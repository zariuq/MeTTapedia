...
\textbf{Proved (D)} &
Meaningful grammatical types live on shared abstract syntax, not on source form. \\
...
Same-text PP-attachment ranked by world evidence &
\textbf{Proved (Sem)} &
Same text, two parses, distinct compiled patterns, world-sensitive evidence reversal. \\
\code{anna\_scene\_rankings\_reverse} &
Locative PP-attachment ranked by world evidence &
\textbf{Proved (Sem)} &
Second same-text case: ``Anna dresses the baby in the crib'' gets reversed
...
\item \textbf{\GF{} (grammar layer).}  \GF{} provides typed abstract syntax trees
  (language-independent structure) plus concrete linearizations (source forms
...
\item \textbf{Contextual ambiguity analysis}: Two independent cases of
  same-text PP-attachment ambiguity are formally verified end-to-end.
  ...
  records the English source forms, and appropriate world states produce
...
\item \textbf{Concrete syntax}: Language-specific linearization rules mapping
  abstract trees to linearized strings, parameterized by morphological features
...
Competing candidate analyses for the same linearized string can satisfy different
...
\paragraph{Same-text PP attachment.}
...
records both analyses for that same text.  In addition, actual \GF{}/PGF
parsing/export recovers the same two abstract trees in both English and Czech:

\begin{lstlisting}[language=ml,caption={Same-text contextual ambiguity}]
theorem telescope_np_text :
    telescopeNPAttachmentText =
      "John sees the man with the telescope"

theorem telescope_vp_text :
    telescopeVPAttachmentText =
      "John sees the man with the telescope"
...
scene the ranking reverses. This is a same-text ambiguity theorem with
world-sensitive evidence, not merely a lexical-profile contrast.
This gives a complete small theorem package: same text, two analyses in the
...
\paragraph{A second same-text case: locative PP attachment.}
...
\begin{lstlisting}[language=ml]
theorem anna_np_text :
    annaNPAttachmentText = "Anna dresses the baby in the crib"

theorem anna_vp_text :
    annaVPAttachmentText = "Anna dresses the baby in the crib"

theorem anna_same_text :
    annaNPAttachmentText = annaVPAttachmentText
...
already proved for the grammar.  The current source-text-side setup has two layers:
