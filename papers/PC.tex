...
\leanname{propext}, \leanname{Classical.choice} and \leanname{Quot.sound}. To keep the
statements readable we give them here as ordinary mathematics; Table~\ref{tab:index}
maps each one to the name you can look up and check.
...
BP, world 3 & 8{,}627 & 11{,}510 & 13{,}267 & 14{,}793 & \emph{in progress} \\
PC, world 1 & 6{,}465 & 6{,}914 & 9{,}067 & 9{,}486 & 10{,}211 \\
PC, world 2 & 6{,}093 & 8{,}031 & 8{,}359 & 9{,}195 & 9{,}411 \\
PC, world 3 & 7{,}548 & 7{,}548 & 8{,}820 & 10{,}111 & \emph{in progress} \\
\midrule
PC/BP ratio & 78\% & 65\% & 66\% & 65\% & 61.4\% (w1--w2) \\
\bottomrule
\end{tabular}
\end{center}

BP compounds metronomically: per-generation deltas agree across worlds within $\pm1.3\%$
at generation~2, endpoints within $\sim$3\%. PC ignites but advances in erratic bursts
--- including one genuine zero-progress generation (world~3, generation~2, byte-identical
solution set) --- and settles between $58\%$ and $65\%$ of BP's frontier (world~1
$64.4\%$, world~2 $58.5\%$ at generation~5). The final three-world verdict follows the
pre-registered analysis when world~3's generation-5 runs seal.
...
\endhead
Thm~\ref{thm:bayes}
  & \leanname{LinearGaussianOperatorModel.equilibrium\_iff\_eq\_posteriorMean}
  & LinearGaussian\-Operator \\
  & \leanname{matrixPCEquilibrium\_iff\_eq\_conditionalPosteriorMean} & MatrixGaussian\-Chain \\
  & \leanname{matrixPC\_not\_equilibrium\_of\_ne\_posteriorMean} & (negative fixture) \\
  & \leanname{pcConditionalPosteriorMean\_equilibrium} & LinearGaussian\-Chain (scalar) \\
Thm~\ref{thm:errrec}
  & \leanname{equilibriumError\_satisfies\_backpropRecursion} & BackpropExactness \\
  & \leanname{tiedEquilibriumError\_satisfies\_bpttRecursion} & (weight tying) \\
Thm~\ref{thm:dagrec}
  & \leanname{sharedDAGEquilibriumError\_satisfies\_parentAggregationRecursion}
  & SharedLatentDAG \\
  & \leanname{dagMultiParent\_parentAggregate\_counts\_both\_slots} & (positive fixture) \\
  & \leanname{dagMultiParent\_single\_slot\_is\_incorrect\_negative\_example}
  & (negative fixture) \\
Thm~\ref{thm:gradne}
  & \leanname{equilibriumGradient\_ne\_backpropGradient\_example} & BackpropExactness \\
  & \leanname{equilibrium\_state\_ne\_forward\_state\_example} & (root cause) \\
Thm~\ref{thm:zil}
  & \leanname{zilScheduledGainGradient\_eq\_backpropGradient} & ZILExactness \\
  & \leanname{zilChainResidual\_at\_schedule\_eq\_backpropError} & (front lemma) \\
  & \leanname{zilOffSchedule\_freeEquilibriumGradient\_failure} & (negative) \\
  & \leanname{residualSkip\_naiveFirstArrivalGradient\_failure\_example} & (negative) \\
Thm~\ref{thm:signed}
  & \leanname{dagScheduleExactAtEveryNode\_iff\_all\_missed\_eq\_zero}
  & DAGSchedule\-Exactness \\
  & \leanname{dagScheduleExactAtEveryNode\_iff\_each\_admissible\_or\_cancellation} & \\
  & \leanname{dagSchedule\_exact\_not\_admissible\_iff\_hasMissedCancellation} & \\
  & \leanname{dagScheduleExactAtEveryNode\_iff\_admissible\_of\_noCancellation}
  & (fusion under no cancellation) \\
Thm~\ref{thm:structsigned}
  & \leanname{dagScheduleUniversallyExactAtEveryNode\_iff\_targetGain\_zero}
  & DAGSchedule\-Exactness \\
  & \leanname{dagFirstArrivalScheduleUniversallyExact\_iff\_targetGain\_zero} & \\
  & \leanname{moduleScheduleUniversallyExactAtNode\_iff\_targetGain\_zero}
  & ModuleSchedule\-Exactness \\
  & \leanname{moduleSignedCancellation\_universallyExact\_positive\_example}
  & (positive fixture) \\
  & \leanname{moduleSingleMissedEdge\_not\_universallyExact\_negative\_example}
  & (negative fixture) \\
Thm~\ref{thm:sched}
  & \leanname{dagFirstArrivalScheduleExact\_iff\_uniformOutgoingTargetRanks}
  & DAGSchedule\-Exactness \\
  & \leanname{uniformLongSkipGraph\_firstArrival\_exact} & (witness) \\
  & \leanname{uniformLongSkipGraph\_not\_unitLevelized} & (witness) \\
Thm~\ref{thm:epc}
  & \leanname{spcErrorStream\_epcStateStream} & ErrorState\-Reparameterization \\
  & \leanname{epcStateStream\_spcErrorStream} & \\
  & \leanname{epcPrefixEnergy\_eq\_spcPrefixEnergy} & \\
  & \leanname{epcCriticalDifferential\_iff\_spcCriticalDifferential} & \\
  & \leanname{epcLocalParameterUpdate\_eq\_spcLocalParameterUpdate} & \\
Thm~\ref{thm:wave}
  & \leanname{spcFirstArrivalSignal\_layer\_closedForm}
  & ErrorState\-Reparameterization \\
  & \leanname{spcWavefront\_eventually\_below\_tolerance} & \\
  & \leanname{spcWavefront\_nonzero\_but\_below\_tolerance\_example} & (positive) \\
  & \leanname{spcWavefront\_unitRate\_not\_below\_small\_tolerance} & (boundary) \\
  & \leanname{epcOneStepParameterUpdate\_unitRate\_eq\_backprop} & \\
Thm~\ref{thm:mupcdepth}
  & \leanname{spcFirstArrivalParameterUpdate\_eq\_depthCreditBackprop}
  & MuPCDepthCredit \\
  & \leanname{chainLinkLoss\_after\_uniformDepthCredit\_exact} & \\
  & \leanname{inferenceRateEqualization\_forces\_unitBackpropBoundary} & \\
  & \leanname{muPCDepthScaling\_cannot\_cancel\_contractingDepthCredit} & \\
  & \leanname{uniformPrecision\_equilibrium\_sub\_forward\_exact} & \\
Thm~\ref{thm:depth}
  & \leanname{pathSlowMode\_eigenrelation} & DepthPathology \\
  & \leanname{pathSlowMode\_iterate\_exact} & \\
  & \leanname{pathRelaxationRate\_gap\_bound} & \\
  & \leanname{pathRelaxationRate\_tendsto\_one} & \\
Thm~\ref{thm:het}
  & \leanname{HeterogeneousDepthBlockFamily.modalSpectralRadius\_lt\_one\_iff}
  & Heterogeneous\-DepthBlocks \\
  & \leanname{HeterogeneousDepthBlockFamily.modalSpectralGap\_le\_depth\_bound} & \\
Thm~\ref{cor:adapter}
  & \leanname{coupledBackboneAdapterStep\_not\_blockDiagonal}
  & FrozenAdapter\-Restriction \\
  & \leanname{frozenAdapterRestriction\_eq\_blockPathJacobiStep\_clamped} & \\
  & \leanname{frozenAdapterRestriction\_eq\_blockPathJacobiStep\_of\_boundary\_clamped} & \\
  & \leanname{isFrozenAdapterSlowModeRate\_iff} & \\
  & \leanname{frozenAdapterRestriction\_gap\_bound} & \\
  & \leanname{activeBackboneAdapter\_backbone\_gap\_bound} & \\
  & \leanname{partialFreezing\_reimports\_backbone\_negative\_fixture} & \\
Thm~\ref{thm:precond}
  & \leanname{chainLinkLoss\_preconditionedUpdate\_exact} & Preconditioned\-Learning \\
  & \leanname{chainLinkResidual\_preconditionedUpdate\_exact} & \\
  & \leanname{chainLink\_zeroLoss\_afterUpdate\_iff\_identity} & \\
  & \leanname{chainLink\_preconditionedUpdate\_strictDescent\_iff} & \\
  & \leanname{chainLink\_lossDecrease\_gap\_from\_identity\_exact} & \\
  & \leanname{chainLink\_lossDescent\_strictly\_degrades\_with\_squaredDeparture} & \\
  & \leanname{unitChain\_large\_preconditioner\_increases\_loss\_negative\_example} & \\
Thm~\ref{thm:spectral}
  & \leanname{spectralRadius\_lt\_one\_iff\_pow\_tendsto\_zero} & Operator\-Stability \\
  & \leanname{complexMatrixPreconditioner\_fullSpectrum\_convergence} & \\
  & \leanname{nonnormalTransientMatrix\_fullSpectrum\_stable} & (nonnormal witness) \\
  & \leanname{nonnormalTransientMatrix\_not\_normal} & \\
Thm~\ref{thm:envelope}
  & \leanname{operatorPowNorm\_geometricEnvelope} & Operator\-Stability \\
  & \leanname{operatorResidualIterate\_norm\_geometricEnvelope} & \\
Thm~\ref{thm:jordan}
  & \leanname{jordanTransientMatrix\_pow\_exact} & Operator\-Stability \\
  & \leanname{jordanTransientMatrix\_spectrum} & \\
  & \leanname{jordanTransientMatrix\_not\_normal} & \\
  & \leanname{spectralRadius\_zero\_allows\_arbitrary\_transientAmplification}
  & (negative boundary) \\
Thm~\ref{thm:mode}
  & \leanname{matrixPreconditionedResidual\_loss\_perMode\_exact}
  & Preconditioned\-Learning \\
  & \leanname{mixedModePreconditioner\_helps\_one\_hurts\_another\_fixture} & \\
Thm~\ref{thm:armijo}
  & \leanname{armijoBacktracking\_terminates} & Backtracking\-Descent \\
  & \leanname{armijoCondition\_energy\_nonincreasing} & \\
  & \leanname{pcEnergyDepth2\_armijoBacktracking\_terminates\_with\_descent} & \\
Thm~\ref{thm:saddles}
  & \leanname{orthogonalDataset\_outputDirection\_curvature}
  & EquilibratedEnergy\-Saddles \\
  & \leanname{orthogonalDataset\_innerStrip\_hasStrictSaddleCertificate} & \\
  & \leanname{orthogonalDataset\_counterexamplePoint\_all\_curvatures\_zero} & \\
  & \leanname{orthogonalDataset\_counterexamplePoint\_twoSidedSaddleCurves} & \\
  & \leanname{orthogonalDataset\_universalStrictSaddleConjecture\_counterexample}
  & (negative boundary) \\
Thm~\ref{thm:coupled}
  & \leanname{coupledTwoTimescale\_multipliers\_stable\_iff} & CoupledDynamics \\
  & \leanname{coupledTwoTimescale\_converges\_below\_threshold} & \\
  & \leanname{coupledTwoTimescale\_diverges\_above\_learning\_threshold} & \\
Thm~\ref{thm:turnover}
  & \leanname{arbitraryDepthLinearEntropyTurnover\_general} & EntropyTurnover \\
  & \leanname{arbitraryDepthTurnoverEntropy\_nonmonotone\_iff} & \\
Thm~\ref{thm:nonlin}
  & \leanname{monotoneContractiveNonlinearEntropyTurnover\_sufficient} & EntropyTurnover \\
  & \leanname{MonotoneContractiveSettling.iterate\_abs\_le} & (contraction bound) \\
  & \leanname{monotoneContractiveNonlinearEntropyTurnover\_uniform\_boundary} & (boundary) \\
Thm~\ref{thm:trainednonlin}
  & \leanname{trainedCenteredSigmoidWeight\_is\_unique\_global\_minimizer}
  & TrainedNonlinear\-Turnover \\
  & \leanname{trainedCenteredSigmoidEntropyTurnover} & \\
  & \leanname{approximateTraining\_preservesCenteredSigmoidTurnoverMargin} & \\
  & \leanname{centeredSigmoidContraction\_does\_not\_reach\_logit\_one} & (boundary) \\
  & \leanname{trainedCenteredSigmoidVectorTurnover} & (vector lift) \\
  & \leanname{centeredSigmoidVectorZeroSeed\_uniform} & (negative fixture) \\
Appendix
  & \leanname{PositiveBivariateCovariance.\allowbreak distinct\_orientations\_same\_observational\_batchEnergy}
  & CausalDirection\-Identifiability \\
  & \leanname{PositiveBivariateCovariance.\allowbreak clamp\_is\_conditioning\_and\_direction\_blind} & \\
  & \leanname{PositiveBivariateCovariance.\allowbreak interventionalEquilibria\_separate\_iff} & \\
  & \leanname{PositiveBivariateCovariance.\allowbreak observationally\_blind\_intervention\_certificate} & \\
  & \leanname{PositiveBivariateCovariance.\allowbreak TwoNodeQuadraticResidualModel.\allowbreak batchEnergy\_eq\_trace\_quadraticForm\_mul\_empiricalSecondMoment} & \\
Thm~\ref{thm:intervention}
  & \leanname{PositiveBivariateCovariance.\allowbreak separatesDirectionsByInterventionalEnergy\_iff}
  & CausalDirection\-Identifiability \\
  & \leanname{PositiveBivariateCovariance.\allowbreak unitNoiseForwardCovariance\_observation\_tied\_intervention\_separates}
  & (positive fixture) \\
  & \leanname{PositiveBivariateCovariance.\allowbreak independentEqualVarianceCovariance\_interventionalEnergy\_fixture}
  & (negative fixture) \\
Thm~\ref{thm:causalselection}
  & \leanname{PositiveBivariateCovariance.\allowbreak forwardOrientationWinsByMinimizedInterventionalEnergy\_iff}
  & CausalDirection\-Selection \\
  & \leanname{PositiveBivariateCovariance.\allowbreak isMinimizedForwardInterventionalBatchEnergy\_iff}
  & (unique forward fit) \\
  & \leanname{PositiveBivariateCovariance.\allowbreak reverseCandidates\_distinct\_but\_intervention\_deletes\_parameter}
  & (anti-vacuity) \\
  & \leanname{PositiveBivariateCovariance.\allowbreak unitNoiseForwardCovariance\_minimizedIntervention\_selects\_forward}
  & (positive fixture) \\
  & \leanname{PositiveBivariateCovariance.\allowbreak independentEqualVariance\_minimizedIntervention\_tied}
  & (negative fixture) \\
Appendix
  & \leanname{sequentialTwoTaskUpdate\_sub\_additive\_exact}
  & ContinualLearning/ QuadraticTwoTask \\
  & \leanname{centered\_sequential\_order\_defect\_eq\_commutator} & \\
  & \leanname{linearTwoTask\_causalCoding\_separation\_crown} & \\
  & \leanname{GaussianEvidence.reused\_contribution\_exact\_excess}
  & ContinualLearning/ EvidenceLedger \\
  & \leanname{linearCausalCodingBridge\_crown} & \\
  & \leanname{generalCausalCodingCorrespondence\_requires\_interventionalClamp}
  & (scope marker) \\
Thm~\ref{thm:yield}
  & \leanname{iidExpectedTargetHits\_eq\_budget\_mul\_targetMass}
  & SearchGuidance/ \mbox{DecisivenessYield} \\
  & \leanname{iidExpectedTargetHits\_strictMono\_targetMass} & \\
Thm~\ref{thm:sep}
  & \leanname{equal\_crossEntropy\_strictly\_ordered\_expectedYield}
  & SearchGuidance/ \mbox{DecisivenessYield} \\
\bottomrule
\end{longtable}
}
