...
scale, and the variable that distinguishes them is where the graph is cut: the unified
carrier's fine multi-site cut leaves little for settling to recover, while the coarse
recurrent cut discards a great deal. We record cut granularity as the candidate control
variable for departure and do not yet claim the relation is causal.
...
\paragraph{Settling depth saturates well before the energy does.}
Settling the same state to fifty times the production depth moves the credit cosine from
$0.241$ at depth five to $0.270$, after which it is immobile to three figures from depth
$32$ through depth $160$---while the energy continues to fall by a further $26\%$
throughout. Roughly nine tenths of all available transport occurs in the first five
steps. Energy descent and credit transport are therefore decoupled: the solver goes on
doing genuine work on the objective and that work buys no further credit. The
consequence for design is immediate and negative---a faster or more stable inference
solver cannot recover credit the graph cut never carried---and the consequence for
theory is a target we state as open: predict the saturation depth from the spectrum
