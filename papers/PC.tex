...
settling failure the cosine falls to $0.0008$: credit essentially orthogonal to the BP
direction, hence carrying no first-order reduction of the task loss. That is not a
stronger departure but a departed and no-longer-descending state, which is exactly the
complement of the \emph{departs-but-descends} band---positive inner product, not a
scalar multiple, squared cosine below one---established constructively in
\S\ref{sec:finite-settling}. The practical consequence is that a monotone-energy gate
cannot detect this failure: energy decreased at every accepted step while the credit was
already orthogonal. The admissible criterion is a floor on credit cosine, which is the
credit-unit stationarity condition that a bare gradient-norm tolerance provably cannot
supply.
