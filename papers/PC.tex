...
\newcommand{\leancommit}{84ec10fdac3c576b725133a70b6e41f45d1874f4}
...
guidance. All statements are machine-checked in Lean~4. In the completed experiments
reported here, PC did not overtake backpropagation: every from-scratch arm failed its
registered bar, and the one ignition that reproduced across seeds started from a
backprop-equivalent anchor. A later typed-GSLT factorial is still running and is excluded
from that verdict until its registered cells and independent checks are sealed.
\end{abstract}
...
\begin{itemize}
\item alignment:
\leansource{lean/mettapedia/Mettapedia/MachineLearning/NeuralNetworks/CreditTransport/WorkNormalizedTruncation.lean}{51}{finiteCredit\_positiveAlignment};
...
\leansource{lean/mettapedia/Mettapedia/MachineLearning/NeuralNetworks/CreditTransport/Float32AuthenticatedCheckpointGeometry.lean}{197}{invocation0\_full\_geometry\_mismatch\_le}.
...
new name. The present registry is intentionally closed:
\leansource{lean/mettapedia/Mettapedia/MachineLearning/NeuralNetworks/CreditTransport/ThirdFamilyGate.lean}{197}{no\_registered\_candidate\_is\_namingLicensed}.
Equivalence and collapse are evidence, not failures to be hidden.
...
experimental verdict, not a theorem about every carrier, schedule, or hardware model; the
new typed-GSLT factorial is not folded into it while that campaign remains live.
...
search (shrink $0.5$, at most twelve backtracks, tolerance $10^{-6}$). Coverage for the
current generations is under independent adjudication and is not reported here.
