...
for ProbMeTTa's source-level query operations, a normalized source syntax
...
\item A formal proof that ProbMeTTa's query/runtime core, source syntax,
...
normalized source syntax, literal nonground matching, literal state-space and
presentation layers, and the end-to-end theorem set.
...
\subsection{Normalized Source Syntax (\texttt{ProbMeTTaSourceSyntax.lean}, 627 lines)}

On top of the runtime/query core, the formalization now includes a normalized
source syntax for the actual top-level ProbMeTTa operators:
...
is now formalized against the existing LP matching layer. The formal API
...
\underbrace{\text{state + source syntax}}_{\texttt{ProbMeTTaStateSpace},\,\texttt{ProbMeTTaSourceProgram}}
...
BDD compilation layer, the normalized source syntax packages the actual
...
ProbMeTTa's source-level query operations, a normalized source syntax for the
