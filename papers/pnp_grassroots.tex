...
available input region and therefore inherits the current finite weighted
recovery theorem.
It also now isolates the exact post-switch visible data domain and proves a clean
finite-summary kernel: any switched family that truly factors through a finite
summary domain automatically inherits an explicit truth-table bit budget equal
to the cardinality of that summary type.
It now also ties the four current exact-data-domain affine candidate classes
...
concrete rho present-moment external channels already realizes arbitrary finite
...
\texttt{PNP/ExactSwitchedFamily.lean} & exact post-switch data domain plus finite-summary compression kernel \\
\texttt{PNP/ABVisibleState.lean} & reduced raw visible data domain $(a,b)$ and its explicit cardinality \\
...
\texttt{PNP/ExactAffineRecovery.lean} & exact-data-domain weighted recovery bounds for the current affine candidate classes \\
\texttt{PNP/SharedAffineFeatureFamilies.lean} & shared-basis exact-data-domain families with combiner-only budgets and recovery bounds \\
\texttt{PNP/ExactABDecisionListFamily.lean} & concrete raw exact-data-domain decision-list family on $(a,b)$ with budget $2k+1$ \\
\texttt{PNP/ABDecisionListRoute.lean} & pullback certificate from reduced $(a,b)$ decision lists to exact-data-domain compression/recovery \\
\texttt{PNP/ExactABAffineFamilies.lean} & affine-feature, sparse-threshold, and decision-list ladders on the raw $(a,b)$ state \\
\texttt{PNP/SharedExactABFeatureFamilies.lean} & shared-basis affine ladders on the raw $(a,b)$ state with combiner-only budgets \\
\texttt{PNP/SharedABFeatureRoutes.lean} & quotient-route transfer from reduced $(a,b)$ families to shared-basis exact-data-domain budgets \\
...
\texttt{PNP/ExactZABDecisionListFamily.lean} & exact-data-domain decision-list family on $(zfeat(z),a,b)$ with budget $r+2k+1$ \\
...
\texttt{PNP/BitVecZABVisibleState.lean} & when $z$ is already bit-valued, the full exact visible data domain $(z,a,b)$ becomes one raw bitvector of length $r+2k$ \\
\texttt{PNP/BitVecZABERMInterface.lean} & concrete identity-extractor specialization of the final exact ERM interface on the raw full visible bit state \\
...
\texttt{PNP/GlobalWeaknessObstruction.lean} & current weakness summaries alone cannot yield a nonconstant predictor \\
\texttt{PNP/InfinitaryHMLObstruction.lean} & infinite minimal contexts force an infinite depth-1 HML observable set \\
...
\begin{enumerate}[leftmargin=1.5em]
  \item one available input region \(\mathrm{AvailableInput}(P,B,G)\),
  \item one \(s\)-bit encoded boolean predictor family on that domain,
...
over this sort of certificate: an explicit visible input region, an explicit
...
The manuscript-visible post-switch datum is the exact local datum
\[
  u = (z,a,b).
\]
The Lean file separates:
\begin{enumerate}[leftmargin=1.5em]
  \item the full exact visible data domain \(u=(z,a,b)\),
...
\[
  \mathrm{view} : U \to V
\]
to a finite summary domain \(V\), then \(G\) admits a concrete truth-table bit
budget of size \(|V|\).
\end{theorem}
...
This does \emph{not} defeat the crux.  Those summary domains may still be far
...
\section{Hyperseed as a Local-View Region}
...
informal phrase, we can work with a typed input region:
\[
...
predictor.  So any semantic rescue that remains close to the
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item Use the current exact present-moment external channels directly: then one already
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \(\mathrm{externalChannels}(\mathrm{state}_i,\mathrm{env}_S)\) iff \(i \in S\);
...
state-indexed external-channel set already realizes arbitrary finite
membership patterns on a natural probe family.  This is not yet a full
``depth-1 HML shattering'' theorem, but it means the large-observable-set
...
already a powerset-like concrete probe family in the rho present-moment external channels.
...
\texttt{ExactAffineRecovery.lean} turns those four exact-data-domain candidate
classes into direct weighted finite-sampling recovery theorems.  Once the
target lies in one of the concrete exact-data-domain affine families, the existing
...
exact-recovery lower bounds on the exact post-switch data domain.
...
\(2k\) raw bits.  This yields a completely concrete exact-data-domain candidate
class with budget
\[
  2k + 1.
\]
The key point is that the feature extractor is no longer existential or
parameterized.  It is the literal raw exact visible data domain restricted to the
...
\texttt{ABVisibleState.lean} and \texttt{ABDecisionListRoute.lean} isolate the
reduced raw visible data domain \((a,b)\) as an explicit quotient candidate and
prove the pullback statement one actually wants: if the switched family factors
through that reduced state and the reduced family is realized by fixed-order
decision lists there, then the exact-data-domain compression and weighted recovery
...
slightly stronger lift: if an exact-data-domain family is invariant under the
...
machinery on the same raw extractor and proves exact-data-domain compression and
...
decision lists respectively.  So the reduced raw state is now a whole
...
that same raw \((a,b)\) state.  If one fixed affine basis on the \(2k\) raw
...
through the reduced raw state \((a,b)\) and the reduced family is realized by
one of those shared-basis raw affine classes, then the exact-data-domain family
...
version: if the exact-data-domain family is already invariant under the raw
...
\begin{enumerate}[leftmargin=1.5em]
  \item as a generic shared-basis target interface on the reduced raw state,
...
fixed-order decision-list family as before.  The exact-data-domain budget is then
\(r+2k+1\), and the matching weighted exact-recovery bound is proved on that
larger visible data domain.
...
that shared-basis route, the exact-data-domain budgets fall to \(2^p\) for arbitrary
...
\texttt{BitVecZABVisibleState.lean} fixes the case where the local datum is
...
to the identity extractor on that raw full visible bit state.  So in that
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item define local views as typed accessible regions,
...
reduced raw visible data domain $(a,b)$ & proved & \texttt{ABVisibleState.lean} \\
...
exact-data-domain affine recovery ladder & proved & \texttt{ExactAffineRecovery.lean} \\
shared-basis affine family ladder & proved & \texttt{SharedAffineFeatureFamilies.lean} \\
raw exact-data-domain decision-list family on $(a,b)$ & proved & \texttt{ExactABDecisionListFamily.lean} \\
...
raw full visible bit state when $z : \mathrm{BitVec}\, r$ & proved & \texttt{BitVecZABVisibleState.lean} \\
identity-extractor ERM interface on the raw full visible bit state & proved & \texttt{BitVecZABERMInterface.lean} \\
...
observer-relative input regions can already be handled cleanly inside one
...
learning domain.  And even once one moves from those global summaries to the
