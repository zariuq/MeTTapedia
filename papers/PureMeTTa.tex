...
\begin{quote}
One source-language \MeTTa{} should elaborate to one shared \MeTTa-Core, which then
...
source language $\to$ elaborated core $\to$ checked kernel and compiled runtime
}}
\end{center}
...
\begin{tikzpicture}[node distance=1.1cm and 1.8cm]
  % Source syntax
  \node[sharedbox, minimum width=8cm] (syntax)
    {Source \MeTTa{} / Shared Substrate\\
     \footnotesize\MeTTaIL{} / Pattern / LanguageDef / artifacts};

  % Elaborated Core
  \node[neutralbox, minimum width=5cm, below=0.8cm of syntax] (elab)
  ...
  \node[sharedbox, minimum width=6cm,
    ...
    {WM consequence / obligation API\\
     \footnotesize\leanref{wmConsequenceRuleOn}};

  % Arrows
  \draw[arrw] (syntax) -- (elab);
  \draw[arrw] (elab) -- (pure);
...
\begin{itemize}
  ...
  \item \codepath{Mettapedia/PLN/Bridges/Languages/WorldModel/PLNWorldModelPureKernelBridge.lean}
...
packages closed source-syntax checking through \leanref{PureCheckSuccess},
\leanref{inferPureSyntax}, and \leanref{checkPureSyntax}. Parser-free checked
...
\begin{itemize}[nosep]
  ...
  \item \leanref{PureCheckingBoundary.checkAndEvalSyntax},
...
\begin{tikzpicture}[node distance=0.9cm and 1.2cm]
  \node[sharedbox, minimum width=5.8cm] (input)
    {Closed Pure term or certificate\\
     \footnotesize trusted input to the live proof-side boundary};

  \node[proofbox, below=0.9cm of input, minimum width=6.2cm] (waist)
    {Pure semantic waist\\
...
concrete syntax, artifact substrate, and elaborated-core story are shared, while
...
conversion rules in the kernel theory. The preferred concrete syntax
...
\begin{itemize}[nosep]
  ...
  are even further away: they may be practical higher-order syntax devices,
...
concrete syntax, that lands in a trusted kernel and shared artifact substrate.
...
\paragraph{Three runtime interfaces.}
The central type is \leanref{MeTTaRuntimeExecInterface}
...
QuerySeam & \leanref{morkRuntimeQueryExec0} &
  Query interface with \emph{soundness + completeness} proofs \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Query interface detail.}
\leanref{MeTTaRuntimeQueryInterface}
...
\begin{itemize}
  ...
  \item \codepath{Mettapedia/Languages/ProcessCalculi/MORK/DirectExecInterface.lean}
...
\begin{center}
closed Pure source terms $\;\to\;$ PureKernel term $\;\to\;$
...
\begin{itemize}[nosep]
  ...
  consequence.
  \item \leanref{pureClosedSyntax\_overlap\_is\_not\_directExec}
  (\codepath{PureCertificateFragment.lean}).
  \item \leanref{coreTypedSyntaxAtom\_overlap\_is\_not\_directExec}
...
\leanref{SyntaxNode} is an inductive with 11~constructors:
...
\midrule
\code{pureClosedSyntax} & pureKernel & \leanref{PureCertificate} \\
...
\leanref{elaborate : SyntaxNode $\to$ ElaboratedNode} maps all
11~source-node variants to their classified outputs.

\paragraph{Preservation theorems (all proved).}
\begin{itemize}[nosep]
  \item 7~region-preservation theorems (e.g.\
  \leanref{elaborate\_pureClosedSyntax\_region}).
  \item 5~artifact-preservation theorems (e.g.\
  \leanref{elaborate\_pureClosedSyntax\_artifact}).
...
\code{SyntaxNode} to these neutral classes.}
...
Direct proof/runtime overlap is \textbf{not yet proved}. No source-syntax
...
\begin{tikzpicture}[node distance=0.9cm and 2.5cm]
  \node[proofbox] (sptm) {PureSyntaxTerm};
...
source-syntax subset that elaborates to both a PureKernel certificate and a
...
\begin{itemize}[nosep]
  ...
  bounded authored \code{ContextualStep.StepAt} $\to$ a compiled
  \code{fireRule} update.
  ...
  (\codepath{DirectExecInterface.lean}) ---
...
\noindent
The query interface \leanref{morkRuntimeQueryExec0}
...
a source term maps to both a trusted PureKernel term and a shared
\MeTTaIL{} artifact, with proven agreement.

\subsection{Source Language}

\leanref{PureSyntaxTerm}
...
Two maps out of \leanref{PureSyntaxTerm}:

\begin{enumerate}
  \item \leanref{toPureTm : PureSyntaxTerm\;n $\to$ PureTm\;n} ---
  structural embedding into the trusted kernel.
  \item \leanref{toClosedPattern : PureSyntaxTerm\;0 $\to$ Pattern} ---
  quotation into the shared \MeTTaIL{} artifact space.
\end{enumerate}

\begin{fact}[Quotation agreement]
\leanref{toClosedPattern\_eq\_quoteClosedTm}:
the source quotation agrees with the kernel quotation for all closed terms.
...
\begin{itemize}
  ...
  \leanref{PureTm\;0}), \code{artifact}, \code{artifact\_eq}.
  \item \leanref{SharedPureOverlapCertificate} --- the first real
  dual-view object: source term, trusted Pure term, shared artifact,
  all proved to agree.
  \item \leanref{certifyPureSyntax} --- constructor that builds the
  overlap certificate from a source term.
  \item \leanref{certifyPureSyntax\_overlapClass} --- proves the
...
The \code{fullLegacyRuntime} constructor exists in \leanref{SyntaxNode}
...
\begin{enumerate}
  ...
  It does not carry embedded proofs the way \leanref{MeTTaRuntimeExecInterface}
  does.
...
\midrule
Source language & Lean 4 & \MeTTa{} syntax \\
...
\begin{itemize}
  ...
  that Lean achieves.
  \item The elaborator is a PoC classifier (11 source-node variants $\to$ 7 elaborated),
...
\begin{itemize}[nosep]
  ...
  $R_{\mathrm{exec}_0}$.
  \item \leanref{pettaEvalStep\_schedulerFires}
  (\codepath{DirectExecInterface.lean}) --- fires through full scheduler.
...
\begin{itemize}[nosep]
  ...
  \leanref{removeAtomCmd\_schedulerFires}
  (\codepath{DirectExecInterface.lean}) --- through full scheduler.
\end{itemize}

\subsection{WM Landing}

All three lanes land on the WM consequence API:
...
PureKernel waist, the runtime interface, or both? Is the overlap currently
...
elaborated source language that classifies \MeTTa{} programs as either
...
\textbf{Pros.}
One source language. One classifier. Two targets. Lean-like.
...
\quad ExecutionBoundary, DirectExecInterface, etc.\ & ${\sim}1200$ \\
\midrule
\textit{WM bridges} & \\
...
\begin{itemize}[nosep]
  ...
  \item \leanref{MeTTaRuntimeExecInterface} translates \leanref{Pattern}
...
\begin{tabular}{@{}lll@{}}
\toprule
Dialect & \code{bindingsMode} & \code{contextMode} \\
\midrule
...
\begin{itemize}[nosep]
  ...
  the Pure runtime spec declares \code{bindingsMode = .none} and
  \code{contextMode = .none}.  There is no implicit binding store;
...
\noindent
``Many spaces'' as concrete syntax is not enough.  True many-space
