...
\newcommand{\formalshort}{f3a31db36bb2}
...
\large A machine-checked waist for neural program synthesis, and the unified-carrier
...
from auxiliary supervision. A later two-world, two-generation carrier--credit factorial
finds $1{,}617$ new entries in union; prospective PC contributes the largest
update-rule union and exclusive marginal, while the carrier and seed interactions rule
out a universal winner. The same interface, pointed at an external dependent-type
benchmark, \emph{measured} the benchmark's definitional-equality demands
($\beta\eta$) through a graded ladder of verified roots. A preregistered
PC-from-update-zero Round~0 then separates mechanism from efficacy: prospective PC
reproduces a strongly non-BP credit direction on an independent seed and contributes one
fixed-benchmark theorem beyond its matched independent-BP control, while still trailing
that control on synthetic held-out coverage. We assemble the architectural alternatives
into one unified carrier:
a recurrent controller, typed workspace, evidence metadata and routed operator bank
share a product state while the checked waist remains unchanged.
A separate two-round relational audit shows that this waist must retain the verified
fact relation rather than only one selected witness per target: selection can preserve
target coverage while discarding program multiplicity, reuse structure and robustness.
\end{abstract}

\paragraph{Open artifacts.}
The \href{https://github.com/zariuq/MeTTapedia}{formal repository},
\href{https://github.com/zariuq/MeTTapedia/blob/main/papers/gslt2gslt.tex}{paper source},
and
\href{https://github.com/zariuq/MeTTapedia/blob/main/papers/gslt2gslt.pdf}{compiled PDF}
are public. Formal links below are immutable source permalinks to commit
\code{\formalshort}.
...
and decides acceptance. The comparison to keep in mind is a chess interface on which an
illegal move cannot be entered---not because the player was trained to avoid illegal
moves, but because the board owns the rules and the move is simply not among the
buttons. Here, wrong arity, an out-of-scope variable, or an ill-typed argument is not a
low-probability output to be filtered; it is not an action at all. The scientific benefit is attributability. When the interface
...
\node[lbl, left=2mm of gru.west, text width=24mm, align=right]
  {\textbf{attention \#2}\\reads state;\\the \emph{write law}\\varies by arm};
...
\subsection{One machine above the waist: the unified carrier}
\label{sec:unified}

The four state carriers in the completed factorial are informative as interventions, but
they are not four mutually exclusive machine designs. A recurrent vector is a controller;
typed slots are memory; belief coordinates are metadata; routed operators are reusable
transformations. The unified carrier gives each role its own field while preserving the
same GSLT2GSLT interface.
...
\draw[flow, provedc] (waist2.west) -- ++(-18mm,0) |- (slots.west)
  node[thm, pos=0.23, left] {typed frontier};
\end{tikzpicture}
\caption{The unified carrier. The address/frontier plane remains checker-owned; all
learned recurrence, storage, evidence and routing remains above the same typed-action
waist. The architecture composes roles that the factorial separated for attribution.}
\label{fig:unified}
\end{figure}
...
\]
where $t_i$ is the injective (role, parent, argument) signature and $u_i^\pm$ records
only evidence created in the current episode. Let
$\Lambda_i=n_i^++n_i^-$ be the featurewise precision vector and write an overbar for
its feature mean. A routed operator proposes $c_{ki}$, learned write gate $g_{ki}$ and
fresh evidence. The derived update is
\[
\begin{aligned}
r_i&=(1+\sigma_{\rm jump}^{2}\Lambda_i)^{-1},&
...
w_i&\leftarrow w_i+\frac1K\sum_k
   \gamma_i g_{ki}(c_{ki}-w_i),&
h&\leftarrow\operatorname{GRU}(x_{\rm carrier},h).
\end{aligned}
\]
The operator chooses the candidate and its learned gate; the evidence chart supplies a
Bayes-structured damping factor. At zero old precision and positive fresh precision,
$\gamma_i=1$ and the content step is exactly the typed-workspace update
...
\[
 \ell_{kj}=\langle q_k,k_j\rangle/\sqrt d+
 b_k^2\log(1+\bar\Lambda_j),
\]
so its unnormalized attention weight is the ordinary content weight multiplied by
$(1+\bar\Lambda_j)^{b_k^2}$. The identity and its strict monotonicity boundary are
machine-checked
(\leansource{lean/mettapedia/Mettapedia/MachineLearning/NeuralNetworks/UnifiedCarrier/Core.lean}{234-L270}{precisionBiasedLogit\_strictMono\_precision}).
Setting $b_k=0$ recovers the workspace read exactly.

Persistent evidence is an ordered, optional outer loop. A fresh hole may read a
type-indexed prior; at episode completion that table decays once and absorbs only the
innovations $u^\pm$. It never reabsorbs the posterior $n^\pm$, which already contains
the imported prior. The double-counting alternative is refuted by an executable fixture
...
This is a refinement, not a relabeling. Under an explicit mode, parameter relation and
state projection, every finite policy trajectory equals the typed-workspace trajectory
...
Positive old precision and distinct recurrent histories each supply a strict separating
witness, so the full carrier is not definitionally the workspace
(\leansource{lean/mettapedia/Mettapedia/MachineLearning/NeuralNetworks/UnifiedCarrier/StateCarrier.lean}{283-L303}{recurrentHistory\_fullPolicy\_separates}).
Most importantly for GSLT2GSLT, the common masked policy contains legal operators and an
illegal operator remains absent after a unified step
(\leansource{lean/mettapedia/Mettapedia/MachineLearning/NeuralNetworks/UnifiedCarrier/StateCarrier.lean}{306-L339}{illegalOperator\_stays\_absent\_after\_unifiedStep}).
No carrier state can manufacture a new refinement action.

The reference instance keeps the existing graph encoder, language table, typed frontier,
masked scorer and external checker unchanged. Its packed state has
$S W_c+4S W_e+2S+W_h+2$ scalars; the factor four is posterior plus innovation evidence.
The exact-real design, an addressed production packet, and one binary32 decision-site
replay are in the public formal snapshot. The architecture did not enter the completed
carrier--credit campaign, so none of those coverage results are evidence for it%
\ifoeisterminalfacts
; it did enter a subsequent mis-specified branch whose record is tabulated in
Section~\ref{sec:gslt-terminal}%
\fi
. Its decisive tests must
separate recurrent control, precision-read bias, Bayes gating and cross-episode persistence
...
$[B,A]\,x + (Ba + b - Ab - a)$
(\leanname{affineCompositionDiscrepancy\_exact}), and
order-independence holds iff the linear parts commute \emph{and} the affine obstruction
vanishes (\leanname{affinePhases\_orderIndependent\_iff}); commuting linear parts alone are
...
(\leanname{linearPhases\_orderIndependent\_iff\_interferenceEnergy\_zero})---a Gram
...
(\leanname{individuallyStable\_switchingDiverges}). Per-command guarantees are therefore
...
(\leanname{commutingCommonQuadraticLyapunov\_crown}). A command-driven decoder should ship
...
\paragraph{The completed carrier campaign.}
The comparison was subsequently run to its registered eight-generation endpoint in two
independently initialized worlds, with four matched-capacity carriers over the same
waist. All $64$ cells passed independent verification. Their union contained $984$
distinct new sequences:
...
The result argues for composition rather than a winner-take-all carrier choice: $669$ of
$984$ sequences were found by exactly one carrier, with pairwise overlaps only
$8$--$16\%$. Yet a fixed-split portfolio reached $399$ sequences against the recurrent
arm's $435$, so complementarity alone does not license naive allocation. A later
generation-nine/ten carrier--credit factorial has now sealed. It keeps all four parents
frozen, trains only matched zero-output residual adapters, and crosses five update rules
in two worlds while preserving identical ledgers and search budgets within each matched
parent.

\begin{center}\small
\begin{tabular}{@{}lrr@{}}
\toprule
update rule & cross-world union & exclusive marginal \\
\midrule
...
The all-cell union is $1{,}617$ new A-numbers. Prospective PC leads the aggregate rule
portfolio but not every carrier--world cell; ordinary BP remains substantially faster.
Grouping those same cells by carrier yields recurrent $738/396$, derived-belief
$643/281$, learned-belief $564/220$ and workspace $517/183$ (union/exclusive), again
supporting compositional roles rather than one interchangeable state mechanism. The
authoritative isolated verifier passes all $749$ checks; its SHA-256 is
{\scriptsize\ttfamily
3f08e6c3a57ba3e13a0e387737bbb31cfd990221a7b3b331214b4f9cab47bc7f}.

\subsection{A selected witness is not the discovery state}
\label{sec:relational-discovery-state}

A separate fresh-start campaign crossed prospective PC and BP in a Unified graph and
a coarse recurrent graph, with one external checker defining acceptance. It does not
identify an effect of the typed waist, because architecture, decoder and graph structure
are bundled. It does, however, reveal a representation requirement for the waist.
The primitive result is an authenticated finite relation
\[
 R\subseteq \mathrm{Program}\times\mathrm{Target},
\]
not a table containing one preferred program for every target.

In round one the witness-complete relation contained $612$ verified edges, $525$
program identities and $493$ targets. The selected union table retained $493$ edges,
$445$ programs and all $493$ targets. In round two the full relation contained $470$
edges, $418$ programs and $396$ targets; selection retained $396$ edges, $346$
programs and all $396$ targets. Thus selection omitted $119$ then $74$ valid facts and
$80$ then $72$ program identities while leaving target coverage unchanged. The formal
criterion states that a curated subrelation preserves program coverage exactly when
every program in the original relation retains at least one selected witness
(\leanname{curated\_programCoverage\_preserved\_iff}).

The omitted relation structure changes the experimental conclusion. In round two the
Unified-PC arm covers $95$ targets through $89$ program identities, whereas Unified BP
covers $82$ through $113$. One previously known PC program supports $36$ targets
exclusively: one same-target reproduction and $35$ cross-target reuses. Removing it
changes PC coverage from $95$ to $59$ and reverses the ordinary PC--BP contrast from
$+13$ to $-21$; the most consequential BP removal changes $82$ to $80$. The exact
leave-one-program-out identity and its degree bound are machine-checked
(\leanname{targetCoverage\_withoutProgram\_add\_exclusive},
\leanname{exclusiveTargetContribution\_le\_programTargetDegree}).

The waist consequence is structural. A selected representative is a derived report,
not the discovery state. The checked record should expose at least authenticated facts,
their program and target projections, fresh-versus-known program provenance,
program degree, exclusive target contribution and causal root. This retains enough
information for relational novelty, overlap correction and leave-one-source-out
robustness without changing the checker-owned acceptance boundary.

The unified carrier of Section~\ref{sec:unified} subsequently entered a registered
three-round branch\footnote{A botched experiment that nonetheless adds data. It was
intended as a from-scratch comparison; it was mis-specified and executed as a
continuation from generation-eight weights (rule-neutral, trained by the earlier
backpropagation loop) over the generation-ten single-world pool. Its verified
discoveries enter the pool with full provenance; its carrier comparisons are recorded
for completeness and are not evidence on the from-scratch question. Row labels 11--13
are pool-accounting indices, not scientific generation numbers.} with persistent
evidence disabled, so it recorded within-episode evidence routing and recurrent
control only.
\ifoeisterminalfacts
The branch record is tabulated below, separately from the generation-nine/ten
factorial.
\else
The branch record will be tabulated separately after its independent seal.
\fi

\ifoeisterminalfacts
\subsection{Recorded branch tables (mis-specified continuation)}
\label{sec:gslt-terminal}

The independent recount records \oeisTerminalCumulativeUnion{} verified new
A-numbers. By carrier:
...
\end{tabular}}
\end{center}
The unified carrier does not win raw totals ($334$/$208$ at $35.7$ h vs the
recurrent carrier's $343$/$271$ at $7.5$ h); in the branch's opportunity replay it is
the best carrier for every predictive-coding rule. Declining yields are a
resource-boundary observation, not saturation. The carrier never acquires authority
to enlarge the legal support; every counted output passes the same external
whole-OEIS checker, so carrier efficacy can change without moving the verified
waist.
\fi
...
\paragraph{Round-zero gate.}
The proof-side boundary for the next dependent-type experiment is now sealed. The
canonical Lean replay contains all $31$ cases and accepts each with the indexed-LF
checker
(\leanname{all\_cases\_accept});
independently, every raw candidate and goal produces a checked $\beta/\eta$
normal-form path
(\leanname{all\_cases\_certified}).
The live MeTTa route reproduces $31/31$. Its largest cases, entries $16$--$28$,
stream proof DAGs in process-isolated chunks of at most $64$ nodes; every chunk
rechecks the admitted GSLT source, and the final worker reconstructs both conversion
goals before invoking the indexed-LF kernel. The Python driver is therefore untrusted
orchestration, not part of the checker. Across these thirteen cases, all $282$
source-bound bundle files rehash exactly and every worker has empty standard error.
A one-rule conversion corruption is rejected explicitly by the checker, while the
mutation-complete binding gate catches all $30/30$ injected live and generic-ABT
breaks.

The neural Round~0 experiment is now complete. The external corpus was rebound to the
same root and split by theorem family; training used $3{,}005$ rows and $13{,}545$
frontier states, while selection used $600$ synthetic family-held-out problems and four
external tutorial problems. The model was deliberately small---an
$84\!\to\!64\!\to\!1$ tanh pointer ranker---so this round tests credit assignment over
the checked frontier, not a new carrier. All five trained profiles used the same
$1{,}060$-batch ledger; the four matched profiles shared the same initial state, and
every PC profile was active from update zero:

\begin{center}\small
\begin{tabular}{@{}lrrr@{}}
\toprule
profile & held-out solves / 604 & final loss & median raw cosine to BP \\
\midrule
...
The screen therefore found both a BP-like PC profile and two mechanism-distinct
profiles, without confusing distinctness with efficacy. The local algebra is itself
formalized: the prospective first step recovers both two-layer BP credits at one common
rate--precision scale
(\leanname{prospectiveFirstCredit\_eq\_scaledBP}),
whereas a hidden-state PC fixture proves that its two layer credits need not share any
common BP scale
(\leanname{hiddenPC\_twoLayerCredit\_not\_commonScaledBP}).

Prospective and incremental PC were then rerun from the independent-BP initialization
and the same ledger. Prospective PC reproduced its mechanism departure: median raw
credit cosine $0.0930$ (screen $0.0979$) and optimizer-displacement cosine $0.3358$,
while its loss fell from $1.7590$ to $0.6073$. Incremental PC again barely learned
($1.7590\to1.7497$; cosines $0.7236/0.8549$). The fixed-endpoint results were:
...
Prospective PC trails BP by $60$ synthetic solves but contributes $40$ synthetic
marginals, two tutorial marginals and \code{Iff\_refl} beyond BP's
\code{False\_elim} on DTTBench. Across the two prospective seeds, synthetic solved-set
Jaccard similarity is $106/172=0.6163$. Incremental PC adds no endpoint marginal and is
not promoted unchanged. The independent verifier re-executed all $1{,}276$ replica
searches, kernel-rechecked $172$ solved traces, recompiled all three DTT candidates in
their original declarations and recomputed every comparison; its SHA-256 is
{\scriptsize\ttfamily
b191d1e779298233d932caf281333fde57c4d4feee79d09c5ac8217ceaf8844d}.

There is one important evidence boundary. The external test and DTTBench endpoints had
already been opened globally before replication training. No endpoint-dependent change
was made after that opening, but the replica endpoint rows are fixed-benchmark
replication, not a second unopened test. The held-out synthetic comparison and repeated
mechanism departure are the cleaner causal evidence. DTTBench-31 remains an
authentication set, never a tuning set.
...
\paragraph{State carriers.} The four experimental carriers differ only in write
semantics; the read over encoder memory and the checker interface are shared. A capacity
test holds all four within two percent of the recurrent baseline. The unified carrier is
a fifth design that composes the roles; it did not enter the historical comparison, but
the registered generation-eleven--thirteen experiment tests it alongside recurrent and
typed-workspace controls.

\begin{center}\footnotesize
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lll>{\raggedright\arraybackslash}p{46mm}@{}}
\toprule
...
unified (registered) & $(w,n^\pm,u^\pm,h)$ &
content gate $\times$ precision gain; innovations add &
60 slots $\times$ content 112 $\times$ evidence 16; control 48; 4 operators;
persistent prior off \\
\bottomrule
\end{tabular}
}
\end{center}
...
precision interpolation in natural coordinates. The registered unified instance has
$5{,}359{,}248$ parameters against the registered recurrent baseline's $5{,}279{,}800$,
a ratio $1.0150$. Its $581{,}400$ trainable scalars occupy $66$ tensors. These are
capacity and implementation-conformance measurements, not efficacy results.
...
component that actually failed. Complete formal sources are in the immutable
\href{https://github.com/zariuq/MeTTapedia/tree/\formalcommit/lean/mettapedia/Mettapedia/GSLT}{GSLT tree},
\href{https://github.com/zariuq/MeTTapedia/tree/\formalcommit/lean/mettapedia/Mettapedia/MachineLearning/NeuralNetworks/WorkspaceDecoder}{workspace tree}, and
\href{https://github.com/zariuq/MeTTapedia/tree/\formalcommit/lean/mettapedia/Mettapedia/MachineLearning/NeuralNetworks/UnifiedCarrier}{unified-carrier tree}.
...
families, five explicit scope boundaries, kernel-computed accounting). Also sealed are
the unified carrier's exact-real workspace refinement, strict separation fixtures,
retention-economics threshold, addressed-packet witness and binary32 decision-site
certificate. The eight-generation, four-carrier comparison is complete; its $64$ cells
and the $984$-sequence union are independently checked. The generation-nine/ten
credit-rule extension is also complete: its checker-audited union is $1{,}617$, and its
isolated verifier passes $749/749$ checks. The unified carrier is implemented, tested and
registered in the later PC-from-update-zero longitudinal campaign, with persistent
cross-episode evidence disabled in the tested instance. The proof/checker boundary for
dependent-type Round~0 is sealed by canonical Lean replay, $31/31$ live source-bound
indexed-LF conversions, process-isolated replay for entries $16$--$28$, and conversion
and binder mutation gates. The PC-from-update-zero screen and two-profile independent
replication are also sealed: prospective PC reproducibly departs from BP and contributes
verified marginals, but BP retains higher synthetic held-out coverage; unchanged
incremental PC does not earn promotion. The endpoint replication is explicitly labelled
previously opened. All coverage numbers above are exact finite-corpus verifications from
immutable, independently recomputed ledgers.
