...
with a perspective-relative notion of accessible input region inspired by the existing
...
\texttt{VisiblePostSwitchData.lean} & isolates the exact visible data domain, invariant projection, and fork-visible projection \\
\texttt{ExactSwitchedFamily.lean} & isolates the exact switched-family compression target and ERM reuse theorem \\
\texttt{ExactVisibleCompressionObstruction.lean} & if the exact family spans all exact visible rules, no sub-data-domain-cardinality code budget can cover it \\
...
\subsection{Hyperseed Perspective Region}
...
input region} as a subtype of the Hyperseed \texttt{availableRegion}.  This does not solve
...
\texttt{VisiblePostSwitchData.lean} defines the exact manuscript-visible
data domain \(u=(z,a,b)\) together with its invariant and fork projections, and
...
exact visible data domain, then no \(s\)-bit code can cover it when
\(s < |\text{data domain}|\).  So any same-route rescue must now prove a genuine
