...
\paragraph{Construction requirements.}
Term sharing must avoid a linear scan per store. Smart constructors should
intern child terms before constructing a parent, so lookup and construction
use the shared representation directly. Performance claims must be measured
...
seam between the language interface and the backend operations.
...
\item Route the streaming builtins (\code{select}, \code{once},
...
builtins routed through the Phase~2 visitor seam.  Positive example:
...
\item Benchmark harness, profiling hooks
