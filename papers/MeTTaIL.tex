...
\item \textbf{Authored contextual closure} (\code{Premise.congruence}):
...
\subsection{The \code{languageDef!} Authored Notation}
...
\subsection{Authored Contextual Semantics and Compiler Equivalence}
...
The least one-step relation generated by authored rewrite rules is
...
\begin{lstlisting}[language={},caption={Least authored contextual semantics and exact compiler}]
...
(\code{languageDef!} presentation) \\[4pt]
...
The \code{languageDef!} presentation \code{metamathCore} defines the full
...
A second, elaborated-record presentation \code{sourceGrammar} carries the
...
The conformance chain between the source presentation and \code{mm-lean4} is
...
pipeline's classified stream to the exact byte presentation the verified
...
\section{The CertificateGSLT Layer: Generic Proof Checking over Presentations}
...
is that any validated presentation whose \code{LanguageDef} declares
...
\begin{itemize}
  ...
  \item least authored contextual relation \code{ContextualStep.Step}, with an
