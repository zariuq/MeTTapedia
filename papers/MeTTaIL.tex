...
target exercising the full stack.  A TPTP/TSTP case study additionally shows
how official syntax, binder-resolving FOF-to-CNF transformations, pluggable
calculus services, finite word compilation, and a source operational GSLT
compose without turning a standard proof format into a hardcoded prover.
\end{abstract}
...
inductive Pattern where
  ...
  | lambda : Option String -> Pattern -> Pattern      -- declared binder spelling
...
\item \textbf{Carrier-aware types} (\code{TypeDecl} with \code{CarrierKind}):
  Metamath concrete syntax requires raw lexical carriers (\code{![raw] as Sym},
  \code{![label] as Label}). Rust already supports these via
  \code{LangType.carrier\_kind}; Lean now matches.
\item \textbf{Preserved binder spellings} (\code{Pattern.lambda : Option String}):
...
\item \textbf{Syntax operators} (\code{SyntaxPatternOp}):
  ...
  declared data, not flattened to terminal/nonTerminal strings.
\item \textbf{Explicit contextual closure} (\code{Premise.congruence}):
  a rewrite rule explicitly states each context in which reduction may recur.
  Collection representation alone grants no reduction authority.
...
\subsection{The \code{languageDef!} Source Notation}
...
\code{languageDef!} macro that parses the \emph{same} concrete syntax into the
\emph{same} \code{LanguageDef} type---no separate ``syntax tree'' is introduced.
The macro \emph{is} the notation layer; the single \code{LanguageDef} is the
core.

\begin{lstlisting}[language={},caption={Lean \code{languageDef!} example
  (\texttt{LanguageDefDSLTests.lean})}]
...
Three additional layers support the source notation:
\begin{enumerate}
...
\item \textbf{Faithful export.}
  \code{Export.renderLanguageWithUserSyntax} preserves declared binder names,
...
\begin{center}
\begin{tabular}{c}
\textbf{Language-Definition Source} \\
...
the Rust \code{language!} macro and the Lean \code{languageDef!} notation.  The
...
Lean now has a directly specified process fragment
...
checks confirm that the Lean-rendered text matches the Rust syntax.

\paragraph{Honest boundary.}
Two Rust rules cannot yet be expressed in Lean:
\begin{itemize}
...
\subsection{Explicit Contextual Semantics and Compiler Equivalence}
...
The least one-step relation generated by declared rewrite rules is
\code{ContextualStep.Step} in
\codepath{Mettapedia/OSLF/MeTTaIL/ContextualStep.lean}.  A
\code{Premise.congruence} recursively invokes that same relation; collection
representation carries no independent reduction authority.  Its executable
compiler is indexed by an explicit contextual-depth bound and is proved exact.

\begin{lstlisting}[language={},caption={Least explicit contextual semantics and exact compiler}]
...
are connected through \code{LanguageDef}:

\begin{center}
\begin{tabular}{c}
\code{LanguageDefDSL.lean} \\
(\code{languageDef!} definition) \\[4pt]
$\swarrow$ \hspace{6em} $\searrow$ \\[4pt]
\begin{tabular}{c}
\code{SourceGSLT.lean} \\
(source-syntax theorem tower)
\end{tabular}
\hspace{2em}
\begin{tabular}{c}
\code{mm-lean4} \\
...
\textbf{Conformance chain} \\
(partially proved; see below)
\end{tabular}
\end{center}

The \code{languageDef!} value \code{metamathCore} defines the full
Metamath statement syntax---\code{Database}, \code{Stmt}, \code{MathString},
\code{ProofString}, normal and compressed proofs, blocks, and includes---with
carrier-aware type declarations (\code{![label] as Label}, \code{![raw] as
Sym}, \code{![proofTok] as ProofTok}, \code{![path] as IncludePath}) and
explicit term constructors for every \code{\$c}, \code{\$v}, \code{\$d},
\code{\$f}, \code{\$e}, \code{\$a}, \code{\$p} declaration form.  Beyond
syntax, its \code{rewrites} block authors the front-end lowering,
environment-loading, linearization, and compilation pipeline as rewrite rules
in the same language object, and the definition renders back to the
\code{language!} concrete syntax with kernel-checked guards on the exported
text.  (A Rust \code{metamath.rs} twin existed historically; the rendered
text is now checked directly, with no in-tree Rust consumer.)

A second, elaborated-record \code{LanguageDef} \code{sourceGrammar} carries the
proof-facing tower: it is validated by the generic well-formation checker
(\code{sourceGrammar\_valid}), declares its five lexical token classes with
byte charsets, and every accepted run of the composed
source pipeline derives the outer database sort in the lexicalized language of
its own classified stream, with no externally supplied lexical or
ordered-token premises (\code{runSource\_selfLexicalized\_derives}).

The \code{mm-lean4} project provides a complete, sorry-free, axiom-free
verification of the Metamath proof checker:
\begin{itemize}
...
\begin{tabular}{lll}
\toprule
\textbf{\code{languageDef!} form} & \textbf{\code{sourceGrammar}} & \textbf{mm-lean4 \code{DeclarativeSpec}} \\
\midrule
\code{Sym} (carrier: raw)   & \code{symbol\_tok} class   & \code{CN} / \code{VR} \\
\code{Label} (carrier: label) & \code{label\_tok} class & labels in frame \\
\code{MathString}            & \code{symbol\_list} rules  & \code{Expr} \\
\code{Database}              & \code{database} rules      & parsed DB structure \\
\code{ProofString}           & \code{proof\_list} rules   & proof witness \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{What is proved and what remains.}
The conformance chain between the source \code{LanguageDef} and \code{mm-lean4} is
now proved at the declaration level: every accepted source transition
(\code{\$c}, \code{\$v}, \code{\$d}, \code{\$f}, \code{\$e}, \code{\$a},
\code{\$p} insertion, scope push/pop/completion) is matched by the shipped
database operation it corresponds to, and the whole statement fold co-evolves
with the implementation database from the initial states with no supplied
premises (\code{default\_initial\_applyPayloads}).  This includes identifying
the implementation's mandatory-frame computation with the specification's
mandatory frame (\code{trimFrame'\_eq\_mandatory}).  Three joints remain
before the chain is a single bidirectional theorem: binding the composed
pipeline's classified stream to the exact byte representation the verified
reader consumes; discharging each \code{\$p} obligation's normal or compressed
proof from the reader's own chronology, with verified and
incomplete~(\code{?}) outcomes kept distinct; and the prefix-wise simulation
between the source fold and the production reader, in which reader microsteps
are administrative and completed statements are the observable steps.  These
are in active development, with the normal-proof discharge recently
constructed against the production reader trace.  Separately, a
statement-fragment translation between \code{metamathCore} and
\code{sourceGrammar} would close the triangle's left edge; it is scoped but
not yet started.
...
Appendix~\ref{app:mq-lean}), giving an explicit audit map that connects
...
\section{The CertificateGSLT Layer: Generic Proof Checking over LanguageDefs}
\label{sec:certificategslt}

Beyond rewriting, a \code{LanguageDef} may declare \emph{judgment forms} and
\emph{inference rules}.  The generic inference checker
(\codepath{Mettapedia/GSLT/LanguageDef/InferenceChecker.lean}) validates
these proof-calculus fields --- V1 fixes exact metavariable occurrence
depths with no implicit lifting; V2 adds finite structural judgment
signatures --- and checks untrusted proof trees against them, reconstructing
\code{Type}-valued \code{Derivation} evidence.  Judgments are ordinary
\code{Pattern} terms of the same syntax IR, so the proof lane introduces no
second syntax authority.

The CertificateGSLT modules
(\codepath{Mettapedia/GSLT/LanguageDef/}, canonical import
\code{CertificateGSLTFramework}, compiled fixtures in
\code{CertificateGSLTInterpretationCanary}) develop the categorical theory of
these validated \code{LanguageDef}s:

\begin{itemize}
\item a strict rule-retaining preorder whose arrows transport derivations,
  checked proofs, and identical chronological proof-DAG artifacts, with a
  compiled negative theorem for missing rules;
\item open derivations with positional premise holes and a proved
  simultaneous-substitution calculus (both unit laws, associativity);
\item a provably non-thin category of derivation-valued interpretations ---
  one primitive rule may be implemented by a composite target proof --- with
  a compiled counterexample showing strict retention cannot express this;
\item set-valued models with contravariant pullback and semantic-transport
  naturality, plus covariant indexed derivation families with concrete
  categories of elements;
\item fuel-free checkers for open proof trees and chronological proof DAGs
  (premise references plus backward node references) with \emph{exact} typed
  reconstruction, and a compiled sharing canary --- two submitted DAG nodes,
  three expanded rule occurrences --- keeping compact evidence identity
  distinct from expanded proof meaning;
\item sharing-preserving substitution of checked DAGs into the premise
  holes of a checked DAG: acceptance preserved, expansion commutes with
  composition exactly, submitted node count exactly additive while
  expanded cost multiplies per citation, with compiled refuters for the
  unshifted and uniformly wired variants;
\item the multisorted-clone structure of open derivations (judgments as
  sorts, proof plugging as simultaneous substitution), the algebraic core of
  a cartesian multicategory.
\end{itemize}

The standalone treatment, with the full theorem inventory and verification
statement, is \code{CertificateGSLT.tex}.  For this companion the operative point
is that any validated \code{LanguageDef} that declares
judgment forms and inference rules is already a CertificateGSLT object: the proof
calculus, interpretation category, models, and both checked evidence formats
come from the framework with no per-language authorship.  The
\code{languageDef!} macro does not yet author the proof-calculus fields;
extending it is a natural seam alongside the next steps below.

\section{TPTP and TSTP as Composed GSLTs}
\label{sec:tptp-gslt}

The TPTP language family is a useful stress test for the preceding claims.
It combines a large versioned concrete grammar, binder-sensitive logical
syntax, semantics-changing transformations such as Skolemization, and TSTP
derivation DAGs whose inference annotations name many different calculi
\cite{tptp-language,tstp-derivations}.  The key architectural separation is:

\begin{quote}
TPTP owns the standard syntax, formula roles, source annotations, and
derivation graph.  It does not own one privileged proof-search strategy or
one universal inference calculus.
\end{quote}

Accordingly, \code{lib\_tptp} is a format, transformation, and verification
library.  Resolution search remains an ordinary MeTTa program in the
examples tree.  A TSTP inference is accepted only by an explicitly selected
calculus service carrying its own semantic soundness theorem.

\subsection{FOF to official CNF: transformations are languages}

The current first-order pipeline is not one opaque clausifier.  Every
intermediate object is a typed carrier, and every computational arrow is an
source \code{LanguageDef} connected to an independent recursive semantics
by exact agreement and no-invention theorems:

\begin{center}
\begin{tikzpicture}[node distance=3.8mm,
  box/.style={draw, rounded corners, align=center, inner sep=4pt,
    font=\small}]
\node[box] (ast) {official TPTP FOF AST};
\node[box, below=of ast] (named) {named FOF};
\node[box, below=of named] (resolved) {binder-resolved FOF\\
  {\scriptsize nearest binder; free variables rejected}};
\node[box, below=of resolved] (normal) {NNF $\to$ prenex $\to$ Skolem form\\
  {\scriptsize semantic preservation/equisatisfiability at each seam}};
\node[box, below=of normal] (cnf) {definitional CNF batches\\
  {\scriptsize source identity and polarity retained}};
\node[box, below=of cnf] (allocated) {collision-free lexical allocation};
\node[box, below=of allocated] (official) {official TPTP CNF AST};
\draw[-{Stealth}] (ast) -- (named)
  node[midway,right,font=\scriptsize]{exact elaboration};
\draw[-{Stealth}] (named) -- (resolved)
  node[midway,right,font=\scriptsize]{capture avoidance};
\draw[-{Stealth}] (resolved) -- (normal)
  node[midway,right,font=\scriptsize]{LanguageDef transformations};
\draw[-{Stealth}] (normal) -- (cnf)
  node[midway,right,font=\scriptsize]{named definitions};
\draw[-{Stealth}] (cnf) -- (allocated)
  node[midway,right,font=\scriptsize]{finite plan};
\draw[-{Stealth}] (allocated) -- (official)
  node[midway,right,font=\scriptsize]{42-rule serializer};
\end{tikzpicture}
\end{center}

For example, the concrete source fixture

\begin{lstlisting}[language={},caption={Nested shadowing at the official FOF boundary}]
fof(shadowing,axiom,![X]:(p(X)=>?[X]:q(X))).
\end{lstlisting}

\noindent
is elaborated to named FOF and then to a locally nameless formula in which
the occurrence in \code{q(X)} resolves to the inner existential binder,
not the outer universal binder.  The composed theorem
\code{endToEndExact\_exists} then obtains an exact official-CNF-AST
serialization result for every evidence-bearing admitted batch.  Its proof
passes source-readiness obligations through binder resolution, NNF,
de~Bruijn shifting, prenexing, Skolemization, definitional naming, and
lexical allocation.  Numeric terms with arguments and nullary defined
predicates are negative controls: they are proved outside the serialization
domain rather than silently rendered.

This endpoint is deliberately an \emph{official abstract syntax tree}.
A canonical AST-to-concrete-text printer and parser/printer round trip are
still separate obligations; the theorem does not call an AST ``a \code{.p}
file.''

\subsection{TSTP derivations: graph format outside, calculus inside}

The first closed verification witness is ground resolution.  The admitted
fixture uses ordinary TSTP inference annotations:

\begin{lstlisting}[language={},caption={A small status-carrying TSTP refutation}]
...
\noindent
The verification architecture keeps the TSTP status and calculus meanings
distinct:
...
\node[box] (dag) {admitted official TSTP DAG\\
  {\scriptsize names, roles, sources, statuses, parents}};
\node[box, below=of dag] (status) {status/provenance-indexed semantic program};
\node[box, below=of status] (words) {compiler-returned finite word artifact\\
  {\scriptsize proof-local arenas and compact records}};
\node[box, below=of words] (trace) {exact derivation-word GSLT trace};
\node[box, below=of trace] (meaning) {verified empty root $\Rightarrow$
  problem unsatisfiable};
\node[service, right=12mm of status] (service)
  {selected calculus service\\
   {\scriptsize inference check $+$ soundness theorem}};
\draw[-{Stealth}] (dag) -- (status);
\draw[-{Stealth}] (status) -- (words)
  node[midway,right,font=\scriptsize]{compile/decode exactness};
\draw[-{Stealth}] (words) -- (trace)
  node[midway,right,font=\scriptsize]{step simulation};
\draw[-{Stealth}] (trace) -- (meaning)
  node[midway,right,font=\scriptsize]{semantic consequence};
\draw[-{Stealth},dashed] (service) -- (status);
\draw[-{Stealth},dashed] (service) -- (meaning);
\end{tikzpicture}
\end{center}

The generic finite compiler allocates arenas for formulas, rules, evidence,
provenance, obligations, and service state, then emits seven compact records
for this canary.  \code{wordArtifact\_decodes} recovers the exact semantic
program.  A program-local state-stability theorem supplies the dynamic arena
obligation.  The shared execution invariant proves a continuous exact
rewrite trace in the actual derivation-word \code{LanguageDef};
\code{wordTrace\_unsatisfiable} derives unsatisfiability from the
calculus service's independent soundness invariant.  Out-of-arena formula
references, malformed parent counts, and unknown opcodes are rejected.

The binary layer is thus a compact representation of a derivation checker,
not another handwritten logical authority.  It also exposes the intended
native compilation shape: a compiler-returned artifact is retained and
reused, rather than rebuilding finite arenas by normalization in every
client theorem.

\paragraph{Current boundary.}
The closed semantic result covers this official ground-resolution profile.
It does not yet verify arbitrary TSTP rule names, \code{status(cth)}, or
\code{status(esa)}.  General TSTP verification requires a registry of
declared calculus services and a status-sensitive composition law; typed
TFF/TCF/THF/NHF carriers, include-DAG resolution, external E/Vampire proof
reconstruction, concrete printing, and generated StructuredC execution
remain separate work.  Whole-language validation also needs compositional
per-row certificates so unchanged \code{LanguageDef}s are not repeatedly
renormalized.
...
\item \code{languageDef!} macro with block-syntax parsing, carrier-aware types,
  declared binder preservation, syntax metasyntax operators, semantic validation,
...
\item Generic inference checker over \code{LanguageDef} judgments/rules,
  with the CertificateGSLT categorical layer (strict refinement, open derivations,
  non-thin interpretations, models, exact DAG evidence);
  Section~\ref{sec:certificategslt} and the standalone \code{CertificateGSLT.tex}.
\item TPTP FOF elaboration through binder resolution, NNF, prenexing,
  Skolemization, definitional CNF, collision-free allocation, and exact
  official-CNF-AST serialization; plus one status-indexed official ground
  TSTP refutation compiled to and executed by the derivation-word
  GSLT (Section~\ref{sec:tptp-gslt}).
\end{itemize}
...
\item Full Hyperon atomspace/runtime integration (beyond encoded relation
  environment subset).
\item General TSTP calculus-service coverage, typed TFF/TCF/THF/NHF
  semantics, concrete TPTP printing and round trips, E/Vampire proof
  reconstruction, and generated native execution of the derivation-word
  checker.
\end{itemize}
...
\item Replace whole-language validation normalization in the derivation
  machines with compositional per-row certificates and transport.
\item Generalize the TSTP word compiler to finite evolving service-state
  arenas, then instantiate further status-sensitive calculus services.
\item Complete concrete TPTP printing and E/Vampire TSTP reconstruction
  before extending the semantic pipeline through TFF and THF.
...
\textbf{B — Elaboration boundary} &
  A (not necessarily trusted) elaborator translating MeTTa concrete syntax into
...
\begin{itemize}
  ...
  \item least explicit contextual relation \code{ContextualStep.Step}, with an
        exact fuel-indexed compiler and no representation-wide descent rule;
...
\subsection{Concrete Syntax (S-Expression Style)}

The kernel interface can remain MeTTa-shaped:

\begin{lstlisting}[language={},caption={Proposed MeTTa-Pure source forms}]
...
At Layer C, \code{=} stays the runtime rewrite operator; the syntax \code{:}
...
\item \codepath{MQCalculus/PaperMap.lean} --- stable audit map (paper $\to$ theorem)
\end{itemize}
...
\subsection*{C. Paper-map audit map}
...
\bibitem{gratzer2020multimodal}
D.~Gratzer, G.~A. Kavvos, A.~Nordvall Forsberg, and L.~Birkedal.
\newblock Multimodal dependent type theory.
\newblock In \emph{Proceedings of the 35th Annual ACM/IEEE Symposium on
  Logic in Computer Science (LICS 2020)}, pages 492--506. ACM, 2020.

\bibitem{tptp-language}
G.~Sutcliffe.
\newblock The TPTP language and syntax BNF.
\newblock TPTP World user documentation,
  \url{https://tptp.org/UserDocs/TPTPLanguage/TPTPLanguage.shtml}.

\bibitem{tstp-derivations}
G.~Sutcliffe.
\newblock The TPTP format for derivations.
\newblock TPTP World quick guide,
