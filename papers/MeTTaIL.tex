...
\section{The ProofGSLT Layer: Generic Proof Checking over Presentations}
\label{sec:proofgslt}
...
The ProofGSLT modules
(\codepath{Mettapedia/GSLT/LanguageDef/}, canonical import
\code{ProofGSLTFramework}, compiled fixtures in
\code{ProofGSLTInterpretationCanary}) develop the categorical theory of
...
statement, is \code{ProofGSLT.tex}.  For this companion the operative point
is that any validated presentation whose \code{LanguageDef} declares
judgment forms and inference rules is already a ProofGSLT object: the proof
...
\item Generic inference checker over \code{LanguageDef} judgments/rules,
  with the ProofGSLT categorical layer (strict refinement, open derivations,
  non-thin interpretations, models, exact DAG evidence);
  Section~\ref{sec:proofgslt} and the standalone \code{ProofGSLT.tex}.
\end{itemize}
