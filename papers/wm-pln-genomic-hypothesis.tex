...
is verified to machine precision on the same benchmark harness.
\end{abstract}
...
theorem live in \code{BioHypothesisGeneration.lean} (${\sim}550$~lines,
...
The module \code{BioHypothesisGeneration.lean} proves the \emph{noisy-OR
...
To verify that the WM-PLN runtime reproduces the ProbLog scoring baseline
...
recover LIFT's learned advantage on this particular benchmark harness.
...
(\code{BioHypothesisGeneration}), syntactic compilation
