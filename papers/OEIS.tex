...
Across the sealed generation-nine/ten run set, the cross-world union contains $1{,}617$
...
\subsection{An equal-work joint-retraining control}
\label{sec:joint-retraining-control}

Four rounds compared credit rules under continuation: every arm inherited its own weights
across rounds. Nothing in that design tests continuation itself. Two further lineages were
therefore registered as observers, one per BP carrier, each trained from the authenticated
initial weights on the frozen final cumulative population for $17{,}500$ updates---the
cumulative budget the continued lineages had spent by round four. Observer proposals were
searched and checked exactly as the registered arms are, and their discoveries were
quarantined from the shared event sequence, so the control cannot alter the lineage it
measures.

The estimand needs stating precisely, because two things change together. The observer
sees the final population from its first update, whereas the continued lineage saw each
round's evidence only once that round produced it. The comparison is therefore between
offline joint retraining on the accumulated corpus and streaming continuation over the
same corpus, not between fresh and inherited weights with the data schedule held fixed.
Snapshots at $10{,}000$, $12{,}500$, and $15{,}000$ updates match the cumulative budgets
of rounds one through three, but they are future-informed budget landmarks rather than
replicas of those rounds; only the terminal comparison is free of that advantage.

Writing $J$ for joint retraining and $C$ for continuation, with every entry recomputed
from the checker's own relation:

\begin{center}\small
\begin{tabular}{@{}clrrrrrr@{}}
\toprule
round & carrier & new $J/C$ & direct $J/C$ & existing $J/C$ & candidates $J/C$
 & yield ratio & $|J\cap C|$ \\
\midrule
...
The round-one evaluation predates the direct-hit field, so both round-one $C$ entries were
recomputed from the generation and coverage relations under the same definition used
elsewhere: a target counts as direct when a program proposed \emph{for} that target solves
it.

Three readings are stable across the eight cells, and one is not.

New-target yield per unique proposed program favours $J$ in every cell, by a factor
between $1.08$ and $1.45$. Joint retraining is the more efficient proposer per candidate
emitted. Retention runs the other way: continuation covers more already-known targets in
seven of eight cells, by $370$ and $270$ targets in the two widest Unified cells, and
total support---new plus existing---favours continuation in six of eight. Efficiency and
breadth are separated, and they point in opposite directions.

Absolute new-target count is architecture-dependent, and this is where care is needed. For
the recurrent carrier $J$ leads in every round, by $+25$, $+89$, $+36$, and $+23$. For the
Unified carrier the sign alternates: $+7$, $-5$, $+8$, $-12$. Only round four is free of
the observer's future-information advantage, and there the carriers disagree, $+23$ against
$-12$, a difference in differences of $+35$. Because the landmark advantage favours $J$,
the round-four Unified deficit is conservative, while the recurrent result is the weaker of
the two, its sign being the one that bias would produce. Four checkpoints of one pair of
training runs are not four replications, and nothing here is a population claim.

Hub robustness separates the round-four results further. With $e_R(p)$ the exclusive
contribution of program $p$ as above, the worst single-program removal costs joint-retrained
Unified $7$ of its $64$ targets and continued Unified $2$ of its $76$; both recurrent cells
lose $2$. Continued Unified's larger count is also the more distributed one, and the
comparison would invert under a hub-discounted reading only for the smaller arm.

Witness redundancy explains why removal costs so little on the recurrent side. Counting new
targets supported by two or more distinct program identities, the round-four Unified cells
carry $19$ of $64$ and $25$ of $76$---about a third---while the recurrent cells carry $14$ of
$225$ and $10$ of $202$, near a twentieth. The recurrent carrier finds many targets each by a
single route; the Unified carrier finds fewer, more often by several. Removing one identity is
cheap in the first case because the targets are nearly disjoint across programs, and cheap in
the second because the survivors re-cover what was removed---the same robustness arising from
opposite relational structures, which is why the bare deletion number needs the redundancy
distribution beside it to be read at all.

The portfolio structure is the most robust feature of the table. The two conditions agree on
very little---the Jaccard index of their new-target sets runs from $0.185$ down to $0.037$,
falling monotonically for the recurrent carrier---so the union exceeds the better single
condition by a factor between $1.54$ and $1.82$ in all eight cells: at round four, $410$
recurrent targets against the best single $225$, and $135$ Unified against $76$. Treating the
conditions as capture and recapture on the reachable undiscovered pool gives Lincoln--Petersen
estimates of $2674$ and $973$ targets at round four. Shared architecture and shared corpus make
the two draws positively dependent, so these are lower bounds on the reachable pool rather than
estimates of it, and the projection caveats of
\leanname{projected\_inclusion\_exclusion} apply to the union counts exactly as before.

The machine-readable comparison has SHA-256
{\scriptsize\ttfamily
78ba544621388a32fa1b0d3e71fb650b55cdb75224f71430ceb27da46011f1f9};
the independent recount from raw checker rows has SHA-256
{\scriptsize\ttfamily
9a85475b7de4301f324121d89f4b9ba6a168c452c3a27c9f307a595dc733ccbf}.

\subsection{Two further rounds, and the corpus the observers cannot enter}
\label{sec:later-rounds}

Two subsequent rounds extended the design: the first added a settling arm on a refined
recurrent cut, the second completed the unified credit-by-history factorial by supplying
its missing cell. Every entry below is recounted from the checker's own rows.

\begin{center}\small
\begin{tabular}{@{}llrrrrr@{}}
\toprule
round & arm & new & existing & candidates & new/1k & direct \\
\midrule
5 & recurrent BP, continued & 240 & 969 & 2787 & 86.11 & 9 \\
5 & recurrent BP, joint & 256 & 891 & 2913 & 87.88 & 9 \\
5 & recurrent PC, refined cut & 0 & 16 & 1843 & 0.00 & 0 \\
5 & Unified BP, continued & 63 & 1598 & 15080 & 4.18 & 8 \\
5 & Unified BP, joint & 60 & 1733 & 14243 & 4.21 & 12 \\
5 & Unified PC, continued & 35 & 1510 & 13630 & 2.57 & 11 \\
\midrule
6 & recurrent BP, continued & 200 & 1080 & 2805 & 71.30 & 11 \\
6 & recurrent BP, joint & 204 & 1007 & 2810 & 72.60 & 9 \\
6 & Unified BP, continued & 29 & 1442 & 12898 & 2.25 & 11 \\
6 & Unified BP, joint & 94 & 1984 & 17373 & 5.41 & 14 \\
6 & Unified PC, continued & 25 & 1475 & 12707 & 1.97 & 10 \\
6 & Unified PC, joint & 44 & 1782 & 17021 & 2.59 & 12 \\
\bottomrule
\end{tabular}
\end{center}

Three readings matter for the loop rather than for the learning rules. The settling arm on
the refined cut returned \emph{zero} new targets from $1{,}843$ candidates, so a round's
arm can consume a full budget and contribute nothing the checker accepts; a discovery
campaign must be able to record that without ambiguity, which is what the fixed
denominators provide. The recurrent carrier remains the collateral-discovery engine: of
the $251$ targets sealed in the later event, $200$ came from the recurrent arm and were
private to it, $29$ and $25$ from the two unified arms, with only three shared between
them, so architectures that differ sharply in global count remain complementary in
coverage.

The third reading is an accounting problem the control created and the registration must
answer. Quarantined observer arms produced $382$ fact witnesses over $339$ target
identities, of which $322$ appear nowhere in the eligible union. Those targets are
verified by the same checker under the same protocol; they are simply ineligible, because
admitting them would let a measurement contaminate the lineage it exists to measure. The
descriptive union across all arms is $573$ targets while the sealed event carries $251$.
Both numbers are correct under their own declaration, and reporting either without the
other misstates the campaign: one confuses what was found with what was admitted, the
other discards evidence that a later prospectively declared pool could legitimately
import. The selected ledger itself partitions into $126$ exact reproductions of catalog
programs and $125$ program identities new to the reference catalog, with no selected row a
known program merely reattributed to another sequence.

\subsection{Target-first replay with a verifier-indexed action datastore}
\label{sec:action-datastore-replay}

A subsequent three-seed study tested a more economical use of the accumulated solution
pool.  Its unit of sampling was a sequence, followed by one unique program generating that
sequence; duplicate receipts did not multiply a program--target relation.  Baseline replay
sampled the accumulated alternatives, while verifier-certified hindsight replay added
deduplicated programs that had been proposed for one query but independently checked as
solving another target.  The eligible population comprised $250$ current checker targets
and four registered replay references, not the full accumulated generation-19 catalog.

Two TGAD backbones were held fixed while a small retrieval module varied along two axes:
feedforward versus fixed-depth recurrent refinement, and backpropagation versus local
predictive coding.  At $500$ updates the eight baseline/hindsight pairs had new-target
effects distributed as eighteen zero, three $+1$, and three $-2$ over the twenty-four
paired runs; the net effect was $-3$, and both replay portfolios covered the same $34$
target identities.  A frozen rule selected a pair when any seed differed in a registered
endpoint, exact generated-program set, or retrieval-cascade count.  All eight pairs
qualified, were replicated from their common parents, and were extended to $2{,}500$
updates.  The larger-budget result is therefore selection-conditioned, and its fresh
searches are separate candidate draws rather than an independent confirmation of the
$500$-update sign.  The checker recount, summed over three paired seeds, is:

\begin{center}\small
\begin{tabular}{@{}llrrrr@{}}
\toprule
backbone & local module & baseline new & hindsight new & difference & candidates B/H \\
\midrule
TGAD--GRU & feedforward BP & 24 & 25 & $+1$ & $9150/9044$ \\
TGAD--GRU & feedforward PC & 24 & 26 & $+2$ & $9002/8854$ \\
TGAD--GRU & recurrent BP & 24 & 25 & $+1$ & $9154/9044$ \\
TGAD--GRU & recurrent PC & 24 & 27 & $+3$ & $8835/8804$ \\
TGAD--Transformer & feedforward BP & 26 & 26 & $0$ & $8376/8619$ \\
TGAD--Transformer & feedforward PC & 26 & 26 & $0$ & $8376/8510$ \\
TGAD--Transformer & recurrent BP & 26 & 26 & $0$ & $8362/8593$ \\
TGAD--Transformer & recurrent PC & 26 & 26 & $0$ & $8203/8255$ \\
\bottomrule
\end{tabular}
\end{center}

The paired count effects are eighteen zero, five $+1$, and one $+2$, with no negative
count effect; the mean is $+0.292$ target, or $+0.051$ target per thousand unique
candidates.  Candidate-normalized signs are less uniform because the replay conditions
produce different candidate counts: fourteen positive and ten negative.  Direct
query-conditioned hits are present---six in each aggregate TGAD--GRU condition and one in
each TGAD--Transformer condition---but their paired treatment effect is zero in all
twenty-four runs.  Leave-one-program-out coverage changes by exactly the same amount as
raw coverage, so no single hub program erases the paired differences.

The portfolio statement is both more robust and more modest than the mean.  Baseline
replay covered $30$ new target identities, hindsight replay $32$, with the baseline set a
subset of the hindsight set.  The two additions were A6355 and A266257, both at seed $43$
under TGAD--GRU.  A266257 was witnessed by the same program in all four GRU hindsight
conditions.  A6355 appeared only under recurrent PC with hindsight replay and had two
distinct program witnesses.  Every one of these witnesses was generated for query
A293553, so they are collateral discoveries rather than query-conditioned successes.
This concentration forbids a population-level claim from three paired seeds, but it
identifies exact targets and witnesses for a cheaper replication.

The complete study---initial factorial, $500$-update replay factorial, and
$2{,}500$-update replication-and-extension---used $126$ training runs, $159{,}000$
optimizer updates, and $1{,}860{,}746$ objective evaluations, or $11.62$ summed
GPU-hours.  One fifth of the last phase was a byte-exact recomputation of the already
authenticated $500$-update prefix.  That duplication bought an integrity check but was
not necessary for the estimand; future designs should continue from the authenticated
checkpoint.  The registered direct query-edge conserved-credit condition is not hidden
inside these results: it had zero eligible direct receipts, changed no retrieval weights,
and was stopped before checker evaluation.  The executed contrast was hindsight replay,
not learned retrieval weighting.

\subsection{Behaviour-preserving translation and a three-language panel}
\label{sec:cross-language-translation}

Comparing synthesis across target languages requires the languages to be posed the same
problems, which in turn requires programs that provably compute the same sequences.  We
built two compositional translators from the synthesis DSL---one into PeTTa, one into
MM2---and admitted a translated program only after its first twenty terms were recomputed
by the target engine and matched the reference evaluator exactly.  Behaviour preservation
is thus established per witness rather than argued once for the translation scheme, and
the resulting corpora are verified by construction: $7{,}773$ PeTTa and $5{,}888$ MM2
programs covering $5{,}265$ behaviours expressible in all three languages.

Two semantic details dominated correctness, and both are invisible until a witness
disagrees.  First, the DSL's values are integer \emph{stacks}, not integers: the binary
operations retain the left operand's tail, and \code{push} and \code{pop} edit the stack,
so any scalar translation is wrong on precisely the structurally interesting programs.
The PeTTa translation is stack-faithful throughout; the MM2 translation carries a scalar
backend for the push-free fragment---$81.8\%$ of the pool, where every stack is provably a
singleton and \code{pop} is the identity---and a stack backend otherwise.  Second, host
arithmetic does not agree with the reference: the DSL floors division and modulo, whereas
the MM2 grounding truncates division and takes the dividend's sign for modulo, and its
modulo aborts rather than faulting on a zero divisor.  Both were corrected branchlessly
and gated, so a demanded division by zero yields no result---mirroring a reference
fault---instead of destroying the run.  These are the ordinary hazards of semantic
preservation, and the per-witness gate is what converts them from silent corruption into
a rejected translation.

On this shared basis a twelve-cell panel crossed the three languages with ordinary versus
structurally constrained decoding at two seeds, training on $4{,}988$ behaviours (a
registered MM2 corpus cap excluded $277$) and holding out $508$.  The constrained arm
guarantees structural completion, which the panel showed to be strictly weaker than
producing an operational program: conditional conversion from complete tree to executing
program fell by $1.5$, $2.1$ and $7.2$ points for A--P, PeTTa and MM2, while absolute
operational output rose ($+12.8\%$, $+11.7\%$, $+8.7\%$) and distinct whole-OEIS coverage
was flat for A--P ($-0.2\%$) but rose for PeTTa ($+8.7\%$) and MM2 ($+14.9\%$).  The two
views were strongly complementary---within-language unions exceeded the better single arm
by $46$--$57\%$, and the union across all twelve cells contains $5{,}567$ confirmed
A-numbers.

The design bounds its own conclusions in a way worth stating plainly.  Because the PeTTa
and MM2 corpora are translations, every target carries the hash of the DSL program it was
compiled from, and the three corpora encode one program population in three notations
rather than three populations.  The idioms that distinguish the languages---PeTTa's
equational definitions, MM2's saturation and reflection---are therefore absent from
training, and median target length inflates from $16$ to $178$ and $680$ decoder actions
under translation.  That inflation is ranked identically to both the parameter increment
of the treatment ($1.0\%$, $12.7\%$, $30.2\%$) and its measured effect, so with three
languages the panel cannot separate authored language structure from target length or
capacity.  Stratifying held-out behaviours by the action length of their reference
programs decides between them on the recorded evidence: the structural gain rises with
target length within every language ($-2.9\%$ to $+1.2\%$ for A--P, $+4.8\%$ to $+16.1\%$
for PeTTa, $+13.7\%$ to $+44.2\%$ for MM2), and since capacity and provenance are constant
inside a language, a single length-dependent curve accounts for the cross-language
gradient.  What the panel measures is thus largely the effect of translation-inflated
target length, not of the target languages themselves.  Ranking these languages as
synthesis targets additionally confounds provenance
with simplicity, since the DSL is at once the population's home language and the most
compact of the three.  Both limitations have the same remedy: natively authored programs
in the non-DSL corpora, which is also the only route by which the behaviour-keyed pool
becomes genuinely multilingual rather than a translation image.
