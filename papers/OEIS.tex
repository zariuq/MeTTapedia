...
\code{canonicalize\_sound} and \code{orgCanonicalize\_sound}. Thus, for example, sorting the
...
modalities. A machine-checked counterexample already settles finite-probe rigidity in the
negative: two programs can agree on a finite probe while differing extensionally. This is
...
mechanism the fit-does-not-imply-guidance theorem predicts under a fixed search budget.
...
across sufficient fuel. Finally, the verified program is not a manual Lean
...
reader can reconstruct exactly which published text was translated.
...
genuinely separate source, and only that can move the second link.
