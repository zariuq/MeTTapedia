...
\item \textbf{Grammar coverage for all prose.}  The V1 parseback grammar is
  generated-text-derived from the current generated set.  Sentences outside that
  generated-text language would fail parseback, not pass it incorrectly.
...
\item \textbf{Shared semantic claim grammar.}
  Replace the generated-text-derived parseback grammar with a structured grammar over
...
\code{roundtrip\_mismatch} & Parsed tree linearized to a different text from the original. \\
\code{coverage\_gap} & Claim-like line is outside the current parseback claim family. \\
\code{semantic\_gap} & Source text passes parseback but fails shared semantic-grammar alignment (V2). \\
\bottomrule
\end{longtable}
