...
computation, so conversion is capability-gated: grounded operations
reduce inside types only from an explicit list of deterministic,
effect-free operations, and anything effectful (space mutation, I/O,
foreign calls) is refused with
\code{(he-reject (type-computation-inadmissible))} and never executes.
The typing operations' declared types stage their term and type
arguments (\code{Atom} parameters), so argument evaluation cannot run
an effect before the checker rules on it.  User functions marked
\code{type-level-function} evaluate against a fuel-bounded snapshot
whose space effects are discarded --- a scoped experimental policy,
stated as such, not a purity judgment.
