...
\paragraph{Purses and funded reactions.} In the \code{.rho} surface, \code{\{P\}alice} is
...
nonempty list of \emph{funding} records naming the purse surface and head signature of each
...
The engine's emission order is just one linearization of that partial order; any two
schedules of the same reactions yield the same pomset, so \emph{schedule-invariance holds by
construction} rather than by a theorem about the scheduler (the Lean theorem
\code{boundedBudgetedCausalPrefix\_emission\_linearizes} confirms the emitted sequence
linearizes the causal order). Locations make purses \emph{fibers} over channels: funding
...
\paragraph{Certified correspondence.} The declarative side lives in Lean (the
\href{https://github.com/zariuq/MeTTapedia/tree/2e832da612270ffc5cb9db52e5d835ec94716f4d/lean/mettapedia/Mettapedia/Languages/ProcessCalculi/RhoCalculus/Costed}{\code{Costed/}}
...
\emph{is} a prefix of a declarative cost path with the same receipt and the correct status,
...
\code{costStep\_complete\_runtime\_up\_to\_struct}).

The pure-rho boundary is now theorem-backed as well.  The matcher and substitution are
compiled from the one authored rho language definition and proved to agree with the
established rho GSLT.  On the derived binder-safe fragment, initialization and funded
reduction preserve scope; each declarative \code{CostStep} erases to one pure
\textsc{comm}, and each declarative funded path erases to a rho rewrite path whose length is
exactly its ordered demand-receipt length.  The equivalence used here is pure structural
congruence: no executable free-Drop step is admitted.  The canonical configuration erasure
is defined by quotient lifting rather than by a noncomputable representative choice.  A
rerunnable Lean audit prints the axioms of the exported results; they use only Lean's
...
occurrences, and causal receipt linearization.  These gates, together with thread and memory
sanitizers, are implementation evidence; the Lean step/path theorem above supplies the
declarative semantic boundary.

\paragraph{One root and the corrected algebra.} The profile implements funded rho, not a
second rho calculus and not a Datalog architecture.  Its pure syntax and \textsc{comm} rule
come from the same authored language definition as the matcher and GSLT bridge.  Other
implementations are bounded encoding references, and their executable free-Drop rule is not
...
~\cite{rhocost-companion}.  The public surface is
...
but the supporting surfaces differ: \PeTTa{} uses its
...
\item \textbf{\PathMap{} algebra (working):}  \CeTTa{} already exposes MORK's
  \PathMap{} substrate through the \code{mork:} surface: the lattice
