...
\emph{generated from the source grammars themselves} (per-language
LanguageDefs compiled to native readers with differential, mutation, and
purity gates --- the pipeline caught two defects in the source grammars
...
checked CertificateGSLT articles, MeTTa inference-kernel (MIK) authority, and
...
An initial \code{lib\_tptp} vertical now exercises the same specification
discipline on a large standard language: a versioned all-family TPTP grammar
generates the native ParserPack and admitted AST/document carriers, while a
Lean development composes source-defined FOF-to-CNF transformations and carries one
status-indexed ground TSTP refutation through a compact derivation-word GSLT
to an independently proved unsatisfiability result.  Calculus-specific proof
search remains example code rather than being built into TPTP or \CeTTa{}.
\end{abstract}
...
\item An executable characterization of the semantic boundary between
  \HE{} and \PeTTa{} on a named pure fragment, together with explicit
  counterexamples at the free-variable, state, and effect boundaries; no
  corpus percentage is promoted to a language theorem.
\item An observer-factored account of non-determinism and mutable state:
  complete-bag observers preserve contents and multiplicity without making
  incidental traversal order semantic; effects remain permissive; and
  \PeTTa{} Hyperpose composes translation-stage callability with a frozen
  definition-occurrence view for each entered call.  \code{with\_mutex},
  \code{transaction}, and \code{with-space-snapshot} provide explicit local
  strengthening rather than a global ban on effectful branches.
...
\item A counted-\PathMap{} query realization whose Rust-backed occurrence
  ...
  declared order, and multiplicity remain observable; append-only index
...
syntactically---to a native $\lambda\Pi$ kernel that decides
$\beta\eta$-conversion directly for the closed, well-scoped fragment,
the same discipline exposed a subtler class: \emph{checker} false
negatives.  The migration keeps the legacy route alive as a separately
compiled test-only oracle (production has no toggle back), and compares
verdicts over a pinned eleven-row corpus
(\code{tests/prime/conformance/closed\_lambda\_pi\_convert.metta}).
Nine rows agree.  Two do not, and in both the legacy verdict is wrong:

\begin{verbatim}
; beta: (\f:(Pi U0 U0). f) (\y:U0. y)  vs  \y:U0. y
!(conversion-status
  (prime-judge &self
    (Convert
      (App (Lam (Pi U0 U0) (idx 0)) (Lam U0 (idx 0)))
      (Lam U0 (idx 0)))))
; native: [Established]     legacy: [Refuted]

; eta: \f:(Pi U0 U0). \x:U0. f x  vs  \f:(Pi U0 U0). f
!(conversion-status
  (prime-judge &self
    (Convert
      (Lam (Pi U0 U0) (Lam U0 (App (idx 1) (idx 0))))
      (Lam (Pi U0 U0) (idx 0)))))
; native: [Established]     legacy: [Refuted]
\end{verbatim}

\noindent
(Terms are de Bruijn: \code{(idx 0)} is the innermost binder.)  The
first pair is one $\beta$-step apart; the second is an
$\eta$-expansion.  The legacy route's normalizer neither reduces this
redex nor knows $\eta$, so it compares two syntactically distinct terms
it wrongly believes normal and \emph{refutes} a true conversion.  The
instructive part is not that a normalizer was incomplete---that is
ordinary---but that its incompleteness was silently promoted to
refutation authority.  An incomplete procedure can honestly answer
``incomplete''; it cannot honestly answer ``no.''
...
migration: the differential gate separates \emph{exact agreements}
(nine rows, including the refutation of unequal well-typed operands,
which is a genuine negative, not a fallback) from \emph{adjudicated
repairs} (the two rows above), and pins both expectation files so the
divergence remains a permanent witness rather than a silent fix---the
same adjudication policy Section~\ref{sec:conformance} applies to
upstream behavioral divergences, now applied to a typing judgment.
Second, the repaired class must earn its authority: mutation canaries
that corrupt the native kernel's context discipline, $\beta$-rule, or
$\eta$-rule are killed by the suite, so the tests demonstrably
constrain the code that produces the new verdicts.  Third, the flip is
class-exact and fail-open: outside the recognized fragment the legacy
route still answers, and exhausting the native budget reports
incomplete rather than silently retrying the other authority---a
verdict must never depend on which checker happened to have budget.
...
across every claim flip.  The pack is, by design, a hand-maintained
...
interface with one admitted eight-rule \code{LanguageDef}: six spatial rules and
...
The bridge in
\href{https://github.com/zariuq/MeTTapedia/blob/main/lean/mettapedia/Mettapedia/OSLF/Framework/NativeTypeCertificateGSLT.lean}{\code{OSLF/Framework/NativeTypeCertificateGSLT.lean}}
...
\subsection{Prime typed construction and request-local native kernels}

The current Prime tranche implements a nontrivial portion of the native
dependent theory directly in C; it has no generated-Lean artifact in its
runtime dependency chain.  NIK does not impose one global ladder of execution
modes.  For each request and dependency revision it considers the licensed
realizations that support the requested capabilities.  A unique strongest
realization is selected only when that request fibre has a licensed common
upgrade for every pair.  Otherwise the incomparable maximal realizations
remain visible and an explicit observation or cost policy selects among them.
Thus native construction, direct decision, admitted flow, and certificate
checking may coexist without pretending that one always subsumes the others.

The RuleMachine remains an untrusted proof producer.  At its ingress boundary,
Prime reconstructs ordinary typed constructors rather than installing
benchmark-specific opcodes or parser authority modes.  The currently recognized
strictly-positive list fragment contains the constructors of
\code{rel:all}, \code{rel:fold}, \code{rel:list:member}, \code{rel:any}, and
\code{rel:case-list}.  Their native values retain recursive proof-tree shape,
intermediate witnesses, ordered answer occurrences, and occurrence identity.
An unrecognized or ill-typed tree obtains no native value; it reaches the
ordinary evidence-bearing checking boundary rather than being silently accepted
or converted into an empty answer.

Two source-grounded Hopper slices exercise this boundary.  The fold slice has
sixteen declared cases (eight positive and eight negative), with all positive
derivations reconstructed natively.  The branching slice has twelve declared
\code{member}/\code{find-dup} cases (seven positive and five negative).  A
separate repeated-member canary produces three ordered occurrences with three
distinct native proof paths, showing that endpoint equality does not collapse
derivation multiplicity.  Malformed \code{member}, \code{any}, and
\code{case-list} trees are rejected by the real checking path.  In the complete
Prime gate this tranche coexists with 395 regular-kernel checks, 352 typed-flow
checks, nine native-calculus-selection checks, and the 90-check fast gate.

This is an implementation and conformance milestone, not a verified C compiler
or a completeness result for all Prime programs.  Leaf terms still enter through
a check-once boundary, and zero-interior-check admitted execution has not yet
replaced every eligible recursive path.  The remaining ILP work extends the same
ordinary-family mechanism to the missing Hopper higher-order and structural
tasks, broadens Popper and IGGP exact images, and qualifies performance only
after answer-bag, provenance, currentness, fault, abstention, and zero-legacy-HE
conditions all remain green.
...
establishes translator correspondence on its explicitly defined fragment;
\CeTTa{}'s runtime correspondence remains a separate executable conformance
claim rather than a blanket theorem about every profile and effect.  The
compiled \code{.act} space format via
...
The mutable-state account is now observer-factored rather than
scheduler-prescriptive.  Complete result observations preserve exact
occurrence multiplicity without promising traversal order; effect traces may
remain permissive; and \PeTTa{} Hyperpose combines translation-stage
callability with a generation-stable view for each entered definition call.
Memory safety and index integrity are runtime obligations even when a user
chooses racy effects.  Mutexes, transactions, and snapshots strengthen local
regions explicitly, while Prime's footprint and fuel structure can justify
more aggressive parallel realizations without narrowing the \PeTTa{}
...
open CertificateGSLT articles cross into MIK authority and NIK replay without
...
\item \textbf{Zero NIK closure:} generate the native checker from the one
  CertificateGSLT acceptance predicate; define authenticated query-row and
...
\item \textbf{Qualify shared \PeTTa{} Hyperpose end to end:} retain the
  translation-stage/per-call-snapshot contract while reviewing the
  revision-bound equation authority and reusable worker lifecycle; select a
  snapshot carrier by allocation and work receipts; recover compiled-plan
  reuse only through an explicit representation morphism; then rerun paired
  ECAN and PeTTaChainer distributions, separating completion, exact bags,
  multiplicity, assertion behavior, median, and tail.
\item \textbf{Versioned heterogeneous realization:} implement the open
  ...
  and a total conservative fallback.  Prime may use proved commutation and
  backend resource compatibility to choose a stronger parallel realization;
  \PeTTa{} continues to permit user-managed effect races while the substrate
  protects representation integrity.  Neither native nor \PathMap{} physics
  becomes language semantics.
...
\item \textbf{Real-project stateful workloads:} restore an end-to-end pattern
  miner driver and keep the official ECAN programs as paired conformance and
  performance portfolios.  Record result bags, multiplicity, effect/assertion
  behavior, completion rate, and tail distributions before making any speed
  claim.
\end{itemize}
