...
Two further pillars extend the architecture: parsers
...
before execution), and \code{--lang prime}, a \emph{distinct} lazy
...
runtime-scope check validates every raw field, including purse surfaces, and is
...
hand-written engine.  The public surface is
...
walkthrough of the calculus, both surfaces, and the profile.
...
The command-line \code{--space-engine pathmap} configuration presently
stores the bootstrap portion in \PathMap{} and then falls back to the
ordinary evaluator space.  It is useful for whole-program conformance but
is not claimed as a pure backend performance result.  All \PathMap{}
timings below use an explicit \code{(new-space pathmap)} and therefore
exercise the backend itself.
...
\CeTTa{}'s explicit \PathMap{} exact-hit cost is dominated by the current
single-pattern adapter and result reconstruction, not by trie lookup.
...
evaluator.  At smaller explicit-store sizes, native \CeTTa{} remains
faster than its present \PathMap{} adapter (31.3\,ms versus 73.6\,ms at
20{,}000 edges).  The appropriate adoption path is therefore to expose
...
\begin{itemize}
\item \textbf{Interning is cluster-free.}  Stored record hashes and the
  slot indices derived from them are distinct roles; an avalanche
  finalizer at the slot boundary prevents structural name families from
  degenerating into shared probe clusters, keeping bulk ingestion
  $O(N)$.
...
grammars themselves}.  Each language's surface syntax is written as a
...
\item \textbf{Per-language ownership.}  The \HE{} family, a two-stage
  ...
  the Prime surface each compile from their \emph{own} LanguageDefs into a
...
\item \textbf{Evidence.}  Compiled-versus-legacy differentials (76/76 on
  the Prime surface, including Unicode, numeric projection, open-name
...
raw-byte characterization theorem remaining future work.
...
This appendix otherwise records design rationale; it does not commit
...
\code{force}, and \code{resample} are the surface (lower-case only);
...
The public suspension-observation surface (constraints and invariants
...
surface syntax; the other-owner forcing protocol; cyclic graph
...
instances of a theorem.  Method: contested surface laws are settled by
...
\item \textbf{What does not change.}  Bag semantics and non-semantic
  ...
  fall-through for surfaces a profile does not provide).
\end{enumerate}
