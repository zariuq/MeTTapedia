...
and a direct-exec API packaging scheduler-level exactness.
%
...
\begin{enumerate}[nosep]
  ...
  \item A \MeTTaIL{} bridge: zero-premise, multi-premise,
        freshness-guarded, and authored contextual reductions imply \MORK{}
        rule firings.  One-hole contexts use a Huet zipper/lens factorization
        (\texttt{LensRel}), while collection shape alone grants no reduction
        authority, with
  ...
  \item A direct-exec API packaging scheduler-level exactness with
...
\texttt{AuthoredContextBridge.lean} & Authored one-hole contexts, exact MORK rule compilation & 321 \\
\texttt{CollectionBridge.lean}    & Collection-shaped lens specialization and negative boundary & 80 \\
...
\texttt{DirectExecInterface.lean} & Scheduler exactness package, PeTTa fragment bridge & 234 \\
...
The \MeTTaIL{} bridge (\texttt{MeTTaILBridge.lean} together with
\texttt{AuthoredContextBridge.lean} and \texttt{CollectionBridge.lean})
connects \MORK{}'s execution model to \MeTTaIL{}'s least reduction generated by
authored rules (\texttt{ContextualStep.Step}).
...
\begin{theorem}[\texttt{stepAt\_compiles\_to\_mork\_fire}]
\label{thm:bridge}
If a bounded \MeTTaIL{} authored contextual derivation relates a source to a
target and both translations are ground, then a rule compiled from exactly
that derivation fires in \MORK{} and replaces the translated source by the
translated target.
\end{theorem}
...
\begin{theorem}[\texttt{languageStep\_implies\_mork\_fireSourceRule}]
\label{thm:multibridge}
If an authored \texttt{ContextualStep.Step} fires in a language whose rules
have only non-contextual, source-translatable premises, and a
\texttt{PremiseChain} witness exists in the workspace, then the corresponding
\texttt{SourceExecRule} compiled from that language fires in
\texttt{fireSourceRule}.
\end{theorem}
...
\begin{theorem}[\texttt{languageStep\_implies\_mork\_fireSourceRuleExt}]
\label{thm:extbridge}
If an authored \texttt{ContextualStep.Step} fires in a language whose rules
have only non-contextual ext-translatable premises
(\texttt{allPremisesTranslatableExt}), a \texttt{PremiseChain} witness exists,
and the extracted guards pass at the final substitution, then the translated
\texttt{SourceExecRule} (with guards) fires in \texttt{fireSourceRule}.
\end{theorem}
...
(\texttt{pln\_step\_implies\_mork\_fireSourceRule} and
\texttt{pln\_guarded\_step\_implies\_mork\_fireSourceRule})
demonstrate the full chain from the least authored contextual relation to
...
a free variable and all premises are non-contextual.  Authored contextual
premises are handled separately by compiling their bounded derivations through
the context bridge (\S\ref{sec:collbridge}).
Beta-redex patterns and non-ground results remain outside the formalized
boundary.
\end{remark}

\subsection{Authored Context and Collection-Shaped Lens Bridge}
\label{sec:collbridge}

\texttt{AuthoredContextBridge.lean}, \texttt{CollectionBridge.lean}, and
\texttt{AtomZipper.lean} prove that an authored one-hole context corresponds to
a \MORK{}-side update through a principled zipper/lens factorization.
Collection shape by itself grants no reduction authority: the language must
contain a rewrite rule whose \texttt{Premise.congruence} explicitly authorizes
the frame.
...
\paragraph{Layer C: Structural context translation.}
The derivative \texttt{OneHoleContext} is translated to a \MORK{}
\texttt{AtomContext}.  Rebuilding commutes with translation, and an
\texttt{AuthoredContextFrame} retains the rule and congruence-premise witness
that authorize the frame.
...
The collection specialization \texttt{collectionContext\_lensRel} proves
that replacing element~$i$ in a \texttt{.expression (ctSym :: elems)}
satisfies \texttt{LensRel}, connecting the zipper decomposition to
the structural derivative.  The stronger
\texttt{authoredCollectionContext\_lensRel} additionally retains the
authorizing language-rule witness.

The canonical authored-context bridge combines the structural and semantic
layers:

\begin{theorem}[\texttt{authoredContextFrame\_lensRel}]
\label{thm:congelemzipper}
An authored one-hole context has an exact \texttt{LensRel} realization after
translation to \MORK{}, and the result retains both the authorizing language
rule and its contextual-premise witness.
\end{theorem}

\paragraph{Layer E: Exact contextual compilation.}
\texttt{compiledContextRules} contains exactly the targets emitted by the
bounded authored contextual compiler, and
\texttt{stepAt\_compiles\_to\_mork\_fire} proves that every such derivation
performs the corresponding translated update.

\paragraph{Layer F: Source-aware contextual compilation.}
\texttt{compiledContextSourceRules} similarly contains exactly the bounded
authored targets as source-aware rules, and
\texttt{stepAt\_compiles\_to\_mork\_sourceRule} proves their workspace update.

\paragraph{Source-rule provenance boundary.}
The rules in \texttt{compiledContextRules} and
\texttt{compiledContextSourceRules} are derived from accepted contextual
reductions; they are not appended as independent language rules.  The ordinary
\texttt{languageDefToSourceExecRules} bridge remains restricted to authored
rules with non-contextual premises.  This keeps contextual authority in the
source language while allowing the backend to realize an accepted contextual
step as a whole-atom replacement.
...
source language is future work.  Non-injective head functions are
excluded by the \texttt{InvertibleHead} predicate.
\end{remark}

%% ============================================================
\section{Direct-Exec API}
\label{sec:directexec}
%% ============================================================

\texttt{DirectExecInterface.lean} (234~lines) packages the scheduler-level
...
\MeTTaIL{} bridge              & Flat rules, multi-premise, freshness guards, authored-context zipper/lens compilation, extended bridge (\texttt{.subst}/rest-var) & Native path-focused execution optimization \\
...
Direct-exec API            & Scheduler exactness, PeTTa eval fragment & add-atom/remove-atom bridge, collection patterns, beta-reduction \\
...
\begin{enumerate}
  ...
  \item \textbf{Native path-focused contextual execution.}
    \label{sec:future}
    The zipper/lens layer provides the structural semantics for
    sub-expression access, and bounded authored contextual steps already
    compile exactly to source-aware whole-atom replacement rules.  A future
    backend optimization could encode the same accepted step with path-aware
    source factors (\texttt{SourceFactor.focus}) and sink actions
    (\texttt{SinkAction.replaceAtPath}); it must prove agreement with the
    authored contextual relation rather than add new reduction authority.
...
module packaging the proven \texttt{morkTranslatable} execution boundary,
...
\texttt{LensRel}, and the lens laws), authored-context bridge theorems
(\texttt{authoredContextLensBridge},
\texttt{contextualRuleCompilation},
\texttt{contextualSourceRuleCompilation},
\texttt{contextualStepFires},
\texttt{contextualStepSourceFires},
\texttt{collectionContextLensBridge}, and
\texttt{authoredCollectionContextLensBridge}),
...
as public \texttt{abbrev}s, providing a stable public API
