...
supplied authored-rule applications belong to the compiled least model;
(4)~\code{PeTTaEval},
...
from \MeTTa{} concrete syntax to SWI-Prolog.
...
\item \textbf{LP kernel} (Sections~\ref{sec:lp-kernel}--\ref{sec:bridge}).
  ...
  proving least-model membership for supplied authored-rule applications.
...
\subsection{Authored Rule-Application Encoding}
...
\begin{theorem}[\code{ruleApplication\_mem\_leastHerbrandModel}]
\label{thm:topRule}
...
\subsection{Authored Contextual Boundary}

The LP compiler does not infer contextual closure from collection
representation.  Such closure would grant every language an accidental
reduction policy.  Instead, contextual authority is expressed by an authored
\code{Premise.congruence} and interpreted by the least relation
\code{ContextualStep.Step}.  An LP backend for that relation must compile the
actual authored contextual rule and prove agreement with it; this module
deliberately introduces no generic collection-descent clauses.

The theorem above is therefore a forward encoding result for a supplied rule
application, not a biconditional between arbitrary LP membership and language
reduction.  The generated clauses are not range-restricted---variables may
occur in a head while the body is empty---so the least model records ground
instances of authored rules and can over-approximate operational reachability.
...
def pettaSpaceToLangDef (s : PeTTaSpace) : LanguageDef where
  rewrites := s.rules   -- LP-safe rules are premise-free, hence root-only
  ...
...
\hline
\multicolumn{4}{l}{\textit{Prolog Goal Language (\code{Languages/Prolog/})}} \\
...
\code{Languages/Prolog/Core.lean} (220 lines) defines a Prolog-style goal language
with 14~constructors covering the ISO Prolog control and meta-predicate
repertoire (re-exported via \code{Languages/Prolog/Prolog.lean} and referenced by
...
(see \code{Mettapedia/PLN/Core/PLNCore.lean} et al.).
...
\begin{itemize}
\item \textbf{Contextual LP compilation and converse direction.} The current
  bridge proves least-model membership for a supplied authored-rule
  application.  A full reduction-level bridge must compile authored
  \code{Premise.congruence} rules rather than collection representation, and a
  converse from LP membership requires range restriction plus a constructive
  lifting theorem from LP witnesses back into \MeTTaIL{} reductions.
  Range restriction (\code{RangeRestriction.lean}) is partially formalized.
...
proved \\
Authored rule-application LP encoding &
\codepath{ruleApplication_mem_leastHerbrandModel} &
proved \\
