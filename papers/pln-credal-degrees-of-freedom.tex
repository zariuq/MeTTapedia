...
pack reproduces every number in the paper. The sealed public Lean endpoint for
the whole characterization is \lpath{Mettapedia/PLN/Core/PLNCanonicalAPI.lean}
under \lid{confidenceCharacterizationEndpointProfile}.
\end{abstract}
...
\lpath{Mettapedia/PLN/TruthValues/PLNConfidenceWeight.lean}; see Appendix~\ref{app:formal}.}
...
\lpath{Mettapedia/ProbabilityTheory/Exchangeability/DeFinettiProjectiveCredalBridge.lean}; full name and
...
\lpath{Mettapedia/PLN/TruthValues/PLNConfidenceDoFCanary.lean};
...
leaving $\kappa$ free (\lpath{Mettapedia/PLN/TruthValues/PLNConfidenceWeight.lean});
strength is \lid{posteriorMeanStrength} (\lpath{Mettapedia/PLN/TruthValues/PLNTruthTower.lean}).}
...
\lpath{Mettapedia/PLN/TruthValues/PLNTruthTower.lean}.} The same counts read differently
...
\lpath{Mettapedia/PLN/TruthValues/PLNTruthTower.lean}.} Strength reads the ratio, confidence the
...
\lpath{Mettapedia/PLN/TruthValues/PLNConfidenceWeight.lean}.} Recovering the evidence count needs only
...
\lid{WidthComplementCompatible} (\lpath{Mettapedia/PLN/TruthValues/PLNTruthTower.lean})
and \lid{walley\_width\_complement\_forces\_plnOdds}
(\lpath{Mettapedia/PLN/TruthValues/PLNConfidenceWeight.lean}).} Setting confidence $=1-\text{width}$
...
\lpath{Mettapedia/PLN/TruthValues/PLNTruthTheoryIndex.lean}.} A real interval $[L,U]$ does not pick its
...
\lid{CredalProjectionTower} in \lpath{Mettapedia/PLN/TruthValues/PLNTruthTower.lean};
\lid{observationCredalSet\_indicatorGamble\_has\_strict\_width\_of\_related\_ne} and
\lid{observationCredalSet\_indicatorGamble\_collapses\_of\_class\_subsingleton} in
\lpath{Mettapedia/PLN/TruthValues/PLNDistinctionCredalBridge.lean}; and the OSLF specialization
\lid{indistObs\_indicatorGamble\_has\_strict\_width} /
\lid{indistObs\_indicatorGamble\_collapses\_of\_class\_subsingleton} in
\lpath{Mettapedia/PLN/Bridges/Languages/PLNDistinctionCredalOSLFBridge.lean}.} Two spreads can agree on one
...
incompatible joint models ``the same'' because their margins match. A second,
operational source of the same width is unresolved observational distinction:
if the observer cannot yet tell two states apart, the relevant truth query can
remain interval-valued until that distinction is resolved.
...
\lpath{Mettapedia/PLN/TruthValues/PLNConfidenceDoFCanary.lean}; see Appendix~\ref{app:formal}.} Feed the \emph{same}
...
\lpath{Mettapedia/PLN/TruthValues/PLNTruthTower.lean}.} A Bayesian interval also carries a
...
\lid{categoricalEncoder\_idmEnvelope\_forced\_by\_aggregate} in
\lpath{Mettapedia/PLN/TruthValues/PLNTruthTower.lean}.} In a multi-class model the total evidence is
...
\lpath{Mettapedia/PLN/TruthValues/PLNConfidenceDoFCanary.lean}.} Evidence can be reweighted for quality,
...
\lpath{Mettapedia/PLN/TruthValues/PLNTruthTower.lean}.} In a built-up model the modeller
...
crown in \lpath{Mettapedia/ProbabilityTheory/Exchangeability/DeFinettiProjectiveCredalBridge.lean}; see
...
\lpath{Mettapedia/ProbabilityTheory/Exchangeability/DeFinettiProjectiveCredalBridge.lean}; see
...
\lpath{Mettapedia/PLN/TruthValues/PLNConfidenceWeightRevision.lean}.} Combining two
...
\lpath{Mettapedia/PLN/TruthValues/PLNConfidenceDoFCanary.lean}.} Same counts, different route, route-specific
...
\lpath{Mettapedia/PLN/TruthValues/PLNAmplitudePhase.lean}.} Some beliefs carry more than
...
For DoF~7 specifically, the pack carries three complementary shadows: the raw
joint-shape loss (\lpath{metta/dof07_credal_shape.metta}), the hidden-joint
distinction shadow (\lpath{metta/dof07b_distinction_credal_shadow.metta}), and
the OSLF observer-resolution shadow
(\lpath{metta/dof07c_oslf_observer_resolution.metta}), where the same target
pattern remains interval-valued under a coarse observational class $\{p,q\}$ but
collapses once the observer class sharpens to $\{p\}$.
...
(the credal stack), \lpath{Mettapedia/ProbabilityTheory/Exchangeability/DeFinettiProjectiveCredalBridge.lean},
