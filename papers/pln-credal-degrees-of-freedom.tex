...
\codepath{Mettapedia.Logic.PLNCanonicalAPI}. It packages the finite forcing
...
\codepath{Mettapedia.Logic.DeFinettiProjectiveCredalBridge} (the F2
...
and are axiom-clean. DoF 14 deliberately points to the existing strict-envelope
theorem rather than inventing a fake ``extreme point'' executable. The table
...
\lpath{Mettapedia/papers/examples/RavenAsymmetricInduction.metta} &
Keep the raven asymmetry example; add explicit confidence readouts for
$5/0$, $5/95$, and $50/950$ under at least two $\kappa$ values. \\

Evidence scale $\kappa$ &
\lpath{Mettapedia/papers/pln-confidence-examples/metta/dof01_kappa_calibration.metta} &
Add a small table showing the same evidence count under $\kappa=1,10,50$.
Use curated clinical, noisy omics, and pathway-mediated labels. \\

Strength prior context &
Lean canaries in \lid{PLNTruthTower} &
Add a PeTTa/CeTTa count example where MLE, uniform prior, and Jeffreys prior
produce different strength displays from the same sparse counts. \\

Confidence chart freedom &
\lid{reserveHalfCoordinate\_not\_walley\_width\_complement} in Lean &
Add a runnable chart comparison: PLN odds chart versus reserve-half display,
same decoded evidence, different displayed confidence. \\

Revision as evidence addition &
\lpath{Mettapedia/papers/pln-confidence-examples/metta/dof17_revision_transport.metta} &
Keep \lid{Truth\_Revision}; add a canary that revising two STVs equals adding
decoded evidence weights and re-encoding. \\

Rule-level confidence transport &
\lpath{Mettapedia/lean/mettapedia/Mettapedia/Logic/PeTTaLibPLNFormalAnalysis.lean} &
Add a PeTTa regression for the raw-min double-damping bug: old formula below
weight-space formula on unit evidence. \\

Interval point selector &
Lean generic ITV selector canary &
Add a three-selector example showing lower, midpoint, and upper produce
different displayed strengths on the same interval. \\

Width-complement range normalization &
Paper convention &
Add binary and dollar-utility examples: binary width uses $1-(U-L)$; bounded
utility uses $1-(U-L)/(b-a)$. \\

Credal shape beyond one interval &
Lean credal projection tower canaries &
Add two small finite credal sets with the same query interval and different
joint/correlation behavior. \\

Backend choice &
Lean typed ITV constructors &
Add side-by-side Bayesian credible interval and Walley IDM interval from the
same counts, with provenance printed in the result. \\

Categorical query choice &
Lean categorical evidence tower &
Add a three-category classifier where total evidence is high but the rare
category remains wide. \\

Feature/evidence discounting &
\lpath{Mettapedia/papers/pln-confidence-examples/metta/dof11_evidence_discounting.metta} &
Add IDF/source-quality examples: duplicate text-mined hits discounted,
curated independent hits amplified. \\

Biomedical route provenance &
\lpath{Mettapedia/papers/pln-confidence-examples/metta/dof18_route_provenance.metta} &
Keep direct-versus-pathway route tests; add route-specific $\kappa$ or
discounting before supported-drug revision. \\

Exchangeable de~Finetti F2 &
Lean F2 crown &
Add a finite-prefix coin/patient-response stream: external readout always
exists; raw all-gambles crown marked as requiring compatibility. Include the
counterexample to pointwise zero-interior necessity as a symbolic canary. \\

Dobrushin F3 collapse &
Lean F3 crown &
Add a weakly coupled sensor-grid story: all finite-query completions agree
under the Dobrushin budget, so the interval collapses. \\

Ising F4 strict width &
Lean F4 crown &
Add a spatial contagion or image-segmentation story: low-temperature coupling
and boundary conditions keep the origin query interval nontrivial. \\

Completion-level extreme point &
Credal-set semantics &
Add a scenario-sampling example where two precise completions are sampled
from the same credal set but the displayed STV remains the envelope summary. \\

Extra hidden structure &
Amplitude/phase Lean boundary &
Add an optional example with two hidden-phase states that share classical STV
but differ under a composition rule that sees phase. \\
\bottomrule
\end{longtable}
