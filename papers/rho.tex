...
pure structural congruence are formalized.  On the binder-safe funded fragment,
initialization and reduction preserve scope, and every declarative funded path
erases to a path of the established pure-rho GSLT with exactly one COMM step per
funded event. Claims are marked as formalized or open; nothing here is asserted
beyond its marker.
\end{abstract}
...
\node[box] (root) {authored definition of pure rho\\(one grammar-and-rules value)};
\node[box, below left=of root] (pure) {pure calculus\\COMM only};
...
\draw[-{Stealth}] (root) -- (pure) node[midway,left,font=\small]{derive};
\draw[-{Stealth}] (root) -- (ext) node[midway,right,font=\small]{derive $+$ delta};
\draw[-{Stealth}] (pure) -- (cost);
\end{tikzpicture}
\end{center}

One authored definition is the sole semantic root. The pure calculus and
each extension are derived from it; extensions import the pure theory and
add a delta, and the pure theory never imports an extension. The cost
profile of the runtime is instantiated from the pure calculus alone. The
direction of these imports is enforced structurally (by module
dependencies and by theorems), not merely by convention.
...
COMM substitution.  Therefore every well-sorted COMM contractum compiled from
the authored rule is a step of the established rho GSLT. \formalized{}
...
derived \emph{binder-safe} judgment.  A quotation seals the surrounding binder,
...
\code{rhoGSLT.Step}.  Induction over declarative paths gives a
\code{rhoGSLT.RewritePath}; its length is exactly the number of funded events
and exactly the length of the path's ordered demand receipt.  Thus erasure
inserts neither administrative reductions nor executable-Drop reductions.
\formalized{}

This is a theorem about the declarative funded relation.  The compiled C engine
is checked independently by differential execution and Lean receipt replay,
including rejection of tampered receipts.  That evidence is deliberately not
presented as a universal theorem about compiled C or about every value accepted
by the low-level raw parser.
...
Authored COMM compiler agreement with rho GSLT & \S7 & formalized \\
Binder-safe initialization and preservation & \S8 & formalized \\
Funded-step erasure to pure rho GSLT & \S8 & formalized \\
Declarative funded paths to pure rewrite paths, exact length & \S8 &
  formalized \\
