...
This is a \textbf{roadmap item}, not current implementation.  Source-syntax
...
\subsection{Source Syntax → Elaborated → Pure + Runtime}

The architecture mirrors Lean's pipeline: source language elaborates to
a shared typed core, which then compiles to two targets:

\begin{center}
\begin{tikzpicture}[node distance=1.2cm and 2.5cm]
  \node[neutralbox] (syntax) {Source MeTTa};
  \node[sharedbox, below=of syntax] (core) {Elaborated MeTTa-Core};
  ...
  \draw[arrw] (syntax) -- node[right,font=\small] {elaborate} (core);
...
\begin{itemize}
\item \textbf{Write once, verify and execute}: Source code elaborates to
