...
\begin{abstract}
We present a Lean~4 formalization of the Markov de~Finetti theorem
(Diaconis--Freedman 1980): every Markov-exchangeable prefix measure on a
finite alphabet with a strongly recurrent probability extension is a mixture
of time-homogeneous Markov chains. The public endpoint is the unconditional
constructor \texttt{MarkovMixture.of\_markovExchangeable\_strongRecurrence};
Lean reports its axiom footprint as exactly
\texttt{[propext, Classical.choice, Quot.sound]}. The active
\texttt{MarkovDeFinetti*.lean} corpus comprises 42,405 lines across 28 files.
Key innovations include a directing measure uniqueness lemma that transfers
de~Finetti structure across measure restrictions, and successor-matrix partial
exchangeability machinery that preserves the initial-state coupling needed by
the cylinder identities. This formalization provides a verified foundation for
structured universal mixtures in the style of Solomonoff induction, where
Markov models replace i.i.d.\ hypotheses for tractable sequential prediction.
\end{abstract}
...
If $\mu$ has a probability extension $P$ whose trajectories satisfy the
formal strong-recurrence condition, then there exists a probability measure
$\pi$ on the space of Markov parameters
...
We present:
\begin{enumerate}
\item A formalization of Theorem~\ref{thm:df80} in Lean~4 with Mathlib,
  exposed through \texttt{MarkovMixture.of\_markovExchangeable\_strongRecurrence}
  and audited by the axiom footprint
  \texttt{[propext, Classical.choice, Quot.sound]}.
\item An active \texttt{MarkovDeFinetti*.lean} corpus of
  \textbf{42,405 lines} across \textbf{28 files}, plus reuse of the local
  \texttt{exchangeability} package for the classical de~Finetti layer.
...
\begin{definition}
A prefix measure $\mu$ is \emph{strongly recurrent} in the formal theorem
theorem set if it admits an extension $P$ to infinite sequences satisfying
\texttt{StrongRecurrence}. This is the recurrence hypothesis used by the
public constructor.
\end{definition}

\begin{lstlisting}
def StrongRecurrence (P : Measure (Nat -> Fin k)) : Prop :=
  forall i : Fin k, forall-ae omega w.r.t. P,
    (exists t : Nat, omega t = i) ->
      forall n : Nat, nthVisitTimeExists omega i n

def StrongRecurrentExtension (mu : PrefixMeasure (Fin k)) : Prop :=
...
The full public theorem derives the Markov-mixture representation from Markov
exchangeability plus a strongly recurrent extension. The endpoint theorem is:

\begin{lstlisting}
theorem markovDeFinetti_strongRecurrence (k : Nat) :
    FortiniSuccessorMatrixInvarianceTheoremStrongRecurrence k
\end{lstlisting}

It is exposed to downstream code through the unconditional public constructor:

\begin{lstlisting}
noncomputable def MarkovMixture.of_markovExchangeable_strongRecurrence
...
The corresponding theorem set is:

\begin{lstlisting}
def FortiniSuccessorMatrixInvarianceTheoremStrongRecurrence
    (k : Nat) : Prop :=
  forall mu : PrefixMeasure (Fin k),
    MarkovExchangeablePrefixMeasure mu ->
    StrongRecurrentExtension mu ->
      exists (pi : Measure (MarkovParam k)),
        IsProbabilityMeasure pi /\
        forall xs, mu xs = integral theta, wordProb theta xs dpi
\end{lstlisting}

The Lean audit for the public constructor reports exactly the standard axiom
footprint \texttt{[propext, Classical.choice, Quot.sound]}; no project-local
axiom or proof placeholder appears in the crown theorem's dependency
footprint.
...
The active Markov de~Finetti core counted here consists of the
\texttt{MarkovDeFinetti*.lean} files in
\texttt{Mettapedia/ProbabilityTheory/Exchangeability/}. The count is obtained
from the current source tree after the public constructor was added.

\begin{table}[h]
\centering
\begin{tabular}{llr}
\toprule
\textbf{Layer} & \textbf{Files} & \textbf{Lines} \\
\midrule
Endpoint and public API & 2 files & 6,605 \\
Successor-matrix PE & 3 files & 5,970 \\
Crux and fiber payload & 5 files & 21,354 \\
Carrier transport and evidence basis & 4 files & 2,067 \\
Euler trail support & 6 files & 1,948 \\
Theorem-set and extension helpers & 8 files & 4,461 \\
\midrule
\textbf{Total} & \textbf{28 files} & \textbf{42,405} \\
\bottomrule
\end{tabular}
...
The formalization builds on the local \texttt{exchangeability} Lake package,
which provides the classical de~Finetti layer:
...
\noindent\textbf{Current theorem set (2026-04):} the post-Markov bridge is
...
\subsubsection{The Markov Xi Transition Theorem Set}

The formalization exposes a direct transition atom theorem set via
...
The public API packages these results in \texttt{ClassRestrictedMarkovMixtureModel},
which carries the extension law, class recurrence hypothesis, and all derived
row-law consequences. The theorem
\texttt{ClassRestrictedMarkovMixtureModel.class\_transition\_structure}
...
\texttt{of\_markovExchangeable\_strongRecurrence}, which discharges the closed
Fortini endpoint and returns a \texttt{MarkovMixture} directly from the public
Markov-exchangeability, extension, and strong-recurrence hypotheses. More
specialized constructors expose internal theorem sets for callers that
already have successor-matrix PE payloads. For class-restricted settings,
\texttt{ClassRestrictedMarkovMixtureModel} provides the analogous object
with honest scope for rows in the class.

\subsection{Kleisli(Giry) Prefix-Law Bridge}

We also connected the measure-theoretic theorem set to the existing
...
This gives an honest categorical restatement of the proved theorem set:
...
\texttt{ProbabilityTheory/Exchangeability/MarkovDeFinettiMomentProblem.lean}:
\begin{itemize}
...
The new file \texttt{ProbabilityTheory/Exchangeability/MarkovDeFinettiSequenceKernelBorel.lean} (1055 lines)
...
\subsection{Categorical Borel-Side Contract}
...
The public factorization contract \texttt{CategoricalBorelMarkovSequenceKernelFactorization}
...
coordinates). We prove the key bridge theorem connecting the two contracts:
...
\item \textbf{Positive example}: the cylinder $[0,0,1]$ on an order-2 binary chain
  now reduces to the honest higher-order probability theorem set via the full
  raw-symbol theorem.
...
This gives a complete finite HMM sequence-law theorem set: the sequence measure
...
\item \textbf{Negative example}: this does not give Baum-Welch, forward-backward,
  or posterior inference; what we proved is the finite sequence-law theorem set.
\end{itemize}
...
The finite HMM theorem set also lifts to the Borel layer, with the latent
...
The action-conditioned extension turns the finite HMM theorem set into a structured
...
filtering and smoothing theorem set for completed action-observation cycles:
\begin{itemize}
...
For controlled finite HMMs, this yields a concrete theorem set, schematically:
\begin{lstlisting}
...
hypothesis would require an additional computable-parameter theorem set, not a
proof cleanup.
...
def higherOrderRowStatistic :
    SufficientStatisticEncoder (HigherOrderObservation k m) (Context k m) (MultiEvidence k)
\end{lstlisting}
...
We have presented a Lean~4 formalization of the Markov de~Finetti theorem
(Diaconis--Freedman 1980), with the public constructor
\texttt{MarkovMixture.of\_markovExchangeable\_strongRecurrence} audited at
the axiom footprint \texttt{[propext, Classical.choice, Quot.sound]}. The
active \texttt{MarkovDeFinetti*.lean} corpus comprises 42,405 lines across
28 files. Key innovations include:
...
\item \textbf{Gap analysis pays off}: A structural audit in April 2026
  ...
  prevent this.
\item \textbf{Reuse existing formalizations}: The local \texttt{exchangeability}
  package was essential. Building row kernels from the directing measure
  machinery avoided reimplementing measure-theoretic foundations.
