...
\item \textbf{source spellings}: a user-written symbol such as
...
\subsection{Lean Certification of the Reaction Semantics}
\label{sec:rho-lean}

The reaction machine's most delicate capability is \emph{eval-at-\textsc{comm}}:
a communication that does not merely move a message but evaluates the
transmitted payload.  The canonical pattern is the two-component process
\[
  \texttt{eval\_drop}(x, p)\;\equiv\;
  x!\big(@p\big)\;\mid\;\mathsf{for}(y \leftarrow x)\{\,*y\,\},
\]
where $@p$ is a deferred (quoted) payload and the receiver immediately drops
($*$, dereferences and runs) whatever it receives.  The single available
\textsc{comm} consumes the message and the drop executes the payload; the
process then runs to quiescence at the evaluated result.

This semantics is formalized in Lean (\code{RhometaReduction.lean}) over the
same locally-nameless syntax, structural congruence, and semantic substitution
used for the rest of the \RhoCalc{} development.  The reduction relation
\code{RhometaReduces} is the rho reaction relation restricted so that the
ordinary core \textsc{comm} cannot silently consume an explicit eval payload,
and \code{RhometaSCObservedOutcomes} collects the
structural-congruence-observable outcomes of its quiescent reducts.  The main
theorem, \code{rhometta\_bridge\_scObserved}, is a \emph{witness-free
operational characterization}: on the ready branch (some payload evaluation is
certified) and for a hash-set-free channel, the concrete quiescent run of
$\texttt{eval\_drop}(x,p)$ equals
\[
  \texttt{RhometaSCObservedOutcomes}\,\big(\texttt{eval\_drop}(x,p)\big),
\]
with \emph{no} certification witness exposed in the carrier.  Earlier bridges
characterized the run only through an explicit \code{CertifiedPayloadResult}
witness; the witness-free form states the run purely in terms of what is
structurally observable---the property a caller of the reaction machine can
actually rely on.

The proof is axiom-clean (\code{\{propext, Classical.choice, Quot.sound\}}, zero
sorries).  Its keystone is a canonical-form argument: a normalizer
\code{nfAtom} that is \emph{complete} for structural congruence (congruent terms
have equal normal forms), which turns the symmetric/transitive structural
closure---where direct inductions stall---into an equation on canonical forms.
The load-bearing substitution lemma carries an opacity side condition---that the
communicated variable does not occur \emph{under a quote}
(\code{noBoundUnderQuote})---and this condition is necessary, not cosmetic:
without it a payload can hide a bound occurrence inside a quoted name and the
characterization fails, as a concrete counterexample witnesses.  The
\code{NoHashSet} hypothesis on the channel records that the result is stated for
the bag-structured reaction fragment, the same opaque-name boundary the
executable index already relies on (quoted process names stay on the fallback
path).

\paragraph{From the bridge to a running cognitive substrate.}
The eval-at-\textsc{comm} bridge is not only a soundness result; it licenses a
programming idiom.  Because a deferred payload may itself evaluate to \emph{more}
rho process, a reaction's payload can plant the next reactions---communication is
at once computation and construction.  Two self-contained demos exercise this on
the executable engine.  A \emph{self-unfolding backward-chaining prover} seeds a
single cell for a goal; each \textsc{comm} evaluates one inference step whose
result is the cells for that goal's subgoals, so the reaction network grows
itself into a proof tree and forks into the may-set wherever a goal has several
rules: \code{rhometta:run} returns one quiescent residual per complete proof
tree (two for the sample goal; a single-rule goal stays confluent at one).  A
larger showcase runs three cognitive services as parallel reactions---a
PLN-style two-hop proof search, overlap-corrected evidence revision across two
agents, and one ECAN-style attention-spreading round---and chains their
channel-gathered results into a final deferred payload that synthesizes one
action plan per proof path.  Every cell in these demos is an \code{eval\_drop},
so \code{rhometta\_bridge\_scObserved} is exactly the theorem that licenses
reading the quiescent residual as the computed result.  Both run on the
\code{make BUILD=core} binary (the showcase additionally importing the shipped
\code{lib\_pln}); they live under \code{benchmarks/rho/rhometta-*}.
...
round-trip fidelity between MM2 source syntax and \CeTTa{}'s internal
representation.
...
The request builder (one per \code{--lang}) normalizes source syntax
...
implementation.  The MAM naming and two-engine strategy remain
design proposals.  The concrete
