#!/usr/bin/env python3
"""Check the authenticated trained-checkpoint full residual replay boundary."""
...
FIXTURE = (
    ...
    / "artifacts"
    / "conformance"
    / "pc_authenticated_full_residual_replay_source_v1.json"
)
LEAN_TARGET = (
    "Mettapedia.MachineLearning.NeuralNetworks.CreditTransport."
    "Float32AuthenticatedThreeHiddenStageResidualReplayInvocation0GeneratedFixture"
)
    ...
    completed = subprocess.run(
        [
            ...
            str(
                source_root
                / "eval"
                / "verify_tgad_authenticated_three_hidden_stage_residual_replay.py"
            ),
            "--json",
        ],
        cwd=source_root,
...
def main() -> None:
    ...
    if args.json:
        ...
        print("authenticated trained-checkpoint full residual replay: PASS")
