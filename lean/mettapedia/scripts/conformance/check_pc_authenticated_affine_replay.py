#!/usr/bin/env python3
"""Check the authenticated trained-checkpoint affine replay boundary."""
...
FIXTURE = (
    ...
    / "artifacts"
    / "conformance"
    / "pc_authenticated_affine_replay_source_v1.json"
)
LEAN_TARGET = (
    "Mettapedia.MachineLearning.NeuralNetworks.CreditTransport."
    "Float32AuthenticatedAffineReplayChunkedGeneratedFixture"
)
...
def run_verifier(source_root: Path, python: str) -> dict[str, Any]:
    ...
    completed = subprocess.run(
        [
            python,
            str(source_root / "eval" / "verify_tgad_authenticated_affine_replay.py"),
            "--json",
        ],
        cwd=source_root,
        env=environment,
...
def main() -> None:
    ...
    if args.json:
        ...
        print("authenticated trained-checkpoint affine replay: PASS")
