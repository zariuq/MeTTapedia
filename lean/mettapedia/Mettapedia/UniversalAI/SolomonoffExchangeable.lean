...
namespace Mettapedia.UniversalAI.SolomonoffExchangeable
...
theorem markovExchangeable_strongRecurrence_restrictClass_rowLaw_factorizes
  ...
  exact
    Mettapedia.ProbabilityTheory.Exchangeability.rowProcessLaw_restrictClass_factorizes_of_markovExchangeable_strongRecurrence
      ...
      (fun r =>
        Measure.pi
          (fun _ : Fin m =>
            (directingRowKernel (k := k) P i r : Measure (Fin k)))) := by
  exact
    Mettapedia.ProbabilityTheory.Exchangeability.rowProcessLaw_restrictClass_factorizes_of_markovExchangeable_strongRecurrenceInClass
...
structure MarkovBinaryPLNDomainConditions
  ...
  lowerSemicomputable :
    Mettapedia.UniversalAI.UniversalPrediction.FiniteAlphabet.LowerSemicomputablePrefixMeasure
      (α := Fin 2) μ
  prior : Fin 2 → Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceDirichlet.DirichletParams 2
    ...
    (∀ n : ℕ,
      ∃ c : ENNReal, c ≠ 0 ∧
        Mettapedia.UniversalAI.UniversalPrediction.FiniteAlphabet.Dominates
          (Mettapedia.UniversalAI.UniversalPrediction.FiniteAlphabet.SolomonoffBridge.M₂
            (α := Fin 2)) μ c ∧
          Mettapedia.UniversalAI.UniversalPrediction.FiniteAlphabet.FiniteHorizon.relEntropy μ
            (Mettapedia.UniversalAI.UniversalPrediction.FiniteAlphabet.SolomonoffBridge.M₂
            (α := Fin 2)) n ≤
...
theorem markovExchangeable_binary_domain_characterization_bool
    ...
    (∀ (xs ys : Mettapedia.UniversalAI.SolomonoffPrior.BinString) (_hx : 0 < xs.length)
      (_hsum : binarySummary xs = binarySummary ys),
...
theorem dirichlet_common_state_update_bool
    (hdom : MarkovBinaryPLNDomainConditions μ)
    (xs ys : Mettapedia.UniversalAI.SolomonoffPrior.BinString) (hx : 0 < xs.length)
    (hsum : binarySummary xs = binarySummary ys) :
    ∃ state : BinarySummaryState,
      binarySummary xs = some state ∧
      binarySummary ys = some state ∧
...
/-- **Practical νPLN Theorem**: For exchangeable Solomonoff priors, counts are sufficient.
    ...
    `Mettapedia.ProbabilityTheory.Exchangeability.DeFinetti` and gives the closed-form Beta-Bernoulli representation.
...
theorem solomonoff_exchangeable_predictBit_same_counts (M : RestrictedSolomonoffPrior) :
  ...
  exact Mettapedia.UniversalAI.SolomonoffExchangeable.RestrictedSolomonoffPrior.predictBit_ofFn_same_counts
    (M := M) xs₁ xs₂ hcount b
...
theorem restrictedSolomonoff_infiniteExchangeable_of_prefixLaw
    ...
    (μ : MeasureTheory.Measure Mettapedia.UniversalAI.SolomonoffPrior.InfBinString)
...
theorem restrictedSolomonoff_infiniteExchangeable_of_noLeakageAtCylindersLaw
    ...
    (hNoLeak :
      Mettapedia.UniversalAI.SolomonoffMeasure.NoLeakageAtCylindersLaw
        (U := M.U) (programs := M.programs) μ) :
    InfiniteExchangeable (fun i ω => ω i) μ := by
  refine restrictedSolomonoff_infiniteExchangeable_of_prefixLaw
    (M := M) (μ := μ) (hμprob := hμprob) ?_
  exact Mettapedia.UniversalAI.SolomonoffMeasure.hprefix_of_noLeakageAtCylindersLaw
...
theorem restrictedSolomonoff_infiniteExchangeable_exists_of_totalOutputOnPrograms
    (M : RestrictedSolomonoffPrior)
    (htot : Mettapedia.UniversalAI.SolomonoffMeasure.TotalOutputOnPrograms M.U M.programs)
    (hroot : M.μ [] = 1) :
    ∃ μ : MeasureTheory.Measure Mettapedia.UniversalAI.SolomonoffPrior.InfBinString,
      ∃ hμprob : MeasureTheory.IsProbabilityMeasure μ,
        μ = Mettapedia.UniversalAI.SolomonoffMeasure.totalOutputProgramMeasure
          (U := M.U) (programs := M.programs) htot ∧
        @InfiniteExchangeable _ _ (fun i ω => ω i) μ hμprob := by
  let μ : MeasureTheory.Measure Mettapedia.UniversalAI.SolomonoffPrior.InfBinString :=
    Mettapedia.UniversalAI.SolomonoffMeasure.totalOutputProgramMeasure
      (U := M.U) (programs := M.programs) htot
...
theorem restrictedSolomonoff_infiniteExchangeable_of_cylinderLaw
    ...
    (hCylinder :
      ∀ x : BinString,
        μ (Mettapedia.UniversalAI.SolomonoffPrior.InfBinString.Cylinder x) =
...
structure RestrictedSolomonoffMarkovWitness (M : RestrictedSolomonoffPrior) where
  law : Mettapedia.UniversalAI.UniversalPrediction.FiniteAlphabet.PrefixMeasure (Fin 2)
  domain : MarkovBinaryPLNDomainConditions law
  prefix_agrees :
    ∀ xs : Mettapedia.UniversalAI.SolomonoffPrior.BinString,
    ...
    (∀ n : ℕ,
      ∃ c : ENNReal, c ≠ 0 ∧
        Mettapedia.UniversalAI.UniversalPrediction.FiniteAlphabet.Dominates
          (Mettapedia.UniversalAI.UniversalPrediction.FiniteAlphabet.SolomonoffBridge.M₂
            (α := Fin 2)) W.law c ∧
          Mettapedia.UniversalAI.UniversalPrediction.FiniteAlphabet.FiniteHorizon.relEntropy W.law
            (Mettapedia.UniversalAI.UniversalPrediction.FiniteAlphabet.SolomonoffBridge.M₂
              (α := Fin 2)) n ≤
            Real.log (1 / c.toReal)) := by
  refine ⟨?_, ?_, ?_⟩
...
theorem restrictedSolomonoff_plnDomainConditions_of_prefixLaw
    ...
    (hprefix :
      ∀ (n : ℕ) (xs : Fin n → Bool),
        μ {ω | ∀ i : Fin n, ω i = xs i} =
          ENNReal.ofReal (M.μ (List.ofFn xs))) :
    Mettapedia.ProbabilityTheory.Exchangeability.DeFinetti.PLNDomainConditions (fun i ω => ω i) μ := by
  simpa [Mettapedia.ProbabilityTheory.Exchangeability.DeFinetti.PLNDomainConditions] using
...
theorem restrictedSolomonoff_nupln_master_chain_of_prefixLaw
    ...
    ∃ (B : Mettapedia.ProbabilityTheory.Exchangeability.DeFinetti.BernoulliMixture),
      ...
      (∀ n_pos n_neg : ℕ,
        Mettapedia.ProbabilityTheory.Exchangeability.evidenceFromCounts n_pos n_neg = (n_pos, n_neg)) ∧
  ...
  exact
    Mettapedia.ProbabilityTheory.Exchangeability.DeFinetti.nupln_master_chain
...
theorem restrictedSolomonoff_nupln_justification_of_prefixLaw
  ...
  have hX : ∀ i : ℕ, Measurable (fun ω : InfBinString => ω i) := by
    ...
    exact measurable_pi_apply i
  exact
    Mettapedia.ProbabilityTheory.Exchangeability.DeFinetti.nupln_justification
      (X := fun i (ω : InfBinString) => ω i)
      (μ := μ)
      hX
      (restrictedSolomonoff_plnDomainConditions_of_prefixLaw
...
end Mettapedia.UniversalAI.SolomonoffExchangeable
