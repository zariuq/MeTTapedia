...
This file is the finite-alphabet analogue of `Mettapedia/UniversalAI/UniversalPrediction/HutterEnumeration.lean`.
...
* `Mettapedia/UniversalAI/UniversalPrediction/FiniteAlphabet/HutterEnumerationTheoremSemimeasure.lean`.
-/
...
noncomputable def xi (E : LSCSemimeasureEnumeration (α := α)) : Semimeasure α :=
  xiSemimeasure (ν := E.eval) (w := Mettapedia.UniversalAI.UniversalPrediction.encodeWeight)
    (hw := Mettapedia.UniversalAI.UniversalPrediction.tsum_encodeWeight_le_one (ι := E.Code))
...
theorem xi_dominates_eval (E : LSCSemimeasureEnumeration (α := α)) (c : E.Code) :
    Dominates (E.xi) (E.eval c) (Mettapedia.UniversalAI.UniversalPrediction.encodeWeight c) := by
  intro x
  -- Termwise dominance inside a `tsum`.
  simpa [LSCSemimeasureEnumeration.xi, xiSemimeasure, xiFun] using
    (xi_dominates_index (ν := E.eval) (w := Mettapedia.UniversalAI.UniversalPrediction.encodeWeight) (i := c) (x := x))

theorem encodeWeight_ne_zero (E : LSCSemimeasureEnumeration (α := α)) (c : E.Code) :
    Mettapedia.UniversalAI.UniversalPrediction.encodeWeight c ≠ 0 := by
  unfold Mettapedia.UniversalAI.UniversalPrediction.encodeWeight
...
theorem relEntropy_le_log_inv_of_LSC (E : LSCSemimeasureEnumeration (α := α)) (μ : PrefixMeasure α)
  ...
  have hdom :
      Dominates (E.xi) μ (Mettapedia.UniversalAI.UniversalPrediction.encodeWeight code) := by
    intro x
    have hdom' : Mettapedia.UniversalAI.UniversalPrediction.encodeWeight code * (E.eval code) x ≤ (E.xi) x := by
      simpa [LSCSemimeasureEnumeration.xi, xiSemimeasure] using
        (xi_dominates_index (ν := E.eval) (w := Mettapedia.UniversalAI.UniversalPrediction.encodeWeight)
          (i := code) (x := x))
  ...
  refine ⟨Mettapedia.UniversalAI.UniversalPrediction.encodeWeight code, E.encodeWeight_ne_zero code, hdom, ?_⟩
