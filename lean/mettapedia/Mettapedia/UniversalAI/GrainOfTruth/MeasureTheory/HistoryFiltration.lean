...
/-- σ-algebra generated by first n steps.
    ...
    Marked `@[reducible]` because it returns a `MeasurableSpace` (a class): this lets it unfold
    at `instances` transparency, as required by Lean 4.31's class-definition lint and by the
    `simp [sigmaAlgebraUpTo]`/`comap`-destructuring used here and in downstream filtration proofs. -/
...
/-- The `Iic`-prefix domain carries the canonical product σ-algebra `MeasurableSpace.pi`.

    `InfiniteHistory.prefixOfMeasurableSpace` (an instance over a generic family `X`) is
    definitionally equal to `MeasurableSpace.pi`, but for a *concrete* family such as
    `StepFamily` it no longer reduces to `MeasurableSpace.pi` at `instances` transparency.
    Mathlib's Ionescu–Tulcea API (`Kernel.traj`, `Kernel.partialTraj`) bakes in the bare
    `MeasurableSpace.pi` for `Iic`-indexed prefixes, so we make that the winning, *reducible*
    instance here. This keeps every prefix measure/kernel definitionally `MeasurableSpace.pi`,
    eliminating the diamond that otherwise breaks `rw`/`simp`/instance synthesis. -/
@[reducible] instance instMeasurableSpaceIicStepFamily (n : ℕ) :
    MeasurableSpace (Π i : Iic n, StepFamily i) := MeasurableSpace.pi
...
theorem initialStepMeasure_isProbability (μ : Environment) (h_stoch : isStochastic μ) :
...
theorem initialStepMeasureWithPolicy_isProbability (μ : Environment) (π : Agent)
    (h_stoch : isStochastic μ) :
...
noncomputable def environmentMeasureWithPolicy (μ : Environment) (π : Agent) (h_stoch : isStochastic μ) :
  ...
  -- Ionescu–Tulcea via `Kernel.traj` directly. Since the `Iic`-prefix domain carries the
  -- canonical `MeasurableSpace.pi` (see `instMeasurableSpaceIicStepFamily`), `Kernel.traj`
  -- and the transition kernels agree definitionally at `instances` transparency.
  haveI : ∀ n, IsMarkovKernel (transitionKernelWithPolicy μ π n) :=
    fun n => transitionKernelWithPolicy_isMarkov μ π n h_stoch
  Kernel.traj (transitionKernelWithPolicy μ π) 0 ∘ₘ initialMeasureIic0WithPolicy μ π
...
noncomputable def environmentMeasure (μ : Environment) (h_stoch : isStochastic μ) :
    MeasureTheory.Measure Trajectory :=
  -- Apply Ionescu–Tulcea via `Kernel.traj` directly (see `environmentMeasureWithPolicy`):
  -- the `Iic`-prefix domain carries the canonical `MeasurableSpace.pi`, so `Kernel.traj`
  -- and the transition kernels agree definitionally at `instances` transparency.
  haveI : ∀ n, IsMarkovKernel (transitionKernel μ n) :=
    fun n => transitionKernel_isMarkov μ n h_stoch
  Kernel.traj (transitionKernel μ) 0 ∘ₘ initialMeasureIic0 μ
...
theorem cylinderSetAt_measurable (t : ℕ) (h : History) :
  ...
  -- Express as preimage under truncate (the goal is a `comap` membership, i.e. `∃ t', …`)
...
theorem measurable_wrt_filtration_iff (f : Trajectory → ℝ) (t : ℕ) :
  ...
  · -- (→) If measurable w.r.t. ℱ_t = comap (truncate t) ⊤, then depends only on first t steps
    ...
    -- `MeasurableSet[comap (truncate t) ⊤]` unfolds definitionally to `∃ s', …`, so destructure directly.
  ...
  · -- (←) If depends only on first t steps, then measurable w.r.t. ℱ_t
    ...
    unfold Measurable
    intro B hB
    -- Need: f⁻¹(B) is in comap (truncate t) ⊤; this goal is the `comap` existential up to defeq.
