...
namespace Mettapedia.ProbabilityTheory.Exchangeability.MarkovDeFinettiHard
...
open Mettapedia.ProbabilityTheory.Exchangeability.PerRowJointPE
open Mettapedia.ProbabilityTheory.Exchangeability.MarkovDeFinettiHardBESTCore
open Mettapedia.ProbabilityTheory.Exchangeability.MarkovDeFinettiHardEulerTrails
open Mettapedia.ProbabilityTheory.Exchangeability.MarkovDeFinettiHardCopyPerm
...
theorem ae_tendsto_rowProcessEmpiricalFreq_to_directingRowKernel
  ...
  have hY_contr : Exchangeability.Contractable ρ Y :=
    Mettapedia.ProbabilityTheory.Exchangeability.DirectingMeasureL1Transfer.contractable_comp_measurable
  ...
  have htail : Exchangeability.Tail.tailProcess Y = tailSigma X := by
    rw [Mettapedia.ProbabilityTheory.Exchangeability.DirectingMeasureL1Transfer.tailProcess_comp_injective
  ...
  have huniq :
      ∀ᵐ r ∂ρ,
        ∀ q : ℝ,
          ...
          q = g r := by
    exact
      _root_.Mettapedia.ProbabilityTheory.Exchangeability.DirectingMeasureL1Transfer.ae_limit_eq_L1_limit_of_ae_tendsto_and_L1_tendsto
...
lemma rowProcessCoordwiseCesaroLimit_restrictClass_of_startRestricted_eval_eq_directingRowKernel
  ...
  rw [RowProcessSingletonCesaroLimit]
  change
    ∀ᵐ r ∂Mettapedia.ProbabilityTheory.Exchangeability.MarkovDeFinettiHard.rowProcessLaw_restrictClass (k := k) C P i,
      Filter.Tendsto
        (fun m => rowProcessEmpiricalFreq (k := k) j r m)
        Filter.atTop
        (nhds (((rowKernel i r) ({j} : Set (Fin k))).toReal))
  rw [Mettapedia.ProbabilityTheory.Exchangeability.MarkovDeFinettiHard.rowProcessLaw_restrictClass_eq_finsetSum
    (k := k) (C := C) (P := P) (i := i)]
  rw [MeasureTheory.ae_finsetSum_measure_iff]
  intro a ha
  by_cases hCa : a ∈ C
...
theorem rowProcessLaw_restrictClass_factorizes_directingRowKernel_of_startRestricted
    ...
    Measure.map
        (fun r : ℕ → Fin k => fun j : Fin m => r (sel j))
        (Mettapedia.ProbabilityTheory.Exchangeability.MarkovDeFinettiHard.rowProcessLaw_restrictClass (k := k) C P i)
      =
    (Mettapedia.ProbabilityTheory.Exchangeability.MarkovDeFinettiHard.rowProcessLaw_restrictClass (k := k) C P i).bind
      (fun r =>
        Measure.pi
          (fun _ : Fin m =>
            (directingRowKernel (k := k) P i r : Measure (Fin k)))) := by
  ...
  have hρC :
    ...
    rw [Measure.sum_fintype]
    simpa [ρC, μC] using
      (Mettapedia.ProbabilityTheory.Exchangeability.MarkovDeFinettiHard.rowProcessLaw_restrictClass_eq_finsetSum
        (k := k) (C := C) (P := P) (i := i))
...
end Mettapedia.ProbabilityTheory.Exchangeability.MarkovDeFinettiHard
