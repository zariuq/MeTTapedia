...
/-!
# Foundation proof terms as ProofGSLT authorities

Foundation distinguishes the proof type `Entailment.Prf system formula` from
its propositional truncation `Entailment.Provable system formula`.  ProofGSLT
...
soundness and completeness classes to obtain semantic ProofGSLT authority.

No executable checker is inferred from an arbitrary Foundation proof type.
Executable replay enters only after a concrete ProofGSLT presentation and a
two-sided proof-term translation have been supplied.
-/

namespace Mettapedia.Logic.Bridges.FoundationProofGSLT
...
/-- A claim-indexed bridge from Foundation proof terms to ProofGSLT.
...
/-- Exact ProofGSLT authority for an independently stated claim meaning.
...
def semanticallyCompleteProofGSLT
    {Meaning : Claim -> Prop}
    (calculus : CalculusExact
      (fun claim => LO.Entailment.Provable
        (bridge.systemOf claim) (bridge.formulaOf claim))
      Meaning) :
    SemanticallyCompleteProofGSLT Claim Meaning where
...
ProofGSLT derivation family.  The maps retain proof objects; an equivalence of
...
/-- Foundation provability and ProofGSLT derivability agree after erasing the
...
/-- The proof-term bridge is exactly a complete ProofGSLT presentation for
...
semantic ProofGSLT presentation for the chosen model or model class. -/
def exactForModel
    {Model : Type uModel} [LO.Semantics Model Formula] (model : Model)
    [LO.Sound system model] [LO.Complete system model] :
    ExactJudgmentPresentation Formula
...
generic exact ProofGSLT wire authority and dependent NIK family. -/
def semanticallyCompleteProofGSLT
    {Model : Type uModel} [LO.Semantics Model Formula] (model : Model)
    [LO.Sound system model] [LO.Complete system model] :
    SemanticallyCompleteProofGSLT Formula
      (fun formula => LO.Semantics.Models model formula) where
  presentation := presentation
...
/-- When both Foundation systems have ProofGSLT presentations, a Foundation
proof translation induces an actual ProofGSLT derivation transformation. -/
def mapDerivation
...
end Mettapedia.Logic.Bridges.FoundationProofGSLT
