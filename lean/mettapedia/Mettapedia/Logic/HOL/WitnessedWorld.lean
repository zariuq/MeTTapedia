...
theorem exists_witnessed_prime_extensionUsing
  ...
  have hHcons : Consistent (Const := WithParams Const) (witnessLimitUsing supply T enum) :=
    witnessLimitUsing_consistent supply T enum hCons hT0
  obtain ⟨U, hExt, hClosed, hUcons, hPrime⟩ :=
    exists_prime_extension_of_consistent (Const := WithParams Const) hHcons
  refine ⟨U, fun hψ => hExt (subset_witnessLimitUsing supply T enum hψ),
    hClosed, hUcons, hPrime, ?_⟩
  intro σ φ hex
...
/-- Identity-supply form of Henkin existence, preserving the original public API. -/
theorem exists_witnessed_prime_extension
    ...
    ∃ U : ClosedTheorySet (WithParams Const),
      ...
      (∀ {σ : Ty Base} {φ : Formula (WithParams Const) [σ]},
        (.ex φ : ClosedFormula (WithParams Const)) ∈ U →
          ∃ t : ClosedTerm (WithParams Const) σ, instantiate (Base := Base) t φ ∈ U) := by
  exact exists_witnessed_prime_extensionUsing identityWitnessSupply enum henum hCons hT0
