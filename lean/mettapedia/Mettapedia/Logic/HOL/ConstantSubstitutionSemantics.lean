...
def constantSubstitutionReduct
  ...
  term_closed := by
    ...
    exact (congrArg (M.adm τ)
      (denote_constantSubstitutionPreModel definitions M term valuation)).mpr
        (M.term_closed (substConst definitions term) valuation admitted)
