import Mettapedia.Logic.BDD.ProbLogPublicTheorems
import Mettapedia.Logic.LP.FunctionFree
import Mathlib.Tactic

/-!
# ProbLog Public Examples

Worked examples for the current first-order ProbLog source theorems.
...
- This file does not yet provide a canonical compiler from a source program to
  ...
  current theorem collection on concrete structured or locally compilable data.
...
/-- End-to-end worked example: the structured weighted first-order AD source language
theorem yields ordered BDDs for the `heads(X)` query of a fair binary choice. -/
theorem fairToss_source_example :
...
witness automatically, then the source theorem yields ordered BDDs for
`heads(X)` conditioned on `coin(X)`. -/
theorem gatedToss_source_example :
