...
- Function-free first-order normal programs with first-order query/evidence
  ...
  `Mettapedia.Logic.BDDCore.problog_functionFree_normal_source_equivalence`
...
- Function-free first-order annotated-disjunction programs with first-order
  ...
  `Mettapedia.Logic.BDDCore.problog_functionFree_ad_source_equivalence`
- Function-free weighted first-order annotated-disjunction programs with
  ...
  `Mettapedia.Logic.BDDCore.problog_functionFree_weighted_ad_source_equivalence`
- The same weighted first-order AD result packaged around a compiled-program
  object:
  `Mettapedia.Logic.BDDCore.FirstOrderWeightedADCompiledProgram.source_equivalence`
- A decomposed-stratification variant for weighted first-order AD programs:
  `Mettapedia.Logic.BDDCore.source_equivalence_of_parts`
- A structured-stratification variant where AD body/head and switch/head
  ordering is proved per grounded AD instance:
  `Mettapedia.Logic.BDDCore.source_equivalence_of_structured`
- The same structured route packaged as a proof-carrying compiled-program
  object:
  `Mettapedia.Logic.BDDCore.FirstOrderWeightedADStructuredCompiledProgram.source_equivalence`
- A source-level structured-compilation wrapper that quantifies over explicit
  ...
  `Mettapedia.Logic.BDDCore.FirstOrderWeightedADSourceStructuredCompilation.source_equivalence`
- The highest-level honest source-facing existence wrapper for that route:
  `Mettapedia.Logic.BDDCore.exists_source_equivalence_of_sourceStructuredEvidenceSatisfiable`
- A local-choice automation route: if each grounded AD instance merely admits a
  ...
  `Mettapedia.Logic.BDDCore.FirstOrderWeightedADProbLogProgram.source_equivalence_of_locallyStructurablyCompilable`
- The corresponding highest-level existence wrapper:
  `Mettapedia.Logic.BDDCore.exists_source_equivalence_of_locallyStructuredEvidenceSatisfiable`
- The highest-level honest source-facing wrapper for the structured route:
  `Mettapedia.Logic.BDDCore.exists_source_equivalence_of_structuredEvidenceSatisfiable`
- Existence-style wrapper that hides the compiled witness in the assumptions:
  `Mettapedia.Logic.BDDCore.exists_source_equivalence_of_exists_compiled`

Positive example:
- If you want the strongest currently formalized source theorem, start from
  `exists_source_equivalence_of_locallyStructuredEvidenceSatisfiable`,
  `FirstOrderWeightedADProbLogProgram.source_equivalence_of_locallyStructurablyCompilable`,
  `exists_source_equivalence_of_sourceStructuredEvidenceSatisfiable`,
  `FirstOrderWeightedADSourceStructuredCompilation.source_equivalence`,
  `source_equivalence_of_structured`,
  `FirstOrderWeightedADStructuredCompiledProgram.source_equivalence`, or
  `exists_source_equivalence_of_structuredEvidenceSatisfiable`.
