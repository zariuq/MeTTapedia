...
/-!
# ProbMeTTa Source Syntax

This file lifts the proved ProbMeTTa runtime/query core to a normalized
source-level syntax for the actual `lib_prob.metta` constructors:
...
The source syntax is intentionally normalized:
...
/-- The normalized normal ProbLog program corresponding to a source syntax
state. -/
...
/-- Source constructor `(:: p goal)`: extend the probabilistic-fact store by a
...
/-- Exact, non-rounded `?prob-bdd` witness for the normalized source syntax. -/
...
/-- Exact, non-rounded `?prob` value on the normalized source syntax. -/
...
/-- Exact, non-rounded `?prob-given` value on the normalized source syntax. -/
