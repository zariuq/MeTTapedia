...
theorem classicalAuthorityTranslation_toTheoryTranslation
    {source target : EquationSystem S}
    (equivalent : EquationSystem.SameConsequences source target) :
    (classicalAuthorityTranslation equivalent).toTheoryTranslation =
      identityOnEquationsTranslation equivalent := by
  apply Mettapedia.GSLT.LanguageDef.NIKAuthorityCategory.TheoryTranslation.ext_data
  · intro kind
    rfl
  · intro signature
    rfl
...
are forgotten. -/
theorem classicalAuthorityTranslation_conservative
    {source target : EquationSystem S}
    (equivalent : EquationSystem.SameConsequences source target) :
    (classicalAuthorityTranslation equivalent).toTheoryTranslation.Conservative := by
  rw [classicalAuthorityTranslation_toTheoryTranslation]
...
noncomputable def derivedExtensionAuthorityTranslation :
...
theorem derived_equation_certificate_transports :
    ∃ sourceCertificate,
      ((contract derivedExtension).checker ()).check
        ...
        ((contract equationSystem).checker
          (derivedExtensionAuthorityTranslation.mapKind ())).check
          (derivedExtensionAuthorityTranslation.mapClaim ()
            (mul (mul one x) one, x))
          (derivedExtensionAuthorityTranslation.mapCertificate ()
  ...
  rw [derivedExtensionAuthorityTranslation.check_commutes]
  exact accepted
...
theorem no_identity_conservative_authorityTranslation_from_collapsingExtension :
    ¬ ∃ translation :
        AuthorityTranslation (contract collapsingExtension)
