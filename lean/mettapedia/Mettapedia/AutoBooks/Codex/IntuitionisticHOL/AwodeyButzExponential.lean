...
theorem continuous_sectionMapOfLocalMorphism {F E : EtaleSpace X} {U : Opens X}
  ...
  unfold expCarrierTopologicalSpace
...
theorem isOpenEmbedding_chartToRange {F E : EtaleSpace X} {U : Opens X}
  ...
  have hComp :
    ...
    change IsOpenEmbedding (sectionMapOfLocalMorphism f)
    exact isOpenEmbedding_sectionMapOfLocalMorphism f
...
theorem evalAtGerm_proof_irrel {F E : EtaleSpace X} (q : ExpCarrier F E) (y : F.Carrier)
    (h₁ h₂ : F.proj y = expProj F E q) :
    evalAtGerm q y h₁ = evalAtGerm q y h₂ := by
...
theorem evalToFun_eq_localMorphism_on_chart {F E : EtaleSpace X} {U : Opens X}
    ...
    (fun p : (prod (exp F E) F).Carrier =>
      evalAtGerm p.fst p.snd (by
        simpa [exp, Function.Pullback.fst, Function.Pullback.snd] using p.property.symm)) p =
      f.toFun ⟨p.snd, by
        have hproj : F.proj p.snd = expProj F E p.fst := by
          simpa [exp, Function.Pullback.fst, Function.Pullback.snd] using p.property.symm
  ...
  simpa [hx, Function.Pullback.fst, Function.Pullback.snd] using
  ...
  let chartMap :
    ...
    fun p => ⟨p.1.snd, by
      have hproj : F.proj p.1.snd = expProj F E p.1.fst := by
        simpa [exp, Function.Pullback.fst, Function.Pullback.snd] using p.1.property.symm
...
noncomputable def evalMorphism (F E : EtaleSpace X) :
  ...
  continuous_toFun := by
    ...
    have hsOpen : IsOpen s := by
      change IsOpen ((prodFst (exp F E) F) ⁻¹' chartRangeOfLocalMorphism f)
      exact (isOpen_chartRangeOfLocalMorphism f).preimage (prodFst (exp F E) F).continuous
...
theorem evalAtGerm_proj {F E : EtaleSpace X} (q : ExpCarrier F E) (y : F.Carrier)
  ...
  intro a h
  have hy : F.proj y ∈ a.2.nbhd := by
    simpa [h, expProj] using a.2.mem_nbhd
  change E.proj (a.2.morphism.toFun ⟨y, hy⟩) = F.proj y
  exact a.2.morphism.fiberwise ⟨y, hy⟩

theorem evalMorphism_proj (F E : EtaleSpace X) (p : (prod (exp F E) F).Carrier) :
    E.proj (evalMorphism F E p) = (prod (exp F E) F).proj p := by
  change E.proj (evalAtGerm p.fst p.snd
    (by simpa [exp, Function.Pullback.fst, Function.Pullback.snd] using p.property.symm)) = expProj F E p.fst
  exact (evalAtGerm_proj p.fst p.snd
    (by simpa [exp, Function.Pullback.fst, Function.Pullback.snd] using p.property.symm)).trans <|
    by simpa [exp, Function.Pullback.fst, Function.Pullback.snd] using p.property.symm
...
theorem uncurry_proj {F E G : EtaleSpace X}
  ...
  have hBase : (prod (exp F E) F).proj q = (prod G F).proj p := by
    simpa [q, prod, prodFst, prodSnd, Function.comp_apply, Function.Pullback.fst,
      Function.Pullback.snd] using congrFun hg p.fst
  ...
  by
      ...
      have hproj :
        ...
        calc
          expProj F E (g (s.toContinuousMap v)) = G.proj (s.toContinuousMap v) := by
            simpa [exp, Function.comp_apply] using congrFun hg (s.toContinuousMap v)
    ...
    have hsame :
      ...
      have hqBase : expProj F E (g gx) = F.proj e.val := by
        calc
          expProj F E (g gx) = G.proj gx := by
            simpa [gx, exp, Function.comp_apply] using congrFun hg gx
          _ = u.1 := by
      ...
      have heval :
        ...
        simpa [evalMorphism, q, Function.comp_apply, Function.Pullback.fst, Function.Pullback.snd] using
    ...
    exact hcurry.trans <| by simpa [ha, exp, sectionMapOfLocalMorphism] using hEq
  ...
  by
    ...
    have heval :
        evalAtGerm
            ...
            p.snd (by simpa [Function.Pullback.fst, Function.Pullback.snd] using p.property.symm) = f p := by
      calc
        evalAtGerm
            (sectionMapOfLocalMorphism (localMorphismAlongSection f hf s) ⟨G.proj p.fst, hx⟩)
            p.snd (by simpa [Function.Pullback.fst, Function.Pullback.snd] using p.property.symm)
            =
              (localMorphismAlongSection f hf s).toFun ⟨p.snd, hbaseF⟩ := by
                simpa [hsArg] using
                  (evalAtGerm_sectionMapOfLocalMorphism
                    (localMorphismAlongSection f hf s)
                    ⟨G.proj p.fst, hx⟩ p.snd
                    (by simpa [Function.Pullback.fst, Function.Pullback.snd] using p.property.symm))
        _ =
        ...
        _ = f p := by
              ...
              simpa using congrArg f hpair
    have htarget : F.proj p.snd = expProj F E ((curry f hf) p.fst) := by
      have hcurBase : expProj F E ((curry f hf) p.fst) = G.proj p.fst := by
        change (exp F E).proj ((curry f hf) p.fst) = G.proj p.fst
        exact congrFun (curry_proj f hf) p.fst
      exact p.property.symm.trans hcurBase.symm
    have hsection :
        F.proj p.snd =
          expProj F E
            (sectionMapOfLocalMorphism (localMorphismAlongSection f hf s) ⟨G.proj p.fst, hx⟩) := by
      simpa [Function.Pullback.fst, Function.Pullback.snd] using p.property.symm
    have hEvalEq :
        evalAtGerm ((curry f hf) p.fst) p.snd htarget =
          evalAtGerm
            (sectionMapOfLocalMorphism (localMorphismAlongSection f hf s) ⟨G.proj p.fst, hx⟩)
            p.snd hsection := by
      cases hcurry
      exact evalAtGerm_proof_irrel _ _ _ _
    simpa [uncurry, evalMorphism, Function.comp_apply, Function.Pullback.fst, Function.Pullback.snd]
      using hEvalEq.trans heval
