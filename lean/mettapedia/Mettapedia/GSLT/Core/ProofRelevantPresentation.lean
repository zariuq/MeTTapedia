...
/-!
# Proof-relevant presentations of GSLT steps
...
`StepPresentation` equips a GSLT with a proof-relevant loose relation whose
...
namespace Mettapedia.GSLT.ProofRelevantPresentation
...
/-- An authored, proof-relevant presentation of every one-step reduction in a
semantic GSLT.  `erases_iff` prevents both missing authored events and events
that invent a semantic step. -/
structure StepPresentation (theory : GSLT.{u}) where
...
namespace StepPresentation

variable {theory : GSLT.{u}}

/-- Erase one authored event to the proposition-valued GSLT step. -/
theorem erase (presentation : StepPresentation theory)
    {source target : theory.Term}
    (evidence : presentation.Evidence source target) :
    theory.Step source target :=
  (presentation.erases_iff source target).mp ⟨evidence⟩

/-- Every semantic GSLT step has at least one authored event. -/
theorem present (presentation : StepPresentation theory)
    {source target : theory.Term} (step : theory.Step source target) :
    Nonempty (presentation.Evidence source target) :=
  (presentation.erases_iff source target).mpr step

end StepPresentation

/-- A GSLT bundled with the proof-relevant presentation of its steps. -/
structure PresentedGSLT where
  theory : GSLT.{u}
  steps : StepPresentation theory

namespace PresentedGSLT

/-- A proof-relevant labeled event retains the exact evidence occurrence. -/
structure Event (system : PresentedGSLT.{u}) where
...
def Event.erase {system : PresentedGSLT.{u}}
...
structure Translation (source target : PresentedGSLT.{u}) where
...
def id (system : PresentedGSLT.{u}) : Translation system system where
...
def comp {first middle last : PresentedGSLT.{u}}
...
def toCovered {source target : PresentedGSLT.{u}}
...
@[simp] theorem comp_mapTerm {first middle last : PresentedGSLT.{u}}
    (earlier : Translation first middle)
...
structure ExactTranslation (source target : PresentedGSLT.{u})
...
/-- Exact identity on a proof-relevant presentation. -/
def id (system : PresentedGSLT.{u}) : ExactTranslation system system := by
...
/-- One state with one extensional self-step.  Different presentations below
...
def boolSteps : StepPresentation loopTheory where
...
def optionSteps : StepPresentation loopTheory where
...
def unitSteps : StepPresentation loopTheory where
...
def boolSystem : PresentedGSLT := ⟨loopTheory, boolSteps⟩
def optionSystem : PresentedGSLT := ⟨loopTheory, optionSteps⟩
def unitSystem : PresentedGSLT := ⟨loopTheory, unitSteps⟩
...
end Mettapedia.GSLT.ProofRelevantPresentation
