...
theorem denoteReferenceBindings_sorts
  ...
  | cons element rest ih =>
      ...
      | literal text span =>
          simp [denoteReferenceBindings, List.filterMap_cons, Element.referenceName?, ih]
...
theorem denoteRules_labels_nodup
  ...
  | cons entry rest ih =>
      ...
      | comment text span =>
          apply ih
          simpa [List.filterMap_cons, Entry.ruleName?] using namesNodup
      | blank span =>
          apply ih
          simpa [List.filterMap_cons, Entry.ruleName?] using namesNodup
      | rule category expression span =>
...
theorem denoteRuleTypes_names (entries : List Entry) :
  ...
  | cons entry rest ih =>
      ...
      | comment text span =>
          simp [denoteRuleTypes, List.filterMap_cons, Entry.ruleName?, ih]
      | blank span =>
          simp [denoteRuleTypes, List.filterMap_cons, Entry.ruleName?, ih]
