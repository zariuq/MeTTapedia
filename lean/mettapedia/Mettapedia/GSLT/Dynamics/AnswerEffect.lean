...
namespace Mettapedia.GSLT.Dynamics.OccurrenceSemantics
...
/-! ## Occurrence-indexed one-step theories -/

/-- The smallest evaluator interface needed to speak about exact answer
occurrences. It fixes neither a query language nor a fallback policy: those
choices belong to the concrete function supplying the occurrence bag. -/
structure OccurrenceSource (Space : Type uSpace) (Request : Type uRequest)
    (Answer : Type uAnswer) where
  occurrences : Space → Request → Multiset Answer

variable {Space : Type uSpace} {Request : Type uRequest}
  {Answer : Type uAnswer}

/-- Canonical request/answer terms generated by an occurrence source. -/
inductive OccurrenceTerm
    (source : OccurrenceSource Space Request Answer) where
  | request (space : Space) (request : Request)
  | answer (space : Space) (request : Request)
      (occurrence : Nat) (answer : Answer)

/-- One occurrence-selection step. -/
inductive OccurrenceStep [DecidableEq Answer]
    (source : OccurrenceSource Space Request Answer) :
    OccurrenceTerm source → OccurrenceTerm source → Prop where
  | found {space request occurrence answer}
      (copy : occurrence < Multiset.count answer
        (source.occurrences space request)) :
      OccurrenceStep source (.request space request)
        (.answer space request occurrence answer)

/-- The canonical one-step GSLT generated by an occurrence source. -/
def occurrenceGSLT [DecidableEq Answer]
    (source : OccurrenceSource Space Request Answer) : GSLT where
  Term := OccurrenceTerm source
  equations := ⟨Eq, ⟨Eq.refl, Eq.symm, Eq.trans⟩⟩
  rewrites := OccurrenceStep source
...
@[simp] theorem occurrenceGSLT_step_iff [DecidableEq Answer]
    (source : OccurrenceSource Space Request Answer)
    (space : Space) (request : Request) (answer : Answer)
    (occurrence : Nat) :
    (occurrenceGSLT source).Step (.request space request)
        (.answer space request occurrence answer) ↔
  ...
  · intro step
    cases step
    assumption
  · exact OccurrenceStep.found

/-- Every occurrence-labelled answer is terminal, independently of how its
source computes the answer bag. -/
theorem occurrenceAnswer_isNormalForm [DecidableEq Answer]
    (source : OccurrenceSource Space Request Answer)
    (space : Space) (request : Request) (occurrence : Nat) (answer : Answer) :
    (occurrenceGSLT source).IsNormalForm
      (.answer space request occurrence answer) := by
  rintro ⟨target, step⟩
  cases step

/-- Every occurrence-selection step lands at a terminal answer. -/
theorem occurrenceStep_target_isNormalForm [DecidableEq Answer]
    (source : OccurrenceSource Space Request Answer)
    {first second : (occurrenceGSLT source).Term}
    (step : (occurrenceGSLT source).Step first second) :
    (occurrenceGSLT source).IsNormalForm second := by
  cases step
  exact occurrenceAnswer_isNormalForm source _ _ _ _
...
theorem occurrenceGSLT_not_hasComposableSteps [DecidableEq Answer]
    (source : OccurrenceSource Space Request Answer) :
    ¬ HasComposableSteps (occurrenceGSLT source) :=
  (occurrenceOneStepTerminal source).not_hasComposableSteps

/-- A request and each of its occurrence-labelled answers denote the same
complete answer bag. -/
def occurrenceMeaning (source : OccurrenceSource Space Request Answer) :
    OccurrenceTerm source → Multiset Answer
  | .request space request => source.occurrences space request
  | .answer space request _ _ => source.occurrences space request

/-- Occurrence selection preserves the complete answer-bag denotation. -/
def occurrenceElaboration [DecidableEq Answer]
    (source : OccurrenceSource Space Request Answer) :
    GSLT.Elaboration (occurrenceGSLT source) (Multiset Answer) where
  elaborate := fun term => some (occurrenceMeaning source term)
  equation := by
...
end Mettapedia.GSLT.Dynamics.OccurrenceSemantics
...
/-! ## Operational specification regions

An occurrence source is only an answer-producing component.  An operational
point additionally selects a host GSLT and a faithful embedding of that
component.  Morphisms retain both translations and the commuting square, so
the component is not identified with its host.

These total categories require no cartesian or cocartesian lifting property.
They therefore express exactly the functorial structure proved below, without
claiming a fibration or opfibration. -/

/-- A selected occurrence semantics hosted by a larger operational theory. -/
structure OccurrencePoint (Request Answer : Type) [DecidableEq Answer] where
  Space : Type
  source : OccurrenceSource Space Request Answer
  host : GSLT
  occurrenceEmbedding : GSLT.Embedding (occurrenceGSLT source) host

namespace OccurrencePoint

/-- A typed arrow between occurrence-hosting points.  The source component
and its host may both change, but their selected embedding square must
commute on every term. -/
structure Hom {Request Answer : Type} [DecidableEq Answer]
    (first second : OccurrencePoint Request Answer) where
  occurrence : OperationalTranslation (occurrenceGSLT first.source)
    (occurrenceGSLT second.source)
  host : OperationalTranslation first.host second.host
  commutes : ∀ term,
    host.mapTerm (first.occurrenceEmbedding.toFun term) =
      second.occurrenceEmbedding.toFun (occurrence.mapTerm term)

namespace Hom

@[ext]
theorem ext {Request Answer : Type} [DecidableEq Answer]
    {first second : OccurrencePoint Request Answer}
    {left right : Hom first second}
    (occurrence : left.occurrence = right.occurrence)
    (host : left.host = right.host) : left = right := by
  cases left
  cases right
  cases occurrence
  cases host
...
/-- Identity retains both selected components and their embedding square. -/
def id {Request Answer : Type} [DecidableEq Answer]
    (point : OccurrencePoint Request Answer) : Hom point point where
  occurrence := OperationalTranslation.id _
  host := OperationalTranslation.id _
  commutes := by intro; rfl

/-- Typed occurrence-host arrows compose componentwise. -/
def comp {Request Answer : Type} [DecidableEq Answer]
    {first middle last : OccurrencePoint Request Answer}
    (earlier : Hom first middle) (later : Hom middle last) :
    Hom first last where
  occurrence := earlier.occurrence.comp later.occurrence
  host := earlier.host.comp later.host
  commutes := by
    intro term
    change later.host.mapTerm
        (earlier.host.mapTerm (first.occurrenceEmbedding.toFun term)) =
      last.occurrenceEmbedding.toFun
        (later.occurrence.mapTerm (earlier.occurrence.mapTerm term))
    rw [earlier.commutes, later.commutes]

end Hom

instance {Request Answer : Type} [DecidableEq Answer] :
    CategoryTheory.Category (OccurrencePoint Request Answer) where
  Hom := Hom
  id := Hom.id
  comp := Hom.comp
  id_comp morphism := by
    apply Hom.ext <;> apply OperationalTranslation.ext <;> rfl
  comp_id morphism := by
    apply Hom.ext <;> apply OperationalTranslation.ext <;> rfl
  assoc first second third := by
    apply Hom.ext <;> apply OperationalTranslation.ext <;> rfl

/-- Forget an occurrence-host point to its selected host theory. -/
def hostFunctor {Request Answer : Type} [DecidableEq Answer] :
    CategoryTheory.Functor (OccurrencePoint Request Answer)
      OperationalTheory where
  obj point := ⟨point.host⟩
  map morphism := morphism.host
  map_id _ := by apply OperationalTranslation.ext; rfl
  map_comp _ _ := by apply OperationalTranslation.ext; rfl

/-- Forget an occurrence-host point to the generated occurrence theory. -/
def occurrenceFunctor {Request Answer : Type} [DecidableEq Answer] :
    CategoryTheory.Functor (OccurrencePoint Request Answer)
      OperationalTheory where
  obj point := ⟨occurrenceGSLT point.source⟩
  map morphism := morphism.occurrence
  map_id _ := by apply OperationalTranslation.ext; rfl
  map_comp _ _ := by apply OperationalTranslation.ext; rfl

/-- A total answer-bag interpretation attached independently to an
operational point. -/
structure BagObservation {Request Answer : Type} [DecidableEq Answer]
    (point : OccurrencePoint Request Answer) where
  meaning : point.host.Term → Multiset Answer
  elaboration : GSLT.Elaboration point.host (Multiset Answer)
  elaborates : ∀ term, elaboration.elaborate term = some (meaning term)
  occurrenceMeaning : ∀ term,
    meaning (point.occurrenceEmbedding.toFun term) =
      OccurrenceSemantics.occurrenceMeaning point.source term
