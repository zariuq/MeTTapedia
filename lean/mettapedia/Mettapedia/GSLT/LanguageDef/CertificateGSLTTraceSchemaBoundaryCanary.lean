...
(`{tick, ...rest} → {...rest}`) is rejected by every definition that
contains it — a universally quantified compiled counterexample.

Positive side: a small hand-authored trace definition (axiom `a → b`, congruence
...
namespace Mettapedia.GSLT.LanguageDef.CertificateGSLT.TraceSchemaBoundaryCanary
...
/-- No definition whatsoever admits the direct bag-step schema. -/
theorem bag_direct_transliteration_rejected
    (definition : CalculusLanguageDef) (mem : bagStepRule ∈ definition.rules) :
    definition.isValid = false :=
  isValid_eq_false_of_collectionRest_rule mem
...
private def authoredTraceDefinition : CalculusLanguageDef :=
...
private theorem stepLanguage_validate :
    authoredTraceDefinition.toLanguageDef.validate = [] := by
  apply LanguageDef.validate_eq_nil_of_constructorOnly <;>
    simp [authoredTraceDefinition, spType, constA, constB, constF, constG,
      LanguageDef.typeNames, TermParam.typeExpr, TypeExpr.baseNames,
      TypeDecl.plain]

private theorem authoredTraceDefinition_valid :
    authoredTraceDefinition.isValid = true := by
  unfold CalculusLanguageDef.isValid CalculusLanguageDef.hasValidLocalRules
  rw [stepLanguage_validate]
  simp [authoredTraceDefinition, CalculusLanguageDef.judgmentSignatureValid,
...
private def stepValidated : ValidatedCalculusLanguageDef :=
  ⟨authoredTraceDefinition, authoredTraceDefinition_valid⟩
...
private theorem axiom_instantiates :
  ...
  simp [instantiateRule?, stepValidated, authoredTraceDefinition, axiomInstance,
    axiomAB, congF, congGLeft, congGRight, reflSteps, transSteps,
    stepJudgment, termA, termB, CalculusLanguageDef.lookupRule?, argumentsValidAt,
...
private theorem congF_instantiates :
  ...
  simp [instantiateRule?, stepValidated, authoredTraceDefinition, congFInstance,
    axiomAB, congF, congGLeft, congGRight, reflSteps, transSteps,
    stepJudgment, termA, termB, termF, CalculusLanguageDef.lookupRule?,
...
private theorem congGLeft_instantiates :
  ...
  simp [instantiateRule?, stepValidated, authoredTraceDefinition,
...
private theorem reflGbb_instantiates :
  ...
  simp [instantiateRule?, stepValidated, authoredTraceDefinition, reflGbbInstance,
    axiomAB, congF, congGLeft, congGRight, reflSteps, transSteps,
    tracesJudgment, gbb, termG, termB, CalculusLanguageDef.lookupRule?,
    argumentsValidAt, argumentValidAt, lookupArgumentAt?, Pattern.isGroundAt,
    Pattern.isGroundListAt, Pattern.hasCanonicalBinderMetadata,
    Pattern.hasCanonicalBinderMetadataList,
    instantiateSchemas?, instantiateSchema?, instantiateSchemaAt?,
...
private theorem transLeftOuter_instantiates :
  ...
  simp [instantiateRule?, stepValidated, authoredTraceDefinition,
    ...
    termG, termA, termB, CalculusLanguageDef.lookupRule?, argumentsValidAt,
...
end Mettapedia.GSLT.LanguageDef.CertificateGSLT.TraceSchemaBoundaryCanary
