...
theorem incompatiblePair_not_compatible :
  ...
  have impossible : (.top : ClosedFormula TwoConstants) = doubledTruth := by
    ...
    have unchanged : henkinConsequence.sentence.map
        (CategoryTheory.CategoryStruct.id sourceSignature)
          (.top : ClosedFormula TwoConstants) = .top :=
      henkinConsequence.sentence.map_id_apply sourceSignature .top
    exact unchanged.symm.trans changedMeaning
  cases impossible
...
theorem incompatiblePair_not_in_joint_image :
  ...
  exact incompatiblePair_not_compatible
    ((BiformTheory.jointProjection_map_range_iff_compatible
      (source := tagSensitiveBiform) (target := tagSensitiveBiform) incompatiblePair).mp route)
