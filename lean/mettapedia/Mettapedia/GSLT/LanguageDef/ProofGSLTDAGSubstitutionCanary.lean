...
namespace Mettapedia.GSLT.LanguageDef.ProofGSLT.DAGSubstitutionCanary

open Mettapedia.OSLF.MeTTaIL.Syntax
open Mettapedia.GSLT.LanguageDef.InferenceChecker
open Mettapedia.GSLT.LanguageDef.ProofGSLT

private def judgmentA : Pattern := .apply "ProofGSLT-Sub-A" []
private def judgmentB : Pattern := .apply "ProofGSLT-Sub-B" []
private def judgmentC : Pattern := .apply "ProofGSLT-Sub-C" []

private def ruleAB : RuleSchema :=
  { id := ⟨"proof-gslt-sub-a-b"⟩
    metavariables := []
    premises := [judgmentA]
    conclusion := judgmentB }

private def ruleShare : RuleSchema :=
  { id := ⟨"proof-gslt-sub-share"⟩
    metavariables := []
    premises := [judgmentB, judgmentB]
    conclusion := judgmentC }

private def subPresentation : Presentation :=
  { language := LanguageDef.empty "proof-gslt-sub"
    calculus :=
      { judgments :=
          [{ head := "ProofGSLT-Sub-A", arity := 0 },
           { head := "ProofGSLT-Sub-B", arity := 0 },
           { head := "ProofGSLT-Sub-C", arity := 0 }]
...
private def instanceAB : RuleInstance := ⟨⟨"proof-gslt-sub-a-b"⟩, []⟩
private def instanceShare : RuleInstance := ⟨⟨"proof-gslt-sub-share"⟩, []⟩
...
end Mettapedia.GSLT.LanguageDef.ProofGSLT.DAGSubstitutionCanary
