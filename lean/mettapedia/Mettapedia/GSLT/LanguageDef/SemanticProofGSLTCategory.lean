...
/-!
# Exact semantic ProofGSLTs and their interpretations

The ordinary category of ProofGSLT presentations remembers derivation-valued
...
assert that filtered colimits exist or are created by the underlying ProofGSLT
category, and therefore does not yet claim an `Ind` completion.
-/

namespace Mettapedia.GSLT.LanguageDef.ProofGSLT.Semantic
...
/-- A ProofGSLT presentation with exact authority for a fixed independent
semantic meaning. -/
structure ExactObject (Claim : Type uClaim) (Meaning : Claim → Prop) where
  toProofGSLT : ProofGSLT.Object
  adequacy : ExactJudgmentPresentation Claim Meaning toProofGSLT.presentation
...
theorem derivation_iff_meaning (object : ExactObject Claim Meaning)
    (claim : Claim) :
    Nonempty (Derivation object.toProofGSLT.presentation (object.encode claim)) ↔
      Meaning claim := by
  constructor
...
theorem theoremhood_iff (source target : ExactObject Claim Meaning)
    (claim : Claim) :
    Nonempty (Derivation source.toProofGSLT.presentation (source.encode claim)) ↔
      Nonempty (Derivation target.toProofGSLT.presentation (target.encode claim)) := by
...
structure Morphism {Claim : Type uClaim} {Meaning : Claim → Prop}
  ...
  interpretation : Interpretation source.toProofGSLT target.toProofGSLT
...
def id (object : ExactObject Claim Meaning) : Morphism object object where
  interpretation := Interpretation.id object.toProofGSLT
  encode_commutes := fun _ => rfl

/-- Composition is recursive ProofGSLT interpretation followed by equality of
...
def mapClaimDerivation (morphism : Morphism source target) (claim : Claim)
    (derivation :
      Derivation source.toProofGSLT.presentation (source.encode claim)) :
    Derivation target.toProofGSLT.presentation (target.encode claim) :=
...
theorem theoremhood_iff (_morphism : Morphism source target) (claim : Claim) :
    Nonempty (Derivation source.toProofGSLT.presentation (source.encode claim)) ↔
      Nonempty (Derivation target.toProofGSLT.presentation (target.encode claim)) :=
...
def mapClaimDerivation (morphism : Morphism source target)
    ...
    (derivation :
      Derivation source.toProofGSLT.presentation (source.encode claim)) :
    Derivation target.toProofGSLT.presentation
...
theorem theoremhood_iff_of_reflects (morphism : Morphism source target)
    ...
    Nonempty (Derivation source.toProofGSLT.presentation (source.encode claim)) ↔
      Nonempty (Derivation target.toProofGSLT.presentation
...
theorem apex_derivation_iff_meaning
    ...
    Nonempty
        (Derivation growth.cocone.pt.toProofGSLT.presentation
...
end Mettapedia.GSLT.LanguageDef.ProofGSLT.Semantic
