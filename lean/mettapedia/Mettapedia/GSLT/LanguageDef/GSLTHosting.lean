...
/-!
# Source GSLT hosting through explicit executable targets
...
An `ObservedRefinement` is the forward compiler square: every source source
...
For textual languages, `ExactLexicalRealization` first relates the source
...
scannerless syntax-GSLT, complete packed forests, source elaboration, and
...
namespace Mettapedia.GSLT.LanguageDef.GSLTHosting
...
/-- A runtime scanner realizes a source lexical GSLT exactly when it
recognizes every source stream and invents none.  `Raw` may be bytes, a
...
lexical productions of that scannerless grammar. -/
structure ExactLexicalRealization
    {Raw : Type uRaw}
    (source runtime : Raw → List Codepoint → Prop) : Prop where
  runtime_iff_source : ∀ raw codepoints,
    runtime raw codepoints ↔ source raw codepoints

namespace ExactLexicalRealization

variable {Raw : Type uRaw}
variable {source middle runtime : Raw → List Codepoint → Prop}

def id (relation : Raw → List Codepoint → Prop) :
    ExactLexicalRealization relation relation where
  runtime_iff_source := fun _ _ => Iff.rfl

def comp (earlier : ExactLexicalRealization source middle)
    (later : ExactLexicalRealization middle runtime) :
    ExactLexicalRealization source runtime where
  runtime_iff_source := fun raw codepoints =>
    (later.runtime_iff_source raw codepoints).trans
      (earlier.runtime_iff_source raw codepoints)

end ExactLexicalRealization

namespace LexicalAlternativeCanary

def source (_ : Unit) (codepoints : List Codepoint) : Prop :=
  codepoints = [] ∨ codepoints = [0]

def incompleteRuntime (_ : Unit) (codepoints : List Codepoint) : Prop :=
  codepoints = []

theorem source_has_second_alternative : source () [0] :=
  Or.inr rfl
...
exact realization, even if all of its returned streams are source-recognized. -/
theorem no_exact_realization :
    ¬ Nonempty (ExactLexicalRealization source incompleteRuntime) := by
  rintro ⟨realization⟩
  exact runtime_misses_second_alternative
    ((realization.runtime_iff_source () [0]).mpr
      source_has_second_alternative)
...
classes as ordinary source grammar definitions. -/
structure WholeTextSurfaceHosting
    {Raw : Type uRaw}
    (sourceLex runtimeLex : Raw → List Codepoint → Prop)
    (grammar :
      Mettapedia.GSLT.Parsing.GuardCorrespondence.SourceDefinition)
    {Value : Type uValue}
    (source target : ObservedOperationalObject Value) where
  lexical : ExactLexicalRealization sourceLex runtimeLex
  forest : List Codepoint → Forest
  complete : ∀ codepoints, Complete (forest codepoints) grammar codepoints
...
variable {Raw : Type uRaw}
variable {sourceLex runtimeLex : Raw → List Codepoint → Prop}
variable {grammar :
  Mettapedia.GSLT.Parsing.GuardCorrespondence.SourceDefinition}
variable {Value : Type uValue}
variable {source target : ObservedOperationalObject Value}

/-- Source surface alternatives retain both the admitted lexical stream and
the parse tree.  This prevents two lexings with equal ASTs from being silently
identified at the source boundary. -/
def SourceSurfaceResults
    (_pipeline : WholeTextSurfaceHosting sourceLex runtimeLex grammar
      source target)
    (raw : Raw) : Set (List Codepoint × ParseTree) :=
  fun result => sourceLex raw result.1 ∧
    result.2 ∈
...
def RuntimeSurfaceResults
    (pipeline : WholeTextSurfaceHosting sourceLex runtimeLex grammar
      source target)
    (raw : Raw) : Set (List Codepoint × ParseTree) :=
  fun result => runtimeLex raw result.1 ∧
    result.2 ∈
      Mettapedia.GSLT.Parsing.PackedForest.packedResults
        (pipeline.forest result.1) grammar result.1
...
theorem surfaceResults_exact
    (pipeline : WholeTextSurfaceHosting sourceLex runtimeLex grammar
      source target)
    (raw : Raw) :
    pipeline.RuntimeSurfaceResults raw = pipeline.SourceSurfaceResults raw := by
  ext result
  constructor
  · rintro ⟨runtimeLexes, packedTree⟩
    exact ⟨(pipeline.lexical.runtime_iff_source raw result.1).mp
        runtimeLexes,
      (Mettapedia.GSLT.Parsing.PackedForest.packedResults_subset_sourceResults
          (pipeline.forest result.1) grammar result.1) packedTree⟩
  · rintro ⟨sourceLexes, sourceTree⟩
    refine ⟨(pipeline.lexical.runtime_iff_source raw result.1).mpr
        sourceLexes, ?_⟩
...
theorem surface_ambiguity_preserved
    ...
    SurfaceAmbiguous (pipeline.RuntimeSurfaceResults raw) ↔
      SurfaceAmbiguous (pipeline.SourceSurfaceResults raw) := by
  ...
  ∃ codepoints tree,
    runtimeLex raw codepoints ∧
      tree ∈ Mettapedia.GSLT.Parsing.PackedForest.packedResults
        (pipeline.forest codepoints) grammar codepoints ∧
      ∃ term, pipeline.Elaborates tree term ∧
...
theorem targetProduces_iff_sourceProduces
  ...
  · rintro ⟨codepoints, tree, runtimeLexes, packedTree, term,
    ...
    refine ⟨codepoints, tree,
      (pipeline.lexical.runtime_iff_source raw codepoints).mp runtimeLexes,
      ?_, term, elaborates, ?_⟩
    · exact
        (Mettapedia.GSLT.Parsing.PackedForest.packedResults_subset_sourceResults
            (pipeline.forest codepoints) grammar codepoints) packedTree
    · exact pipeline.operational.noInvention term value targetProduces
  · rintro ⟨codepoints, tree, sourceLexes, sourceTree, term,
      elaborates, sourceProduces⟩
    refine ⟨codepoints, tree,
      (pipeline.lexical.runtime_iff_source raw codepoints).mpr sourceLexes,
      ?_, term, elaborates, ?_⟩
    · rw [Mettapedia.GSLT.Parsing.PackedForest.complete_result_set_agreement
...
/-- A complete packed parser, a source elaboration relation, and one
two-sided operational hosting proof.  `Elaborates` is a relation rather than a
function so source ambiguity may remain explicit. -/
structure MaySetSurfaceHosting {Value : Type uValue}
    ...
    (source target : ObservedOperationalObject Value) where
  forest : Forest
  complete : Complete forest grammar input
  Elaborates : ParseTree → source.operational.theory.Term → Prop
  operational : BehavioralHosting source target
...
/-- Semantics from source parse trees through source elaboration and
source execution. -/
def SourceProduces (pipeline :
    MaySetSurfaceHosting grammar input source target) (value : Value) : Prop :=
  ∃ tree, tree ∈
      Mettapedia.GSLT.Parsing.GuardCorrespondence.sourceResults
        grammar input ∧
    ∃ term, pipeline.Elaborates tree term ∧
...
theorem targetProduces_iff_sourceProduces
    (pipeline : MaySetSurfaceHosting grammar input source target)
    (value : Value) :
  ...
  · rintro ⟨tree, packedTree, term, elaborates, targetProduces⟩
    ...
    · exact
        (Mettapedia.GSLT.Parsing.PackedForest.packedResults_subset_sourceResults
          pipeline.forest grammar input) packedTree
    · exact pipeline.operational.noInvention term value targetProduces
...
/-- All source syntactic alternatives remain represented before the
source elaboration relation is consulted. -/
...
end Mettapedia.GSLT.LanguageDef.GSLTHosting
