...
The syntax deliberately does not call a route exact.  Exactness, coverage,
conservativity, and realization adequacy are proof obligations on a route, not
unchecked authoring labels.

One syntax step elaborates to one step of the typed `at`/`via` IR.  Repeated
stepping is the separate reflexive-transitive closure of this relation.
-/

namespace Mettapedia.GSLT.LanguageDef.GSLTIL.Syntax
...
/-- Compilation retains the complete authored identity of a local rule. -/
theorem SpaceRule.toFibreRow_injective :
    Function.Injective SpaceRule.toFibreRow := by
  intro first second equal
  cases first
  cases second
  simp [SpaceRule.toFibreRow] at equal
  simp_all
...
minimal syntax permits a relation; functionality, totality, exactness, and
...
/-- Syntax commands elaborate nondeterministically when the same public name
...
/-- Every direct syntax step is preserved by one independently defined wire
...
/-- On commands obtained from the syntax elaborator, the generated IR
catalog does not invent a syntax transition. -/
...
/-- Consequently every syntax step is accepted by the authored GSLT after
elaboration. -/
...
/-- Syntax chaining is the ordinary reflexive-transitive closure of the
...
end Mettapedia.GSLT.LanguageDef.GSLTIL.Syntax
