...
/-!
# ProofGSLT instances of the ambient-first GSLT theory

The abstract theory lives in `Mettapedia.GSLT.Core.Ultrainfinite`.  This file
contains only its ProofGSLT-specific realizations:
...
`ProofGSLTUltrainfinite`: it is one application of finite evidence to an
