...
The five original fields are untouched and remain canonical: `toLanguageDef` is
a plain field selection, and `ofNested`/`toNested` round-trip by `rfl`, so the
older two-part spelling stays available and nothing is invented either way.
...
/-- A language together with its proof calculus, extended as one record.

The first five fields are the object language; the last three are the calculus
over it.  Nothing else is stored, and nothing is derived that could have been
written. -/
...
The object-language projection is generated by Lean from the literal
`extends LanguageDef`; the calculus projection is the three added fields. -/

/-- The proof calculus over it. -/
abbrev toCalculus : ProofCalculus where
  judgments := extended.judgments
  rules := extended.rules
  conversion := extended.conversion

/-- The nested spelling, for consumers that still expect it. -/
abbrev toNested : Presentation where
  language := extended.toLanguageDef
  calculus := extended.toCalculus

@[simp] theorem toLanguageDef_eq :
    extended.toNested.language = extended.toLanguageDef := rfl

@[simp] theorem toCalculus_eq :
    extended.toNested.calculus = extended.toCalculus := rfl

/-- Recover the flat record from a nested presentation.  With `ofNested_toNested`
this makes the two spellings interchangeable, so migrating costs nothing. -/
def ofNested (presentation : Presentation) : CalculusLanguageDef where
  toLanguageDef := presentation.language
  judgments := presentation.calculus.judgments
  rules := presentation.calculus.rules
  conversion := presentation.calculus.conversion

@[simp] theorem ofNested_toNested :
    ofNested extended.toNested = extended := rfl

@[simp] theorem toNested_ofNested (presentation : Presentation) :
    (ofNested presentation).toNested = presentation := rfl
...
abbrev isAdmitted : Bool := extended.toNested.isValidV2

/-- The admitted presentation. -/
abbrev checked (admitted : extended.isAdmitted = true) : ValidatedPresentation :=
  ⟨extended.toNested, admitted⟩
...
def toGSLT (admitted : extended.isAdmitted = true)
  ...
  combinedGSLT (extended.checked admitted) laws
...
theorem toGSLT_rewrites (admitted : extended.isAdmitted = true)
    ...
    (extended.toGSLT admitted laws).rewrites =
      GSLT.SumStep (languageGSLT extended.toLanguageDef laws)
        (proofSearchGSLT (extended.checked admitted)) := rfl
...
theorem toGSLT_object_step (admitted : extended.isAdmitted = true)
  ...
  combinedGSLT_language_step (extended.checked admitted) laws source target
...
theorem toGSLT_proof_step (admitted : extended.isAdmitted = true)
    ...
    (extended.toGSLT admitted laws).Step
      ...
      (proofSearchGSLT (extended.checked admitted)).Step source target :=
  combinedGSLT_calculus_step (extended.checked admitted) laws source target
...
theorem toGSLT_no_crossing (admitted : extended.isAdmitted = true)
  ...
  combinedGSLT_no_crossing (extended.checked admitted) laws pattern state

/-- **Derivability is reduction**, inside the one theory. -/
theorem toGSLT_derivability (admitted : extended.isAdmitted = true)
    (laws : ReductionRespectsEquations extended.toLanguageDef)
    (goals : GoalState) :
    Nonempty (DerivationList (extended.checked admitted) goals) ↔
      (extended.toGSLT admitted laws).MultiStep
        (inCalculus goals) (inCalculus []) :=
  derivability_iff_combinedGSLT (extended.checked admitted) laws goals
...
theorem evenNumbers_admitted : evenNumbers.isAdmitted = true := by
  ...
  unfold isAdmitted Presentation.isValidV2 Presentation.isValidV1
  rw [show evenNumbers.toNested.language.validate = [] from validate]
  simp [evenNumbers, toNested, toCalculus,
