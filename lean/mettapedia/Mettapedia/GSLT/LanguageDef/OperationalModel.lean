...
def hiddenBitRoute : ModelledTheory.Hom (equipped visibleSystem) hiddenBitModelled where
  ...
  satisfaction_natural := by
    ...
    have mapped := institution.sentence.map_id_apply
      (X := (logicalTheory visibleSystem).signature) formula
    have applied := congrArg (fun f => f (projectVisible (insertHiddenBit state))) mapped
    exact applied ▸ (iff_of_eq (congrArg formula (projectVisible_insertHiddenBit state).symm))
