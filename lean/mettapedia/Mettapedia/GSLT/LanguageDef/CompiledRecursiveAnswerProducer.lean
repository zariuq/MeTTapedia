...
def additiveDenotation {State : Type u} {Answer : Type v}
  ...
  unfold residual := by
    ...
    | call state =>
        ...
        simp [system, optionBag, foldValues_branchResidual denotation, bagEffect]
...
theorem completed_controller_exact_bag
  ...
  simpa [foldValues, additiveDenotation, residualBagValue, bagEffect] using exact
...
theorem frontierListValue_append {State : Type u} {Answer : Type v}
  ...
  | nil => rfl
  | cons node rest ih =>
      cases node <;> simp [frontierListValue, listEffect, ih, List.append_assoc]
