...
namespace Mettapedia.GSLT.LanguageDef.ProofGSLT.WireFormatCanary
...
private def wireLanguage : LanguageDef :=
  { name := "proof-gslt-wire-format-canary"
    types := [wireType]
    terms := [wireA]
    equations := []
    rewrites := [] }
...
end Mettapedia.GSLT.LanguageDef.ProofGSLT.WireFormatCanary
