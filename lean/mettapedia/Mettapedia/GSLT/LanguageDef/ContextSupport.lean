...
/-- Whether a constructor is declared as a reflective quotation boundary by
the sole authored language definition. -/
def isQuoteConstructor (language : LanguageDef) (constructor : String) : Bool :=
  language.reflectivePresentations.any
...
def substituteAt (language : LanguageDef) (support : ContextSupport.Support)
  ...
  | .apply constructor arguments =>
      ...
      .apply constructor
        (arguments.map (substituteAt language support assignment childDepth))
  | .lambda binderName body =>
      .lambda binderName
        (substituteAt language support assignment (availableDepth + 1) body)
  | .multiLambda arity binderNames body =>
      .multiLambda arity binderNames
        (substituteAt language support assignment
...
context. -/
def substitute (language : LanguageDef) (support : ContextSupport.Support)
    (assignment : ContextSupport.Assignment) (bound : List TypeExpr)
    (pattern : Pattern) : Pattern :=
  substituteAt language support assignment bound.length pattern
...
theorem substituteAt_eq_of_inputsAgreeOn
    ...
    substituteAt language firstSupport firstAssignment availableDepth pattern =
      substituteAt language secondSupport secondAssignment availableDepth
        pattern := by
...
theorem substitute_eq_of_inputsAgreeOn
    ...
    (agree : SubstitutionInputsAgreeOn pattern firstSupport secondSupport
      firstAssignment secondAssignment) :
    substitute language firstSupport firstAssignment bound pattern =
      substitute language secondSupport secondAssignment bound pattern := by
  exact substituteAt_eq_of_inputsAgreeOn language bound.length pattern agree
...
mutual
  ...
  inductive ReflectiveFreeVariableOccursAt
    ...
    | here {name type} {lookup : free name = some type}
        ...
        ReflectiveFreeVariableOccursAt
    ...
    | constructorQuote {bound rule arguments}
        ...
        (occurs : ReflectiveArgumentsFreeVariableOccursAt argumentsTyped
    ...
    | lambda {bound binder body domain codomain}
        {bodyTyped : HasType language free (domain :: bound) body codomain}
        {available name occurrenceSupport}
        (occurs : ReflectiveFreeVariableOccursAt bodyTyped
    ...
    | substReplacement {bound body replacement domain codomain}
        ...
        {replacementTyped : HasType language free bound replacement domain}
        {available name occurrenceSupport}
        (occurs : ReflectiveFreeVariableOccursAt replacementTyped
          available name occurrenceSupport) :
        ReflectiveFreeVariableOccursAt
          (HasType.subst bodyTyped replacementTyped)
          available name occurrenceSupport
    | collection {bound collectionType elements rest elementType}
        {elementsTyped : ElementsHaveType language free bound elements elementType}
        {available name occurrenceSupport}
        (occurs : ReflectiveElementsFreeVariableOccursAt elementsTyped
          available name occurrenceSupport) :
        ReflectiveFreeVariableOccursAt
          (HasType.collection (collectionType := collectionType)
  ...
  inductive ReflectiveArgumentsFreeVariableOccursAt
    ...
    | head {argument arguments parameter parameters expected}
        ...
        {argumentsTyped : ArgumentsHaveTypes language free bound
          arguments parameters}
        {available name occurrenceSupport}
        (occurs : ReflectiveFreeVariableOccursAt argumentTyped
          available name occurrenceSupport) :
        ReflectiveArgumentsFreeVariableOccursAt
          (ArgumentsHaveTypes.cons representation parameterType
  ...
  inductive ReflectiveElementsFreeVariableOccursAt
    ...
    | head {element elements}
        ...
        {elementsTyped : ElementsHaveType language free bound
          elements elementType}
        {available name occurrenceSupport}
        (occurs : ReflectiveFreeVariableOccursAt elementTyped
          available name occurrenceSupport) :
        ReflectiveElementsFreeVariableOccursAt
          (ElementsHaveType.cons elementTyped elementsTyped)
          available name occurrenceSupport
    | tail {element elements}
        {elementTyped : HasType language free bound element elementType}
...
def HasType.RespectsReflectiveSupportAt
  ...
  ∀ name occurrenceSupport,
    ReflectiveFreeVariableOccursAt typed available name occurrenceSupport →
...
def ArgumentsHaveTypes.RespectsReflectiveSupportAt
  ...
  ∀ name occurrenceSupport,
    ReflectiveArgumentsFreeVariableOccursAt typed
        available name occurrenceSupport →
      ∃ inner, occurrenceSupport = inner ++ support name
...
def ElementsHaveType.RespectsReflectiveSupportAt
  ...
  ∀ name occurrenceSupport,
    ReflectiveElementsFreeVariableOccursAt typed
        available name occurrenceSupport →
      ∃ inner, occurrenceSupport = inner ++ support name
...
mutual
  ...
  inductive HasType.ReflectiveSupportSafeAt
    ...
    | bvar {bound index type} (lookup : bound[index]? = some type)
        ...
        HasType.ReflectiveSupportSafeAt support
    ...
    | constructorQuote {bound rule arguments}
        ...
        (argumentsSafe : ArgumentsHaveTypes.ReflectiveSupportSafeAt
          support argumentsTyped [] binderImage) :
        HasType.ReflectiveSupportSafeAt support
    ...
    | constructorOrdinary {bound rule arguments}
        ...
        (ordinary : ReflectiveContextSupport.isQuoteConstructor
          language rule.label = false)
        (argumentsSafe : ArgumentsHaveTypes.ReflectiveSupportSafeAt
          support argumentsTyped available binderImage) :
        HasType.ReflectiveSupportSafeAt support
          (HasType.constructor membership notBare argumentsTyped) available
          binderImage
    | lambda {bound binder body domain codomain}
        {bodyTyped : HasType language free (domain :: bound) body codomain}
        {available : List TypeExpr} {binderImage : TypeExpr → TypeExpr}
        (bodySafe : HasType.ReflectiveSupportSafeAt support bodyTyped
    ...
    | subst {bound body replacement domain codomain}
        ...
        (bodySafe : HasType.ReflectiveSupportSafeAt support bodyTyped
          (binderImage domain :: available) binderImage)
        (replacementSafe : HasType.ReflectiveSupportSafeAt support replacementTyped
    ...
    | collection {bound collectionType elements rest elementType}
        ...
        (elementsSafe : ElementsHaveType.ReflectiveSupportSafeAt
          support elementsTyped available binderImage) :
        HasType.ReflectiveSupportSafeAt support
          (HasType.collection (collectionType := collectionType)
  ...
  inductive ArgumentsHaveTypes.ReflectiveSupportSafeAt
    ...
    | nil (bound available : List TypeExpr)
        ...
        ArgumentsHaveTypes.ReflectiveSupportSafeAt support
    ...
    | cons {bound argument arguments parameter parameters expected}
        ...
        (argumentSafe : HasType.ReflectiveSupportSafeAt
          support argumentTyped available binderImage)
        (argumentsSafe : ArgumentsHaveTypes.ReflectiveSupportSafeAt
          support argumentsTyped available binderImage) :
        ArgumentsHaveTypes.ReflectiveSupportSafeAt support
          (ArgumentsHaveTypes.cons representation parameterType
  ...
  inductive ElementsHaveType.ReflectiveSupportSafeAt
    ...
    | nil (bound : List TypeExpr) (elementType : TypeExpr)
        (available : List TypeExpr) {binderImage : TypeExpr → TypeExpr} :
        ElementsHaveType.ReflectiveSupportSafeAt support
    ...
    | cons {bound element elements elementType}
        ...
        (elementSafe : HasType.ReflectiveSupportSafeAt
          support elementTyped available binderImage)
        (elementsSafe : ElementsHaveType.ReflectiveSupportSafeAt
          support elementsTyped available binderImage) :
        ElementsHaveType.ReflectiveSupportSafeAt support
...
theorem HasType.ReflectiveSupportSafeAt.castTyping
    ...
    {available : List TypeExpr} {binderImage : TypeExpr → TypeExpr}
    (safe : source.ReflectiveSupportSafeAt support available binderImage) :
    target.ReflectiveSupportSafeAt support available binderImage := by
...
theorem ElementsHaveType.ReflectiveSupportSafeAt.forall_mem
    ...
    ∀ element ∈ elements,
      ∃ elementTyped : HasType language free bound element elementType,
        elementTyped.ReflectiveSupportSafeAt support available binderImage := by
  exact ElementsHaveType.ReflectiveSupportSafeAt.rec
    ...
    (motive_3 := fun {bound} {elements} {elementType} _ available
      ...
      ∀ element ∈ elements,
        ∃ elementTyped : HasType language free bound element elementType,
          elementTyped.ReflectiveSupportSafeAt support available binderImage)
    (by intros; trivial)
    (by intros; trivial)
    (by intros; trivial)
    (by intros; trivial)
...
theorem ElementsHaveType.ReflectiveSupportSafeAt.of_forall_mem
  ...
  | cons head tail inductionHypothesis =>
      ...
      have tailPointwise : ∀ element ∈ tail,
          ∃ elementTyped : HasType language free bound element elementType,
            elementTyped.ReflectiveSupportSafeAt support available
              binderImage := by
        intro element membership
...
theorem multiLambda_respectsReflectiveSupportAt
    ...
    {available : List TypeExpr} {support : ContextSupport.Support}
    (bodySupported : bodyTyped.RespectsReflectiveSupportAt
      (List.replicate arity domain ++ available) support) :
...
structure SupportedOpenAssignment (language : LanguageDef)
    (source target : FreeTypeContext)
    (support : ContextSupport.Support)
  ...
  reflectiveScopeSafe : ∀ {name type}, source name = some type →
    ReflectiveScopeSafeAt language (support name).length (assignment name)
...
mutual
  ...
  theorem HasType.ReflectiveSupportSafeAt.substituteBinderSafeAt
      ...
      (membership : presentation ∈ language.reflectivePresentations)
        ...
        | cons argument remainder =>
            ...
            | nil =>
                ...
                · have inputList :
                  ...
                  have selectedQuote :
                      ReflectiveContextSupport.isQuoteConstructor language
...
theorem HasType.ReflectiveSupportSafeAt.substituteReflectiveScopeSafe
    ...
    {typed : HasType language source bound pattern type}
    (assignment : SupportedOpenAssignment language source target support)
    (safe : typed.ReflectiveSupportSafeAt support bound)
    (scope : ReflectiveScopeSafeAt language bound.length pattern) :
    ReflectiveScopeSafeAt language bound.length
...
theorem MatchesParameterRepresentation.substituteReflectiveAt
    (language : LanguageDef) (parameter : TermParam) (pattern : Pattern)
    (support : ContextSupport.Support)
    (assignment : ContextSupport.Assignment) (availableDepth : Nat) :
    MatchesParameterRepresentation parameter pattern →
      MatchesParameterRepresentation parameter
...
mutual
  ...
  theorem HasType.substituteReflectiveSupportedAt
      ...
      (typed : HasType language source bound pattern type)
      (supported : typed.RespectsReflectiveSupportAt available support) :
    ...
    | @constructor bound rule arguments membership notBare argumentsTyped =>
        cases quoteStatus : ReflectiveContextSupport.isQuoteConstructor
            language rule.label with
        | false =>
            have argumentsSupported : ∀ name occurrenceSupport,
                ReflectiveArgumentsFreeVariableOccursAt argumentsTyped
                    available name occurrenceSupport →
    ...
    | @multiLambda bound arity binders body domain codomain bodyTyped =>
        have bodySupported : bodyTyped.RespectsReflectiveSupportAt
            (List.replicate arity domain ++ available) support := by
          intro name occurrenceSupport occurs
          exact supported name occurrenceSupport
    ...
    | @subst bound body replacement domain codomain bodyTyped replacementTyped =>
        ...
        have replacementSupported :
            replacementTyped.RespectsReflectiveSupportAt available support := by
          intro name occurrenceSupport occurs
          exact supported name occurrenceSupport
    ...
    | @collection bound collectionType elements rest elementType elementsTyped =>
        have elementsSupported : ∀ name occurrenceSupport,
            ReflectiveElementsFreeVariableOccursAt elementsTyped
                available name occurrenceSupport →
              ∃ inner, occurrenceSupport = inner ++ support name := by
          intro name occurrenceSupport occurs
          exact supported name occurrenceSupport
...
theorem HasType.substituteReflectiveSupported
    ...
    (assignment : SupportedAssignment language source target support)
    (typed : HasType language source bound pattern type)
    (supported : typed.RespectsReflectiveSupportAt bound support) :
    HasType language target bound
      (ReflectiveContextSupport.substitute language support
...
theorem HasType.ReflectiveSupportSafeAt.substituteOpenTermWellSorted
    ...
    (scope : ReflectiveScopeSafeAt language bound.length pattern) :
    OpenTermWellSorted language target bound sort
      (ReflectiveContextSupport.substitute language support
        assignment.assignment bound pattern) := by
  exact ⟨safe.substituteRoot assignment.toSupportedAssignment,
...
specialization. -/
def OpenPattern.substituteReflectiveSupported
    {language : LanguageDef} {source target : FreeTypeContext}
  ...
  ⟨ReflectiveContextSupport.substitute language support
    ...
    safe.substituteRoot assignment.toSupportedAssignment,
    safe.substituteCanonicalBinderMetadata assignment pattern.2.2.1,
    safe.substituteObjectPattern assignment pattern.2.2.2.1,
    safe.substituteReflectiveScopeSafe assignment pattern.2.2.2.2⟩

@[simp]
theorem OpenPattern.substituteReflectiveSupported_pattern
    {language : LanguageDef} {source target : FreeTypeContext}
    {support : ContextSupport.Support} {bound : List TypeExpr}
    {type : TypeExpr}
    (pattern : OpenPattern language source bound type)
    (assignment : SupportedOpenAssignment language source target support)
    (safe : pattern.2.1.ReflectiveSupportSafeAt support bound) :
    (pattern.substituteReflectiveSupported assignment safe).1 =
      ReflectiveContextSupport.substitute language support
        assignment.assignment bound pattern.1 :=
  rfl
...
def OpenTerm.substituteReflectiveSupported
    {language : LanguageDef} {source target : FreeTypeContext}
    {support : ContextSupport.Support} {bound : List TypeExpr}
    {sort : LangSort language}
  ...
  OpenPattern.substituteReflectiveSupported term assignment safe

@[simp]
theorem OpenTerm.substituteReflectiveSupported_pattern
    {language : LanguageDef} {source target : FreeTypeContext}
    {support : ContextSupport.Support} {bound : List TypeExpr}
    {sort : LangSort language}
    (term : OpenTerm language source bound sort)
    (assignment : SupportedOpenAssignment language source target support)
    (safe : term.2.1.ReflectiveSupportSafeAt support bound) :
    (term.substituteReflectiveSupported assignment safe).1 =
      ReflectiveContextSupport.substitute language support
...
theorem HasType.support_eq_nil_of_reflective_occurrence
    ...
    (supported : typed.RespectsReflectiveSupportAt available support)
    (occurs : ReflectiveFreeVariableOccursAt typed available name []) :
    support name = [] := by
...
theorem ArgumentsHaveTypes.length_eq_one_of_quote
  ...
  have presentationValid :=
    LanguageDef.reflectivePresentation_validate_of_validate_eq_nil language
      valid presentation presentationMembership
  obtain ⟨witness⟩ :=
...
theorem isWellScopedListAt_zero_of_typed_quote
    ...
    (quoted : ReflectiveContextSupport.isQuoteConstructor language
      rule.label = true)
    (scope : ReflectiveScopeSafeAt language bound.length
      (.apply rule.label arguments)) :
    Pattern.isWellScopedListAt 0 arguments = true := by
  have argumentLength := typed.length_eq_one_of_quote valid membership quoted
...
theorem reflectiveScopeSafeListAt_zero_of_typed_quote
  ...
  subst arguments
  have ordinaryScope :=
    isWellScopedListAt_zero_of_typed_quote valid membership typed quoted scope
