...
/-!
# Compatibility import for the former CertificateGSLT-local ambient layer

The ambient-first definitions now live at their correct abstraction level in
`Mettapedia.GSLT.Core.Ultrainfinite`.  CertificateGSLT-specific instances live in
`CertificateGSLTUltrainfiniteInstances`.
