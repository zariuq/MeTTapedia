...
def comorphismProjection
  ...
  preserves := by
    ...
    apply (route.satisfaction_condition signature targetModel conclusion).mpr
    exact (derives_iff_entails source signature premises conclusion).mp derives
      ((route.mapModel.app (Opposite.op signature)).toFunctor.obj targetModel) <| by
        ...
        exact (route.satisfaction_condition signature targetModel premise).mp
          (satisfiesMappedPremises (route.mapSentence.app signature premise)
