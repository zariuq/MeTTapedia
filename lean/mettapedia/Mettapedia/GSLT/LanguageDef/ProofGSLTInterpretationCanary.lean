...
namespace Mettapedia.GSLT.LanguageDef.ProofGSLT.InterpretationCanary
...
private def judgmentA : Pattern := .apply "ProofGSLT-A" []
private def judgmentB : Pattern := .apply "ProofGSLT-B" []
private def judgmentC : Pattern := .apply "ProofGSLT-C" []
...
private def targetRuleShare : RuleSchema :=
  { id := ⟨"proof-gslt-share-b"⟩
    metavariables := []
    premises := [judgmentB, judgmentB]
    conclusion := judgmentC }

private def sourcePresentation : Presentation :=
  { language := LanguageDef.empty "proof-gslt-source"
    calculus :=
      { judgments :=
          [{ head := "ProofGSLT-A", arity := 0 },
           { head := "ProofGSLT-C", arity := 0 }]
        rules := [sourceRule] } }

private def targetPresentation : Presentation :=
  { language := LanguageDef.empty "proof-gslt-target"
    calculus :=
      { judgments :=
          [{ head := "ProofGSLT-A", arity := 0 },
           { head := "ProofGSLT-B", arity := 0 },
           { head := "ProofGSLT-C", arity := 0 }]
...
/-- Positive side: a general proof-GSLT interpretation exists. -/
...
theorem strict_rule_retaining_interpretation_is_empty :
  ...
  apply RuleRetaining.hom_isEmpty_of_missing
      (ruleId := ⟨"proof-gslt-a-c"⟩) (rule := sourceRule)
  · rfl
  · rfl
...
end Mettapedia.GSLT.LanguageDef.ProofGSLT.InterpretationCanary
