...
/-!
# Authored source-to-target transformations

A compiler may be handwritten, but its output must be determined by the
authored source and target definitions that it receives.  This module gives
...
definitions equipped elsewhere with proofs that they denote their authored
GSLTs.  A bare semantic `GSLT` is not inspectable in general: its rewrite field
is a proposition-valued relation.  Keeping authored data explicit avoids
replacing compilation by a filename, digest, or fixed-callback dispatch.
-/

namespace Mettapedia.GSLT.LanguageDef.AuthoredTransformation
...
/-- Run an authored transformation in source-first order.
...
keeping the authored source and target definitions as ordinary
function inputs.
-/
...
/-- A real transformation reads authored semantic content, not merely its
identity. -/
...
end Mettapedia.GSLT.LanguageDef.AuthoredTransformation
