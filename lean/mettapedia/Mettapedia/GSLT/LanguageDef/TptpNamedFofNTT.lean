...
theorem shadowing_formula_inhabits_native_type :
    (langOSLF language "TptpNamedFof:Formula").satisfies
      Canary.encoded formulaNativeType.pred := by
...
theorem official_ast_is_not_named_formula :
    ¬ (langOSLF language "TptpNamedFof:Formula").satisfies
      TptpOfficialAbstractSyntax.fofAnnotatedExample formulaNativeType.pred := by
  change ¬ (checkHasType language WellSorted.FreeTypeContext.empty []
