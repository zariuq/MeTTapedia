...
/-!
# Language-definition-sensitive source-to-target transformations

A compiler may be handwritten, but its output must be determined by the
source and target definitions that it receives.  This module gives
...
definitions equipped elsewhere with proofs that they denote their operational
GSLTs.  A bare semantic `GSLT` is not inspectable in general: its rewrite field
is a proposition-valued relation.  Keeping source data explicit avoids
replacing compilation by a filename, digest, or fixed-callback dispatch.
-/

namespace Mettapedia.GSLT.LanguageDef.LanguageDefTransformation
...
definition.  `compile target source` is the existing certified realization
...
/-- Run a language-definition-sensitive transformation in source-first order.
...
under the supplied target definition. -/
...
/-- If two source definitions have different declared observations, an
...
keeping the source and target definitions as ordinary
function inputs.
-/
...
/-- A source- and target-definition-sensitive compiler whose successful
...
/-- Run the actual compiler on its supplied definitions.  Rejection is an
...
def positiveSource : SourceDefinition :=
  { identity := 7, meaning := true }

def negativeSource : SourceDefinition :=
  { identity := 7, meaning := false }

/-- A real transformation reads source semantic content, not merely its
identity. -/
def semanticTransformation :
    Transformation SourceDefinition Unit (fun _ => Bool) (fun _ => Bool) where
...
def IdentityOnly
    (transformation :
      Transformation SourceDefinition Unit (fun _ => Bool) (fun _ => Bool)) :
    Prop :=
  forall left right
...
/-- No adequate transformation that observes source meaning and accepts both
...
end Mettapedia.GSLT.LanguageDef.LanguageDefTransformation
