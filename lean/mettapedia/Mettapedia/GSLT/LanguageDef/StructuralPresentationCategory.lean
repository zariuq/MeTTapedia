...
/-!
# The extensional category of structural presentations

`PresentationSymbols` is intentionally total on strings, while a language
uses only finitely many authored declarations.  Raw equality of total symbol
...
source presentation.  Composition respects that equality because structural
morphisms carry authored declarations to authored declarations.
-/

namespace Mettapedia.GSLT.LanguageDef.StructuralPresentationCategory
...
every authored declaration of the source to the same target declaration. -/
...
/-- Structural maps modulo equality on the exact authored presentation. -/
...
def id (presentation : ValidatedLanguageDef) : Arrow presentation presentation :=
  ofMorphism (StructuralMorphism.id presentation)
...
/-- Validated presentations and extensional structural arrows form the
category in which disjoint presentation sums have their intended universal
property. -/
abbrev StructuralPresentation := ValidatedLanguageDef

instance : Category StructuralPresentation where
  Hom := Arrow
  id := Arrow.id
  comp := Arrow.comp
  id_comp morphism := by
...
the two images are observationally irrelevant to the source presentation. -/
...
noncomputable def copairSymbols
    ...
    (rightMap : StructuralMorphism right target) : PresentationSymbols where
...
theorem leftSymbols_comp_copairSymbols
    ...
    PresentationSymbols.comp leftSymbols
...
theorem rightSymbols_comp_copairSymbols
    ...
    (leftMap : StructuralMorphism left target)
    (rightMap : StructuralMorphism right target) :
    PresentationSymbols.comp rightSymbols
...
its arbitrary action on strings absent from the coproduct presentation. -/
noncomputable def copairMorphism
    ...
    (leftMap : StructuralMorphism left target)
    (rightMap : StructuralMorphism right target) :
    StructuralMorphism compatible.presentation target where
...
theorem equivalent_of_component_equivalent
    ...
    {left right target : ValidatedLanguageDef}
    (compatible : Compatibility name leftSymbols rightSymbols left right)
    {first second : StructuralMorphism compatible.presentation target}
...
/-- Categorical injection of the left presentation. -/
def leftArrow
    {name : String} {leftSymbols rightSymbols : PresentationSymbols}
    {left right : ValidatedLanguageDef}
    (compatible : Compatibility name leftSymbols rightSymbols left right) :
    left ⟶ compatible.presentation :=
  Arrow.ofMorphism compatible.leftInclusion

/-- Categorical injection of the right presentation. -/
def rightArrow
    {name : String} {leftSymbols rightSymbols : PresentationSymbols}
    {left right : ValidatedLanguageDef}
    (compatible : Compatibility name leftSymbols rightSymbols left right) :
    right ⟶ compatible.presentation :=
...
noncomputable def copair
    ...
    compatible.presentation ⟶ target :=
...
theorem hom_ext
    ...
    {left right target : ValidatedLanguageDef}
    (compatible : Compatibility name leftSymbols rightSymbols left right)
    {first second : compatible.presentation ⟶ target}
...
declaration-observational category of validated presentations. -/
...
end Mettapedia.GSLT.LanguageDef.StructuralPresentationCategory
