...
/-!
# SPD-preconditioned updates and the BP--PC chord

Exact two-dimensional constructions separating four update-geometry claims:
escaping the affine BP--PC chord is achievable by an SPD-preconditioned BP
step; additive broadcast/local hybrids can collapse exactly to the identity;
warm starts move finite trajectories but not a contractive endpoint; and a
family of probes admitting no single linear map can still be realized by
per-probe SPD metrics.  A skew (rotation-derived) transport supplies the
non-symmetric boundary case with explicit descent and ascent step sizes.
-/

namespace Mettapedia.MachineLearning.NeuralNetworks.CreditTransport

namespace SPDPreconditionedUpdateChord
...
theorem chordEscape_not_mem_chord_and_spdPreconditioned :
    (¬ ∃ coefficient : ℝ,
...
theorem additiveHybrid_mulVec_eq_self (update : Vec2) :
...
theorem warmStart_trajectory_ne_and_fixedPoint_unique :
...
/-! ## No single linear map, yet per-probe SPD metrics suffice -/

def probe : Fin 3 → Vec2
  | 0 => ![1, 0]
  | 1 => ![0, 1]
  | 2 => ![1, 1]

def targetUpdate : Fin 3 → Vec2
  | 0 => ![1, 0]
  | 1 => ![0, 1]
  | 2 => ![2, 1]

def spdMetric : Fin 3 → Mat2
  | 0 => !![1, 0; 0, 1]
  | 1 => !![2, 0; 0, 1]
  | 2 => !![2, 0; 0, 1]

theorem not_exists_fixedMap_forall_mulVec_eq :
    ¬ ∃ fixedMap : Mat2,
      ∀ index : Fin 3, fixedMap *ᵥ probe index = targetUpdate index := by
  intro hexists
  ...
  norm_num [probe, targetUpdate, Matrix.mulVec, Fin.sum_univ_two] at h00 h10 h01 h11 hthird
  linarith
...
theorem gateMetric_posDef (index : Fin 3) :
    (spdMetric index).PosDef := by
  fin_cases index
  · exact diagonalMetric_posDef 1 1 (by norm_num) (by norm_num)
  · exact diagonalMetric_posDef 2 1 (by norm_num) (by norm_num)
  · exact diagonalMetric_posDef 2 1 (by norm_num) (by norm_num)

theorem spdMetric_mulVec_eq (index : Fin 3) :
    spdMetric index *ᵥ probe index = targetUpdate index := by
  fin_cases index <;> ext coordinate <;> fin_cases coordinate <;>
    norm_num [spdMetric, probe, targetUpdate, Matrix.mulVec,
      Fin.sum_univ_two]

theorem not_exists_fixedMap_and_forall_spdMetric_mulVec_eq :
    (¬ ∃ fixedMap : Mat2,
        ∀ index : Fin 3, fixedMap *ᵥ probe index = targetUpdate index) ∧
      (∀ index : Fin 3,
        (spdMetric index).PosDef ∧
          spdMetric index *ᵥ probe index = targetUpdate index) := by
  refine ⟨not_exists_fixedMap_forall_mulVec_eq, ?_⟩
  intro index
  exact ⟨gateMetric_posDef index, spdMetric_mulVec_eq index⟩

/-! ## Skew transport: asymmetric geometry with explicit descent/ascent steps -/
...
theorem skewTransport_geometry :
...
#print axioms chordEscape_not_mem_chord_and_spdPreconditioned
#print axioms additiveHybrid_mulVec_eq_self
#print axioms warmStart_trajectory_ne_and_fixedPoint_unique
#print axioms not_exists_fixedMap_forall_mulVec_eq
#print axioms not_exists_fixedMap_and_forall_spdMetric_mulVec_eq
#print axioms skewTransport_geometry

end

end SPDPreconditionedUpdateChord
