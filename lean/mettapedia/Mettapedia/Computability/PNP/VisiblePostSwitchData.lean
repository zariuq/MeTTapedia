...
/-!
# P vs NP crux: the exact visible post-switch data domain
...
* the full visible data domain is the exact manuscript input itself;
* the invariant visible data domain is the projection `(z, a)`;
* the fork data domain is the exact retained data `((z, a), b)`.
...
/-- The manuscript's exact visible post-switch data domain. -/
abbrev ExactVisiblePostSwitchData (Z : Type*) (k : ℕ) := PostSwitchInput Z k

/-- The always-invariant visible projection obtained by dropping the VV right-hand side. -/
abbrev InvariantPostSwitchData (Z : Type*) (k : ℕ) := Z × BitVec k

/-- The exact fork data domain keeping the invariant projection together with the side channel `b`. -/
abbrev ForkPostSwitchData (Z : Type*) (k : ℕ) :=
  InvariantPostSwitchData Z k × BitVec k

/-- The full visible data map, written explicitly for theorem interfaces. -/
def fullVisibleData (u : ExactVisiblePostSwitchData Z k) :
    ExactVisiblePostSwitchData Z k := u

/-- The manuscript's invariant projection `(z, a)`. -/
def invariantVisibleData (u : ExactVisiblePostSwitchData Z k) :
    InvariantPostSwitchData Z k :=
  invariantProjection u

/-- The exact fork-visible data `((z, a), b)`. -/
def forkVisibleData (u : ExactVisiblePostSwitchData Z k) :
    ForkPostSwitchData Z k :=
  (invariantVisibleData u, u.b)

@[simp] theorem fullVisibleData_eq (u : ExactVisiblePostSwitchData Z k) :
    fullVisibleData u = u := rfl

@[simp] theorem invariantVisibleData_eq (u : ExactVisiblePostSwitchData Z k) :
    invariantVisibleData u = (u.z, u.a) := rfl

@[simp] theorem forkVisibleData_eq (u : ExactVisiblePostSwitchData Z k) :
...
/-- The exact fork data domain is definitionally equivalent to the full manuscript input. -/
def forkVisibleEquiv :
    ExactVisiblePostSwitchData Z k ≃ ForkPostSwitchData Z k where
