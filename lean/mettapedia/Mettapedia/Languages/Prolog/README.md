...
Import via `Mettapedia.Languages.Prolog.Prolog` ([Prolog.lean](Prolog.lean)).

## Build

```bash
# from repository root
lake build Mettapedia.Languages.Prolog.Prolog
```
...
- [PeTTa layer](../MeTTa/PeTTa) — evaluation, effects, expression compilation

## Formalization status

All 5 `.lean` files in this directory build with closed proofs. The semantics is
an inductive relation (`PrologEval`); the 242 fixture theorems and the 4
runtime-error boundary declarations are discharged constructively (many by `rfl`).
