...
/-!
# IntrinsicPure Embedding Into MeTTa Core Profiles

Scaffold linking the trusted scoped kernel (`PureTm`) with the MeTTa profile
interface through contextual quotation.
-/

namespace Mettapedia.Languages.MeTTa.Pure.Intrinsic.CoreEmbedding
...
/-- Minimal embedding contract for closed IntrinsicPure terms into a MeTTa profile. -/
...
/-- Canonical embedding of IntrinsicPure into the Pure core profile. -/
def pureKernelIntoPureProfile : KernelEmbedding where
  profile := pureProfile
  kernel := mettaIntrinsicPureTyped
...
def checkedNoValuesDeclKernelIntoPureProfileOfPackage
    ...
    (hPkg :
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSpec.DeclSpecAndNoValuesPackage
        specs hNone) :
...
def checkedChurchRosserDeclKernelIntoPureProfileOfPackage
    ...
    (hSig :
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSpec.SignatureWellFormed
        specs)
    (hPkg :
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSpec.DeclSpecChurchRosserPackage
        specs) :
    KernelEmbedding where
  profile := pureProfile
...
def checkedChurchRosserDeclKernelIntoPureProfile
    ...
    (hCR :
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSemantics.DeclChurchRosser
        (Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSpec.envOfSpecs specs)) :
    KernelEmbedding :=
...
theorem checkedNoValuesDeclKernel_sound_pureProfileTheoryStep_quoteClosed
    ...
    (hcompat0 : QuoteCompat defaultBinderName 0 emptyEnv)
    {t u : PureTm 0}
    (h : Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSemantics.RedDecl
      (Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSpec.envOfSpecs specs) t u) :
    PureProfileTheoryStep (quoteClosedTm t) (quoteClosedTm u) := by
  have hNoValues :=
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSpec.valueOf_envOfSpecs_eq_none_of_all_none
      specs hNone
  have hCore :
      Red t u :=
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSemantics.redDecl_to_core_of_no_values
      hNoValues h
...
theorem checkedNoValuesDeclKernel_star_sound_pureProfileTheoryStepStar_quoteClosed
  ...
  have hCore :
      RedStar t u :=
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSemantics.redStarDecl_to_core_of_no_values
      hNoValues h
...
end Mettapedia.Languages.MeTTa.Pure.Intrinsic.CoreEmbedding
