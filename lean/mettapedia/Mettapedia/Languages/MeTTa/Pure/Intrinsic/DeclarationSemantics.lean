...
namespace Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSemantics
...
theorem parDecl_to_cdev_of_no_values {E : DeclEnv}
    (hNone : ∀ c : DeclName, valueOf? E c = none)
    {t u : PureTm n} (h : ParRedDecl E t u) :
    ParRedDecl E u (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.cdev t) :=
  parDecl_core <|
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.par_to_cdev <|
...
theorem diamond_parRedDecl_of_no_values {E : DeclEnv}
    ...
    ∃ u, ParRedDecl E t₁ u ∧ ParRedDecl E t₂ u :=
  ⟨ Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.cdev s
...
theorem redStarDecl_confluence_of_no_values {E : DeclEnv}
  ...
  have h₂core : RedStar s t₂ := redStarDecl_to_core_of_no_values hNone h₂
  rcases Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.redStar_confluence h₁core h₂core with
    ⟨u, hu₁, hu₂⟩
...
theorem convDecl_to_cdev_of_no_values {E : DeclEnv}
    ...
    (t : PureTm n) :
    ConvDecl E t (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.cdev t) :=
  conv_core_to_decl (Mettapedia.Languages.MeTTa.Pure.Intrinsic.conv_to_cdev t)

theorem convDecl_of_cdev_eq_of_no_values {E : DeclEnv}
    (_hNone : ∀ c : DeclName, valueOf? E c = none)
    {A B : PureTm n}
    (h : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.cdev A =
        Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.cdev B) :
    ConvDecl E A B := by
  exact conv_core_to_decl
    (Mettapedia.Languages.MeTTa.Pure.Intrinsic.conv_of_cdev_eq h)
...
def defEqByNormalizationDeclOfNoValues?
  ...
  if h :
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.cdev A =
        Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.cdev B then
...
theorem defEqByNormalizationDeclOfNoValues_not_complete {E : DeclEnv}
  ...
  rcases Mettapedia.Languages.MeTTa.Pure.Intrinsic.defEqByNormalization_not_complete with
    ⟨Γ, t, u, A, ht, hu, hconv, hnone⟩
  have hcdev :
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.cdev t ≠
        Mettapedia.Languages.MeTTa.Pure.Intrinsic.Confluence.cdev u := by
    intro hEq
    have hSome :
        Mettapedia.Languages.MeTTa.Pure.Intrinsic.defEqByNormalization? t u ≠ none := by
      simp [Mettapedia.Languages.MeTTa.Pure.Intrinsic.defEqByNormalization?, hEq]
    exact hSome hnone
...
end Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSemantics
