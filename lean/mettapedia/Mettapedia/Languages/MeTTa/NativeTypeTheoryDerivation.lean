...
behaviour.  The bags below are *auditable data*: correcting them is cheap and
changes the forced requirement set mechanically. -/
...
/-- Prime: the full dialect — all eight observations. -/
...
/-- HE: collections, metavariables, reflection, spaces, inert unknowns. -/
...
/-- PeTTa: same observable interface as HE for typing purposes. -/
...
theorem currentZeroPresentation_ne_currentPrimePresentation :
  ...
  change "metta-zero-query-kernel" = "metta-prime-nucleus" at names
  have distinct : "metta-zero-query-kernel" ≠ "metta-prime-nucleus" := by decide
...
structure PureEqualityProfile where
  ...
  subst_closed : ∀ {n m}
    ...
    (∀ index, Rel (leftSub index) (rightSub index)) → Rel left right →
      Rel
        (Mettapedia.Languages.MeTTa.PureKernel.Substitution.subst leftSub left)
...
def rename (profile : PureEqualityProfile) {n m : Nat}
    ...
    Quotient (profile.setoid n) → Quotient (profile.setoid m) :=
  profile.mapOne
    (Mettapedia.Languages.MeTTa.PureKernel.Renaming.rename ρ)
...
def substRaw (profile : PureEqualityProfile) {n m : Nat}
    ...
    Quotient (profile.setoid n) → Quotient (profile.setoid m) :=
  profile.mapOne
    (Mettapedia.Languages.MeTTa.PureKernel.Substitution.subst σ)
...
@[simp] theorem rename_mk (profile : PureEqualityProfile) {n m : Nat}
    ...
    profile.rename ρ (Quotient.mk (profile.setoid n) term) =
      Quotient.mk (profile.setoid m)
        (Mettapedia.Languages.MeTTa.PureKernel.Renaming.rename ρ term) :=
  rfl
...
@[simp] theorem substRaw_mk (profile : PureEqualityProfile) {n m : Nat}
    ...
    profile.substRaw σ (Quotient.mk (profile.setoid n) term) =
      Quotient.mk (profile.setoid m)
        (Mettapedia.Languages.MeTTa.PureKernel.Substitution.subst σ term) :=
  rfl
...
theorem substRaw_ids (profile : PureEqualityProfile) {n : Nat}
    ...
    profile.substRaw
      (Mettapedia.Languages.MeTTa.PureKernel.Substitution.ids (n := n)) term =
      term := by
...
theorem substRaw_comp (profile : PureEqualityProfile) {n m k : Nat}
    (τ : Mettapedia.Languages.MeTTa.PureKernel.Substitution.Sub m k)
    (σ : Mettapedia.Languages.MeTTa.PureKernel.Substitution.Sub n m)
    (term : Quotient (profile.setoid n)) :
    profile.substRaw τ (profile.substRaw σ term) =
      profile.substRaw
        (fun index =>
          Mettapedia.Languages.MeTTa.PureKernel.Substitution.subst τ (σ index))
        term := by
  refine Quotient.inductionOn term ?_
  intro syntaxTerm
  simp
...
abbrev IntrinsicPureConv {n : Nat} :=
  @Mettapedia.Languages.MeTTa.PureKernel.Typing.Conv n

abbrev IntrinsicPureRed {n : Nat} :=
  @Mettapedia.Languages.MeTTa.PureKernel.Reduction.Red n
...
theorem intrinsicPureConv_subst_pointwise {n m : Nat}
    ...
    ∀ term : PureTm n, IntrinsicPureConv
      (Mettapedia.Languages.MeTTa.PureKernel.Substitution.subst leftSub term)
      (Mettapedia.Languages.MeTTa.PureKernel.Substitution.subst rightSub term) := by
  intro term
  ...
  | pi domain body domainIH bodyIH =>
      ...
      · apply bodyIH
        ...
        · exact .refl _
        · intro predecessor
          exact Mettapedia.Languages.MeTTa.PureKernel.Typing.conv_rename
...
theorem intrinsicPureConv_subst_congr {n m : Nat}
    ...
    IntrinsicPureConv
      ...
      (Mettapedia.Languages.MeTTa.PureKernel.Substitution.subst rightSub right) :=
  Relation.EqvGen.trans _ _ _
    (Mettapedia.Languages.MeTTa.PureKernel.Typing.conv_subst leftSub terms)
...
inductive NativeRawTm : Nat → Nat → Type where
  | var {stage binders : Nat} : Fin binders → NativeRawTm stage binders
  | const {stage binders : Nat} : DeclName → NativeRawTm stage binders
  | u0 {stage binders : Nat} : NativeRawTm stage binders
  | u1 {stage binders : Nat} : NativeRawTm stage binders
  | pi {stage binders : Nat} : NativeRawTm stage binders →
      NativeRawTm stage (binders + 1) → NativeRawTm stage binders
  | sigma {stage binders : Nat} : NativeRawTm stage binders →
      NativeRawTm stage (binders + 1) → NativeRawTm stage binders
  | id {stage binders : Nat} : NativeRawTm stage binders →
      NativeRawTm stage binders → NativeRawTm stage binders →
      NativeRawTm stage binders
  | lam {stage binders : Nat} : NativeRawTm stage (binders + 1) →
      NativeRawTm stage binders
  | app {stage binders : Nat} : NativeRawTm stage binders →
      NativeRawTm stage binders → NativeRawTm stage binders
  | pair {stage binders : Nat} : NativeRawTm stage binders →
      NativeRawTm stage binders → NativeRawTm stage binders
  | fst {stage binders : Nat} : NativeRawTm stage binders →
      NativeRawTm stage binders
  | snd {stage binders : Nat} : NativeRawTm stage binders →
      NativeRawTm stage binders
  | refl {stage binders : Nat} : NativeRawTm stage binders →
      NativeRawTm stage binders
  ...
  | quote {high low binders : Nat} : StageHom high low →
      NativeRawTm high binders → NativeRawTm low binders
...
def nativeRename (rho : NativeRen source target) :
    NativeRawTm stage source → NativeRawTm stage target
...
abbrev NativeSub (source target : Nat) :=
  (stage : Nat) → Fin source → NativeRawTm stage target
...
theorem nativeLiftRen_eq_pureLiftRen (rho : NativeRen source target) :
    nativeLiftRen rho =
      Mettapedia.Languages.MeTTa.PureKernel.Renaming.liftRen rho := by
  funext index
  refine Fin.cases ?_ ?_ index
  · rfl
  · intro previous
...
theorem nativeRename_ext {left right : NativeRen source target}
    ...
    ∀ {stage} (term : NativeRawTm stage source),
...
@[simp] theorem nativeRename_id :
    ∀ {stage} (term : NativeRawTm stage binders),
...
@[simp] theorem nativeSubst_quote
    ...
    (term : NativeRawTm high source) :
    nativeSubst substitution (NativeRawTm.quote route term) =
      NativeRawTm.quote route (nativeSubst substitution term) :=
  rfl
...
@[simp] theorem nativeSubst_liftSub_wk
    (substitution : NativeSub source target)
    {stage : Nat} (term : NativeRawTm stage source) :
...
abbrev NativeTermFamily (binders : Nat) :=
  (stage : Nat) → NativeRawTm stage binders
...
@[simp] theorem nativeInlineLet_var_zero
    ...
    nativeInlineLet value
        (NativeRawTm.var (0 : Fin (binders + 1)) :
          NativeRawTm stage (binders + 1)) =
      value stage :=
  rfl
...
theorem nativeLet_is_primitive_before_inlining :
    (NativeRawTm.letE .u0 (.var (0 : Fin 1)) : NativeRawTm 0 0) ≠
...
abbrev nativeSyntaxAlgebra : NativeRawAlgebra where
  atStage := fun stage =>
    { Carrier := NativeRawTm stage
      var := NativeRawTm.var
      const := NativeRawTm.const
      u0 := NativeRawTm.u0
      u1 := NativeRawTm.u1
      pi := NativeRawTm.pi
      sigma := NativeRawTm.sigma
      id := NativeRawTm.id
      lam := NativeRawTm.lam
      app := NativeRawTm.app
      pair := NativeRawTm.pair
      fst := NativeRawTm.fst
      snd := NativeRawTm.snd
      refl := NativeRawTm.refl }
  pattern := NativeRawTm.pattern
  empty := NativeRawTm.empty
  superpose := NativeRawTm.superpose
  letE := NativeRawTm.letE
  language := NativeRawTm.language
  quote := NativeRawTm.quote

/-- Structural interpretation of native raw syntax in any native algebra. -/
def nativeRawFold (target : NativeRawAlgebra.{uRawTarget}) :
    {stage binders : Nat} → NativeRawTm stage binders →
      (target.atStage stage).Carrier binders
...
theorem nativeRawFold_unique_pointwise
    ...
    ∀ {stage binders} (term : NativeRawTm stage binders),
...
stage. -/
def embedPureSub
    (substitution : Mettapedia.Languages.MeTTa.PureKernel.Substitution.Sub
...
theorem nativeRename_embedPure (rho : NativeRen source target) (stage : Nat) :
    ∀ term : PureTm source,
      nativeRename rho (embedPure stage term) =
        embedPure stage
          (Mettapedia.Languages.MeTTa.PureKernel.Renaming.rename rho term) := by
  intro term
  induction term generalizing target with
  | var index => rfl
  | const name => rfl
  ...
  | fst pair pairIH =>
      simp [embedPure, nativeRename,
        Mettapedia.Languages.MeTTa.PureKernel.Renaming.rename, pairIH rho]
  | snd pair pairIH =>
      simp [embedPure, nativeRename,
        Mettapedia.Languages.MeTTa.PureKernel.Renaming.rename, pairIH rho]
  | refl value valueIH =>
      simp [embedPure, nativeRename,
        Mettapedia.Languages.MeTTa.PureKernel.Renaming.rename, valueIH rho]
...
theorem nativeSubst_embedPure
  ...
  | lam body bodyIH =>
      ...
      rw [nativeLiftSub_embedPureSub]
      exact bodyIH
        (Mettapedia.Languages.MeTTa.PureKernel.Substitution.liftSub substitution)
  | app function argument functionIH argumentIH =>
      simp [embedPure, nativeSubst,
        Mettapedia.Languages.MeTTa.PureKernel.Substitution.subst,
...
recursively. -/
def NativeRawTm.pureProjection :
    {stage binders : Nat} → NativeRawTm stage binders → Option (PureTm binders)
  ...
  | _, _, NativeRawTm.fst value =>
      return PureTm.fst (← value.pureProjection)
  | _, _, NativeRawTm.snd value =>
      return PureTm.snd (← value.pureProjection)
  | _, _, NativeRawTm.refl value =>
...
theorem embedPure_injective (stage : Nat) {binders : Nat} :
    Function.Injective (embedPure stage : PureTm binders → NativeRawTm stage binders) := by
  intro left right equal
  have projected := congrArg NativeRawTm.pureProjection equal
...
theorem stageSensitiveSub_zero_component :
    stageSensitiveSub 0 (0 : Fin 1) = (NativeRawTm.u0 : NativeRawTm 0 0) := by
  rfl

theorem stageSensitiveSub_one_component :
    stageSensitiveSub 1 (0 : Fin 1) = (NativeRawTm.u1 : NativeRawTm 1 0) := by
  rfl

/-- Unquoted syntax selects the current stage's component. -/
theorem stageSensitiveSub_unquoted :
    nativeSubst stageSensitiveSub (NativeRawTm.var (0 : Fin 1) :
      NativeRawTm 0 1) = .u0 := by
  rfl
...
theorem stageSensitiveSub_quoted :
    nativeSubst stageSensitiveSub
        (NativeRawTm.quote oneToZeroQuotation
          (NativeRawTm.var (0 : Fin 1) : NativeRawTm 1 1)) =
      NativeRawTm.quote oneToZeroQuotation
        (NativeRawTm.u1 : NativeRawTm 1 0) := by
  rfl
...
def quotedPureUniverse : NativeRawTm 0 0 :=
  .quote oneToZeroQuotation (.u0 : NativeRawTm 1 0)
...
def mixedNativeApplication : NativeRawTm 0 0 :=
...
theorem pureProjection_none_not_in_pure_image
    {stage binders : Nat} (native : NativeRawTm stage binders)
...
def nativeRuntimePattern : NativeRawTm 0 0 :=
...
def nativeUniverseSuperposition : NativeRawTm 0 0 :=
  .superpose .u0 .u1
...
term. -/
def nativeCurrentPrimeLanguage : NativeRawTm 0 0 :=
  .language
    Mettapedia.Languages.MeTTa.Prime.LanguageDef.currentPrimePresentation
...
structure TypingAt (stage : Nat) (context : Ctx binders)
    (term type : NativeRawTm stage binders) where
...
def TypingAt.subst {stage : Nat} {sourceContext : Ctx source}
    {targetContext : Ctx target} {term type : NativeRawTm stage source}
...
def u0Typing (stage : Nat) (context : Ctx binders) :
    TypingAt stage context (.u0 : NativeRawTm stage binders) .u1 :=
...
def Context.lookup : Context binders →
    (stage : Nat) → Fin binders → NativeRawTm stage binders
...
structure ConversionPolicy where
  ...
  subst_closed : ∀ {stage source target}
    (substitution : NativeSub source target)
    {left right : NativeRawTm stage source},
    Rel left right →
...
@[simp] theorem subst0_rename_wk (argument : TermFamily binders)
    (term : NativeRawTm stage binders) :
...
theorem subst_inst0 (substitution : NativeSub source target)
    ...
    (body : NativeRawTm stage (source + 1)) :
...
inductive HasType (conversion : ConversionPolicy) :
    Context binders → NativeRawTm stage binders →
      NativeRawTm stage binders → Prop where
  ...
  | pi_form {context : Context binders} {domain : TermFamily binders}
      {body : NativeRawTm stage (binders + 1)} :
  ...
  | id_form {context : Context binders} {type left right : NativeRawTm stage binders} :
  ...
  | refl_intro {context : Context binders} {term type : NativeRawTm stage binders} :
...
theorem primitiveLet_has_native_modal_type :
    HasType syntacticConversion .nil
      (NativeRawTm.letE .u0 (.var (0 : Fin 1)) : NativeRawTm 0 0) .u1 := by
...
theorem primitiveLet_typable_but_not_syntactically_inlined :
    Nonempty (HasType syntacticConversion .nil
      (NativeRawTm.letE .u0 (.var (0 : Fin 1)) : NativeRawTm 0 0) .u1) ∧
      (NativeRawTm.letE .u0 (.var (0 : Fin 1)) : NativeRawTm 0 0) ≠
...
theorem typing_rename {conversion : ConversionPolicy}
    {context : Context source} {term type : NativeRawTm stage source}
...
theorem weakening {conversion : ConversionPolicy}
    {context : Context binders} {term type : NativeRawTm stage binders}
    (typing : HasType conversion context term type)
...
def embedPureContext :
    Mettapedia.Languages.MeTTa.PureKernel.Context.Ctx binders →
      Context binders
  | .nil => .nil
  | .snoc context type =>
...
theorem lookup_embedPureContext
    (context : Mettapedia.Languages.MeTTa.PureKernel.Context.Ctx binders)
    (stage : Nat) (index : Fin binders) :
    (embedPureContext context).lookup stage index =
      embedPure stage
        (Mettapedia.Languages.MeTTa.PureKernel.Context.lookup context index) := by
  ...
  | @snoc binders context type contextIH =>
      ...
      · change nativeRename nativeWk (embedPure stage type) =
          embedPure stage
            (Mettapedia.Languages.MeTTa.PureKernel.Renaming.rename
              Mettapedia.Languages.MeTTa.PureKernel.Renaming.wk type)
...
theorem subst0_embedPure (argument : PureTm binders) :
    subst0 (fun stage => embedPure stage argument) =
      embedPureSub
        (Mettapedia.Languages.MeTTa.PureKernel.Substitution.subst0 argument) := by
  funext stage index
  refine Fin.cases ?_ ?_ index
  · rfl
  · intro previous
...
@[simp] theorem inst0_embedPure (argument : PureTm binders)
  ...
  exact nativeSubst_embedPure
    (Mettapedia.Languages.MeTTa.PureKernel.Substitution.subst0 argument)
    stage body
...
theorem typing_embedPure {conversion : ConversionPolicy}
    ...
    {term type : PureTm binders}
    (typing :
      Mettapedia.Languages.MeTTa.PureKernel.Typing.HasType context term type) :
    ∀ stage,
...
theorem quotedPureUniverse_has_native_modal_type :
    HasType syntacticConversion .nil quotedPureUniverse
      (.quote oneToZeroQuotation (.u1 : NativeRawTm 1 0)) := by
...
theorem quotation_typing_separates_native_from_intrinsic :
    HasType syntacticConversion .nil quotedPureUniverse
        (.quote oneToZeroQuotation (.u1 : NativeRawTm 1 0)) ∧
      ¬ ∃ type : NativeRawTm 0 0,
        Nonempty
...
def ConversionPolicy.Extends (larger smaller : ConversionPolicy) : Prop :=
  ∀ {stage binders} {left right : NativeRawTm stage binders},
...
structure TypingAlgebra (target : NativeRawAlgebra.{uRawTarget})
  ...
  u0_type : ∀ {stage binders} (context : Context binders),
    Judges (foldContext contexts context)
      (nativeRawFold target (.u0 : NativeRawTm stage binders))
      (nativeRawFold target (.u1 : NativeRawTm stage binders))
  var : ∀ {stage binders} (context : Context binders)
      (index : Fin binders),
    Judges (foldContext contexts context)
      (nativeRawFold target (.var index : NativeRawTm stage binders))
  ...
  pi_form : ∀ {stage binders} {context : Context binders}
    ...
    Judges (foldContext contexts (.snoc context domain))
      (nativeRawFold target body)
      (nativeRawFold target (.u1 : NativeRawTm stage (binders + 1))) →
    Judges (foldContext contexts context)
  ...
  app_elim : ∀ {stage binders} {context : Context binders}
      {function : NativeRawTm stage binders}
  ...
  id_form : ∀ {stage binders} {context : Context binders}
      {type left right : NativeRawTm stage binders},
    Judges (foldContext contexts context)
  ...
  pattern_intro : ∀ {stage binders} (context : Context binders)
      (value : Pattern),
    Judges (foldContext contexts context)
      (nativeRawFold target (.pattern value : NativeRawTm stage binders))
      (nativeRawFold target (.u0 : NativeRawTm stage binders))
  language_intro : ∀ {stage binders} (context : Context binders)
      (value : Mettapedia.GSLT.LanguageDef.ValidatedLanguageDef),
    Judges (foldContext contexts context)
      (nativeRawFold target (.language value : NativeRawTm stage binders))
      (nativeRawFold target (.u0 : NativeRawTm stage binders))
  empty_intro : ∀ {stage binders} {context : Context binders}
      {type : NativeRawTm stage binders},
    Judges (foldContext contexts context)
      (nativeRawFold target type)
      (nativeRawFold target (.u1 : NativeRawTm stage binders)) →
    Judges (foldContext contexts context)
      (nativeRawFold target (.empty : NativeRawTm stage binders))
      (nativeRawFold target type)
  ...
  conv : ∀ {stage binders} {context : Context binders}
      {term left right : NativeRawTm stage binders},
    Judges (foldContext contexts context)
...
theorem universe_has_image (model : Model.{uRawTarget}) :
    model.typing.Judges (foldContext model.contexts (.nil : Context 0))
      (nativeRawFold model.raw (.u0 : NativeRawTm 0 0))
      (nativeRawFold model.raw (.u1 : NativeRawTm 0 0)) :=
...
def embedRuntimePattern (value : Pattern) : NativeRawTm stage binders :=
  .pattern value

/-- Partial projection onto the direct live runtime-Pattern image. -/
def NativeRawTm.runtimePattern? :
    NativeRawTm stage binders → Option Pattern
...
theorem runtimePattern_image_iff (term : NativeRawTm stage binders) :
  ...
  · rintro ⟨value, rfl⟩
    exact ⟨value, rfl⟩
  · cases term <;>
      simp [NativeRawTm.runtimePattern?, embedRuntimePattern]
...
theorem currentPrimeReflectedDemandProgram_roundtrip :
    (embedRuntimePattern currentPrimeReflectedDemandProgram :
      NativeRawTm 0 0).runtimePattern? =
...
theorem quotedPureUniverse_not_runtimePattern_image :
    ¬ ∃ value : Pattern,
      (embedRuntimePattern value : NativeRawTm 0 0) = quotedPureUniverse := by
  rintro ⟨value, equal⟩
  cases equal

/-! ### Cost and evidence remain a fibred decoration

Putting cost or evidence inside `NativeRawTm` would make operational metadata
...
def undecorated {stage binders : Nat} (term : NativeRawTm stage binders) :
...
def withCost {stage binders : Nat} (term : NativeRawTm stage binders)
...
def withEvidence {stage binders : Nat} (term : NativeRawTm stage binders)
...
@[simp] theorem withCost_term {stage binders : Nat}
    (term : NativeRawTm stage binders) (account : NativeCostAccount) :
    (withCost term account).term = term := rfl

@[simp] theorem withEvidence_term {stage binders : Nat}
    (term : NativeRawTm stage binders) (evidence : NativeEvidence) :
...
def nativeNodeCount {stage binders : Nat} (term : NativeRawTm stage binders) : Nat :=
...
theorem nativeNodeCount_positive {stage binders : Nat}
    (term : NativeRawTm stage binders) : 0 < nativeNodeCount term := by
  induction term <;>
...
theorem nodeCount_primitiveLet_image :
    nodeCountModel.typing.Judges
      ...
      (nativeRawFold nodeCountModel.raw
        (NativeRawTm.letE .u0 (.var (0 : Fin 1)) : NativeRawTm 0 0))
      (nativeRawFold nodeCountModel.raw (.u1 : NativeRawTm 0 0)) := by
...
structure NativeRawEqualityProfile where
  setoid : (stage binders : Nat) → Setoid (NativeRawTm stage binders)
...
def Rel (profile : NativeRawEqualityProfile)
    {stage binders : Nat} (left right : NativeRawTm stage binders) : Prop :=
...
theorem nativeBooleanEqualityProfile_separates_universes :
    ¬ nativeBooleanEqualityProfile.Rel
      (NativeRawTm.u0 : NativeRawTm 0 0) .u1 := by
  intro related
...
theorem nativeBoolean_is_strict_nondegenerate_typing_extension :
    nativeBooleanConversionPolicy.Extends
      ...
      ¬ nativeBooleanConversionPolicy.Rel
        (NativeRawTm.u0 : NativeRawTm 0 0) .u1 :=
...
def nativeGradedCost {stage binders : Nat}
    (term : NativeRawTm stage binders) : NativeCostGrade binders :=
...
structure GradedCostAlgebra where
  grade : {stage binders : Nat} → NativeRawTm stage binders →
    NativeCostGrade binders
  rename_comp : ∀ {source target} (rho : NativeRen source target)
      {stage} (term : NativeRawTm stage source) environment,
...
def nativeStructuralCost {stage binders : Nat}
    (term : NativeRawTm stage binders) : Nat :=
...
theorem nativeLet_structural_cost :
    nativeStructuralCost
        (NativeRawTm.letE .u0 (.var (0 : Fin 1)) : NativeRawTm 0 0) = 3 :=
  rfl
...
theorem graded_cost_not_profile_invariant
  ...
  let shared : NativeRawTm 0 0 := .letE .u0 (.var (0 : Fin 1))
  let inlined : NativeRawTm 0 0 :=
    nativeInlineLet nativeU0Family (.var (0 : Fin 1))
...
Tarski-classified carrier, the existing `NativeRawTm.quote` is the only
...
def nativeQuoteNext (level : Nat)
    (term : NativeRawTm (level + 1) binders) :
    NativeRawTm level binders :=
...
def NativeRawTm.reflectiveDepth
    (term : NativeRawTm stage binders) : Nat :=
  nativeRawFold nativeReflectiveDepthAlgebra term

@[simp] theorem NativeRawTm.reflectiveDepth_quote
    (route : StageHom high low) (term : NativeRawTm high binders) :
...
theorem NativeRawTm.quote_strictly_raises_reflectiveDepth
    (route : StageHom high low) (positive : 0 < route.quoteDepth)
    (term : NativeRawTm high binders) :
    term.reflectiveDepth <
      (NativeRawTm.quote route term).reflectiveDepth := by
...
def nativeQuotedLanguage (level : Nat)
  ...
  nativeQuoteNext level
    (NativeRawTm.language value : NativeRawTm (level + 1) 0)

/-- Partial observation of the language-value fragment inside quoted code. -/
def NativeRawTm.quotedLanguage? :
    NativeRawTm stage binders →
...
theorem nativeQuotedLanguage_has_type (level : Nat)
    ...
    NativeModalTyping.HasType NativeModalTyping.syntacticConversion .nil
      ...
      (nativeQuoteNext level
        (NativeRawTm.u0 : NativeRawTm (level + 1) 0)) := by
...
structure QuoteCodeUnificationWitness where
  quoteAtNext : ∀ {binders}, (level : Nat) →
    NativeRawTm (level + 1) binders → NativeRawTm level binders
  ...
  quoteLanguage : (level : Nat) →
    Mettapedia.GSLT.LanguageDef.ValidatedLanguageDef → NativeRawTm level 0
...
theorem no_unit_to_native_syntax_hom :
  ...
  have universesEqual : (NativeRawTm.u0 : NativeRawTm 0 0) = .u1 :=
...
theorem nativeSyntax_incompatible_with_universe_collapse
  ...
  change (NativeRawTm.u0 : NativeRawTm stage 0) = .u1 at equalUniverses
...
#print axioms NativeRawTm.reflectiveDepth_quote
#print axioms NativeRawTm.quote_strictly_raises_reflectiveDepth
...
end Mettapedia.Languages.MeTTa.NativeTypeTheory
