...
/-- Evaluation requests and occurrence-labelled answers. -/
inductive EvaluationTerm (model : Model) where
  | request (space : model.Space) (subject : Pattern)
  | answer (space : model.Space) (subject : Pattern)
      (occurrence : Nat) (result : Pattern)

/-- One evaluation rewrite emits exactly one copy from the query-derived
answer multiset. -/
inductive EvaluationStep (model : Model) :
    EvaluationTerm model → EvaluationTerm model → Prop where
  | found {space subject occurrence result}
      (copy : occurrence < Multiset.count result
        (evaluateOne model space subject)) :
      EvaluationStep model (.request space subject)
        (.answer space subject occurrence result)

/-- Query-derived one-step evaluation as a genuine `(T,E,R)` theory. -/
def evaluationGSLT (model : Model) : GSLT where
  Term := EvaluationTerm model
  equations := ⟨Eq, ⟨Eq.refl, Eq.symm, Eq.trans⟩⟩
  rewrites := EvaluationStep model
  rewrites_resp_left := by
    intro source source' target equal step
    subst source'
    exact ⟨target, step, rfl⟩
...
def evaluationMeaning (model : Model) :
  ...
  | .request space subject => evaluateOne model space subject
  | .answer space subject _ _ => evaluateOne model space subject
...
def evaluationElaboration (model : Model) :
    GSLT.Elaboration (evaluationGSLT model) (Multiset Pattern) where
  elaborate := fun term => some (evaluationMeaning model term)
  equation := by
    intro source target equivalent
    cases equivalent
    rfl
