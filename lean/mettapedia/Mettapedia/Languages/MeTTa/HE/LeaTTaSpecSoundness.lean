...
This module sits downstream of both direct spec/LeaTTa conformance and
semantic completeness of the executable-independent spec matcher.  It keeps
...
private def PrincipalModel
  ...
  theory general ∧
    ∀ specific, theory specific →
      Spec.Match.ModelTheory.ValuationRefines specific general
...
def LeaCanonicalSolutionInvariant (bindings : Metta.Bindings) : Prop :=
  LeaBindingSatisfied (leaClassSolution bindings) bindings ∧
    ∀ specific, LeaBindingSatisfied specific bindings →
      Spec.Match.ModelTheory.ValuationRefines specific
...
equation.  This is the Lea-side companion of the corresponding spec-binding
...
private theorem valuationRefines_leaBindingSatisfied
    ...
    (hrefines : Spec.Match.ModelTheory.ValuationRefines specific general)
    (hsatisfied : LeaBindingSatisfied general bindings) :
    LeaBindingSatisfied specific bindings := by
  obtain ⟨post, happly⟩ :=
    Spec.Match.ModelTheory.ValuationRefines.apply_eq_applyClassSolution
      hrefines
  constructor
...
private theorem principalModel_and_equation_of_pullback
  ...
  have hrefinesGeneral :
      Spec.Match.ModelTheory.ValuationRefines composed general := by
  ...
  · unfold MettaEquationSatisfied at hpost ⊢
    simpa only [composed, Spec.Match.ModelTheory.applyClassSolution_comp]
      using hpost.1
  · intro specific hspecific
    ...
    ∃ general : String → Metta.Atom,
      LeaBindingSatisfied general [] ∧
        ∀ specific, LeaBindingSatisfied specific [] →
          Spec.Match.ModelTheory.ValuationRefines specific general := by
...
/-- A successful loop-free merge of a reachable accumulator with any
host-float-free binding record preserves the complete runtime binding-state
invariant.  The merge itself normalizes variable assignments and reflexive
equalities, so the right input needs no representation invariant beyond the
semantic-domain restriction. -/
theorem LeaRuntimeBindingInvariant.merge
    {left right out : Metta.Bindings}
    (hinvariant : LeaRuntimeBindingInvariant left)
    (hrightNoFloat : LeaBindingsNoFloat right)
    (hmerge : out ∈ Metta.Bindings.merge left right)
    (houtLoop : out.hasLoop = false) :
  ...
  · exact leaMerge_canonicallyPrincipal_of_loop
      ...
      hinvariant.equalitiesIrreflexive hrightNoFloat hmerge houtLoop
  · exact leaMerge_result_noFloat
      hinvariant.noFloat hrightNoFloat hmerge
...
private theorem singletonValueBinding_principal
  ...
  have hvalue : applyClassSolution general value = value := by
    calc
      ...
      _ = value := Spec.Match.ModelTheory.applyClassSolution_identity value
  refine ⟨general, ?_, ?_⟩
  · constructor
...
the executable-independent spec specification with the same complete binding
solution theory. -/
theorem leaMatch_observational_sound_of_satisfiable
  ...
  have hspecEquation : HEAtomEquationSatisfied valuation query pattern := by
    simpa [HEAtomEquationSatisfied, MettaEquationSatisfied] using
      hmettaEquation.symm
  obtain ⟨specOut, hspec, _hspecSatisfied⟩ :=
...
default matcher output for translated spec atoms has a derivation in the
executable-independent spec specification and presents exactly the same
...
/-- **Repaired-LeaTTa ↔ specification observational conformance seal.**  The
first projection is direct soundness of every public LeaTTa output.  The
second projection is direct completeness of every spec derivation under the
...
theorem specLeaMatch_observational_conformance
    (query pattern : Atom) :
  ...
  · intro specOut hspec hdisjoint
    exact specMatch_observational_complete hspec hdisjoint

/-- Direct observational soundness is unconditional for every translated
spec atom pair outside expression/expression matching. -/
