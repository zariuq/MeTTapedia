/-
σ-canonical transfer: if a substitution σ identifies two atoms, then a
canonical valuation χ also identifies them, provided variable-to-variable
σ-steps are χ-identified and non-variable σ-images have compatible
canonical picks.
-/
...
/-- If `σ` identifies two atoms, so does `χ`, provided variable images
and non-variable canonical picks agree at the interface. -/
