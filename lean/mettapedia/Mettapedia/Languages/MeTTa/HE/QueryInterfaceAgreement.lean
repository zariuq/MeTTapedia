...
/-!
# G3 staged agreement for the HE equation-query interface

This file banks the **bounded** G3 theorem after the public query interface moved
...
- For any **fixed freshened equation pair** `(lhs', rhs')`, the legacy
  one-way matcher and the repaired `matchAtoms`/`mergeBindings` interface agree
...
`QueryInterfaceAgreement`; kept local so bounded G3 does not depend on the
LeaTTa bridge layer. -/
...
the staged G3 comparison theorem. The live runtime interface is `queryEquations`
...
`matchAtoms`/`mergeBindings` interface with the outer space scan and freshening
...
/-- The repaired public query interface inherits the same no-equalities property
...
/-- The visible-avoid interface has the same bounded no-equality property on
ground query atoms. -/
...
/-- On a ground queried atom, the fixed-freshening faithful matcher interface
...
/-- Expression-case inversion for the fixed-freshening faithful interface on a
...
/-- Head/tail inversion of the expression-case faithful interface on a ground
...
interface and the legacy one-way matcher produce exactly the same query result at
...
patterns, the whole faithful fixed-freshening expression query interface
...
one-way matcher and the repaired faithful matcher/merge interface agree on the
...
/-- Public-interface staged G3 theorem, legacy-to-faithful direction:
...
result reappears on the repaired public `queryEquations` interface at some
...
/-- Public-interface staged G3 theorem, faithful-to-legacy direction:
...
`queryEquations` interface. The two directions live at different abstractions:
legacy hits need freshening adequacy to lift into the repaired interface, and
