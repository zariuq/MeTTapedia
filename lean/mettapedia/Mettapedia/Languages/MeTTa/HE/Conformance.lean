...
/-- HE premise-core steps embed in the least relation generated by the rules
    authored in the HE `LanguageDef`.  The HE matcher itself stays the
    dedicated `matchAtoms`/`Bindings` relation; OSLF supplies only the rule and
    explicitly authored contextual-premise layer. -/
theorem he_premise_core_step_models_oslf_language_step
    ...
    Mettapedia.OSLF.MeTTaIL.ContextualStep.Step
      ...
      Mettapedia.Languages.MeTTa.HE.LanguageDef.mettaHE p q := by
  exact Mettapedia.Languages.MeTTa.HE.CoreFragment.premiseCoreToLanguageStep h
