...
/-!
# First-order unification bridge for the spec matcher

The spec match/merge relation remains executable-independent.  This module
...
namespace Mettapedia.Languages.MeTTa.HE.Spec.Match.LPBridge
...
inductive MatchConstant where
  | symbol : String → MatchConstant
  | grounded : Metta.Ground → MatchConstant

noncomputable instance specConstantDecidableEq :
    DecidableEq MatchConstant := Classical.decEq _

/-- An expression node remembers its list arity. -/
inductive MatchFunction where
  | expression : Nat → MatchFunction
  deriving DecidableEq

/-- First-order signature of the spec matching atom fragment. -/
def specSignature : LPSignature where
  constants := MatchConstant
  vars := String
  relationSymbols := Empty
  relationArity := fun relation => nomatch relation
  functionSymbols := MatchFunction
  functionArity
    | .expression arity => arity

abbrev MatchTerm := Term specSignature
abbrev MatchSubst := Subst specSignature

instance specVarsDecidableEq : DecidableEq specSignature.vars := by
  change DecidableEq String
  infer_instance

noncomputable instance specConstantsDecidableEq :
    DecidableEq specSignature.constants := by
  change DecidableEq MatchConstant
  exact specConstantDecidableEq

instance specFunctionsDecidableEq :
    DecidableEq specSignature.functionSymbols := by
  change DecidableEq MatchFunction
  infer_instance

mutual

/-- Structural embedding of one spec atom into the generic first-order term
algebra. -/
def atomToTerm : Atom → MatchTerm
...
def atomsToTerms : List Atom → List MatchTerm
  | [] => []
...
def termToMettaAtom : MatchTerm → Metta.Atom
  | .var name => .var name
...
def mettaAtomToTerm : Metta.Atom → MatchTerm
...
def mettaAtomsToTerms : List Metta.Atom → List MatchTerm
  | [] => []
...
theorem termToMettaAtom_injective : Function.Injective termToMettaAtom := by
  ...
  | app function terms ih =>
      cases function with
      | expression arity =>
          change Fin arity → MatchTerm at terms
          change ∀ index : Fin arity, ∀ {right : MatchTerm},
...
def substValuation (substitution : MatchSubst) : String → Metta.Atom :=
  fun name => termToMettaAtom (substitution name)

/-- Encode a total MeTTa valuation as a first-order substitution. -/
def valuationSubst (valuation : String → Metta.Atom) : MatchSubst :=
...
theorem termToMettaAtom_applyTerm (substitution : MatchSubst) :
    ∀ term : MatchTerm,
...
/-- Substitution commutes with the spec-atom embedding. -/
theorem termToMettaAtom_apply_atomToTerm
    (substitution : MatchSubst) (atom : Atom) :
...
/-- Pure first-order equation presentation of a spec binding record. -/
def bindingEquations (bindings : Bindings) :
    List (MatchTerm × MatchTerm) :=
...
theorem bindingSatisfied_of_unifies
    {bindings : Bindings} {substitution : MatchSubst}
...
/-- A model of the spec binding equations yields a fuel-free Robinson
...
/-- For spec binding records, the generic Robinson certificate is exactly
...
theorem incompatibleAcyclicProbe_not_unifyDerives :
    ¬UnifyDerives
      (bindingEquations Spec.Match.ModelTheory.incompatibleAcyclicProbe) := by
  intro hderives
  exact Spec.Match.ModelTheory.incompatibleAcyclicProbe_has_no_model
    (has_model_of_unifyDerives hderives)

end Mettapedia.Languages.MeTTa.HE.Spec.Match.LPBridge
