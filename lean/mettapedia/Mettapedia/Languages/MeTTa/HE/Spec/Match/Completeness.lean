...
/-!
# Semantic completeness of the spec HE matcher/merge relation

This module constructs specification matcher and merger derivations directly
from a common valuation.  The construction is independent of both executable
matchers and both executable binding mergers.
-/

namespace Mettapedia.Languages.MeTTa.HE.Spec.Match.Completeness
...
private abbrev Spec.Match := Spec.Match.Merge.MatchRel
...
private theorem specVarNonVar_of_solution
    {valuation : String → Metta.Atom} {varName : String} {value : Atom}
    {bound : Nat}
    (hnonvar : Spec.Match.Merge.isVariableB value = false)
    ...
    ∃ out, Spec.Match (.var varName) value out ∧
  ...
  have hdeclNonvar : DeclMatchSpec.Atom.isVarB value = false := by
    cases value <;> simp_all [Spec.Match.Merge.isVariableB,
  ...
  have hadmissible : Spec.Match.Merge.SemanticLoopFree out :=
...
private theorem specNonVarVar_of_solution
    ...
    (hvalueBound : HESolutionAtomSize valuation value < bound)
    (hvarBound : (valuation varName).size < bound) :
    ∃ out, Spec.Match value (.var varName) out ∧
      HEBindingSatisfied valuation out ∧
      HEAssignmentsNonVariable out ∧
      HEBindingSolutionSizeBound valuation out bound := by
  let out := Bindings.empty.assign varName value
...
theorem exists_specMatch_leaf_of_solution
  ...
  | var leftName =>
      cases right with
      | symbol rightName =>
          exact specVarNonVar_of_solution
            (by simp [Spec.Match.Merge.isVariableB])
            (by simpa [HEAtomEquationSatisfied, toLeaTTaAtom,
              applyClassSolution] using hequation)
      ...
      | grounded rightGround =>
          ...
          exact ⟨Bindings.empty,
            .groundedLeftCustom
              ...
              ⟨rfl, rfl⟩ Spec.Match.Merge.semanticLoopFree_empty,
            hesat_empty valuation, assignmentsNonVariable_empty,
            heBindingSolutionSizeBound_empty valuation bound⟩
      | expression rightAtoms =>
          simp [HEAtomEquationSatisfied, toLeaTTaAtom,
...
/-- Bounded semantic completeness of the spec merge relation.  The right
record supplies the recursion ceiling; the live seed is already bounded at
that same ceiling. -/
def SatisfiedMergeRelCompleteBelow
  ...
  ∀ {ambientBound : Nat} {seed right : Bindings},
    ...
    ∃ out,
      Spec.Match.Merge.MergeRel
        Spec.Match.Merge.equalityGroundedSemantic seed right out ∧
      HEBindingSatisfied valuation out ∧
      HEAssignmentsNonVariable out ∧
...
private theorem specBoundedMatcherPack
    {valuation : String → Metta.Atom} {bound : Nat}
    (hmerge : SatisfiedMergeRelCompleteBelow valuation bound) :
    (∀ {left right : Atom},
  ...
  have hrec : ∀ left, AtomGoal left := by
    ...
    · intro symbol right hequation hleft hright
      exact exists_specMatch_leaf_of_solution hequation
...
/-- Fold semantic value/equality insertion callbacks through the spec
...
private theorem specMergeCompleteBelow_of_adds
    {valuation : String → Metta.Atom} {bound : Nat}
    (haddValue : ∀ {ambientBound : Nat} {seed : Bindings}
      ...
      ∃ out,
        Spec.Match.Merge.AddVarBindingRel
          Spec.Match.Merge.equalityGroundedSemantic
          seed key value out ∧
        HEBindingSatisfied valuation out ∧
        HEAssignmentsNonVariable out ∧
    ...
    (haddEquality : ∀ {ambientBound : Nat} {seed : Bindings}
      ...
      ∃ out,
        Spec.Match.Merge.AddVarEqualityRel
          Spec.Match.Merge.equalityGroundedSemantic
          seed left right out ∧
        HEBindingSatisfied valuation out ∧
        HEAssignmentsNonVariable out ∧
        HEBindingSolutionSizeBound valuation out ambientBound) :
    SatisfiedMergeRelCompleteBelow valuation bound := by
  ...
  have foldConstraints : ∀
      ...
      (∀ constraint ∈ constraints,
        Spec.Match.SolutionTheory.ConstraintSatisfied valuation constraint) →
      (∀ key value,
      ...
      (∀ constraint ∈ constraints,
        ...
        | .equality left right =>
            (valuation left).size < bound ∧
              (valuation right).size < bound) →
      ∃ after,
        Spec.Match.Merge.MergeConstraintsRel
          Spec.Match.Merge.equalityGroundedSemantic
    ...
    | cons constraint rest ih =>
        ...
        have hrestSatisfied : ∀ item ∈ rest,
            Spec.Match.SolutionTheory.ConstraintSatisfied valuation item := by
          intro item hmem
          exact hsatisfied item (by simp [hmem])
        have hrestNonvariable : ∀ key value,
            Spec.Match.Merge.Constraint.value key value ∈ rest →
  ...
  have hconstraintsSatisfied : ∀ constraint ∈
      ...
      Spec.Match.SolutionTheory.ConstraintSatisfied valuation constraint :=
    (Spec.Match.SolutionTheory.constraintsSatisfied_constraints_iff
  ...
  have hconstraintsNonvariable : ∀ key value,
    ...
    exact hrightNonvariable.isVarB_eq_false_of_assignment
      (Spec.Match.SolutionTheory.value_mem_constraints_iff.mp hmem)
  have hconstraintsBound : ∀ constraint ∈
    ...
    | equality left rightKey =>
        exact hrightBound.2 left rightKey
          (Spec.Match.SolutionTheory.equality_mem_constraints_iff.mp hmem)
  obtain ⟨out, hfold, houtSatisfied, houtNonvariable, houtBound⟩ :=
    foldConstraints (Spec.Match.Merge.constraints right) seed
...
private theorem exists_specExpressionMatch_at_solutionSize
    ...
    ∃ out, Spec.Match (.expression lefts) (.expression rights) out ∧
      HEBindingSatisfied valuation out ∧
      HEAssignmentsNonVariable out ∧
      HEBindingSolutionSizeBound valuation out bound := by
  have hlists : MettaAtomListsSatisfied valuation
  ...
  have hrightBound : HESolutionAtomsSizeBound valuation rights bound := by
    ...
    exact heSolutionAtomSize_lt_expression_of_mem valuation hmem
  obtain ⟨out, hitems, hsatisfied, hnonvariable, hbound⟩ :=
    (specBoundedMatcherPack hmerge).2
      (seed := Bindings.empty)
      (hesat_empty valuation) assignmentsNonVariable_empty
      (heBindingSolutionSizeBound_empty valuation bound)
...
private theorem exists_specNonvariableMatch_at_solutionSize
    {valuation : String → Metta.Atom} {left right : Atom} {bound : Nat}
    (hmerge : SatisfiedMergeRelCompleteBelow valuation bound)
    (hequation : HEAtomEquationSatisfied valuation left right)
  ...
  · subst right
    ...
    | symbol name =>
        exact ⟨Bindings.empty,
          .symSym name Spec.Match.Merge.semanticLoopFree_empty,
          hesat_empty valuation, assignmentsNonVariable_empty,
          heBindingSolutionSizeBound_empty valuation bound⟩
    ...
    | grounded ground =>
        exact ⟨Bindings.empty,
          ...
          heBindingSolutionSizeBound_empty valuation bound⟩
    | expression atoms =>
        exact exists_specExpressionMatch_at_solutionSize hmerge
...
private theorem exists_specNonvariableMatchList_at_solutionSize
    {valuation : String → Metta.Atom} {bound : Nat}
    (hmerge : SatisfiedMergeRelCompleteBelow valuation bound) :
    ∀ {lefts rights : List Atom} {seed : Bindings},
      HEBindingSatisfied valuation seed →
  ...
  | cons left lefts ih =>
      ...
      | cons right rights =>
          ...
          obtain ⟨matched, hmatched, hmatchedSatisfied,
              hmatchedNonvariable, hmatchedBound⟩ :=
            exists_specNonvariableMatch_at_solutionSize hmerge
...
/-- Semantic completeness of the spec merge relation.  Strong induction is
...
theorem specSatisfiedMergeRelCompleteBelow
    (valuation : String → Metta.Atom) :
    ∀ bound, SatisfiedMergeRelCompleteBelow valuation bound := by
  intro bound
  induction bound using Nat.strong_induction_on with
  | h bound ih =>
      apply specMergeCompleteBelow_of_adds
      · intro ambientBound seed key value hboundAmbient
        ...
        | nil =>
            ...
            let hadd : Spec.Match.Merge.AddVarBindingRel
                Spec.Match.Merge.equalityGroundedSemantic
        ...
        | cons first rest =>
            have hvalues : (first :: rest).Perm (seed.classValues key) := by
              rw [hclass]
            by_cases hagree :
                Spec.Match.Merge.ValuesAgree (first :: rest)
            · by_cases hequal : value = first
              ...
              · have hfirstMem : first ∈ seed.classValues key := by
                ...
                subst first
                subst value
                have hlocal : SatisfiedMergeRelCompleteBelow
                    valuation (valuation key).size :=
                  ih _ hkeyBound
                obtain ⟨matched, hmatch, hmatchedSatisfied,
                    hmatchedNonvariable, hmatchedBound⟩ :=
                  exists_specExpressionMatch_at_solutionSize hlocal
            ...
            · have hfirstMem : first ∈ seed.classValues key := by
              ...
              obtain ⟨matched, hlist, hmatchedSatisfied,
                  hmatchedNonvariable, hmatchedBound⟩ :=
                exists_specNonvariableMatchList_at_solutionSize hlocal
                  (seed := Bindings.empty)
                  (hesat_empty valuation) assignmentsNonVariable_empty
      ...
      · intro ambientBound seed left right hboundAmbient
        ...
        | nil =>
            let hadd : Spec.Match.Merge.AddVarEqualityRel
                Spec.Match.Merge.equalityGroundedSemantic
                seed left right candidate :=
              .consistent hclass (by simp [Spec.Match.Merge.ValuesAgree])
...
theorem exists_specMatch_of_solution
  ...
  obtain ⟨out, hmatch, hsatisfied, _hnonvariable, _hbound⟩ :=
    (specBoundedMatcherPack
      (specSatisfiedMergeRelCompleteBelow valuation bound)).1
      hequation hleftBound hrightBound
  exact ⟨out, hmatch, hsatisfied⟩

/-- Every finite spec binding record has a strict semantic-size ceiling at
any fixed valuation. -/
theorem exists_specBindingSolutionSizeBound
...
/-- Any two non-variable spec binding records with a common solution have a
declarative spec merge.  This is the unbounded public projection of the
semantic-size construction. -/
theorem exists_specMerge_of_solution
  ...
  obtain ⟨leftBound, hleftBound⟩ :=
    exists_specBindingSolutionSizeBound valuation left
  obtain ⟨rightBound, hrightBound⟩ :=
    exists_specBindingSolutionSizeBound valuation right
  let bound := max leftBound rightBound
  obtain ⟨out, hmerge, houtSatisfied, houtNonvariable, _houtBound⟩ :=
    specSatisfiedMergeRelCompleteBelow valuation bound
...
images yields a spec match without passing through an executable matcher. -/
example : ∃ out, Spec.Match
  ...
  obtain ⟨out, hmatch, _⟩ := exists_specMatch_of_solution
    (valuation := valuation)
...
end Mettapedia.Languages.MeTTa.HE.Spec.Match.Completeness
