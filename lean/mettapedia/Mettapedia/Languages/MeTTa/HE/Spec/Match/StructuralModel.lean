...
/-!
# Structural reconciliation certificate for the spec matcher

This file isolates the representation-independent certificate needed by the
canonical spec binding model.  Two atoms are structurally solved by a binding
...
namespace Mettapedia.Languages.MeTTa.HE.Spec.Match.StructuralModel

open Mettapedia.Languages.MeTTa.HE
open Mettapedia.Languages.MeTTa.HE.LeaTTaBridge
open Mettapedia.Languages.MeTTa.OSLFCore (Atom GroundedValue)
open Spec.Match.ModelTheory
...
spec rules choose one class value and recursively match it against all the
...
def AtomClassOccurs (bindings : Bindings) (atom : Atom)
    (name : String) : Prop :=
  ∃ occurrence,
    Spec.Match.Merge.AtomOccurs atom occurrence ∧
...
def AtomDependenciesBelow
    ...
    (atom : Atom) : Prop :=
  ∀ name, Spec.Match.Merge.AtomOccurs atom name →
...
theorem classDepends_of_source_mem_eqClass
    ...
    Spec.Match.Merge.ClassDepends bindings replacement target := by
...
theorem dependencyOrder_of_source_mem_eqClass
  ...
  change Relation.TransGen
      (Spec.Match.Merge.ClassDepends bindings) source.1 target.1 at hlt
  change Relation.TransGen
      (Spec.Match.Merge.ClassDepends bindings) replacement.1 target.1
  let motive := fun start
      (_ : Relation.TransGen
        (Spec.Match.Merge.ClassDepends bindings) start target.1) =>
      ∀ replacement,
        replacement ∈ bindings.eqClass start →
        Relation.TransGen
          (Spec.Match.Merge.ClassDepends bindings)
...
def ConstraintDependenciesBelow (target : Bindings)
    ...
    Spec.Match.Merge.Constraint → Prop
  | .value key value =>
...
def ConstraintsDependenciesBelow (target : Bindings)
    (source : DependencyNode target)
    (constraints : List Spec.Match.Merge.Constraint) : Prop :=
...
/-- The spec match/merge derivation never invents an out-of-scope variable.
...
theorem specMatch_dependenciesBelow
  ...
  refine (Spec.Match.Merge.MatchRel.rec
  ...
  next =>
      ...
      apply ihFold hleft
      intro constraint hmem
      have hrightMem : constraint ∈ Spec.Match.Merge.constraints right :=
        (List.Perm.mem_iff horder).mp hmem
      cases constraint with
      | value key value =>
          exact hright.assignments key value
            (Spec.Match.SolutionTheory.value_mem_constraints_iff.mp hrightMem)
      | equality edgeLeft edgeRight =>
          exact hright.equalities edgeLeft edgeRight
            (Spec.Match.SolutionTheory.equality_mem_constraints_iff.mp hrightMem)
...
/-- Structural presentation of one neutral spec merge constraint. -/
...
theorem ConstraintSolved.transport
    {source target : Bindings}
    (hpresentation : BindingPresentation target source)
    {constraint : Spec.Match.Merge.Constraint}
...
theorem ConstraintsSolved.transport
    {source target : Bindings}
    (hpresentation : BindingPresentation target source)
    {constraints : List Spec.Match.Merge.Constraint}
...
theorem BindingPresentation.of_constraintsSolved
    {source target : Bindings}
    {order : List Spec.Match.Merge.Constraint}
    (horder : order.Perm (Spec.Match.Merge.constraints source))
  ...
  assignments key value hmem := by
    ...
    exact (List.Perm.mem_iff horder).mpr
      (Spec.Match.SolutionTheory.value_mem_constraints_iff.mpr hmem)
  classes := eqClass_mono_of_equalities_solved fun left right hmem => by
    apply hsolved (.equality left right)
    exact (List.Perm.mem_iff horder).mpr
      (Spec.Match.SolutionTheory.equality_mem_constraints_iff.mpr hmem)
...
/-- Successful spec matching preserves the binding representation's unique
assignment-key invariant through every recursive reconciliation branch. -/
theorem specMatch_assignmentsNodup
    {query pattern : Atom} {out : Bindings}
    (hmatch : Spec.Match.Merge.MatchRel
      Spec.Match.Merge.equalityGroundedSemantic query pattern out) :
...
theorem valuesAgree_eq_of_mem
    {values : List Atom}
    (hagree : Spec.Match.Merge.ValuesAgree values)
    {left right : Atom}
...
theorem structurallySolved_addEquality_of_valuesAgree
    ...
    (hsolved : ClassAssignmentsStructurallySolved bindings)
    (hnodup : AssignmentsNodup bindings)
    (hagree : Spec.Match.Merge.ValuesAgree
...
private theorem solved_canonical_pair
    ...
    :
    (∀ left right (_ : AtomSolved bindings left right),
      (∀ name, Spec.Match.Merge.AtomOccurs left name →
        AtomClassOccurs bindings leftRoot.1.2 name) →
      (∀ name, Spec.Match.Merge.AtomOccurs right name →
...
theorem AtomSolved.canonical_eq_of_smaller
    ...
    (hsolved : AtomSolved bindings left right)
    (hleftRoot : ∀ name,
      Spec.Match.Merge.AtomOccurs left name →
      AtomClassOccurs bindings leftRoot.1.2 name)
    (hrightRoot : ∀ name,
      Spec.Match.Merge.AtomOccurs right name →
      AtomClassOccurs bindings rightRoot.1.2 name) :
    applyClassSolution
        (canonicalValuation bindings hloopFree hnonvariable)
        (toLeaTTaAtom left) =
...
theorem mergeConstraintsRel_seed_subrecord
    {seed : Bindings} {constraints : List Spec.Match.Merge.Constraint}
    {out : Bindings}
    (hfold : Spec.Match.Merge.MergeConstraintsRel
      Spec.Match.Merge.equalityGroundedSemantic seed constraints out) :
    BindingSubrecord seed out := by
  apply Spec.Match.Merge.MergeConstraintsRel.rec
  ...
  apply Spec.Match.Merge.MergeRel.rec
    ...
    (motive_5 := fun _ constraints result _ =>
      ∀ addedLeft addedRight,
        Spec.Match.Merge.Constraint.equality addedLeft addedRight ∈
          constraints →
        addedRight ∈ result.eqClass addedLeft)
...
theorem canonical_assignment_eq_of_key_below
    {target : Bindings}
    (hloopFree : Spec.Match.Merge.SemanticLoopFree target)
    (hnonvariable : AssignmentsNonVariable target)
    {source : DependencyNode target}
...
/-- Simultaneous strict-scope and semantic replay for a spec merge.  Its
...
private theorem mergeRel_below_pack
  ...
  apply Spec.Match.Merge.MergeRel.rec
    ...
    (motive_5 := fun seed constraints result _ =>
      ...
      (BindingSubrecord result target →
        ConstraintsDependenciesBelow target source constraints →
          Spec.Match.SolutionTheory.ConstraintsSatisfied valuation
            constraints))
    (motive_6 := fun left right result _ =>
      (BindingDependenciesBelow target source left →
        BindingDependenciesBelow target source right →
  ...
  next =>
      ...
      have hmatched := specMatch_dependenciesBelow hhead hleft hright
  ...
  next =>
      ...
      · intro hseed hkey hvalue
        ...
        exact ihMerge.1 hseed
          (specMatch_dependenciesBelow hhead hfirst hvalue)
      · intro hout hkey hvalue
        ...
        have hmatched := specMatch_dependenciesBelow hhead hfirst hvalue
  ...
  next =>
      ...
      · intro hout hkey hvalue
        ...
        have hmatchedSatisfied := ihMerge.2 hout hmatched
        have hitems :=
          (Spec.Match.SolutionTheory.matchListAccRel_solution_iff
  ...
  next =>
      ...
      · intro hleft hright
        ...
        have hrightMem : constraint ∈
            Spec.Match.Merge.constraints mergedRight :=
          (List.Perm.mem_iff horder).mp hmem
        cases constraint with
        | value key value =>
            exact hright.assignments key value
              (Spec.Match.SolutionTheory.value_mem_constraints_iff.mp
                hrightMem)
        | equality edgeLeft edgeRight =>
            exact hright.equalities edgeLeft edgeRight
              (Spec.Match.SolutionTheory.equality_mem_constraints_iff.mp
                hrightMem)
      · intro hout hright
        ...
        apply (Spec.Match.SolutionTheory.constraintsSatisfied_constraints_iff
          valuation mergedRight).mp
        exact (Spec.Match.SolutionTheory.constraintsSatisfied_iff_of_perm
...
theorem matchListAccRel_dependenciesBelow
    ...
    (hlist : Spec.Match.Merge.MatchListAccRel
  ...
  refine (Spec.Match.Merge.MatchListAccRel.rec
    (motive_1 := fun _ _ _ _ => True)
    (motive_2 := fun left right seed result _ =>
      AtomsDependenciesBelow target source left →
      AtomsDependenciesBelow target source right →
  ...
  next =>
      ...
      have hmatched :=
        specMatch_dependenciesBelow hhead hheadLeft hheadRight
      have hnext :=
...
/-- At one dependency frontier, every live accumulator produced by a spec
...
private theorem specMatch_evalCoherentAt
    {query pattern : Atom} {out : Bindings}
    (hmatch : Spec.Match.Merge.MatchRel
      Spec.Match.Merge.equalityGroundedSemantic query pattern out)
    (hloopFree : Spec.Match.Merge.SemanticLoopFree out)
  ...
  next =>
      intro seed addedLeft addedRight values hvalues hagree
        hout hseedNodup hcoherent
      have hagree' : Spec.Match.Merge.ValuesAgree
  ...
  · exact specMatch_assignmentsNodup hmatch

/-- Every successful executable-independent spec match has coherent final
...
same class. -/
theorem specMatch_canonicalClassCoherent
    {query pattern : Atom} {out : Bindings}
    (hmatch : Spec.Match.Merge.MatchRel
      Spec.Match.Merge.equalityGroundedSemantic query pattern out)
    (hloopFree : Spec.Match.Merge.SemanticLoopFree out)
  ...
  | some chosen =>
      have hchosenToAssignment : assignment.1.1 ∈
        ...
        rw [EqualityClosure.mem_eqClass_iff_reachable]
          at hchosenClass hassignmentClass ⊢
        exact hchosenClass.symm.trans hassignmentClass
      have hagree := (specMatch_evalCoherentAt
...
/-- Consequently every successful executable-independent spec match has a
concrete canonical model. -/
theorem specMatch_has_model
    {query pattern : Atom} {out : Bindings}
    (hmatch : Spec.Match.Merge.MatchRel
      Spec.Match.Merge.equalityGroundedSemantic query pattern out)
    (hloopFree : Spec.Match.Merge.SemanticLoopFree out)
  ...
  exact canonicalValuation_satisfies_of_classCoherent
    hloopFree hnonvariable
    (specMatch_canonicalClassCoherent hmatch hloopFree hnonvariable)
...
theorem specMatch_pathReconciled_of_reconcilePaths
    (hreconcilePaths : ∀
      {bindings : Bindings} {left right : String}
...
joined class's stored values; the remaining premise contains only the spec
list-match and merge derivations and the direct `AtomsReconciled` conclusion. -/
theorem specMatch_pathReconciled_of_reconcileAtoms
    (hreconcileAtoms : ∀
      {bindings : Bindings} {left right : String}
      {first : Atom} {rest : List Atom} {matched out : Bindings},
      Spec.Match.Merge.MatchListAccRel
    ...
    (hnonvariable : AssignmentsNonVariable out) :
    ClassAssignmentsPathReconciled out := by
  apply specMatch_pathReconciled_of_reconcilePaths
...
end Mettapedia.Languages.MeTTa.HE.Spec.Match.StructuralModel
