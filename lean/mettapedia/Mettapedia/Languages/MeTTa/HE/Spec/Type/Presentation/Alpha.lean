...
namespace Mettapedia.Languages.MeTTa.HE.Spec.Type.Presentation.Alpha
...
theorem functionTypeRel_renameTypeVars_iff
    (rename : String → String) (source : Atom)
    (argumentTypes : List Atom) (returnType : Atom) :
    FunctionTypeRel (renameTypeVars rename source)
        argumentTypes returnType ↔
      ∃ sourceArguments sourceReturn,
        FunctionTypeRel source sourceArguments sourceReturn ∧
        argumentTypes =
          sourceArguments.map (renameTypeVars rename) ∧
        returnType = renameTypeVars rename sourceReturn := by
  constructor
  · intro hfunction
    cases source with
    | symbol name =>
        simp [FunctionTypeRel, renameTypeVars] at hfunction
    | var name =>
        simp [FunctionTypeRel, renameTypeVars] at hfunction
    | grounded value =>
        simp [FunctionTypeRel, renameTypeVars] at hfunction
    | expression atoms =>
        simp only [FunctionTypeRel, renameTypeVars,
...
private theorem mapped_types_are_alpha_siblings
    ...
    ∀ sourceTypes : List Atom,
      List.Forall₂ ObservedTypeAlphaRel
        (sourceTypes.map (renameTypeVars leftRename))
        (sourceTypes.map (renameTypeVars rightRename)) := by
  intro sourceTypes
...
theorem ObservedTypeAlphaRel.function_skeleton
  ...
  obtain ⟨sourceArguments, sourceReturn, sourceFunction,
    ...
    (functionTypeRel_renameTypeVars_iff
      leftRename source leftArguments leftReturn).mp leftFunction
  subst leftArguments
  subst leftReturn
  refine ⟨sourceArguments.map (renameTypeVars rightRename),
    renameTypeVars rightRename sourceReturn, ?_, ?_, ?_⟩
  · rw [hright]
    exact (functionTypeRel_renameTypeVars_iff
...
theorem distinct_symbols_not_alpha_siblings :
  ...
  cases source with
  | symbol name =>
      simp [renameTypeVars] at hleft hright
      have hAB : ("A" : String) = "B" := hleft.trans hright.symm
      simp at hAB
  | var name => simp [renameTypeVars] at hleft
  | grounded value => simp [renameTypeVars] at hleft
  | expression atoms => simp [renameTypeVars] at hleft

end Mettapedia.Languages.MeTTa.HE.Spec.Type.Presentation.Alpha
