import Mettapedia.Languages.MeTTa.HE.Spec.Type.Presentation.Algebra
...
namespace Mettapedia.Languages.MeTTa.HE.Spec.Type.Presentation.MatchSolutionTheory
...
/-- Ordinary presentation matching adds exactly one syntactic equation to
the incoming finite presentation.  Unlike the reduced-type lane, this
theorem has no `%Undefined%` or `Atom` wildcard cases. -/
theorem AppliedTypePresentationMatchRel.solutions
    {substitution output : TypeSubst} {left right : Atom}
    (derivation : AppliedTypePresentationMatchRel
      substitution left right output)
    (normal : substitution.Normal) (valuation : String → Atom) :
    TypeSubstSatisfied valuation output ↔
      TypeSubstSatisfied valuation substitution ∧
        applyTypeValuation valuation left =
          applyTypeValuation valuation right :=
  appliedPresentationSolutions derivation normal valuation
...
/-- Regard a finite type presentation as an ordinary spec binding record
with assignments only. -/
def typeSubstAsBindings (substitution : TypeSubst) :
    Mettapedia.Languages.MeTTa.HE.Bindings where
  assignments := substitution
  equalities := []

/-- The assignment-only spec binding semantics is exactly finite
presentation satisfaction. -/
theorem typeBindingSatisfied_asBindings_iff
  ...
  simp [TypeBindingSatisfied, typeSubstAsBindings,
    TypeSubstSatisfied]
...
end Mettapedia.Languages.MeTTa.HE.Spec.Type.Presentation.MatchSolutionTheory
