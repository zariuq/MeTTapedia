...
/-!
# Named runtime refinements of the published spec type layer

`Spec.Type` remains the literal published core.  This module separates two
...
namespace Mettapedia.Languages.MeTTa.HE.Spec.Type.RuntimeRefinement
...
/-- A valuation satisfies every assignment and equality carried by a spec
binding record. -/
def TypeBindingSatisfied
...
def R1ReturnTypeObserved
  ...
  (∃ valuation, TypeBindingSatisfied valuation bindings) ∧
    ∀ valuation,
...
def renameTypeVars (ρ : String → String) : Atom → Atom
  | .symbol name => .symbol name
  | .var name => .var (ρ name)
  | .grounded value => .grounded value
  | .expression atoms => .expression (atoms.map (renameTypeVars ρ))
...
def TypeVariableRenamingOf (type type' : Atom) : Prop :=
  ∃ ρ : String → String, Function.Injective ρ ∧
    type' = renameTypeVars ρ type

private theorem renameTypeVars_id :
    ∀ type : Atom, renameTypeVars id type = type := by
  intro type
...
@[refl] theorem TypeVariableRenamingOf.refl (type : Atom) :
  ...
  ⟨id, Function.injective_id, (renameTypeVars_id type).symm⟩
...
published `ApplicationSuccessRel` remains unchanged in `Spec.Type`. -/
...
theorem r1_nullary_f_does_not_infer_B :
  ...
  | mk hshape hoperator hrenaming hfunction harguments _ =>
      ...
      simp [renameTypeVars, FunctionTypeRel] at hfunction
...
private theorem r1TBinding_satisfied_iff (valuation : String → Atom) :
    TypeBindingSatisfied valuation r1TBinding ↔
      valuation "t" = .symbol "A" := by
  simp [r1TBinding, TypeBindingSatisfied, Bindings.empty,
...
theorem r1_t_not_observed_as_B :
  ...
  have hmodel : TypeBindingSatisfied valuation r1TBinding := by
    rw [r1TBinding_satisfied_iff]
    simp [valuation]
...
private theorem r1TBindingB_satisfied_iff (valuation : String → Atom) :
    TypeBindingSatisfied valuation r1TBindingB ↔
      valuation "t" = .symbol "B" := by
  simp [r1TBindingB, TypeBindingSatisfied, Bindings.empty,
    Bindings.assign, Bindings.isBound, Bindings.lookup,
    applyTypeValuation]
...
private theorem runtimeListExpected_actual_r2 :
  ...
  · intro valuation
    simp [runtimeListExpected, runtimeListActual,
      TypeBindingSatisfied, Bindings.empty,
      CorePlusR2TypeConsistent, ReducedTypeConsistent,
...
private theorem runtimeListExpected_actual_not_core
  ...
  intro hmatch
  obtain ⟨matched, hspec, _⟩ := TypeMatchRel.structural_of_nonWildcard
...
end Mettapedia.Languages.MeTTa.HE.Spec.Type.RuntimeRefinement
