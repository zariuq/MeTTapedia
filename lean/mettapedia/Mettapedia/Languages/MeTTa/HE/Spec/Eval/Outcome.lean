...
The fuel-free spec evaluator describes semantic results only.  An executable
may additionally stop because its resource budget is exhausted.  This module
keeps those two outcomes disjoint without importing fuel into the spec
semantics.
-/

namespace Mettapedia.Languages.MeTTa.HE.Spec.Eval.Outcome
...
/-- A semantic outcome is justified by the fuel-free spec evaluator. -/
def SemanticOutcome
...
end Mettapedia.Languages.MeTTa.HE.Spec.Eval.Outcome
