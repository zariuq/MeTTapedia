...
namespace Mettapedia.Languages.MeTTa.HE.HumanTypePresentationWildcardProvenanceCounterexample
...
/-- The corrected human relation keeps wildcard provenance at the raw layer
and therefore rejects the substitution-produced false positive. -/
theorem corrected_human_relation_rejects_resolved_wildcard_conflict :
    ¬∃ output,
...
theorem corrected_human_relation_rejects_nested_resolved_wildcard_conflict :
    ¬∃ output,
      ReducedTypePresentationMatchRel legacyIncoming
  ...
  | expression children =>
      cases children with
      | cons head tail =>
          exact corrected_human_relation_rejects_resolved_wildcard_conflict
            ⟨_, head⟩
...
end Mettapedia.Languages.MeTTa.HE.HumanTypePresentationWildcardProvenanceCounterexample
