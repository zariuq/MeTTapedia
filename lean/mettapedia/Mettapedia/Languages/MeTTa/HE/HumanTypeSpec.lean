...
namespace Mettapedia.Languages.MeTTa.HE.HumanTypeSpec
...
other case uses the already independent human match and merge relations. -/
...
ordinary human match followed by one human merge. -/
theorem TypeMatchRel.structural_of_nonWildcard
  ...
  | structural _ _ _ _ hhuman hmerge => exact ⟨_, hhuman, hmerge⟩

/-- Relational `type_cast`, including the specification's first-success rule.
On total failure, one error derivation is available for every discovered
actual type. -/
inductive TypeCastRel (space : Space) (atom expectedType : Atom)
  ...
  | success {types earlierTypes laterTypes : List Atom} {actualType : Atom}
      ...
      (∀ earlier ∈ earlierTypes, ∀ candidate,
        ¬TypeMatchRel earlier expectedType bindings candidate) →
      TypeMatchRel actualType expectedType bindings output →
  ...
  | failure {types : List Atom} {actualType : Atom} :
      ...
      (∀ candidateType ∈ types, ∀ candidate,
        ¬TypeMatchRel candidateType expectedType bindings candidate) →
...
private theorem intBoolTypeMismatch (candidate : Bindings) :
    ¬TypeMatchRel (.symbol "Int") (.symbol "Bool")
...
example : TypeCastRel declaredIntSpace (.symbol "x") (.symbol "Bool")
  ...
  · intro candidateType hmem candidate
    ...
    subst candidateType
    exact intBoolTypeMismatch candidate
...
end Mettapedia.Languages.MeTTa.HE.HumanTypeSpec
