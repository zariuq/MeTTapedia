...
/-!
# Model theory for the executable-independent human matcher

This file develops the finite dependency order used to construct models of
admitted human matcher derivations.  The order is semantic: one equality class
...
namespace Mettapedia.Languages.MeTTa.HE.HumanMatchModelTheory
...
equality endpoints in a human binding record. -/
...
theorem assignment_value_var_mem_bindingSupport
    ...
    (hassignment : (key, value) ∈ bindings.assignments)
    (hdependency : HumanMatchMergeSpec.AtomOccurs value dependency) :
...
def DependencyOrder (bindings : Bindings) :
  ...
  fun target source =>
    Relation.TransGen (HumanMatchMergeSpec.ClassDepends bindings)
      source.1 target.1

/-- The human semantic loop filter is precisely enough to make recursive
...
theorem dependencyNode_lt
    ...
    {target : String}
    (hdepends : HumanMatchMergeSpec.ClassDepends
...
/-- Every stored assignment value is non-variable.  Human matcher outputs have
...
def AssignmentsNonVariable (bindings : Bindings) : Prop :=
  ∀ key value, (key, value) ∈ bindings.assignments →
    HumanMatchMergeSpec.isVariableB value = false

/-- A variable occurring properly inside a non-variable human atom becomes
...
theorem size_valuation_lt_apply_of_atomOccurs_nonvariable
    ...
    (hnonvariable : HumanMatchMergeSpec.isVariableB value = false)
    (hoccurs : HumanMatchMergeSpec.AtomOccurs value dependency) :
    (valuation dependency).size <
      (applyClassSolution valuation (toLeaTTaAtom value)).size := by
  cases value with
  | symbol symbol => cases hoccurs
  | var name => simp [HumanMatchMergeSpec.isVariableB] at hnonvariable
...
theorem semanticLoopFree_of_satisfied_nonvariable
  ...
  have hstrict : ∀ {source target},
      HumanMatchMergeSpec.ClassDepends bindings source target →
...
private theorem classDepends_of_chosen_assignment
    ...
    {dependency : String}
    (hnonvariable : AssignmentsNonVariable bindings)
    (hoccurs : HumanMatchMergeSpec.AtomOccurs assignment.1.2 dependency) :
    HumanMatchMergeSpec.ClassDepends bindings source.1
  ...
  intro halias
  have hvalueVar :
      HumanMatchMergeSpec.isVariableB assignment.1.2 = true := by
    rw [halias.1]
    rfl
...
noncomputable def canonicalNodeValue
  ...
  exact (dependencyOrder_wellFounded hloopFree).fix fun source recurse =>
      ...
      | some assignment =>
          applyClassSolution
            (fun dependency =>
              if hoccurs : HumanMatchMergeSpec.AtomOccurs
...
/-- The remaining consistency obligation for a human derivation: every stored
...
/-- Literal record inclusion.  Human merge-back never removes a constraint
...
theorem BindingSubrecord.classDepends {before after : Bindings}
    ...
    {source target : String}
    (hdepends : HumanMatchMergeSpec.ClassDepends before source target) :
    HumanMatchMergeSpec.ClassDepends after source target := by
  rcases hdepends with
  ...
  intro hproperAlias
  have hvalueVariable : HumanMatchMergeSpec.isVariableB value = true := by
    rw [hproperAlias.1]
    rfl
...
theorem BindingSubrecord.semanticLoopFree {before after : Bindings}
    (hsubrecord : BindingSubrecord before after)
    (hnonvariable : AssignmentsNonVariable before)
    (hafter : HumanMatchMergeSpec.SemanticLoopFree after) :
    HumanMatchMergeSpec.SemanticLoopFree before := by
  intro name hcycle
  apply hafter name
...
/-- Every declarative human merge contains its entire live left accumulator.
...
theorem matchListAccRel_seed_subrecord
    ...
    (hmatch : HumanMatchMergeSpec.MatchListAccRel
...
theorem addVarBindingRel_seed_subrecord
    ...
    (hadd : HumanMatchMergeSpec.AddVarBindingRel
      HumanMatchMergeSpec.equalityGroundedSemantic
...
theorem addVarEqualityRel_seed_subrecord
    ...
    (hadd : HumanMatchMergeSpec.AddVarEqualityRel
      HumanMatchMergeSpec.equalityGroundedSemantic seed left right out) :
    BindingSubrecord seed out := by
  cases hadd with
...
theorem semanticLoopFree_of_subrecord_of_match
    ...
    (hnonvariable : AssignmentsNonVariable before) :
    HumanMatchMergeSpec.SemanticLoopFree before := by
  have houtLoopFree : HumanMatchMergeSpec.SemanticLoopFree out := by
...
constructor heads.  It is a well-formed record shape, yet no successful human
...
theorem incompatibleAcyclicProbe_loopFree :
    HumanMatchMergeSpec.SemanticLoopFree incompatibleAcyclicProbe := by
  have hnostep : ∀ source target,
      ¬HumanMatchMergeSpec.ClassDepends
...
theorem satisfiableUnreconciledProbe_loopFree :
    HumanMatchMergeSpec.SemanticLoopFree satisfiableUnreconciledProbe := by
  ...
  have hreach : ∀ {source target},
      Relation.TransGen
          (HumanMatchMergeSpec.ClassDepends satisfiableUnreconciledProbe)
          source target →
        source = "x" ∧ target = "y" := by
...
and the human merge fold can insert the fresh right assignment.  Their union
...
theorem cyclicMerge_rel :
  ...
  apply HumanMatchMergeSpec.MergeRel.mk
      (order := [.value "y" (.expression [.var "x"])])
  · change List.Perm
      [HumanMatchMergeSpec.Constraint.value "y" (.expression [.var "x"])]
      ([HumanMatchMergeSpec.Constraint.value "y"
        (.expression [.var "x"])]).eraseDups
    rw [show ([HumanMatchMergeSpec.Constraint.value "y"
        (.expression [.var "x"])]).eraseDups =
      [HumanMatchMergeSpec.Constraint.value "y"
        (.expression [.var "x"])] by decide]
  · apply HumanMatchMergeSpec.MergeConstraintsRel.value
    · apply HumanMatchMergeSpec.AddVarBindingRel.fresh
...
/-- The same probe confirms exactly why the outer human matcher carries its
semantic loop filter. -/
theorem cyclicMergeOut_not_loopFree :
    ¬HumanMatchMergeSpec.SemanticLoopFree cyclicMergeOut := by
  intro hloopFree
  have hxy : HumanMatchMergeSpec.ClassDepends
    ...
    · simp [cyclicMergeOut, cyclicMergeLeft, Bindings.eqClass,
        ...
        Bindings.assign, Bindings.isBound, Bindings.lookup]
    · simp
  have hyx : HumanMatchMergeSpec.ClassDepends
...
end Mettapedia.Languages.MeTTa.HE.HumanMatchModelTheory
