...
re-presentation of the existing sealed `IntrinsicPure` grammar and typing
...
theorem old_liftRen_eq
    ...
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.Renaming.liftRen ρ =
      liftRen ρ := by
  funext i
...
@[simp] theorem ofPure_rename
    ...
    ofPure (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Renaming.rename ρ t) =
  ...
  | pi A B ihA ihB => simp only [
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.Renaming.rename, ofPure,
...
theorem ofPure_liftSub
    ...
    (fun i => ofPure
      (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Substitution.liftSub σ i)) =
  ...
  · intro j
    exact ofPure_rename
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.Renaming.wk (σ j)

/-- Re-presentation commutes with simultaneous substitution. -/
@[simp] theorem ofPure_subst
    (σ : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Substitution.Sub n m)
    (t : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Syntax.PureTm n) :
    ofPure (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Substitution.subst σ t) =
  ...
  | u1 => rfl
  | pi A B ihA ihB =>
      simp only [Mettapedia.Languages.MeTTa.Pure.Intrinsic.Substitution.subst,
...
@[simp] theorem ofPure_inst0
    (u : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Syntax.PureTm n)
    (body : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Syntax.PureTm (n + 1)) :
    ofPure (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Substitution.inst0 u body) =
      inst0 (ofPure u) (ofPure body) := by
  rw [Mettapedia.Languages.MeTTa.Pure.Intrinsic.Substitution.inst0,
    inst0, ofPure_subst]
  congr 1
  funext i
  refine Fin.cases ?_ ?_ i
...
presentation. -/
def ofPureCtx :
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.Context.Ctx n → Legacy.Ctx n
  | .nil => .nil
  | .snoc Γ A => .snoc (ofPureCtx Γ) (ofPure A)

/-- Inverse context translation. -/
def toPureCtx : Legacy.Ctx n →
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.Context.Ctx n
  | .nil => .nil
  | .snoc Γ A => .snoc (toPureCtx Γ) (toPure A)

@[simp] theorem toPureCtx_ofPureCtx
    (Γ : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Context.Ctx n) :
...
@[simp] theorem lookup_ofPureCtx
    ...
    Ctx.lookup (ofPureCtx Γ) i = ofPure
      (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Context.lookup Γ i) := by
  induction Γ with
  | nil => exact Fin.elim0 i
  | @snoc n Γ A ih =>
      refine Fin.cases ?_ ?_ i
      · exact (ofPure_rename
          Mettapedia.Languages.MeTTa.Pure.Intrinsic.Renaming.wk A).symm
      · intro j
        change rename wk (Ctx.lookup (ofPureCtx Γ) j) =
          ofPure (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Renaming.rename
            Mettapedia.Languages.MeTTa.Pure.Intrinsic.Renaming.wk
            (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Context.lookup Γ j))
        rw [ih]
        exact (ofPure_rename
          Mettapedia.Languages.MeTTa.Pure.Intrinsic.Renaming.wk
          (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Context.lookup Γ j)).symm

/-- The inverse lookup square. -/
@[simp] theorem lookup_toPureCtx (Γ : Legacy.Ctx n) (i : Fin n) :
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.Context.lookup (toPureCtx Γ) i =
...
theorem step_of_oldRed {t u :
    ...
    (h : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Reduction.Red t u) :
...
theorem conv_of_oldConv {t u :
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.Syntax.PureTm n}
    (h : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.Conv t u) :
...
private theorem oldConv_map
    {F : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Syntax.PureTm n →
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.Syntax.PureTm m}
    (hF : ∀ {left right},
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.Reduction.Red left right →
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.Reduction.Red (F left) (F right))
    {left right : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Syntax.PureTm n}
    (h : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.Conv left right) :
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.Conv (F left) (F right) := by
  induction h with
  | rel x y hred =>
...
theorem oldConv_of_step {t u : Legacy.Tm n} (h : Step HeadEq t u) :
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.Conv (toPure t) (toPure u) := by
  induction h with
  | betaPi body a =>
      simpa only [toPure, toPure_inst0] using
        (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.red_implies_conv
...
theorem hasType_of_oldHasType
    {Γ : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Context.Ctx n}
    {t A : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Syntax.PureTm n}
    (h : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType Γ t A) :
...
theorem oldHasType_of_hasType {Γ : Legacy.Ctx n} {t A : Legacy.Tm n}
  ...
  | headType hHead =>
      cases hHead
      exact Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.u0_type _
  | @var n Γ i =>
  ...
  | piForm hA hu hB hv hj ihA ihB =>
      ...
      cases hv
      cases hj
      exact Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.pi_form
        ihA ihB
  | sigmaForm hA hu hB hv hj ihA ihB =>
      cases hu
      cases hv
      cases hj
      exact Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.sigma_form
        ihA ihB
  | lamIntro hBody ihBody =>
      exact Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.lam_intro
        ihBody
  | appElim hg ha ihg iha =>
      simpa only [toPure, toPure_inst0] using
        (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.app_elim
          ihg iha)
  | @pairIntro n Γ a b A B ha hb iha ihb =>
      have ihb' : Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType
          (toPureCtx Γ) (toPure b)
          (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Substitution.inst0
            (toPure a) (toPure B)) := by
        simpa only [toPure_inst0] using ihb
      exact Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.pair_intro
        iha ihb'
  | fstElim hp ihp =>
      exact Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.fst_elim ihp
  | sndElim hp ihp =>
      simpa only [toPure, toPure_inst0] using
        (Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.snd_elim ihp)
  | idForm hA hu ha hb ihA iha ihb =>
      cases hu
      exact Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.id_form
        ihA iha ihb
  | reflIntro ha iha =>
      exact Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.refl_intro iha
  | cumul ht hFalse iht => exact False.elim hFalse
  | conv ht hAB iht =>
      exact Mettapedia.Languages.MeTTa.Pure.Intrinsic.Typing.HasType.conv
...
theorem towerHeadEq_of_headEq {left right : Legacy.Head}
  ...
  cases left <;>
    simp [Legacy.Head.embed, Tower.HeadEq, Tower.zero,
      Mettapedia.Languages.MeTTa.TypeTheory.CumulativeTower.LevelExpr.eval]
...
theorem sealedUniverseApplication_hasNoType {n : Nat} (Γ : Tower.Ctx n)
    (level : Mettapedia.Languages.MeTTa.TypeTheory.CumulativeTower.LevelExpr)
    (A : Tower.Tm n) :
