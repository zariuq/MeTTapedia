...
namespace Mettapedia.Languages.MeTTa.TypeTheory.CumulativeTower.DeclarationAwareCheckedContext
...
def contextFormationDelta : CalculusLanguageExtension :=
...
private theorem contextFormationTarget_valid :
    (contextFormationDelta.apply Structural.checked.1).isValid = true := by
  have validatedLanguage :
      (contextFormationDelta.apply Structural.checked.1).toLanguageDef.validate =
        [] := by
    apply LanguageDef.validate_eq_nil_of_constructorOnly <;>
      simp [contextFormationDelta, CalculusLanguageExtension.apply,
...
def contextFormationExtension : ValidatedCalculusLanguageExtension Structural.checked where
...
theorem contextQuery_hasTargetJudgmentShape
  ...
  simp [contextFormationExtension, ValidatedCalculusLanguageExtension.target,
    contextFormationDelta, CalculusLanguageExtension.apply, Structural.checked,
...
def primeBaseSemantics :
    CalculusLanguageSemantics Structural.checked PrimeMeaning where
  ruleMeaning := by
...
private theorem instantiateContextSnocRule {n : Nat}
  ...
  simp [contextFormationExtension, ValidatedCalculusLanguageExtension.target,
    ...
    DeclarationAwareStructuralTyping.piFormRule,
    contextNilRule, contextSnocRule, instantiateRule?,
    CalculusLanguageDef.lookupRule?, argumentsValidAt, arityValid, levelsValid,
    levelValid,
...
end Mettapedia.Languages.MeTTa.TypeTheory.CumulativeTower.DeclarationAwareCheckedContext
