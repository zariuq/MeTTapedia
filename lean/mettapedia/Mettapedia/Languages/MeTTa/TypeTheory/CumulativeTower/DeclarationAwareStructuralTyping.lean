...
namespace Mettapedia.Languages.MeTTa.TypeTheory.CumulativeTower.DeclarationAwareStructuralTyping
...
@[simp] theorem definition_language_eq_data :
    definition.toLanguageDef = DeclarationAwareDataLanguage.language := by
  rfl

private theorem legacyGroundRule_validV1 :
    RuleSchema.isLocallyValid legacyGroundRule = true := by
  simp [RuleSchema.isLocallyValid, RuleSchema.metavariableNames,
...
private theorem sortRule_validV1 :
    RuleSchema.isLocallyValid sortRule = true := by
  simp [RuleSchema.isLocallyValid, RuleSchema.metavariableNames,
    RuleSchema.occurrences, RuleSchema.patterns,
    patternMetavariableOccurrencesAt, patternsMetavariableOccurrencesAt,
    patternHasNoCollectionRest, patternsHaveNoCollectionRest,
...
private theorem reflRule_validV1 :
    RuleSchema.isLocallyValid reflRule = true := by
  simp [RuleSchema.isLocallyValid, RuleSchema.metavariableNames,
    RuleSchema.occurrences, RuleSchema.patterns,
    patternMetavariableOccurrencesAt, patternsMetavariableOccurrencesAt,
    patternHasNoCollectionRest, patternsHaveNoCollectionRest,
...
private theorem piFormRule_validV1 :
    RuleSchema.isLocallyValid piFormRule = true := by
  simp [RuleSchema.isLocallyValid, RuleSchema.metavariableNames,
    RuleSchema.occurrences, RuleSchema.patterns,
    patternMetavariableOccurrencesAt, patternsMetavariableOccurrencesAt,
    patternHasNoCollectionRest, patternsHaveNoCollectionRest,
...
@[simp] theorem hasTypingJudgment :
    (definition.lookupJudgment? "prime-has-type" 4).isSome = true := by
  decide

private theorem legacyGroundRule_validIn :
    RuleSchema.isValidIn definition legacyGroundRule = true := by
  ...
  simp [
    ...
    CalculusLanguageDef.judgmentSchemaValid, hasTypingJudgment,
    fixedConstructorListsValid, fixedConstructorsValid,
    definition_language_eq_data,
    DeclarationAwareDataLanguage.fixed_encodeTowerHead]

private theorem sortRule_validIn :
    RuleSchema.isValidIn definition sortRule = true := by
  ...
  simp [RuleSchema.patterns,
    ...
    levelSuccPattern, CalculusLanguageDef.judgmentSchemaValid, hasTypingJudgment,
    fixedConstructorListsValid, fixedConstructorsValid,
    definition_language_eq_data]

private theorem reflRule_validIn :
    RuleSchema.isValidIn definition reflRule = true := by
...
private theorem piFormRule_validIn :
    RuleSchema.isValidIn definition piFormRule = true := by
  ...
  simp [RuleSchema.patterns, piFormRule, hasTypePattern, tmHeadPattern,
    ...
    ctxSnocPattern, CalculusLanguageDef.judgmentSchemaValid, hasTypingJudgment,
    fixedConstructorListsValid, fixedConstructorsValid,
    definition_language_eq_data]

theorem definition_valid : definition.isValid = true := by
  have validatedLanguage : definition.toLanguageDef.validate = [] := by
    simpa [definition, language] using language_validate
  have signatureValid : definition.judgmentSignatureValid = true := by
    have dataValid := DeclarationAwareDataLanguage.definition_valid
    unfold CalculusLanguageDef.isValid at dataValid
    simp only [Bool.and_eq_true] at dataValid
    have dataSignature := dataValid.1.1.2
    change DeclarationAwareDataLanguage.definition.judgmentSignatureValid =
      true
    exact dataSignature
  have conversionValid : definition.conversionDeclarationValid = true := by
    have dataValid := DeclarationAwareDataLanguage.definition_valid
    unfold CalculusLanguageDef.isValid at dataValid
    simp only [Bool.and_eq_true] at dataValid
    have dataConversion := dataValid.2
    change
      DeclarationAwareDataLanguage.definition.conversionDeclarationValid =
        true
    exact dataConversion
  have v1Valid : definition.hasValidLocalRules = true := by
    have validEmpty : definition.toLanguageDef.validate.isEmpty = true :=
...
def structuralSemantics :
    CalculusLanguageSemantics checked CanonicalMeaning where
  ruleMeaning := by
...
/-- One reusable waist joins the authored language definition, independent
...
private theorem instantiateLegacyPattern
  ...
  simp [instantiateRule?, checked, definition, Data.definition,
    CalculusLanguageDef.lookupRule?, ruleInstance, legacyGroundRule, sortRule,
...
end Mettapedia.Languages.MeTTa.TypeTheory.CumulativeTower.DeclarationAwareStructuralTyping
