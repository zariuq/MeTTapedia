...
theorem sequence_application {Value : Type}
  ...
  calc
    _ = (Program.map next program).bind (fun function => application function argument) :=
      application_action (Program.map next program) argument
    _ = _ := ContextualComputationKleisli.Program.bind_assoc program
