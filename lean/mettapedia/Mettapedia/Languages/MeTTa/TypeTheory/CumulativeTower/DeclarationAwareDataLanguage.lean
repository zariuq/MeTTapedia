...
namespace Mettapedia.Languages.MeTTa.TypeTheory.CumulativeTower.DeclarationAwareDataLanguage
...
def kernelDataType : TypeDecl := TypeDecl.plain "PrimeTypeTheoryData"

def dataConstructor (head : String) (arity : Nat) : GrammarRule :=
  { label := head
    category := "PrimeTypeTheoryData"
    params := (List.range arity).map fun index =>
      .simple s!"field{index}" (.base "PrimeTypeTheoryData")
    syntaxPattern := [] }
...
theorem definition_valid : definition.isValid = true := by
  ...
  simp [definition, constructorArities,
...
theorem encodeTowerTypingClaim_judgmentSchemaValid
  ...
  | mk arity context subject type =>
      change
        ((definition.lookupJudgment? "prime-has-type" 4).isSome &&
...
end Mettapedia.Languages.MeTTa.TypeTheory.CumulativeTower.DeclarationAwareDataLanguage
