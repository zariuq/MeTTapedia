...
def constantFactDelta (source : SourceDocument) : CalculusLanguageExtension :=
  { newTerms := []
    newJudgments := [{ head := "prime-authored-constant", arity := 3 }]
    newRules := generatedRules (elaborate source) }

abbrev basePresentation : ValidatedCalculusLanguageDef := formedTypingExtension.target
...
private theorem example_target_language_validate :
    ((constantFactDelta exampleSource).apply
      basePresentation.1).toLanguageDef.validate = [] := by
  simpa [constantFactDelta, CalculusLanguageExtension.apply] using
...
private theorem example_delta_policy :
  ...
  simp [CalculusLanguageExtension.policyHolds, basePresentation,
    ...
    constantFactPattern, formedTypingExtension, ValidatedCalculusLanguageExtension.target,
