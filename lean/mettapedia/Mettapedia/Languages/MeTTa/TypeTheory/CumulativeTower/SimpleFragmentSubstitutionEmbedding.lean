...
/-!
# Substitution-natural embedding of the simple fragment

The simple-function fragment used by the four-face comparison embeds into the
cumulative tower on every typed term.  This module proves that the embedding
...
This is a fragment comparison theorem.  It does not identify simple and
dependent type theory: genuinely varying dependent families remain outside the
simple image.
-/
...
namespace SimpleFragmentSubstitutionEmbedding
...
/-- Typed term renaming commutes with the simple-fragment embedding. -/
@[simp]
...
/-- Simultaneous substitution commutes with the simple-fragment embedding. -/
@[simp]
...
/-- Opening the newest binder commutes exactly with the embedding. -/
@[simp]
...
/-- The fragment embedding remains proper: the varying Boolean family in the
families CwF is not the image of any simple type. -/
theorem dependent_family_still_outside_simple_image :
...
#print axioms dependent_family_still_outside_simple_image

end SimpleFragmentSubstitutionEmbedding
