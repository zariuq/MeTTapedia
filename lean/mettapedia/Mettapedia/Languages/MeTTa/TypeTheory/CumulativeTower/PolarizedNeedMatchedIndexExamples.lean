...
theorem vector_selected (values : List Pattern) (index : Fin values.length) :
  ...
  simp [select?, vectorRequest, vectorReceipt, matchPattern, Bindings.lookup,
    MatchedIndexJudgment.checkIndex]
...
def duplicateBindings : Bindings :=
  [("rest", .collection .hashBag [duplicateVector] none), ("items", duplicateVector)]
...
private def isReply (reply : Wire) : Outcome Tower.Head Operation Nat Empty Empty 0 → Bool
  | .value (.returned (.native term)) => decide (term = NativeWireData.encode reply)
  | _ => false

private theorem isReply_iff (reply : Wire) (result : Outcome Tower.Head Operation Nat Empty Empty 0) :
    isReply reply result = true ↔ result = replyOutcome reply := by
  cases result <;> simp [isReply, replyOutcome]
  rename_i value
  cases value <;> simp
  rename_i value
  cases value <;> simp
...
#eval (answers canonical.request canonical.request emptyWorld {} 24).any
  (isReply (admittedWire canonical))
