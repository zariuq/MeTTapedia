import Mettapedia.Languages.MeTTa.PureKernel.Universe.DeclarationAwareCheckedContext
...
namespace Mettapedia.Languages.MeTTa.PureKernel.Universe.DeclarationAwareFormedTyping
...
def formedTypingDelta : PresentationExtension :=
  { newTerms := []
...
private theorem formedTypingTarget_valid :
    (formedTypingDelta.apply contextFormationExtension.target.1).isValidV2 = true := by
  have validatedLanguage :
      (formedTypingDelta.apply
        contextFormationExtension.target.1).language.validate = [] := by
    apply LanguageDef.validate_eq_nil_of_constructorOnly <;>
      simp [formedTypingDelta, contextFormationExtension,
        ValidatedExtension.target, contextFormationDelta,
  ...
  simp [formedTypingDelta, PresentationExtension.apply, formedTypingRule,
...
def formedTypingExtension :
    ValidatedExtension contextFormationExtension.target where
...
private theorem formedCodec_avoids_baseConclusions :
    PresentationConclusionsAvoid contextFormationExtension.target
...
noncomputable def primeRootBaseSemantics :
    PresentationSemantics contextFormationExtension.target
      PrimeRootMeaning :=
...
private theorem structuralClaim_hasJudgmentShape
  ...
  simp [Structural.checked, DeclarationAwareStructuralTyping.checked,
    Presentation.hasJudgmentShape, encodeTypingClaim,
...
private theorem structuralClaim_hasContextTargetJudgmentShape
  ...
  simpa [contextFormationExtension, ValidatedExtension.target,
    contextFormationDelta, PresentationExtension.apply,
    Presentation.hasJudgmentShape, Presentation.lookupJudgment?,
...
theorem RawFormedProof.derived_accepted_iff_components
  ...
  have contextIff :=
    InferenceNewJudgmentConservativity.ValidatedExtension.checkRaw_iff_base_of_newJudgmentsOnly
...
end Mettapedia.Languages.MeTTa.PureKernel.Universe.DeclarationAwareFormedTyping
