...
/-!
# A checked MIL chain over a validated presentation extension

This is the finite bridge canary between the intrinsic Prime metarule and the
generic proof checker.  The base presentation declares the relational judgment
...
The learned rule remains a presentation extension, not a new Prime primitive.
Its semantic use is governed by `InferenceSemanticExtension.SemanticExtension`.
-/

namespace Mettapedia.Languages.MeTTa.PureKernel.Universe.MILCheckedChain
...
private theorem basePresentation_valid :
  ...
  have hvalidate : basePresentation.language.validate = [] := by
    simpa [basePresentation] using relationLanguage_validate
  unfold Presentation.isValidV2 Presentation.isValidV1
  rw [hvalidate]
  simp [basePresentation, relationLanguage, entityConstructor,
...
private theorem learned_disjoint :
  ...
  simp [learnedDelta, PresentationExtension.disjointFrom, base,
    basePresentation, relationLanguage, motherRule, fatherRule, chainRule]
...
private theorem learned_policy :
  ...
  simp [learnedDelta, PresentationExtension.policyHolds, base,
    basePresentation, motherRule, fatherRule, chainRule, relates]
...
private theorem learned_target_valid :
  ...
  have hvalidate : (learnedDelta.apply base.1).language.validate = [] := by
    simpa [learnedDelta, PresentationExtension.apply, base,
      basePresentation] using relationLanguage_validate
  unfold Presentation.isValidV2 Presentation.isValidV1
  rw [hvalidate]
  simp [learnedDelta, PresentationExtension.apply, base, basePresentation,
    ...
    relates, alice, bob, carol, Presentation.ruleIds,
...
theorem grandparentProof_checked :
  ...
  simp [checkRaw, checkRawChildren, grandparentProof, motherProof, fatherProof,
    ...
    PresentationExtension.apply, base, basePresentation, motherRule,
    fatherRule, chainRule, instantiateRule?, Presentation.lookupRule?,
...
/-- Independent relational meaning for the presentation. -/
...
def learnedSemantics : SemanticExtension base learned Meaning where
  baseRuleSound := by
    ...
    | intro rule lookup =>
        simp [base, basePresentation, Presentation.lookupRule?] at lookup
  addedRuleSound := by
...
end Mettapedia.Languages.MeTTa.PureKernel.Universe.MILCheckedChain
