...
def equationFactDelta (source : SourceDocument) : PresentationExtension :=
  { newTerms := []
...
abbrev basePresentation : ValidatedPresentation :=
...
private theorem example_target_language_validate :
    ((equationFactDelta exampleSource).apply
      basePresentation.1).language.validate = [] := by
  simpa [equationFactDelta, PresentationExtension.apply] using
...
private theorem example_delta_disjoint :
  ...
  rw [example_elaborates]
  simp [PresentationExtension.disjointFrom, basePresentation,
    ...
    ValidatedExtension.target, DeclarationAwareFormedTyping.formedTypingDelta,
...
private theorem example_target_valid :
  ...
  simp [basePresentation, generatedRules, exampleDeclarations,
    ...
    Tower.zero, PresentationExtension.apply, Presentation.ruleIds,
