...
derivations.  A validated presentation extension and its interpretation are
then composed by the ordinary revision-indexed NIK admission doctrine.
...
/-- Exact relational evidence for the three-rule learned presentation. -/
...
/-- The base presentation has no rules, so its proof-relevant semantics is
...
def baseSemantics : PresentationSemantics MILCheckedChain.base Meaning where
  ruleMeaning := by
