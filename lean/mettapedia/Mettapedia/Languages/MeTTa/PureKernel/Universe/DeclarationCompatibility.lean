...
abbrev OldEntry :=
  Mettapedia.Languages.MeTTa.PureKernel.DeclarationEnv.DeclEntry
abbrev OldEnvironment :=
  Mettapedia.Languages.MeTTa.PureKernel.DeclarationEnv.DeclEnv
...
@[simp] theorem typeOf_ofOldEnvironment (environment : OldEnvironment)
    ...
    (ofOldEnvironment environment).typeOf? name =
      (Mettapedia.Languages.MeTTa.PureKernel.DeclarationEnv.typeOf?
...
@[simp] theorem valueOf_ofOldEnvironment (environment : OldEnvironment)
    ...
    (ofOldEnvironment environment).valueOf? name =
      (Mettapedia.Languages.MeTTa.PureKernel.DeclarationEnv.valueOf?
        environment name).map Legacy.ofPure := by
...
@[simp] theorem ofPure_oldLiftClosed
    (term : Mettapedia.Languages.MeTTa.PureKernel.Syntax.PureTm 0) :
    Legacy.ofPure
      (Mettapedia.Languages.MeTTa.PureKernel.DeclarationEnv.liftClosed
        (n := n) term) =
      (liftClosed (Legacy.ofPure term) : Legacy.Tm n) := by
  unfold Mettapedia.Languages.MeTTa.PureKernel.DeclarationEnv.liftClosed
    liftClosed
...
theorem step_of_redDecl {environment : OldEnvironment}
  ...
  | core reduction =>
      exact
        _root_.Mettapedia.Languages.MeTTa.PureKernel.Universe.Presentation.Declaration.StepCore.includeSignature
  ...
  | deltaConst lookup =>
      ...
      change RootStep Legacy.rules (ofOldEnvironment environment) _
        (.const _) (Legacy.ofPure
          (Mettapedia.Languages.MeTTa.PureKernel.DeclarationEnv.liftClosed _))
...
theorem convDecl_map {environment : OldEnvironment}
    ...
    {function :
      Mettapedia.Languages.MeTTa.PureKernel.Syntax.PureTm sourceDepth →
      Mettapedia.Languages.MeTTa.PureKernel.Syntax.PureTm targetDepth}
...
theorem convDecl_of_step {environment : OldEnvironment}
  ...
  | betaSigmaFst first second =>
      exact redDecl_implies_conv (by
        simpa only [Legacy.toPure] using
          (RedDecl.core
            (Mettapedia.Languages.MeTTa.PureKernel.Reduction.Red.betaSigmaFst
              (Legacy.toPure first) (Legacy.toPure second))))
  | betaSigmaSnd first second =>
      exact redDecl_implies_conv (by
        simpa only [Legacy.toPure] using
          (RedDecl.core
            (Mettapedia.Languages.MeTTa.PureKernel.Reduction.Red.betaSigmaSnd
              (Legacy.toPure first) (Legacy.toPure second))))
  | head equality =>
      cases equality
      exact .refl _
  ...
  | congLam step ih =>
      simpa only [Legacy.toPure] using convDecl_map
        (function := fun term =>
          Mettapedia.Languages.MeTTa.PureKernel.Syntax.PureTm.lam term)
  ...
  | congFst step ih =>
      simpa only [Legacy.toPure] using convDecl_map
        (function := fun term =>
          Mettapedia.Languages.MeTTa.PureKernel.Syntax.PureTm.fst term)
        (fun reduction => RedDecl.congFst reduction) ih
  | congSnd step ih =>
      simpa only [Legacy.toPure] using convDecl_map
        (function := fun term =>
          Mettapedia.Languages.MeTTa.PureKernel.Syntax.PureTm.snd term)
        (fun reduction => RedDecl.congSnd reduction) ih
  | congRefl step ih =>
      simpa only [Legacy.toPure] using convDecl_map
        (function := fun term =>
          Mettapedia.Languages.MeTTa.PureKernel.Syntax.PureTm.refl term)
...
theorem typeOf_toSignature {environment : OldEnvironment}
    {name : DeclName}
    {type : Mettapedia.Languages.MeTTa.PureKernel.Syntax.PureTm 0}
    (lookup :
      Mettapedia.Languages.MeTTa.PureKernel.DeclarationEnv.typeOf?
...
theorem hasType_of_hasTypeDecl {environment : OldEnvironment}
    {context : Mettapedia.Languages.MeTTa.PureKernel.Context.Ctx n}
    {term type : Mettapedia.Languages.MeTTa.PureKernel.Syntax.PureTm n}
