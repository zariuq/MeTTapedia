...
validated learned presentation contains an element relation and the two
...
finite presentation.  It retains one element witness and one recursive witness
...
namespace MILCheckedNativeListPresentation
...
/-! ## A validated recursive List-relator presentation -/
...
private theorem basePresentation_valid :
  ...
  have hvalidate : basePresentation.language.validate = [] := by
    simpa [basePresentation] using listRelationLanguage_validate
  unfold Presentation.isValidV2 Presentation.isValidV1
  rw [hvalidate]
  simp [basePresentation, listRelationLanguage, valueConstructor,
...
def base : ValidatedPresentation :=
...
def learnedDelta : PresentationExtension :=
  { newTerms := []
    newJudgments := []
...
private theorem learned_disjoint :
  ...
  simp [learnedDelta, PresentationExtension.disjointFrom, base,
    basePresentation, listRelationLanguage, successorRule, mapNilRule,
    mapConsRule]

private theorem learned_policy :
    learnedDelta.policyHolds base.1
      (.extendsBaseJudgments ["MIL.Rel", "MIL.MapRel"]) = true := by
  simp [learnedDelta, PresentationExtension.policyHolds, base,
    basePresentation, successorRule, mapNilRule, mapConsRule, relates, mapRel]

private theorem learned_target_valid :
    (learnedDelta.apply base.1).isValidV2 = true := by
  have hvalidate : (learnedDelta.apply base.1).language.validate = [] := by
    simpa [learnedDelta, PresentationExtension.apply, base,
      basePresentation] using listRelationLanguage_validate
  unfold Presentation.isValidV2 Presentation.isValidV1
  rw [hvalidate]
  simp [learnedDelta, PresentationExtension.apply, base, basePresentation,
...
def learned : ValidatedExtension base where
...
theorem singletonMapProof_checked :
  ...
  simp [checkRaw, checkRawChildren, singletonMapProof, successorProof,
    nilProof, learned, ValidatedExtension.target, learnedDelta,
    PresentationExtension.apply, base, basePresentation, successorRule,
    mapNilRule, mapConsRule, instantiateRule?, Presentation.lookupRule?,
...
theorem missingTailProof_rejected :
  ...
  simp [checkRaw, checkRawChildren, missingTailProof, successorProof,
    learned, ValidatedExtension.target, learnedDelta,
    PresentationExtension.apply, base, basePresentation, successorRule,
    mapNilRule, mapConsRule, instantiateRule?, Presentation.lookupRule?,
    argumentsValidAt, argumentValidAt, instantiateSchema?,
    instantiateSchemaAt?, instantiateSchemas?, instantiateSchemasAt?,
...
theorem ListStep.constructorView {relation sources targets : Pattern}
    ...
    (sources = MILCheckedNativeListPresentation.nil ∧
      targets = MILCheckedNativeListPresentation.nil ∧
      ...
      ∃ (sourceHead targetHead sourceTail targetTail : Pattern)
        ...
        sources = MILCheckedNativeListPresentation.cons sourceHead sourceTail ∧
          targets = MILCheckedNativeListPresentation.cons targetHead targetTail ∧
...
def baseSemantics : PresentationSemantics base Meaning where
  ruleMeaning := by
...
theorem NativeImage.targetShape {sources targets : Pattern}
    (image : NativeImage sources targets) :
    targets = MILCheckedNativeListPresentation.nil ∨
      ∃ tail, targets = MILCheckedNativeListPresentation.cons two tail := by
  cases image with
...
/-- The checked recursive List presentation, its independent semantics, and
...
end MILCheckedNativeListPresentation
