...
The intrinsic `PureKernel` syntax contains global declaration constants, while
...
`PureKernel.HasType` rules omit several of those premises.
...
declaration to `PureKernel` never silently enlarges the Pattern Pure grammar.
-/
...
def quoteNameNat : Nat → Pattern
  | 0 => .apply "PureKernel.name.nat.zero" []
  | n + 1 => .apply "PureKernel.name.nat.succ" [quoteNameNat n]
...
def quoteDeclName : DeclName → Pattern
  | .anonymous => .apply "PureKernel.name.anonymous" []
  | .str pre component =>
      .apply "PureKernel.name.str" [quoteDeclName pre, .apply component []]
  | .num pre component =>
      .apply "PureKernel.name.num" [quoteDeclName pre, quoteNameNat component]

def quoteTaggedConst (c : DeclName) : Pattern :=
  .apply "PureKernel.const" [quoteDeclName c]
...
def isTaggedConst : Pattern → Bool
  | .apply name _ => name == "PureKernel.const"
  | _ => false
...
def ConstantFreeRed (t u : PureTm n) : Prop :=
  Mettapedia.Languages.MeTTa.PureKernel.Reduction.Red t u ∧
...
theorem ConstantFreeConv.toConv {t u : PureTm n} (h : ConstantFreeConv t u) :
    Mettapedia.Languages.MeTTa.PureKernel.Typing.Conv t u := by
  induction h with
...
theorem rawConstantDetour_reduces (c : DeclName) :
    Mettapedia.Languages.MeTTa.PureKernel.Reduction.Red
...
theorem raw_conversion_has_impure_detour (c : DeclName) :
    ∃ middle : PureTm 0,
      Mettapedia.Languages.MeTTa.PureKernel.Typing.Conv .u0 middle ∧
      Mettapedia.Languages.MeTTa.PureKernel.Typing.Conv middle .u0 ∧
...
Unlike the older `PureKernel.HasType`, every rule which consumes a dependent
...
theorem RegularHasType.toHasType (h : RegularHasType Γ t A) :
    Mettapedia.Languages.MeTTa.PureKernel.Typing.HasType Γ t A := by
  induction h with
...
theorem raw_allows_untyped_lambda_domain :
    Mettapedia.Languages.MeTTa.PureKernel.Typing.HasType (.nil : Ctx 0)
      (.lam (.var 0)) (.pi .u1 .u1) := by
  apply Mettapedia.Languages.MeTTa.PureKernel.Typing.HasType.lam_intro
  exact Mettapedia.Languages.MeTTa.PureKernel.Typing.HasType.var 0
...
def rawRegularRuleModel : RegularRuleModel
    (fun Γ t A =>
      Mettapedia.Languages.MeTTa.PureKernel.Typing.HasType Γ t A) where
