...
/-!
# MeTTa Runtime Execution Interface

Packages the first concrete runtime-execution backend interface, `R_exec₀`, around
...
/-- First source-rule firing contract used by the current `R_exec₀` interface.

This packages exactly the theorem already proved for the no-premise, `fvar`-LHS
fragment of the least relation generated by authored rewrite rules.
-/
...
/-- First theoremic backend interface for MeTTa-family runtime execution. -/
structure MeTTaRuntimeExecInterface where
  backendName : String
...
/-- Extended theoremic execution interface over the same backend. This packages
the already-proved premise-aware and guard-aware source-rule bridges without
changing the live runtime semantics. -/
structure MeTTaRuntimeExecExtendedInterface extends MeTTaRuntimeExecInterface where
...
/-- Query-side sibling of the current theoremic runtime execution interface.
...
-/
structure MeTTaRuntimeQueryInterface where
  backendName : String
  objectLanguage : String
  patternTranslation : ILP → Atom
  bindingsTranslation : ILBind → Subst
...
def morkRuntimeExec0 : MeTTaRuntimeExecInterface where
...
def morkRuntimeExec0Ext : MeTTaRuntimeExecExtendedInterface where
  toMeTTaRuntimeExecInterface := morkRuntimeExec0
...
rule firing.
-/
noncomputable def morkRuntimeQueryExec0 : MeTTaRuntimeQueryInterface where
  backendName := "MORK/MM2"
  objectLanguage := "MeTTaIL"
  patternTranslation := patternTranslation
  bindingsTranslation := bindingsTranslation
...
/-- Space-effect sibling interface for `add-atom` and `remove-atom`. -/
structure MeTTaRuntimeSpaceEffectInterface where
  backendName : String
  objectLanguage : String
  baseExecInterface : MeTTaRuntimeExecInterface
...
/-- Oracle request/response interface for ACT and Z3 resources.
...
-/
structure MeTTaRuntimeOracleInterface where
  backendName : String
  objectLanguage : String
...
structure MeTTaRuntimeKernelTriad where
  ruleExec : MeTTaRuntimeExecInterface
  query : MeTTaRuntimeQueryInterface
  spaceEffect : MeTTaRuntimeSpaceEffectInterface

/-- Canonical space-effect sibling of `R_exec₀`: the current direct-execution
interface for `add-atom` and `remove-atom` over the default atomspace. -/
def morkRuntimeSpaceEffectExec0 : MeTTaRuntimeSpaceEffectInterface where
  backendName := "MORK/MM2"
  objectLanguage := "MeTTaIL"
  baseExecInterface := morkRuntimeExec0
...
def morkRuntimeOracleExec0 : MeTTaRuntimeOracleInterface where
  backendName := "MORK/MM2"
  objectLanguage := "MeTTaIL"
  requestEncoding := OracleQuery.resourceRequest
