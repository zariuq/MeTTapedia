...
Pure rho has one authored static equation, one authored reflective
presentation, and collection laws generated by its parallel constructor.
Every proof-relevant generator in the first Cost language retains its exact
base-or-wrapped source declaration. This is declaration classification only.
...
/-- Equation, collection-law, and reflection classification for one proof-relevant generated
rho Cost occurrence. -/
def RhoCostAuthoredGeneratorOrigin {left right : Pattern}
  ...
  | .core (.derived _ lawWitness) =>
      CostCollectionDeclarationOrigin rhoCIGSLT lawWitness.declaration
...
image of an equation, collection, or reflective declaration. -/
theorem nonempty_rhoCostAuthoredGeneratorOrigin
  ...
  | core witness =>
      ...
      | derived context lawWitness =>
          exact nonempty_costCollectionDeclarationOrigin rhoCIGSLT lawWitness
...
declaration to rho's QuoteDrop equation, collection laws, or reflective presentation. -/
...
authored-declaration occurrence above it. -/
...
#print axioms nonempty_rhoCostTypedGeneratorOccurrence
