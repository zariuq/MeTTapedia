...
support or the empty sealed support.  At that depth an exposed atom is
unshifted, while a sealed atom's well-scoped normal is invariant under the
remaining lift, so the key equality recovers equality of the stored normals. -/
theorem atom_of_keyEqAt_exposedDepth_of_support_eq_or_nil
    {source : CIGSLT} {leftColor rightColor : CostStaticColor}
    {targetFree : WellSorted.FreeTypeContext}
    {leftOccurrences rightOccurrences : List CostRegionOccurrence}
    {leftTable : TypedCostRegionBoundaryTable source leftColor targetFree
...
/-- Equality at any semantic-key depth aligns two atoms whose supports are
either one shared exposed support or the empty sealed support.  Lifting bound
variables is injective, while a sealed atom is fixed by every such lift, so a
key collision still recovers equality of the stored normal forms. -/
