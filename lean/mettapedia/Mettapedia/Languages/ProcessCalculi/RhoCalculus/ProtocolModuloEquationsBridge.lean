...
/-!
# The rho calculus as a dependent protocol modulo equations

The generic rewriting-modulo-equations construction is instantiated on the existing
closed rho carrier, canonical process equations, and declaration-derived raw
rewrite relation.  The resulting GSLT modulo equations and the established rho GSLT
...
namespace Mettapedia.Languages.ProcessCalculi.RhoCalculus.ProtocolModuloEquationsBridge
...
/-- Generic modulo-equations occurrences specialized to rho. -/
abbrev SaturatedOccurrence (source target : RhoProcess) :=
  Mettapedia.GSLT.Dynamics.ProtocolModuloEquations.Occurrence
    rhoProcessEquations MiddleReceipt source target

/-- The hand-written rho occurrence and the generic modulo-equations
...
/-- The generic modulo-equations system for rho. -/
...
abbrev saturatedRhoProtocol :=
  Mettapedia.GSLT.Dynamics.ProtocolModuloEquations.protocol
    rhoProcessEquations MiddleReceipt

/-- Its literal-equality endpoint GSLT. -/
abbrev saturatedRhoProtocolSystem : GSLT :=
  Mettapedia.GSLT.Dynamics.ProtocolModuloEquations.protocolSystem
    rhoProcessEquations MiddleReceipt

/-- Generic closure under equations and the established rho GSLT have exactly the
same endpoint steps. -/
...
relation, by composition through generic closure under equations. -/
theorem protocol_step_iff_languageDef_step
    {source target : RhoProcess} :
    saturatedRhoProtocolSystem.Step source target <->
      rhoLanguageDefGSLT.Step source target := by
  exact
    (Mettapedia.GSLT.Dynamics.ProtocolModuloEquations.protocol_step_iff_saturated_step
...
end Mettapedia.Languages.ProcessCalculi.RhoCalculus.ProtocolModuloEquationsBridge
