...
mutual
  /-- Quote-aware scope safety of a cost name survives erasure whenever each
  encoded signature is itself a closed rho name. -/
  theorem CostName.BinderSafeAt.erase_binderSafeAt
      {Ground : Type u} {signatureName : SignatureNameEncoding Ground}
      (signatureSafe : ∀ signature,
        binderSafeAt "NQuote" 0 (signatureName signature) = true)
      {depth : Nat} {name : CostName Ground} (safe : name.BinderSafeAt depth) :
      binderSafeAt "NQuote" depth (name.erase signatureName) = true := by
    cases safe with
    | bvar inScope => simpa [CostName.erase, binderSafeAt] using inScope
    | quote termSafe =>
        simpa [CostName.erase, binderSafeAt] using
          termSafe.erase_binderSafeAt signatureSafe
    | signature =>
        exact binderSafeAt_mono "NQuote" (signatureSafe _) (Nat.zero_le depth)

  /-- Process form of quote-aware scope preservation by erasure. -/
  theorem CostProc.BinderSafeAt.erase_binderSafeAt
      {Ground : Type u} {signatureName : SignatureNameEncoding Ground}
      (signatureSafe : ∀ signature,
        binderSafeAt "NQuote" 0 (signatureName signature) = true)
      {depth : Nat} {process : CostProc Ground}
      (safe : process.BinderSafeAt depth) :
      binderSafeAt "NQuote" depth (process.erase signatureName) = true := by
    cases safe with
    | nil => simp [CostProc.erase, binderSafeAt, binderSafeListAt]
    | par leftSafe rightSafe =>
        simp [CostProc.erase, binderSafeAt, binderSafeListAt,
          leftSafe.erase_binderSafeAt signatureSafe,
          rightSafe.erase_binderSafeAt signatureSafe]
    | send channelSafe payloadSafe =>
        simp [CostProc.erase, binderSafeAt, binderSafeListAt,
          channelSafe.erase_binderSafeAt signatureSafe,
          payloadSafe.erase_binderSafeAt signatureSafe]
    | recv channelSafe bodySafe =>
        simp [CostProc.erase, binderSafeAt, binderSafeListAt,
          channelSafe.erase_binderSafeAt signatureSafe,
          bodySafe.erase_binderSafeAt signatureSafe]

  /-- Wrapped-term form of quote-aware scope preservation by erasure. -/
  theorem CostTerm.BinderSafeAt.erase_binderSafeAt
      {Ground : Type u} {signatureName : SignatureNameEncoding Ground}
      (signatureSafe : ∀ signature,
        binderSafeAt "NQuote" 0 (signatureName signature) = true)
      {depth : Nat} {term : CostTerm Ground} (safe : term.BinderSafeAt depth) :
      binderSafeAt "NQuote" depth (term.erase signatureName) = true := by
    cases safe with
    | nil => simp [CostTerm.erase, binderSafeAt, binderSafeListAt]
    | signed processSafe =>
        exact processSafe.erase_binderSafeAt signatureSafe
    | par leftSafe rightSafe =>
        simp [CostTerm.erase, binderSafeAt, binderSafeListAt,
          leftSafe.erase_binderSafeAt signatureSafe,
          rightSafe.erase_binderSafeAt signatureSafe]
    | drop nameSafe =>
        simp [CostTerm.erase, binderSafeAt, binderSafeListAt,
          nameSafe.erase_binderSafeAt signatureSafe]
    | purse => simp [CostTerm.erase, binderSafeAt, binderSafeListAt]
end
