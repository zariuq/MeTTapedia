...
theorem stoppedBoundary_targetSupport_eq_nil_of_quoteRoot
...
theorem decodeDeclared_foreignQuote_eq_none
...
theorem rhoCrossColor_foreignNameRoot_escape
    {leftColor rightColor : CostStaticColor}
  ...
  · exact ⟨wire, arguments.map (canonicalize declaration), canonicalShape,
      decodeDeclared_foreignQuote_eq_none different⟩

structure RhoCrossColorSealedDescent
...
theorem rhoCrossColor_leftCollapsing_sealedDescent
    ...
    (canonical : canonicalize
      ...
      canonicalize
        (costStaticReflectivePresentationDecl rhoCIGSLT declarationColor
          rhoReflectivePresentation.toReflectivePresentationDecl)
        rightPattern) :
    Nonempty (RhoCrossColorSealedDescent
  ...
  have targetEscape : RhoDescendEscape leftColor
      (canonicalize
        (costStaticReflectivePresentationDecl rhoCIGSLT leftColor
          rhoReflectivePresentation.toReflectivePresentationDecl)
        rightPattern) :=
    rhoCrossColor_foreignNameRoot_escape rightView different rightName
  ...
  have boundarySupport : state.certified.typed.boundary.targetSupport = [] :=
    stoppedBoundary_targetSupport_eq_nil_of_quoteRoot leftView.node state
      skeletonShape
  exact ⟨
    { payload := payload
      state := state
...
theorem stoppedBoundary_targetType_eq_of_nameRoot
    {color : CostStaticColor} {targetFree : FreeTypeContext}
    (node : CostStaticRegionNode rhoCIGSLT color targetFree)
    {payload : Pattern}
    (state : CostStaticPlanStopped rhoCIGSLT color targetFree payload
...
noncomputable def sealedStoppedCollapseOfCloseSmaller
...
theorem rhoCrossColor_leftCollapsing_leafExposure
    {declarationColor leftColor rightColor : CostStaticColor}
    {targetFree : FreeTypeContext} {available outer : List TypeExpr}
    {leftPattern rightPattern : Pattern} {type : TypeExpr}
    {left : CostRegionTree rhoCIGSLT targetFree available outer leftPattern
    ...
    Nonempty (RhoCollapsingLeafExposure leftView.node leftView.children
      right) := by
  obtain ⟨descent⟩ := rhoCrossColor_leftCollapsing_sealedDescent
    leftView rightView different sourceSortEq notInteracting collapsing
      canonical
  have leftName : leftView.node.sourceSort.1 = "Name" :=
    rho_sourceSort_eq_name_of_ne_interacting notInteracting
  have boundaryName := stoppedBoundary_targetType_eq_of_nameRoot
  ...
  exact ⟨sealedStoppedCollapseOfCloseSmaller left right leftView
...
theorem rhoCrossColor_rightCollapsing_leafExposure
    {declarationColor leftColor rightColor : CostStaticColor}
    {targetFree : FreeTypeContext} {available outer : List TypeExpr}
    {leftPattern rightPattern : Pattern} {type : TypeExpr}
    {left : CostRegionTree rhoCIGSLT targetFree available outer leftPattern
  ...
  have closeSwapped : RhoPairCloseSmaller declarationColor targetFree
    ...
    rw [Nat.add_comm]
    exact closeSmaller
  exact rhoCrossColor_leftCollapsing_leafExposure
