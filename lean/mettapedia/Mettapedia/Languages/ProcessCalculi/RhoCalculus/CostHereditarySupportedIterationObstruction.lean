...
/-!
# The exact Cost² boundary for rho's supported hereditary executor
...
def hereditarySupported
    (laws : CIGSLT.CostOneObjectLawsFor rhoCIGSLT
      rhoCostNormalizeOpenHereditarySupported) : RhoCostOneConfiguration where
...
representative premise used by the parameterized Cost² obstruction. -/
theorem hereditarySupported_emptyParallelSourceRepresentative
    (laws : CIGSLT.CostOneObjectLawsFor rhoCIGSLT
      rhoCostNormalizeOpenHereditarySupported) :
    RhoEmptyParallelSourceRepresentative
      (RhoCostOneConfiguration.hereditarySupported laws) := by
...
theorem rhoHereditarySupportedCostOne_not_compactCostNormalizationCoherent
    (laws : CIGSLT.CostOneObjectLawsFor rhoCIGSLT
      rhoCostNormalizeOpenHereditarySupported) :
  ...
  exact rhoCostOneFor_not_compactCostNormalizationCoherent
    (RhoCostOneConfiguration.hereditarySupported laws)
...
normalizer-indexed Cost₁ object.  The hypotheses are precisely the two
remaining semantic crown witnesses used to construct that object. -/
theorem rhoHereditaryCostOneDomainObject_not_compactCostNormalizationCoherent_of
    ...
    ¬ CompactCostNormalizationCoherent
      (rhoHereditaryCostOneDomainObject_of alignable preservesReflectiveSupport
        ).compactOutput.toCIGSLT := by
  exact rhoHereditarySupportedCostOne_not_compactCostNormalizationCoherent
    (rhoHereditaryCostOneObjectLaws_of alignable preservesReflectiveSupport)
