...
/-!
# Structural and modal formulas derived from a language
...
This module takes the first step of OSLF → structural-modal formula theory by naming that
...
* **Structural-modal formulas are derived.**  `satisfies_congr` shows the whole semantics
  ...
  So a structural-modal formula is a *view* of the core fields, never extension data.
...
namespace Mettapedia.OSLF.StructuralModal
...
/-! ## The structural-modal formula language
...
/-- A type of the structural-modal layer of some language. -/
...
/-- Structural-modal satisfaction over an explicit reduction span.  This is the
...
changing the structural-modal formula language. -/
def satisfiesOver (span : ReductionSpan Pattern) : Formula → Pattern → Prop
  | .top, _ => True
  | .bot, _ => False
...
def satisfiesAllOver (span : ReductionSpan Pattern) :
    List Formula → List Pattern → Prop
  | [], [] => True
...
/-- Structural-modal satisfaction for a language under an explicit relation
...
def satisfiesUsing (relEnv : MeTTaIL.Engine.RelationEnv)
    (lang : LanguageDef) : Formula → Pattern → Prop :=
  satisfiesOver (langSpanUsing relEnv lang)

/-- Pointwise structural-modal satisfaction under an explicit relation environment. -/
def satisfiesAllUsing (relEnv : MeTTaIL.Engine.RelationEnv)
    (lang : LanguageDef) : List Formula → List Pattern → Prop :=
  satisfiesAllOver (langSpanUsing relEnv lang)

/-- What it means for a pattern to inhabit a structural-modal formula under the default
empty relation environment. -/
def satisfies (lang : LanguageDef) : Formula → Pattern → Prop :=
  satisfiesUsing MeTTaIL.Engine.RelationEnv.empty lang

/-- Pointwise default inhabitation of an argument vector. -/
def satisfiesAll (lang : LanguageDef) : List Formula → List Pattern → Prop :=
...
/-- A finite positive certificate for structural-modal satisfaction. -/
inductive Certificate (span : ReductionSpan Pattern) :
    Formula → Pattern → Type where
...
inductive CertificateAll (span : ReductionSpan Pattern) :
    List Formula → List Pattern → Type where
...
/-- Every finite structural-modal certificate denotes true structural-modal satisfaction. -/
theorem Certificate.sound {span : ReductionSpan Pattern} :
    ∀ {formula pattern}, Certificate span formula pattern →
      satisfiesOver span formula pattern
  | _, _, .top _ => trivial
...
/-- `bot` has no positive structural-modal certificate. -/
...
def Formula.finitelyCertifiable : Formula → Bool
  | .top | .bot => true
...
def formulasFinitelyCertifiable : List Formula → Bool
  | [] => true
  | formula :: formulas =>
      formula.finitelyCertifiable &&
        formulasFinitelyCertifiable formulas
...
theorem Certificate.complete {span : ReductionSpan Pattern} :
    ∀ (formula : Formula) (pattern : Pattern),
      formula.finitelyCertifiable = true →
      satisfiesOver span formula pattern →
...
theorem CertificateAll.complete {span : ReductionSpan Pattern} :
    ∀ (arguments : List Formula) (children : List Pattern),
      formulasFinitelyCertifiable arguments = true →
      satisfiesAllOver span arguments children →
  ...
  | argument :: arguments, child :: children, supported, inhabited => by
      simp only [formulasFinitelyCertifiable, Bool.and_eq_true] at supported
      obtain ⟨headEvidence⟩ :=
...
/-- The constructor and arity visible in a spatial structural-modal formula. -/
...
def Formula.headSignature? : Formula → Option HeadSignature
...
theorem patternHeadSignature_of_satisfiesOver_headed
    ...
    (arguments : List Formula) (pattern : Pattern)
...
/-- Every declared constructor yields a structural-modal formula: the terms headed by it,
...
def constructorFormulas (lang : LanguageDef) : List Formula :=
  lang.terms.map fun rule =>
    Formula.headed rule.label (rule.params.map fun _ => Formula.top)
...
theorem constructorFormulas_factors_through_terms {first second : LanguageDef}
    (sameTerms : first.terms = second.terms) :
    constructorFormulas first = constructorFormulas second := by
  simp [constructorFormulas, sameTerms]

/-- Stated as a factorization in the criterion's own vocabulary. -/
theorem constructorFormulas_factors : Factors LanguageDef.terms constructorFormulas :=
...
/-- **Structural-modal satisfaction is determined by the step relation.**  Two languages
whose derived reductions agree assign every structural-modal formula the same meaning.
...
theorem satisfies_congr {first second : LanguageDef}
    ...
    ∀ (formula : Formula) (pattern : Pattern),
      satisfies first formula pattern ↔ satisfies second formula pattern := by
  intro formula pattern
  change satisfiesOver (langSpan first) formula pattern ↔
    satisfiesOver (langSpan second) formula pattern
  rw [sameStep]
...
/-- **The boundary, stated once.**  What the structural-modal layer says is derived —
...
theorem derived_and_authored_boundary (lang : LanguageDef) :
    Factors LanguageDef.terms constructorFormulas ∧
      ¬ Factors (fun typed : TypedLanguage => typed.1) (fun typed => typed.2) :=
  ⟨constructorFormulas_factors, authored_typing_not_determined_by_language lang⟩

end Mettapedia.OSLF.StructuralModal
