...
4. `langSpanUsing` — presents its equation-saturated reduction graph
...
/-- Declarative/internal one-step reduction induced by a `LanguageDef`.
    ...
    consumes `langSemanticReducesUsing`, its equation-saturated closure.  A
...
/-! ## Step 2: Equation-saturated GSLT semantics -/
...
def langGSLTUsing (relEnv : RelationEnv) (lang : LanguageDef) :
  ...
  equationSaturatedGSLT (engineBasePremises relEnv) lang

/-- Default equation-saturated GSLT using `RelationEnv.empty`. -/
...
theorem langReducesUsing_to_semantic
  ...
  step_to_equationSaturatedStep step
...
theorem langSemanticReducesUsing_iff_langReducesUsing_of_equation_free
  ...
  equationSaturatedStep_iff_step_of_no_generators equationFree source target
...
/-- The equation-saturated reduction span for a LanguageDef and relation env.
...
respect authored equations, and diamond/box range over the equation-saturated
one-step relation.  Raw predicate generators are admitted only through the
least equation-saturated predicate. -/
...
/-- OSLF-modal corollary of SC-empty irreducibility: no `◇⊤` at SC-empty
    representatives, provided the equation-saturated generated step has been
