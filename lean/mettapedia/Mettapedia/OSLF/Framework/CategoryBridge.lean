...
The predicate category is the frame of predicates on semantic equation
classes.  Its authored-term presentation is `EquationPredicate`: a predicate
paired with its proof of equation invariance.
-/

/-- The semantic predicate lattice generated by a language. -/
...
def PredLattice.get {lang : LanguageDef} (φ : PredLattice lang) : Pattern → Prop :=
  φ.1

/-- Lift `langDiamond` to operate on `PredLattice`. -/
def langDiamondL (lang : LanguageDef) : PredLattice lang → PredLattice lang :=
  langDiamond lang

/-- Lift `langBox` to operate on `PredLattice`. -/
def langBoxL (lang : LanguageDef) : PredLattice lang → PredLattice lang :=
  langBox lang
...
noncomputable def langOSLFFibrationUsing_presheafAgreement
  ...
  (sortFamilySubfunctorEquivPred
      (R := langRewriteSystem lang procSort)
      (T := fun _ => Quotient (langGSLT lang).equations)).trans
    (Equiv.piCongrRight fun _ => (equationPredicatesEquiv (langGSLT lang)).symm)
...
This is a concrete instance of the bridge on the actual executable language
wrapper used by OSLF.
-/
noncomputable def rhoLangOSLFFibrationUsing_presheafAgreement :
    (predFibrationPresheafPrimary
      ...
      (sortFamilyPresheaf (langRewriteSystem rhoCalc "Proc")
        (fun _ => Quotient (langGSLT rhoCalc).equations))
      ≃
...
theorem languageSortFiber_ofPatternPred_mem_iff_satisfies
    ...
    (hNat : languageSortPredNaturality lang s seed φ.1)
    {X : Opposite (ConstructorObj lang)}
    (h : (languageSortRepresentableObj lang s).obj X) :
    h ∈ (languageSortFiber_ofPatternPred lang s seed φ.1 hNat).obj X
      ↔
...
theorem rhoProc_langOSLF_predicate_to_fiber_mem_iff
    ...
    (hNat : languageSortPredNaturality rhoCalc rhoProc seed φ.1)
    {X : Opposite (ConstructorObj rhoCalc)}
    (h : (languageSortRepresentableObj rhoCalc rhoProc).obj X) :
  h ∈ (languageSortFiber_ofPatternPred rhoCalc rhoProc seed φ.1 hNat).obj X
      ↔
...
noncomputable def rhoProcOSLFUsingPred_to_languageSortFiber
  ...
  languageSortFiber_ofPatternPred rhoCalc rhoProc seed φ.1 hNat
...
theorem rho_proc_commDiWitnessLifting_of_pkg
    ...
    commDiWitnessLifting rhoCalc rhoProc seed q φ.1 := by
  exact commDiWitnessLifting_of_pathSemLift rhoCalc rhoProc seed q φ.1 hPkg.1 hPkg.2
