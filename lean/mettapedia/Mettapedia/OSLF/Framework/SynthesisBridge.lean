...
On the syntax derived from the rho presentation, `rhoStep_sound` proves that
the contextual relation compiled from `rhoCalc` is sound for `Reduces`.
...
The generated relation is the least finite relation induced by the authored
rho rules.  On a source admitted by the presentation-derived process judgment,
its soundness for the established COMM/PAR/EQUIV relation is unconditional.
Completeness is deliberately separate: this section does not claim that every
low-level `Reduces` derivation is generated by `rhoCalc`. -/

/-- A contextual step compiled from `rhoCalc` is a step of the established
    rho reduction relation on presentation-derived process syntax. -/
theorem langReduces_rhoCalc_sound
    {free : FreeSortContext} {bound : List String} {p q : Pattern}
    (typed : ProcWellSorted rhoReflectivePresentation free bound p)
    (step : langReduces rhoCalc p q) :
    Nonempty (Reduces p q) := by
  exact rhoStep_sound typed step

/-- The generated rho possibility modality soundly maps into the established
    possibility modality on presentation-derived process syntax. -/
theorem langDiamond_rhoCalc_implies_possibly
    {free : FreeSortContext} {bound : List String}
    (φ : Pattern → Prop) (p : Pattern)
    (typed : ProcWellSorted rhoReflectivePresentation free bound p)
    (holds : langDiamond rhoCalc φ p) :
    possiblyProp φ p := by
  obtain ⟨q, step, satisfies⟩ :=
    (langDiamond_spec (lang := rhoCalc) (φ := φ) (p := p)).1 holds
  exact ⟨q, langReduces_rhoCalc_sound typed step, satisfies⟩
...
Layer 2 ↔ Layer 3 depends on agreement between the established relation and
the generic `LanguageDef` interpretation.  Currently:
...
- The generic interpreter compiles the authored reflective substitution;
  unrestricted completeness remains separate
...
2. **Layer 3 → Layer 2** (proven on derived rho syntax):
   `langDiamond_rhoCalc_implies_possibly`
...
The sound connection from the authored rho `LanguageDef` to `Reduces` is
`langReduces_rhoCalc_sound`.  A converse theorem would require a distinct
