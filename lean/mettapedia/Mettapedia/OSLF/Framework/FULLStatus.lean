...
def tracker : List Milestone :=
  ...
  , { area := "Premise Semantics"
      ...
      status := .done
      codeRef := "Mettapedia/OSLF/MeTTaIL/ContextualStep.lean: mem_rewriteAt_iff_stepAt"
      note := "The fuel-indexed compiler is exact for the least relation generated by authored rules; congruence premises alone carry contextual authority." }
  , { area := "Canonical vs Extension Policy"
      ...
      note := "Pure rho authors ParCong; the set extension adds ParSetCong explicitly. Collection representation carries no reduction authority." }
  , { area := "Category Lift"
