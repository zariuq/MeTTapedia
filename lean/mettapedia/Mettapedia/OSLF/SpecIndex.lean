...
- Lean: `Reduces` (RhoCalculus/Reduction.lean:50)
  - The pure relation contains COMM, equivalence closure, and parallel-context
    closure only; set-context closure is an explicit extension relation.
  - `.comm`: {n!(q) | for(<-n){p} | rest} ⇝ {substBVar p (@q) | rest}
  - `.equiv`: p ≡ p' ⇝ q' ≡ q ⇒ p ⇝ q

### Canonical vs Extension Language Policy
- Canonical language: `rhoCalc`, with an explicit `ParCong` rewrite rule.
- Optional extension: `rhoCalcSetExt`, adding the explicit `ParSetCong` rule.
...
### Authored Contextual Reduction
- Declarative relation: `ContextualStep.Step`, the least relation generated by
  authored rewrite rules.
- Bounded relation: `ContextualStep.StepAt`, indexed by contextual derivation
  depth.
- Executable compiler: `ContextualStep.rewriteAt`.
- Exactness: `mem_rewriteAt_iff_stepAt` and
  `exists_mem_rewriteAt_iff_step`.
- Relation environment: `RelationEnv` supplies non-contextual
  `relationQuery` tuples; a `Premise.congruence` recursively invokes the same
  generated relation.
- Representation forms grant no independent contextual reduction authority.
...
- Exact executable/relational characterization:
...
| 10 | `matchPattern_iff_matchRel` | MatchSpec.lean | matcher exactness |
...
#check @StepAt
#check @rewriteAt
#check @mem_rewriteAt_iff_stepAt
...
#check @Mettapedia.OSLF.PresheafNativeType.TheoryMorphism.PiSigmaPropColaxRuleSet
...
#check @Mettapedia.OSLF.PresheafNativeType.TheoryMorphism.comp_piSigmaProp_colax_rules
...
#check @Mettapedia.OSLF.PresheafNativeType.TheoryMorphism.piOmegaProp_with_constructor_transport_bundle
...
#check @Mettapedia.OSLF.PresheafNativeType.TheoryMorphism.id_piSigmaOmegaProp_translation_endpoint
...
#check @Mettapedia.OSLF.PresheafNativeType.constructorNativeType_obj_roundtrip
...
#check @Mettapedia.OSLF.PresheafNativeType.fullGrothObj_to_scopedConstructorPred_at_representable
