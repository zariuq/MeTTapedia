...
-/
structure NativeType (L : LambdaTheory) where
...
the induced map preserves the predicate. See `EvidenceFibration.lean`
...
def NativeTypeFiber (L : LambdaTheory) (S : L.Obj) := L.fibration.Sub S

instance (L : LambdaTheory) (S : L.Obj) : Preorder (NativeTypeFiber L S) :=
  inferInstanceAs (Preorder (L.fibration.Sub S))

instance (L : LambdaTheory) (S : L.Obj) : PartialOrder (NativeTypeFiber L S) :=
  inferInstanceAs (PartialOrder (L.fibration.Sub S))

/-- The fiber over S has Frame structure (complete Heyting algebra) -/
instance (L : LambdaTheory) (S : L.Obj) : Order.Frame (NativeTypeFiber L S) :=
  L.fibration.frame S

/-- For same-sort native types, morphisms are predicate inclusion -/
def NativeTypeLeq {L : LambdaTheory} (A B : NativeType L) (h : A.sort = B.sort) : Prop :=
  A.pred ≤ (h ▸ B.pred)

theorem NativeTypeLeq.refl {L : LambdaTheory} (A : NativeType L) : NativeTypeLeq A A rfl :=
  le_refl _
...
/-- Grothendieck preorder on native types: `A ≤ B` iff same sort and predicate
    ...
    preorder, giving `NativeType L` a `SmallCategory` instance via Mathlib. -/
instance nativeTypePreorder {L : LambdaTheory} : Preorder (NativeType L) where
...
- `NativeTypeTransport` packages a base sort-morphism layer with reindexing.
- `NativeTypeHom` packages Grothendieck-style morphisms between `(sort,predicate)` pairs.
- `equalityNativeTypeTransport` gives a concrete endpoint today (equality-indexed
...
structure NativeTypeTransport (L : LambdaTheory) where
...
structure NativeTypeHom {L : LambdaTheory}
    (T : NativeTypeTransport L) (A B : NativeType L) where
  sortMap : T.SortHom A.sort B.sort
  predLe : A.pred ≤ T.reindex sortMap B.pred

namespace NativeTypeHom

/-- Identity Grothendieck-style morphism. -/
def id {L : LambdaTheory} (T : NativeTypeTransport L) (A : NativeType L) :
    NativeTypeHom T A A where
  sortMap := T.id A.sort
  predLe := by
    exact (T.reindex_id (φ := A.pred)).symm ▸ (le_rfl : A.pred ≤ A.pred)

/-- Composition of Grothendieck-style morphisms. -/
def comp {L : LambdaTheory} (T : NativeTypeTransport L)
    {A B C : NativeType L}
    (f : NativeTypeHom T A B) (g : NativeTypeHom T B C) :
    NativeTypeHom T A C where
...
def equalityNativeTypeTransport (L : LambdaTheory) : NativeTypeTransport L where
...
composition is available for `NativeTypeHom` via the equality-indexed transport. -/
def equalityNativeTypeTransport_crossSort_comp
    (L : LambdaTheory)
    {A B C : NativeType L}
    (f : NativeTypeHom (equalityNativeTypeTransport L) A B)
    (g : NativeTypeHom (equalityNativeTypeTransport L) B C) :
    NativeTypeHom (equalityNativeTypeTransport L) A C :=
  NativeTypeHom.comp (T := equalityNativeTypeTransport L) f g

/-- Canonical identity endpoint for the concrete cross-sort transport layer. -/
def equalityNativeTypeTransport_endpoint
    (L : LambdaTheory)
    (A : NativeType L) :
    NativeTypeHom (equalityNativeTypeTransport L) A A :=
  NativeTypeHom.id (T := equalityNativeTypeTransport L) A
...
section ConstructorNativeTypeTransport
...
structure ConstructorNativeType (lang : LanguageDef) where
...
structure ConstructorNativeTypeHom (lang : LanguageDef)
    (A B : ConstructorNativeType lang) where
  sortMap : A.sort ⟶ B.sort
  predLe : A.pred ≤ constructorReindex lang sortMap B.pred

namespace ConstructorNativeTypeHom

/-- Identity cross-sort morphism in the constructor-category native endpoint. -/
def id (lang : LanguageDef) (A : ConstructorNativeType lang) :
    ConstructorNativeTypeHom lang A A where
...
def comp (lang : LanguageDef) {A B C : ConstructorNativeType lang}
    (f : ConstructorNativeTypeHom lang A B) (g : ConstructorNativeTypeHom lang B C) :
    ConstructorNativeTypeHom lang A C where
...
end ConstructorNativeTypeHom

/-- Concrete nontrivial endpoint: composition for constructor-path native morphisms. -/
def constructorNativeTypeTransport_crossSort_comp
    (lang : LanguageDef)
    {A B C : ConstructorNativeType lang}
    (f : ConstructorNativeTypeHom lang A B)
    (g : ConstructorNativeTypeHom lang B C) :
    ConstructorNativeTypeHom lang A C :=
  ConstructorNativeTypeHom.comp lang f g

/-- Canonical identity endpoint for constructor-category native morphisms. -/
def constructorNativeTypeTransport_endpoint
    (lang : LanguageDef)
    (A : ConstructorNativeType lang) :
    ConstructorNativeTypeHom lang A A :=
  ConstructorNativeTypeHom.id lang A

/-! ### rhoCalc canaries for the constructor-category endpoint -/

def rhoProcTopConstructorNativeType : ConstructorNativeType rhoCalc where
  sort := rhoProcObj
  pred := fun _ => True

def rhoNameTopConstructorNativeType : ConstructorNativeType rhoCalc where
  sort := rhoNameObj
  pred := fun _ => True

def rho_nquote_constructorNativeTypeHom :
    ConstructorNativeTypeHom rhoCalc
      rhoProcTopConstructorNativeType
      rhoNameTopConstructorNativeType where
  sortMap := nquoteMor
  predLe := by
    intro _ _
    trivial

def rho_pdrop_constructorNativeTypeHom :
    ConstructorNativeTypeHom rhoCalc
      rhoNameTopConstructorNativeType
      rhoProcTopConstructorNativeType where
  sortMap := pdropMor
  predLe := by
    intro _ _
    trivial
...
def rho_roundtrip_constructorNativeTypeHom :
    ConstructorNativeTypeHom rhoCalc
      rhoProcTopConstructorNativeType
      rhoProcTopConstructorNativeType :=
  constructorNativeTypeTransport_crossSort_comp rhoCalc
    rho_nquote_constructorNativeTypeHom
    rho_pdrop_constructorNativeTypeHom

end ConstructorNativeTypeTransport
...
def constructorNativeType_toGrothObj {lang : LanguageDef}
    (A : ConstructorNativeType lang) : ConstructorGrothendieckDual lang :=
...
def grothObj_to_constructorNativeType {lang : LanguageDef}
    (X : ConstructorGrothendieckDual lang) : ConstructorNativeType lang :=
...
theorem constructorNativeType_obj_roundtrip {lang : LanguageDef}
    (A : ConstructorNativeType lang) :
...
def constructorNativeTypeHom_to_grothHom {lang : LanguageDef}
    {A B : ConstructorNativeType lang} (h : ConstructorNativeTypeHom lang A B) :
    constructorNativeType_toGrothObj B ⟶ constructorNativeType_toGrothObj A :=
...
def grothHom_to_constructorNativeTypeHom {lang : LanguageDef}
    {A B : ConstructorNativeType lang}
    (k : constructorNativeType_toGrothObj B ⟶ constructorNativeType_toGrothObj A) :
    ConstructorNativeTypeHom lang A B :=
...
constructor transport -> Grothendieck morphism -> constructor transport. -/
  theorem constructorNativeTypeHom_groth_roundtrip {lang : LanguageDef}
    {A B : ConstructorNativeType lang} (h : ConstructorNativeTypeHom lang A B) :
    grothHom_to_constructorNativeTypeHom (constructorNativeTypeHom_to_grothHom h) = h := by
  cases h
  simp [constructorNativeTypeHom_to_grothHom, grothHom_to_constructorNativeTypeHom]
  rfl
...
def ScopedConstructorPred.toConstructorNativeType {lang : LanguageDef}
    (A : ScopedConstructorPred lang) : ConstructorNativeType lang where
  sort := ⟨A.sort⟩
  pred := A.pred
...
theorem scoped_full_constructor_obj_comparison {lang : LanguageDef}
    ...
    grothObj_to_constructorNativeType
      (constructorNativeType_toGrothObj A.toConstructorNativeType) =
      A.toConstructorNativeType := by
  exact constructorNativeType_obj_roundtrip A.toConstructorNativeType
...
def mapNativeType (A : NativeType L₁) : NativeType L₂ where
...
/-- Equivalent Ω-preservation phrasing at the `NativeType.full` level. -/
theorem preserves_fullNativeType_pred (S : L₁.Obj) :
    (F.mapNativeType (NativeType.full (L := L₁) S)).pred =
      (NativeType.full (L := L₂) (F.mapSort S)).pred := by
  simpa [mapNativeType, NativeType.full] using F.preserves_omegaTop S
...
theorem piOmega_translation_endpoint
    ...
    (F.mapNativeType (NativeType.full (L := L₁) S)).pred =
      (NativeType.full (L := L₂) (F.mapSort S)).pred := by
  exact ⟨F.preserves_piType S types, F.preserves_fullNativeType_pred S⟩
...
theorem piOmegaProp_translation_endpoint
    ...
    ∧
    (F.mapNativeType (NativeType.full (L := L₁) S)).pred =
      (NativeType.full (L := L₂) (F.mapSort S)).pred
    ∧
    F.mapPred (implType L₁ S φ ψ) =
      implType L₂ (F.mapSort S) (F.mapPred φ) (F.mapPred ψ) := by
  exact ⟨F.preserves_piType S types, F.preserves_fullNativeType_pred S, F.preserves_propImp S φ ψ⟩
...
theorem piSigmaOmegaProp_translation_endpoint
  ...
  exact ⟨F.preserves_piType S types, F.preserves_sigmaType S types,
    F.preserves_fullNativeType_pred S, F.preserves_propImp S φ ψ⟩
...
theorem piOmegaProp_with_constructor_transport_bundle
    ...
    (F.mapPred (implType L₁ S φ ψ) =
      implType L₂ (F.mapSort S) (F.mapPred φ) (F.mapPred ψ))
    ∧
    Nonempty (ConstructorNativeTypeHom lang A C) := by
  refine ⟨?_, ?_, ?_, ?_⟩
  · exact F.preserves_piType S types
  · exact F.preserves_fullNativeType_pred S
  · exact F.preserves_propImp S φ ψ
  · exact ⟨constructorNativeTypeTransport_crossSort_comp lang f g⟩
...
theorem comp_piOmegaProp_with_constructor_transport_bundle
    ...
    ∧
    (((TheoryMorphism.comp G F).mapNativeType (NativeType.full (L := L₁) S)).pred =
      (NativeType.full (L := L₃) ((TheoryMorphism.comp G F).mapSort S)).pred)
    ∧
...
theorem id_piOmega_translation_endpoint
    ...
    ∧
    ((id L).mapNativeType (NativeType.full (L := L) S)).pred =
      (NativeType.full (L := L) ((id L).mapSort S)).pred := by
...
theorem id_piOmegaProp_translation_endpoint
    ...
    ∧
    ((id L).mapNativeType (NativeType.full (L := L) S)).pred =
      (NativeType.full (L := L) ((id L).mapSort S)).pred
    ∧
...
Each `LambdaTheory L` gives a preorder category `NativeType L` (via `nativeTypePreorder`).
Each `TheoryMorphism F : L₁ ⟶ L₂` induces a monotone (hence functorial) map
`mapNativeType F : NativeType L₁ → NativeType L₂`.  Together these define a functor
...
/-- `mapNativeType` is monotone with respect to the Grothendieck preorder. -/
theorem TheoryMorphism.mapNativeType_monotone {L₁ L₂ : LambdaTheory}
    (F : TheoryMorphism L₁ L₂) :
    Monotone (F.mapNativeType) := by
  intro A B ⟨hs, hp⟩
...
def theoryMorphismNativeTypeFunctor {L₁ L₂ : LambdaTheory}
    (F : TheoryMorphism L₁ L₂) :
...
def nativeTypeFunctor :
  ...
  map F := (theoryMorphismNativeTypeFunctor F).toCatHom
  map_id L := by ext; rfl
...
end Mettapedia.OSLF.PresheafNativeType
