...
theorem predicateVocabularyIntensionalPairSubsetRel_of_sameIntent
  ...
  have hLeftMutual :=
    (Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLSimilarityBridge.predicateVocabularySameIntent_iff_mutualIntensional
...
theorem predicateVocabularyIntensionalInherits_of_intentMap
    ...
    (I : Mettapedia.KR.ConceptGeometry.AbstractInheritance.Interpretation Pattern Obj Attr)
    (M : HenkinModel.{u, v, w} Base Const)
    (σ : Ty Base)
    {Pred : Type}
    (code : Pattern → Pred)
...
theorem assocEvidence_eq_of_predicateVocabularySameIntent
  ...
  have hLeftSymm :
    ...
    unfold Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLSimilarityBridge.predicateVocabularySameIntent at hLeft ⊢
    exact hLeft.symm
  have hRightSymm :
      Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLSimilarityBridge.predicateVocabularySameIntent
        (Base := Base) (Const := Const) M σ decode d b := by
    unfold Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLSimilarityBridge.predicateVocabularySameIntent at hRight ⊢
    exact hRight.symm
  exact le_antisymm
