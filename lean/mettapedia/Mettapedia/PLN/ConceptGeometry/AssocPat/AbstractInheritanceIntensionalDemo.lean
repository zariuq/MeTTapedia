...
theorem positive_pair_subset_rel_transports_to_predicateVocabularyIntensionalPairSubsetRel
  ...
  exact
    Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.predicateVocabularyIntensionalPairSubsetRel_of_interpretationPairSubsetRel
        (I := baseInterpretation)
        (M := M) (σ := σ) (code := code) (decode := decode)
        hTransport (1 : DemoState)
        (by
          simpa [_root_.Mettapedia.KR.ConceptOntology.AbstractExtentPairSubsetRel, baseInterpretation] using
...
theorem positive_pair_subset_rel_transports_to_predicateVocabularyIntensionalPairSubsetRel_viaIntentMap
    ...
    Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.predicateVocabularyIntensionalPairSubsetRel
      ...
      (code Concept.penguin) (code Concept.bird) := by
  exact
    Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.predicateVocabularyIntensionalPairSubsetRel_of_interpretationPairSubsetRel_viaIntentMap
      (I := baseInterpretation)
      (M := M) (σ := σ) (code := code) (decode := decode)
...
theorem positive_pair_subset_rel_weightedPredicateVocabularyAssocPatEvidence_mono_viaIntentCalibration
  ...
  have hRel :
      Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.predicateVocabularyIntensionalPairSubsetRel
        ...
        (code Concept.bird) (code Concept.bird)
        (code Concept.penguin) (code Concept.bird) := by
    simpa [Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.predicateVocabularyIntensionalPairSubsetRel]
      using hRelDemo
  have hAssocSubset :
      InheritanceQueryBuilder.AssocSubsetSemantics
        (State := State) (Atom := Pred) (Query := PairQuery)
        ...
        (Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.predicateVocabularyIntensionalPairSubsetRel
          (Base := Base) (Const := Const) M σ decode) :=
    Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.assocSubsetSemantics_of_predicateVocabularyWeightedPairOrderRankScore
  ...
  have hPatSubset :
      InheritanceQueryBuilder.PATSubsetSemantics
        ...
        (Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.predicateVocabularyIntensionalPairSubsetRel
          (Base := Base) (Const := Const) M σ decode) :=
    Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.patSubsetSemantics_of_predicateVocabularyWeightedPairOrderRankScore
      (Base := Base) (Const := Const) M σ decode pairEnc model
      hPatLeftWeight hPatRightWeight hPatScore
  exact
    ⟨Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.assocEvidence_mono_of_predicateVocabularyIntensionalSubsetSemantics
        (Base := Base) (Const := Const) M σ decode pairEnc model hAssocSubset hRel,
      Mettapedia.PLN.ConceptGeometry.AssocPat.PLNHigherOrderHOLAssocPatBridge.patEvidence_mono_of_predicateVocabularyIntensionalSubsetSemantics
...
theorem tweety_mem_bird_finiteExtent :
    Creature.tweety ∈
      DualConcept.finiteExtent (baseInterpretation.meaning Concept.bird) := by
  simp only [DualConcept.mem_finiteExtent_iff, baseInterpretation,
    MembershipQueryBuilder.abstractInterpretationAt,
    ...
    Mettapedia.KR.ConceptGeometry.AbstractInheritance.DualConcept.ofConcept_extent,
    Mettapedia.KR.ConceptGeometry.AbstractInheritance.crispBaseConcept_extent,
    Mettapedia.KR.ConceptOntology.crispExtent,
    lowerPolar, Set.mem_setOf_eq, Set.mem_singleton_iff, forall_eq]
  change 0 <
    (natScaleEvidence 1
      (toyWM ⟨ToySort.membership,
        ToyQueryFamily.member Creature.tweety Concept.bird⟩)).pos
  simp [natScaleEvidence, toyWM, strongYes]

/-- The consequent extent channel used by the demo PAT score is genuinely
present for `bird`. -/
theorem extentCard_bird_pos : (0 : NNReal) < extentCard Concept.bird := by
  have hcard :
      0 < (DualConcept.finiteExtent
        (baseInterpretation.meaning Concept.bird)).card :=
    Finset.card_pos.mpr ⟨Creature.tweety, tweety_mem_bird_finiteExtent⟩
  dsimp [extentCard]
  exact_mod_cast hcard

/-- In the live toy ontology, PAT is a distinct structural channel from ASSOC:
the PAT score adds a second copy of the consequent extent, so any nonempty
consequent extent separates the two channels.

This is a guardrail against collapsing Chapter-12 PAT into the ASSOC/log-ratio
readout. -/
theorem assocBaseScore_birdBird_lt_patBaseScore_birdBird :
    assocBaseScore Concept.bird Concept.bird <
      patBaseScore Concept.bird Concept.bird := by
  simp [assocBaseScore, patBaseScore, add_assoc]
...
theorem assocEvidence_birdBird_ne_patEvidence_birdBird :
    InheritanceQueryBuilder.intensionalAssocEvidence
      ...
      InheritanceQueryBuilder.intensionalPATEvidence
        (State := DemoState) (Atom := Concept) (Query := DemoPairQuery)
        1 pairEnc Concept.bird Concept.bird := by
  change
    natScaleEvidence 1
        (scoreToEvidenceNNReal (assocBaseScore Concept.bird Concept.bird)) ≠
      natScaleEvidence 1
        (scoreToEvidenceNNReal (patBaseScore Concept.bird Concept.bird))
  intro h
  have hp := congrArg BinaryEvidence.pos h
  have hne :
      assocBaseScore Concept.bird Concept.bird ≠
        patBaseScore Concept.bird Concept.bird :=
...
/-- `pingu` is a concrete positive extent witness for `penguin`. -/
theorem pingu_mem_penguin_finiteExtent :
    Creature.pingu ∈
      DualConcept.finiteExtent (baseInterpretation.meaning Concept.penguin) := by
  simp only [DualConcept.mem_finiteExtent_iff, baseInterpretation,
    MembershipQueryBuilder.abstractInterpretationAt,
    Mettapedia.KR.ConceptGeometry.AbstractInheritance.crispInterpretation,
    Mettapedia.KR.ConceptGeometry.AbstractInheritance.ofCrispBaseConcept,
  ...
  change 0 <
    (natScaleEvidence 1
      (toyWM ⟨ToySort.membership,
        ToyQueryFamily.member Creature.pingu Concept.penguin⟩)).pos
  simp [natScaleEvidence, toyWM, strongYes]

/-- `pingu` is also a concrete positive extent witness for `bird`. -/
theorem pingu_mem_bird_finiteExtent :
    Creature.pingu ∈
      DualConcept.finiteExtent (baseInterpretation.meaning Concept.bird) := by
  simp only [DualConcept.mem_finiteExtent_iff, baseInterpretation,
    MembershipQueryBuilder.abstractInterpretationAt,
  ...
  change 0 <
    (natScaleEvidence 1
      (toyWM ⟨ToySort.membership,
        ToyQueryFamily.member Creature.pingu Concept.bird⟩)).pos
  simp [natScaleEvidence, toyWM, strongYes]
...
/-- The `penguin -> bird` stamped evidence carries `pingu` as a positive
extensional provenance stamp. -/
theorem posExtPingu_mem_penguinBirdStampedEvidence :
    DualConcept.WitnessStamp.posExt Creature.pingu ∈
      penguinBirdStampedEvidence.stamp := by
  rw [penguinBirdStampedEvidence,
    Interpretation.finiteInheritanceStampedEvidence_stamp]
  have hPosExt : Creature.pingu ∈
    ...
    exact Finset.mem_inter.mpr
      ⟨pingu_mem_penguin_finiteExtent, pingu_mem_bird_finiteExtent⟩
  have hMap :
      DualConcept.WitnessStamp.posExt (Attr := Concept) Creature.pingu ∈
        (DualConcept.finitePositiveExtensionalWitnesses
          (baseInterpretation.meaning Concept.penguin)
...
/-- The `bird -> bird` stamped evidence carries the same `pingu` positive
extensional provenance stamp. -/
theorem posExtPingu_mem_birdBirdStampedEvidence :
    DualConcept.WitnessStamp.posExt Creature.pingu ∈
      birdBirdStampedEvidence.stamp := by
  ...
  have hPosExt : Creature.pingu ∈
      DualConcept.finitePositiveExtensionalWitnesses
        ...
        (baseInterpretation.meaning Concept.bird) := by
    exact Finset.mem_inter.mpr
      ⟨pingu_mem_bird_finiteExtent, pingu_mem_bird_finiteExtent⟩
  have hMap :
      DualConcept.WitnessStamp.posExt (Attr := Concept) Creature.pingu ∈
        (DualConcept.finitePositiveExtensionalWitnesses
          (baseInterpretation.meaning Concept.bird)
...
/-- The same provenance-overlap guard can be witnessed extensionally: both
premise packets mention `pingu` as positive extensional support. -/
theorem premiseStampedEvidence_overlap_on_pingu_extension :
    ¬ StampedBinaryEvidence.StampDisjoint
      penguinBirdStampedEvidence birdBirdStampedEvidence := by
  exact StampedBinaryEvidence.not_stampDisjoint_of_mem
    posExtPingu_mem_penguinBirdStampedEvidence
    posExtPingu_mem_birdBirdStampedEvidence
...
theorem premiseStampedEvidence_guardedRevise_eq_none :
  ...
  exact StampedBinaryEvidence.guardedRevise_eq_none_of_shared_stamp
    posIntBird_mem_penguinBirdStampedEvidence
    posIntBird_mem_birdBirdStampedEvidence

/-- Guarded revision also refuses the premise merge when the overlap is exposed
through the positive extensional witness channel. -/
theorem premiseStampedEvidence_guardedRevise_eq_none_of_extensional_overlap :
    StampedBinaryEvidence.guardedRevise
      penguinBirdStampedEvidence birdBirdStampedEvidence = none := by
  exact StampedBinaryEvidence.guardedRevise_eq_none_of_shared_stamp
    posExtPingu_mem_penguinBirdStampedEvidence
...
the same premise merge shares a positive intensional `bird` stamp and a
positive extensional `pingu` stamp, either of which is enough to reject
double-counting. -/
theorem assoc_pat_dual_channel_provenance_canary :
    (DualConcept.WitnessStamp.posInt Concept.bird ∈
        penguinBirdStampedEvidence.stamp ∧
      DualConcept.WitnessStamp.posInt Concept.bird ∈
        birdBirdStampedEvidence.stamp)
    ∧
    (DualConcept.WitnessStamp.posExt Creature.pingu ∈
        penguinBirdStampedEvidence.stamp ∧
      DualConcept.WitnessStamp.posExt Creature.pingu ∈
...
theorem assoc_pat_guardedListRevise_eq_none_of_dual_channel_overlap :
    StampedBinaryEvidence.guardedListRevise
      [penguinBirdStampedEvidence, birdBirdStampedEvidence] = none := by
  rw [StampedBinaryEvidence.guardedListRevise_eq_none_iff_not_pairwise]
  intro hPair
  have hDisjoint :
      StampedBinaryEvidence.StampDisjoint
        penguinBirdStampedEvidence birdBirdStampedEvidence := by
    exact (List.pairwise_cons.mp hPair).1
      birdBirdStampedEvidence (by simp)
  exact premiseStampedEvidence_overlap_on_pingu_extension hDisjoint

theorem penguinBirdStampedEvidence_exact :
    DualConcept.ExactStampPacket penguinBirdStampedEvidence := by
  change DualConcept.ExactStampPacket
    (DualConcept.finiteInheritanceStampedEvidence
      (baseInterpretation.meaning Concept.penguin)
      (baseInterpretation.meaning Concept.bird))
  exact DualConcept.finiteInheritanceStampedEvidence_exact _ _

theorem birdBirdStampedEvidence_exact :
    DualConcept.ExactStampPacket birdBirdStampedEvidence := by
  change DualConcept.ExactStampPacket
    (DualConcept.finiteInheritanceStampedEvidence
      (baseInterpretation.meaning Concept.bird)
      (baseInterpretation.meaning Concept.bird))
...
theorem assoc_pat_exactPacket_consumer_canary :
    DualConcept.ExactStampPacket penguinBirdStampedEvidence ∧
      DualConcept.ExactStampPacket birdBirdStampedEvidence ∧
      StampedBinaryEvidence.guardedListRevise
        [penguinBirdStampedEvidence, birdBirdStampedEvidence] = none := by
  exact ⟨penguinBirdStampedEvidence_exact, birdBirdStampedEvidence_exact,
    assoc_pat_guardedListRevise_eq_none_of_dual_channel_overlap⟩

theorem assoc_pat_packetJointMerge_eq_source_union :
    DualConcept.packetJointMerge [penguinBirdStampedEvidence, birdBirdStampedEvidence] =
      DualConcept.stampSetPacket
        (DualConcept.packetListUnion
          [penguinBirdStampedEvidence, birdBirdStampedEvidence]) := by
  apply DualConcept.packetJointMerge_eq_stampSetPacket_packetListUnion_of_exact
  intro x hx
  simp at hx
  rcases hx with rfl | rfl
  · exact penguinBirdStampedEvidence_exact
  · exact birdBirdStampedEvidence_exact

theorem penguinBird_packetJointMerge_duplicate_eq_single :
    DualConcept.packetJointMerge
        [penguinBirdStampedEvidence, penguinBirdStampedEvidence] =
      DualConcept.stampSetPacket penguinBirdStampedEvidence.stamp := by
  exact DualConcept.packetJointMerge_duplicate_eq_single_of_exact
    penguinBirdStampedEvidence penguinBirdStampedEvidence_exact

theorem assoc_pat_packetJointMerge_duplicate_absorb :
    DualConcept.packetJointMerge
        [penguinBirdStampedEvidence, penguinBirdStampedEvidence,
          birdBirdStampedEvidence] =
      DualConcept.packetJointMerge
        [penguinBirdStampedEvidence, birdBirdStampedEvidence] := by
  apply DualConcept.packetJointMerge_cons_duplicate_absorb_of_exact
  intro x hx
  simp at hx
  rcases hx with rfl | rfl
  · exact penguinBirdStampedEvidence_exact
...
end Mettapedia.PLN.ConceptGeometry.AssocPat.AbstractInheritanceIntensionalDemo
