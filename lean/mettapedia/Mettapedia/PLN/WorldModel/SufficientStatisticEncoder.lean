...
structure SufficientStatisticEncoder (Obs Query Ev : Type*) where
  observe : Obs → Query → Ev

namespace SufficientStatisticEncoder

variable {Obs Query Ev : Type*}

/-- Lift a query-independent observation statistic into a query-indexed encoder. -/
def ofObservationMap (f : Obs → Ev) : SufficientStatisticEncoder Obs Query Ev where
  observe o _ := f o

section Additive

variable [AddCommMonoid Ev] (S : SufficientStatisticEncoder Obs Query Ev)
...
/-- The aggregation induced by a sufficient-statistic encoder is the canonical
...
/-- The aggregation induced by a sufficient-statistic encoder is uniquely
...
/-- The canonical additive extension of the encoder exists uniquely. -/
...
/-- The sufficient-statistic encoder induces a generic world model over multisets
of observations. -/
...
the sufficient-statistic encoder. This states the universal property directly
...
/-- Extract the singleton observation encoder from a generic multiset world
model. This is the atomic observation encoder that the classification theorem
recovers. -/
def singletonEncoder (G : MultisetGenericWorldModel Obs Query Ev) :
    SufficientStatisticEncoder Obs Query Ev where
  observe o q :=
    letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
...
@[simp] theorem singletonEncoder_observe_eq_evidence_singleton
    (G : MultisetGenericWorldModel Obs Query Ev) (o : Obs) (q : Query) :
    letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
    letI : AdditiveWorldModel (Multiset Obs) Query Ev := G
    (singletonEncoder (Obs := Obs) (Query := Query) (Ev := Ev) G).observe o q =
      AdditiveWorldModel.extract
...
its singleton observation encoder. This is the paper-facing classification
...
theorem evidence_isAdditiveExtension_of_zero
    ...
    GenIsAdditiveExtension
      (singletonEncoder (Obs := Obs) (Query := Query) (Ev := Ev) G).observe
      (fun σ q =>
        letI : AdditiveWorldModel (Multiset Obs) Query Ev := G
        AdditiveWorldModel.extract
...
model is recovered pointwise by aggregating its singleton observation encoder. -/
@[simp] theorem evidence_eq_aggregate_singletonEncoder_of_zero
    (G : MultisetGenericWorldModel Obs Query Ev)
    (hzero :
      ∀ q,
        letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
    ...
    AdditiveWorldModel.extract
      ...
      aggregate
        (singletonEncoder (Obs := Obs) (Query := Query) (Ev := Ev) G) σ q := by
  letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
  letI : AdditiveWorldModel (Multiset Obs) Query Ev := G
  have hEq :=
    aggregate_eq_of_isAdditiveExtension
      (S := singletonEncoder (Obs := Obs) (Query := Query) (Ev := Ev) G)
      (E := fun σ q =>
        AdditiveWorldModel.extract
          (State := Multiset Obs) (Query := Query) (Ev := Ev) σ q)
...
/-- The induced world model built from the singleton encoder of a zero-preserving
generic multiset world model recovers the original evidence extractor pointwise. -/
@[simp] theorem inducedWorldModel_evidence_eq_of_singletonEncoder_zero
    (G : MultisetGenericWorldModel Obs Query Ev)
    (hzero :
      ∀ q,
        letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
    ...
    letI : AdditiveWorldModel (Multiset Obs) Query Ev :=
      (singletonEncoder (Obs := Obs) (Query := Query) (Ev := Ev) G).inducedWorldModel
    AdditiveWorldModel.extract
        (State := Multiset Obs) (Query := Query) (Ev := Ev) σ q =
      letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
      letI : AdditiveWorldModel (Multiset Obs) Query Ev := G
  ...
  exact
    (evidence_eq_aggregate_singletonEncoder_of_zero
...
encoder. -/
theorem existsUnique_additiveExtension_of_singletonEncoder_zero
    (G : MultisetGenericWorldModel Obs Query Ev)
    (hzero :
      ∀ q,
        letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
    ...
    ∃! E : Multiset Obs → Query → Ev,
      GenIsAdditiveExtension
        (singletonEncoder (Obs := Obs) (Query := Query) (Ev := Ev) G).observe E := by
  letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
  refine ⟨
    (fun σ q =>
      letI : AdditiveWorldModel (Multiset Obs) Query Ev := G
  ...
  rw [aggregate_eq_of_isAdditiveExtension
    (S := singletonEncoder (Obs := Obs) (Query := Query) (Ev := Ev) G) hE]
  exact
    (evidence_eq_aggregate_singletonEncoder_of_zero
      (Obs := Obs) (Query := Query) (Ev := Ev) G hzero σ q).symm
...
variable (S : SufficientStatisticEncoder Obs Query BinaryEvidence)
...
/-- A multiset-level conjugate-posterior model over an additive sufficient
...
structure ConjugatePosteriorModel (Obs Query Ev Prior : Type*) [AddCommMonoid Ev] where
  stat : SufficientStatisticEncoder Obs Query Ev
...
namespace ConjugatePosteriorModel

variable {Prior : Type*}
variable (P : ConjugatePosteriorModel Obs Query Ev Prior)
...
theorem posterior_eq_of_isAdditiveExtension
    ...
    (hlift :
      ∀ prior σ q,
        P.posterior prior σ q = lift prior q (SufficientStatisticEncoder.aggregate P.stat σ q))
    {E : Multiset Obs → Query → Ev}
    (hE : GenIsAdditiveExtension P.stat.observe E)
    (prior : Prior) (σ : Multiset Obs) (q : Query) :
    P.posterior prior σ q = lift prior q (E σ q) := by
  rw [hlift]
  have hEq : E = SufficientStatisticEncoder.aggregate P.stat :=
    SufficientStatisticEncoder.aggregate_eq_of_isAdditiveExtension (S := P.stat) hE
  simp [hEq]

/-- The posterior also factors through the evidence extracted by the induced
generic world model of the sufficient-statistic encoder. -/
theorem posterior_eq_of_inducedWorldModelEvidence
  ...
  letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
  letI : AdditiveWorldModel (Multiset Obs) Query Ev := P.stat.inducedWorldModel
  simpa [SufficientStatisticEncoder.inducedWorldModel_evidence_eq_aggregate (S := P.stat)] using
    hlift prior σ q
...
theorem posterior_ne_of_aggregate_ne
    ...
    (hneq :
      SufficientStatisticEncoder.aggregate P.stat σ₁ q ≠
        SufficientStatisticEncoder.aggregate P.stat σ₂ q) :
...
theorem posterior_ne_of_isAdditiveExtension_ne
    ...
    (hneq : E σ₁ q ≠ E σ₂ q) :
    P.posterior prior σ₁ q ≠ P.posterior prior σ₂ q := by
  have hEqE := SufficientStatisticEncoder.aggregate_eq_of_isAdditiveExtension (S := P.stat) hE
  exact
    P.posterior_ne_of_aggregate_ne
      ...
      (by
        intro hAgg
        apply hneq
        have hσ₁ : E σ₁ q = SufficientStatisticEncoder.aggregate P.stat σ₁ q :=
          congrFun (congrFun hEqE σ₁) q
        have hσ₂ : E σ₂ q = SufficientStatisticEncoder.aggregate P.stat σ₂ q :=
          congrFun (congrFun hEqE σ₂) q
        calc
          E σ₁ q = SufficientStatisticEncoder.aggregate P.stat σ₁ q := hσ₁
          _ = SufficientStatisticEncoder.aggregate P.stat σ₂ q := hAgg
          _ = E σ₂ q := hσ₂.symm)
...
end ConjugatePosteriorModel

end Posterior

section Conjugate

variable [ConjugateEvidence Ev] (S : SufficientStatisticEncoder Obs Query Ev)
...
/-- The generic world model induced by a unit-observation encoder has observation
...
/-- The generic world model induced by a unit-observation encoder has confidence
...
through the sufficient-statistics encoder. -/
...
/-- For any inhabited unit-observation encoder, the induced generic world model
...
A `SufficientStatisticEncoder Obs Query Ev` encodes raw observations into an
...
5. **Posterior factoring** (via `ConjugatePosteriorModel`):
...
variable {Obs Query Ev : Type*} [ConjugateEvidence Ev]
    (S : SufficientStatisticEncoder Obs Query Ev)
...
/-! ## Canonical Conjugate-Family Encoders -/
...
/-- Query-indexed Bernoulli/Beta sufficient-statistic encoder. -/
def bernoulliStatistic (classify : Obs → Query → Bool) :
    SufficientStatisticEncoder Obs Query BinaryEvidence where
...
theorem bernoulliStatistic_beta_conjugate_update
    ...
    let n₂_pos := σ₂.countP (fun o => classify o q = true)
    let n₂_neg := σ₂.countP (fun o => classify o q = false)
    let params₁ : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams :=
    ...
    Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams.alpha paramsCombined =
      Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams.alpha params₁ + n₂_pos ∧
    Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams.beta paramsCombined =
      Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams.beta params₁ + n₂_neg := by
  simpa using
...
/-- Bernoulli/Beta posterior model over batches of Boolean-classified
observations. -/
def bernoulliConjugatePosteriorModel
    (classify : Obs → Query → Bool) :
    ConjugatePosteriorModel Obs Query BinaryEvidence Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams where
...
theorem bernoulliConjugatePosteriorModel_eq_bernoulliPosteriorFromCounts
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams)
    (σ : Multiset Obs) (q : Query) :
    (bernoulliConjugatePosteriorModel classify).posterior params σ q =
...
theorem bernoulliConjugatePosteriorModel_eq_of_countExtensions
    (classify : Obs → Query → Bool)
  ...
  rw [bernoulliConjugatePosteriorModel_eq_bernoulliPosteriorFromCounts]
  have hpos :
      Epos σ q =
        σ.countP (fun o => classify o q = true) := by
    calc
...
theorem bernoulliStatistic_inducedWorldModelEvidence_pos_neg
  ...
  letI : AdditiveWorldModel (Multiset Obs) Query BinaryEvidence :=
    (bernoulliStatistic classify).inducedWorldModel
  constructor
  · rw [SufficientStatisticEncoder.inducedWorldModel_evidence_eq_aggregate
      (S := bernoulliStatistic classify)]
...
theorem bernoulliConjugatePosteriorModel_exists_counts_of_inducedWorldModelEvidence
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams)
    (σ : Multiset Obs) (q : Query) :
    letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
  ...
  · exact bernoulliConjugatePosteriorModel_eq_bernoulliPosteriorFromCounts
      classify params σ q
...
theorem bernoulliConjugatePosteriorModel_eq_of_inducedWorldModelEvidence
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams)
    (σ : Multiset Obs) (q : Query) :
    letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
  ...
  letI : AdditiveWorldModel (Multiset Obs) Query BinaryEvidence :=
    (bernoulliStatistic classify).inducedWorldModel
  refine
    ConjugatePosteriorModel.posterior_eq_of_inducedWorldModelEvidence
      (P := bernoulliConjugatePosteriorModel classify)
  ...
  have hpos :
      (SufficientStatisticEncoder.aggregate (bernoulliStatistic classify) τ r).pos =
        (τ.countP (fun o => classify o r = true) : ℝ≥0∞) := by
    simpa using bernoulliStatistic_aggregate_pos classify τ r
  have hneg :
      (SufficientStatisticEncoder.aggregate (bernoulliStatistic classify) τ r).neg =
  ...
  have hcounts :
      bernoulliCountsOfEvidence
          (SufficientStatisticEncoder.aggregate (bernoulliStatistic classify) τ r) =
        (τ.countP (fun o => classify o r = true),
          τ.countP (fun o => classify o r = false)) :=
    bernoulliCountsOfEvidence_eq
      (SufficientStatisticEncoder.aggregate (bernoulliStatistic classify) τ r) hpos hneg
  change
...
theorem bernoulliConjugatePosteriorModel_ne_of_inducedWorldModelEvidence_ne
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams)
    {σ₁ σ₂ : Multiset Obs} (q : Query)
    (hneq :
      ...
      AdditiveWorldModel.extract (State := Multiset Obs) (Query := Query) (Ev := BinaryEvidence) σ₁ q ≠
        AdditiveWorldModel.extract (State := Multiset Obs) (Query := Query) (Ev := BinaryEvidence) σ₂ q) :
    (bernoulliConjugatePosteriorModel classify).posterior params σ₁ q ≠
      (bernoulliConjugatePosteriorModel classify).posterior params σ₂ q := by
  letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
  letI : AdditiveWorldModel (Multiset Obs) Query BinaryEvidence :=
    (bernoulliStatistic classify).inducedWorldModel
  intro hEq
  ...
  have hEqLift :
    ...
    rw [← bernoulliConjugatePosteriorModel_eq_of_inducedWorldModelEvidence
        (classify := classify) (params := params) (σ := σ₁) (q := q),
      ← bernoulliConjugatePosteriorModel_eq_of_inducedWorldModelEvidence
        (classify := classify) (params := params) (σ := σ₂) (q := q)]
    simpa [e₁, e₂] using hEq
  have hpos₁ :
...
theorem bernoulliConjugatePosteriorModel_alpha_beta
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams)
    (σ : Multiset Obs) (q : Query) :
    let params' := (bernoulliConjugatePosteriorModel classify).posterior params σ q
    ...
    Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams.beta params' =
      Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams.beta params +
        σ.countP (fun o => classify o q = false) := by
  cases params
  simp [bernoulliConjugatePosteriorModel,
...
theorem bernoulliConjugatePosteriorModel_alpha_beta_via_evidenceBeta
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams)
    (σ : Multiset Obs) (q : Query) :
    let params' := (bernoulliConjugatePosteriorModel classify).posterior params σ q
  ...
  | mk prior_param prior_pos evidence_pos evidence_neg =>
      dsimp [bernoulliConjugatePosteriorModel, bernoulliPosteriorFromCounts,
        Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams.alpha,
...
theorem bernoulliConjugatePosteriorModel_totalPseudoCount
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams)
    (σ : Multiset Obs) (q : Query) :
    let params' := (bernoulliConjugatePosteriorModel classify).posterior params σ q
    ...
    Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams.alpha params +
        ...
        (σ.card : ℝ) := by
  dsimp
  rcases bernoulliConjugatePosteriorModel_alpha_beta classify params σ q with ⟨hα, hβ⟩
  rw [hα, hβ]
  have hcount :
...
theorem bernoulliConjugatePosteriorModel_ne_of_nonempty
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams)
    {σ : Multiset Obs} (hσ : σ ≠ 0) (q : Query) :
    (bernoulliConjugatePosteriorModel classify).posterior params σ q ≠ params := by
  intro hEq
  have hsum :=
    bernoulliConjugatePosteriorModel_totalPseudoCount
      (classify := classify) (params := params) (σ := σ) (q := q)
  rw [hEq] at hsum
...
theorem bernoulliConjugatePosteriorModel_double_ne_single_of_nonempty
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams)
    {σ : Multiset Obs} (hσ : σ ≠ 0) (q : Query) :
    (bernoulliConjugatePosteriorModel classify).posterior params (σ + σ) q ≠
      (bernoulliConjugatePosteriorModel classify).posterior params σ q := by
  exact
    ConjugatePosteriorModel.posterior_double_ne_single_of_nonempty
      (P := bernoulliConjugatePosteriorModel classify)
      (hneq := fun prior {τ} hτ =>
        bernoulliConjugatePosteriorModel_ne_of_nonempty
          (classify := classify) (params := prior) hτ q)
      params hσ

theorem bernoulliConjugatePosteriorModel_not_add_idempotent
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams)
    (o : Obs) (q : Query) :
    ¬ ∀ σ : Multiset Obs,
        (bernoulliConjugatePosteriorModel classify).posterior params (σ + σ) q =
          (bernoulliConjugatePosteriorModel classify).posterior params σ q := by
  exact
    ConjugatePosteriorModel.not_posterior_add_idempotent_of_observation
      (P := bernoulliConjugatePosteriorModel classify)
      (hneq := fun prior {τ} hτ =>
        bernoulliConjugatePosteriorModel_ne_of_nonempty
          (classify := classify) (params := prior) hτ q)
...
theorem bernoulliConjugatePosteriorModel_not_add_idempotent_global
    [Nonempty Obs] [Nonempty Query]
    (classify : Obs → Query → Bool)
    (params : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.EvidenceBetaParams) :
    ¬ ∀ q σ,
        (bernoulliConjugatePosteriorModel classify).posterior params (σ + σ) q =
          (bernoulliConjugatePosteriorModel classify).posterior params σ q := by
  exact
    ConjugatePosteriorModel.not_posterior_add_idempotent
      (P := bernoulliConjugatePosteriorModel classify)
      (prior := params)
...
/-- Query-indexed categorical/Dirichlet sufficient-statistic encoder. -/
def categoricalStatistic {k : ℕ} (classify : Obs → Query → Fin k) :
    SufficientStatisticEncoder Obs Query (MultiEvidence k) where
...
/-- Categorical/Dirichlet posterior model over batches of classified
observations. -/
noncomputable def categoricalConjugatePosteriorModel {k : ℕ}
    (classify : Obs → Query → Fin k) :
    ConjugatePosteriorModel Obs Query (MultiEvidence k) (EvidenceDirichletParams k) where
...
theorem categoricalConjugatePosteriorModel_eq_categoricalPosteriorFromAggregate {k : ℕ}
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k)
    (σ : Multiset Obs) (q : Query) :
    (categoricalConjugatePosteriorModel classify).posterior params σ q =
      categoricalPosteriorFromAggregate params
        (aggregate (categoricalStatistic classify) σ q) := by
  rfl

theorem categoricalConjugatePosteriorModel_eq_of_isAdditiveExtension {k : ℕ}
    (classify : Obs → Query → Fin k)
  ...
  exact
    ConjugatePosteriorModel.posterior_eq_of_isAdditiveExtension
      (P := categoricalConjugatePosteriorModel classify)
      (lift := fun prior _ e => categoricalPosteriorFromAggregate prior e)
      (hlift := fun prior τ r =>
        categoricalConjugatePosteriorModel_eq_categoricalPosteriorFromAggregate
          (classify := classify) (params := prior) (σ := τ) (q := r))
      hE params σ q

theorem categoricalConjugatePosteriorModel_eq_of_inducedWorldModelEvidence {k : ℕ}
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k)
    (σ : Multiset Obs) (q : Query) :
    letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
...
theorem categoricalConjugatePosteriorModel_ne_of_inducedWorldModelEvidence_ne {k : ℕ}
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k)
    {σ₁ σ₂ : Multiset Obs} (q : Query)
    (hneq :
      ...
      AdditiveWorldModel.extract
        ...
        AdditiveWorldModel.extract
          (State := Multiset Obs) (Query := Query) (Ev := MultiEvidence k) σ₂ q) :
    (categoricalConjugatePosteriorModel classify).posterior params σ₁ q ≠
      (categoricalConjugatePosteriorModel classify).posterior params σ₂ q := by
  letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
  letI : AdditiveWorldModel (Multiset Obs) Query (MultiEvidence k) :=
    (categoricalStatistic classify).inducedWorldModel
  exact
    ConjugatePosteriorModel.posterior_ne_of_inducedWorldModelEvidence_ne
      (P := categoricalConjugatePosteriorModel classify)
      (lift := fun prior _ e => categoricalPosteriorFromAggregate prior e)
      (hlift := fun prior τ r =>
        categoricalConjugatePosteriorModel_eq_categoricalPosteriorFromAggregate
...
theorem categoricalConjugatePosteriorModel_update {k : ℕ}
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k)
    (σ : Multiset Obs) (q : Query) (i : Fin k) :
    let params' := (categoricalConjugatePosteriorModel classify).posterior params σ q
...
theorem categoricalConjugatePosteriorModel_update_via_evidenceDirichlet {k : ℕ}
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k)
    (σ : Multiset Obs) (q : Query) (i : Fin k) :
    let params' := (categoricalConjugatePosteriorModel classify).posterior params σ q
  ...
  cases params with
  | mk prior evidence =>
      simpa [categoricalConjugatePosteriorModel,
...
theorem categoricalConjugatePosteriorModel_totalConcentration {k : ℕ}
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k)
    (σ : Multiset Obs) (q : Query) :
    ((categoricalConjugatePosteriorModel classify).posterior params σ q).toPosterior.totalConcentration =
      params.toPosterior.totalConcentration + (σ.card : ℝ) := by
  rw [categoricalConjugatePosteriorModel_eq_categoricalPosteriorFromAggregate]
  ...
  have hcard :
      ((genAdditiveExtension (categoricalStatistic classify).observe σ q).total : ℝ) =
        σ.card := by
    simpa [SufficientStatisticEncoder.aggregate] using
      congrArg (fun n : ℕ => (n : ℝ))
        (categoricalStatistic_aggregate_total classify σ q)
  rw [hcard]
  ring

theorem categoricalConjugatePosteriorModel_toPosterior_ne_of_nonempty {k : ℕ}
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k)
    {σ : Multiset Obs} (hσ : σ ≠ 0) (q : Query) :
    ((categoricalConjugatePosteriorModel classify).posterior params σ q).toPosterior ≠
      params.toPosterior := by
  intro hEq
  have hconc := congrArg DirichletParams.totalConcentration hEq
  rw [categoricalConjugatePosteriorModel_totalConcentration (classify := classify)
...
theorem categoricalConjugatePosteriorModel_ne_of_nonempty {k : ℕ}
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k)
    {σ : Multiset Obs} (hσ : σ ≠ 0) (q : Query) :
    (categoricalConjugatePosteriorModel classify).posterior params σ q ≠ params := by
  intro hEq
  apply categoricalConjugatePosteriorModel_toPosterior_ne_of_nonempty
    (classify := classify) (params := params) hσ q
  exact congrArg EvidenceDirichletParams.toPosterior hEq

theorem categoricalConjugatePosteriorModel_double_ne_single_of_nonempty {k : ℕ}
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k)
    {σ : Multiset Obs} (hσ : σ ≠ 0) (q : Query) :
    (categoricalConjugatePosteriorModel classify).posterior params (σ + σ) q ≠
      (categoricalConjugatePosteriorModel classify).posterior params σ q := by
  exact
    ConjugatePosteriorModel.posterior_double_ne_single_of_nonempty
      (P := categoricalConjugatePosteriorModel classify)
      (hneq := fun prior {τ} hτ =>
        categoricalConjugatePosteriorModel_ne_of_nonempty
          (classify := classify) (params := prior) hτ q)
      params hσ

theorem categoricalConjugatePosteriorModel_not_add_idempotent {k : ℕ}
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k)
    (o : Obs) (q : Query) :
    ¬ ∀ σ : Multiset Obs,
...
theorem categoricalConjugatePosteriorModel_not_add_idempotent_global {k : ℕ}
    [Nonempty Obs] [Nonempty Query]
    (classify : Obs → Query → Fin k)
    (params : EvidenceDirichletParams k) :
    ¬ ∀ q σ,
...
/-- Query-indexed Gaussian/Normal-Gamma sufficient-statistic encoder. -/
def gaussianStatistic (value : Obs → Query → ℝ) :
    SufficientStatisticEncoder Obs Query NormalGammaEvidence where
...
/-- Gaussian/Normal-Gamma posterior model over batches of numeric
observations. -/
noncomputable def gaussianConjugatePosteriorModel
    (value : Obs → Query → ℝ) :
    ConjugatePosteriorModel Obs Query NormalGammaEvidence NormalGammaPrior where
...
theorem gaussianConjugatePosteriorModel_eq_gaussianPosteriorFromAggregate
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior)
    (σ : Multiset Obs) (q : Query) :
    (gaussianConjugatePosteriorModel value).posterior prior σ q =
      gaussianPosteriorFromAggregate prior
        (aggregate (gaussianStatistic value) σ q) := by
  rfl

theorem gaussianConjugatePosteriorModel_eq_of_isAdditiveExtension
    (value : Obs → Query → ℝ)
  ...
  exact
    ConjugatePosteriorModel.posterior_eq_of_isAdditiveExtension
      (P := gaussianConjugatePosteriorModel value)
      (lift := fun prior' _ e => gaussianPosteriorFromAggregate prior' e)
      (hlift := fun prior' τ r =>
        gaussianConjugatePosteriorModel_eq_gaussianPosteriorFromAggregate
          (value := value) (prior := prior') (σ := τ) (q := r))
      hE prior σ q

theorem gaussianConjugatePosteriorModel_eq_of_inducedWorldModelEvidence
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior)
    (σ : Multiset Obs) (q : Query) :
    letI : EvidenceType (Multiset Obs) := multisetEvidenceType Obs
...
theorem gaussianConjugatePosteriorModel_ne_of_inducedWorldModelEvidence_ne
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior)
    {σ₁ σ₂ : Multiset Obs} (q : Query)
    (hneq :
      ...
      AdditiveWorldModel.extract
        ...
        AdditiveWorldModel.extract
          (State := Multiset Obs) (Query := Query) (Ev := NormalGammaEvidence) σ₂ q) :
    (gaussianConjugatePosteriorModel value).posterior prior σ₁ q ≠
      (gaussianConjugatePosteriorModel value).posterior prior σ₂ q := by
  let e₁ := aggregate (gaussianStatistic value) σ₁ q
  let e₂ := aggregate (gaussianStatistic value) σ₂ q
  have hneqAgg : e₁ ≠ e₂ := by
    intro hEqAgg
    apply hneq
    simpa [e₁, e₂,
      SufficientStatisticEncoder.inducedWorldModel_evidence_eq_aggregate
  ...
  exact
    gaussianPosteriorFromAggregate_eq_imp_eq_of_realizable
      ...
      (hreal₂ := by
        simpa [e₂] using gaussianStatistic_aggregate_realizable value σ₂ q)
      (by
        rw [← gaussianConjugatePosteriorModel_eq_gaussianPosteriorFromAggregate
            (value := value) (prior := prior) (σ := σ₁) (q := q),
          ← gaussianConjugatePosteriorModel_eq_gaussianPosteriorFromAggregate
            (value := value) (prior := prior) (σ := σ₂) (q := q)]
        exact hEq)

theorem gaussianConjugatePosteriorModel_hplus
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior)
    (σ₁ σ₂ : Multiset Obs) (q : Query) :
    let P := gaussianConjugatePosteriorModel value
    P.posterior prior (σ₁ + σ₂) q =
      P.posterior (P.posterior prior σ₁ q) σ₂ q := by
  simpa using
    (gaussianConjugatePosteriorModel value).posterior_add prior σ₁ σ₂ q

theorem gaussianConjugatePosteriorModel_hplus_via_evidenceNormalGamma
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior)
    (σ₁ σ₂ : Multiset Obs) (q : Query) :
    let e₁ := aggregate (gaussianStatistic value) σ₁ q
    let e₂ := aggregate (gaussianStatistic value) σ₂ q
    (gaussianConjugatePosteriorModel value).posterior prior (σ₁ + σ₂) q =
      posterior (posterior prior e₁) e₂ := by
...
theorem gaussianConjugatePosteriorModel_kappa
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior)
    (σ : Multiset Obs) (q : Query) :
    ((gaussianConjugatePosteriorModel value).posterior prior σ q).κ₀ =
      prior.κ₀ + (σ.card : ℝ) := by
  rw [gaussianConjugatePosteriorModel, posterior_kappa, gaussianStatistic_aggregate_n]

theorem gaussianConjugatePosteriorModel_alpha
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior)
    (σ : Multiset Obs) (q : Query) :
    ((gaussianConjugatePosteriorModel value).posterior prior σ q).α₀ =
      prior.α₀ + (σ.card : ℝ) / 2 := by
  rw [gaussianConjugatePosteriorModel, posterior_alpha, gaussianStatistic_aggregate_n]

theorem gaussianConjugatePosteriorModel_ne_of_nonempty
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior)
    {σ : Multiset Obs} (hσ : σ ≠ 0) (q : Query) :
    (gaussianConjugatePosteriorModel value).posterior prior σ q ≠ prior := by
  intro hEq
  have hkappa := congrArg NormalGammaPrior.κ₀ hEq
  rw [gaussianConjugatePosteriorModel_kappa (value := value) (prior := prior)
      (σ := σ) (q := q)] at hkappa
  have hcard_pos_nat : 0 < σ.card := Multiset.card_pos.mpr hσ
  have hcard_pos : (0 : ℝ) < (σ.card : ℝ) := by
    exact_mod_cast hcard_pos_nat
...
theorem gaussianConjugatePosteriorModel_double_ne_single_of_nonempty
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior)
    {σ : Multiset Obs} (hσ : σ ≠ 0) (q : Query) :
    (gaussianConjugatePosteriorModel value).posterior prior (σ + σ) q ≠
  ...
  exact
    ConjugatePosteriorModel.posterior_double_ne_single_of_nonempty
      (P := gaussianConjugatePosteriorModel value)
      (hneq := fun prior' {τ} hτ =>
        gaussianConjugatePosteriorModel_ne_of_nonempty
          (value := value) (prior := prior') hτ q)
      prior hσ

theorem gaussianConjugatePosteriorModel_not_add_idempotent
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior)
    (o : Obs) (q : Query) :
    ¬ ∀ σ : Multiset Obs,
...
theorem gaussianConjugatePosteriorModel_not_add_idempotent_global
    [Nonempty Obs] [Nonempty Query]
    (value : Obs → Query → ℝ)
    (prior : NormalGammaPrior) :
    ¬ ∀ q σ,
...
using only the WM contract, without touching family-specific posterior
...
/-- Contract use-site: for any unit-observation encoder, a nonempty observation
...
/-- Contract use-site: for any unit-observation encoder, trivial revision and
...
end SufficientStatisticEncoder
