...
namespace Mettapedia.PLN.Bridges.ProbabilityTheory.ConjugateEvidenceCore
...
theorem observationCount_eq_zero_of_add_idempotent {Ev : Type*} [ConjugateEvidence Ev]
  ...
  exact
    Mettapedia.PLN.WorldModel.PLNWorldModelAdditiveNoGo.EvidenceQuantale.BinaryEvidence.finite_coord_add_idempotent_eq_zero
      hfin hcountIdem
...
theorem beta_hplus_is_aggregation (e₁ e₂ : BinaryEvidence) :
    (e₁ + e₂).pos = e₁.pos + e₂.pos ∧
    (e₁ + e₂).neg = e₁.neg + e₂.neg :=
  Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceBeta.hplus_is_beta_aggregation e₁ e₂
...
end Mettapedia.PLN.Bridges.ProbabilityTheory.ConjugateEvidenceCore
