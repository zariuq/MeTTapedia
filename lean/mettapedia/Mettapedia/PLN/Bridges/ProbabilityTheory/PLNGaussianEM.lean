...
namespace Mettapedia.PLN.Bridges.ProbabilityTheory.PLNGaussianEM
...
@[simp] theorem WNGE_single_weight (w : NNReal) (x : ℝ) :
    (Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceWeightedNormalGamma.WeightedNormalGammaEvidence.single w x).weight = w := rfl
...
noncomputable def mStepPosterior (S : GaussianMixtureState ι) (σ : Multiset ℝ) (i : ι) :
    NGPrior :=
  Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceWeightedNormalGamma.WeightedNormalGammaEvidence.posterior
...
end Mettapedia.PLN.Bridges.ProbabilityTheory.PLNGaussianEM
