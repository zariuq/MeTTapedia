...
namespace Mettapedia.PLN.Bridges.ProbabilityTheory.WeightedNormalGammaAdapter
...
open Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceWeightedNormalGamma.WeightedNormalGammaEvidence
...
/-- Query-indexed weighted Gaussian sufficient-statistic adapter.
...
def weightedGaussianStatistic
    ...
    (value : Obs → Query → ℝ) :
    Mettapedia.PLN.WorldModel.SufficientStatisticEncoder Obs Query WeightedNormalGammaEvidence where
...
theorem weightedGaussianStatistic_one_posterior_eq_gaussian
    (prior : Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceNormalGamma.NormalGammaPrior)
    (value : Obs → Query → ℝ)
    (σ : Multiset Obs) (q : Query) :
    WeightedNormalGammaEvidence.posterior prior
        (aggregate (weightedGaussianStatistic (fun _ _ => (1 : NNReal)) value) σ q) =
      Mettapedia.PLN.Bridges.ProbabilityTheory.EvidenceNormalGamma.posterior prior
        (aggregate (gaussianStatistic value) σ q) := by
...
end Mettapedia.PLN.Bridges.ProbabilityTheory.WeightedNormalGammaAdapter
