...
/-!
# IntrinsicPure -> WM Obligation Bridge (A/B/C Aligned)

This module provides an explicit interpretation interface from closed IntrinsicPure
judgments into WM strength obligations, while keeping kernel/profile bridge
contracts explicit.

It does **not** alter IntrinsicPure semantics; it only consumes already-proved
...
- this file is the IntrinsicPure/DTT-specific producer of WM obligations
- the OSLF/NTT modules are the generic formula/evidence closure route
-/

namespace Mettapedia.PLN.Bridges.Languages.WorldModel.PLNWorldModelIntrinsicPureBridge
...
/-- Closed A -> C1 bridge alias from the canonical IntrinsicPure A/B/C interface. -/
...
/-- Closed B -> C1 bridge alias from the canonical IntrinsicPure A/B/C interface. -/
...
/-- One-step closed IntrinsicPure reduction transports to a WM strength obligation,
...
theorem checkedNoValuesDeclKernelBoundary_closedSubjectReduction_and_wmBridge
    ...
    {hNone : ∀ s ∈ specs, s.value? = none}
    (hBoundary :
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.Assembly.CheckedNoValuesDeclKernelBoundary
        hSig hNone)
    {W : State} (hW : I.side W)
    {t u A : PureTm 0}
    (ht :
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSemantics.HasTypeDecl
        (Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSpec.envOfSpecs specs) .nil t A)
    (hred :
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSemantics.RedStarDecl
        (Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSpec.envOfSpecs specs) t u) :
    Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSemantics.HasTypeDecl
        (Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSpec.envOfSpecs specs) .nil u A ∧
      WMStrengthObligation State Query W
        (I.encode (quoteClosedTm t))
        (I.encode (quoteClosedTm u)) := by
  exact
...
theorem checkedNoValuesDeclKernelBoundary_closedCommonReduct_wmBridge
    ...
    {t u : PureTm 0}
    (hconv :
      Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSemantics.ConvDecl
        (Mettapedia.Languages.MeTTa.Pure.Intrinsic.DeclarationSpec.envOfSpecs specs) t u) :
    ∃ w : PureTm 0,
      WMStrengthObligation State Query W
        (I.encode (quoteClosedTm t))
...
/-- Star closed IntrinsicPure reduction transports to a WM strength obligation,
...
/-- Package a closed IntrinsicPure one-step as a state-indexed WM consequence rule. -/
...
/-- Package a closed IntrinsicPure one-step as a WM consequence rule using the
...
/-- Package a closed IntrinsicPure star reduction as a state-indexed WM consequence rule. -/
...
/-- Package a closed IntrinsicPure star reduction as a WM consequence rule using
...
/-- Categorical-aligned packaging of a closed IntrinsicPure one-step as a
...
/-- Categorical-aligned packaging of a closed IntrinsicPure star reduction as a
state-indexed WM consequence rule. -/
...
/-! ## WM-side regression canaries (consume existing IntrinsicPure regressions) -/
...
end Mettapedia.PLN.Bridges.Languages.WorldModel.PLNWorldModelIntrinsicPureBridge
