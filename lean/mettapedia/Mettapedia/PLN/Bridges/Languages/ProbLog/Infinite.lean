...
namespace Mettapedia.PLN.Bridges.Languages.ProbLog.Infinite
...
noncomputable def infiniteMassSemantics (p : ℕ → ℝ≥0) (hp : ∀ i, p i ≤ 1)
    (Q : Set (ℕ → Bool)) (_hQ : MeasurableSet Q) :
    Mettapedia.Logic.MarkovLogicAbstract.MassSemantics Unit where
...
theorem infiniteMassSemantics_queryProb (p : ℕ → ℝ≥0) (hp : ∀ i, p i ≤ 1)
  ...
  simp [Mettapedia.Logic.MarkovLogicAbstract.MassSemantics.queryProb, infiniteMassSemantics,
        div_one]
...
theorem wm_queryStrength_eq_infiniteQueryProb (p : ℕ → ℝ≥0) (hp : ∀ i, p i ≤ 1)
    ...
    BinaryWorldModel.queryStrength
      ({infiniteMassSemantics p hp Q hQ} :
        Mettapedia.Logic.MarkovLogicAbstract.MassState Unit) ()
    = infiniteFactMeasure p hp Q := by
  rw [Mettapedia.Logic.MarkovLogicAbstract.MassState.queryStrength_singleton_eq_queryProb]
  exact infiniteMassSemantics_queryProb p hp Q hQ

end Mettapedia.PLN.Bridges.Languages.ProbLog.Infinite
