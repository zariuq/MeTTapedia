...
/-!
# PLN evidence fibration via a Grothendieck-style construction

This file isolates the PLN-specific evidence fibration that was previously
described as a complete Native Type Theory construction.  Its base is
`PLNObj`, every fibre is `BinaryEvidence`, and its morphisms retain only the
evidence order.  It is therefore a useful Grothendieck-style shell and a
client of categorical ideas from NTT, but not the Williams--Stay presheaf
topos or its full internal dependent language.
...
instance objCategoryStruct : CategoryTheory.CategoryStruct EvidenceObject where
  Hom := Hom
  id := idHom
  comp := compHom

instance objCategory : CategoryTheory.Category EvidenceObject where
...
end Mettapedia.PLN.Bridges.CategoryTheory.EvidenceFibration
