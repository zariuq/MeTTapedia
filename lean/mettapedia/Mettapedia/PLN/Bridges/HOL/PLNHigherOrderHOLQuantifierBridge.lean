...
/-- A finite conjunction is true exactly when every indexed conjunct is true. -/
theorem denote_finiteConjunctionFormula_iff_forall
    (M : HenkinModel.{u, v, w} Base Const)
    {Γ : Ctx Base} {n : Nat}
    (f : Fin n → Formula Const Γ)
    (ρ : HenkinModel.Valuation M Γ) :
    (HenkinModel.denote M (finiteConjunctionFormula f) ρ).down ↔
      ∀ i : Fin n, (HenkinModel.denote M (f i) ρ).down := by
  induction n with
  | zero =>
      simp [finiteConjunctionFormula]
  | succ n ih =>
      change ((HenkinModel.denote M (f 0) ρ).down ∧
        (HenkinModel.denote M
          (finiteConjunctionFormula (fun i : Fin n => f i.succ)) ρ).down) ↔
          ∀ i : Fin (n + 1), (HenkinModel.denote M (f i) ρ).down
      rw [ih]
      constructor
      · rintro ⟨h0, htail⟩ i
        cases i using Fin.cases with
...
/-- The newest-witness distinctness formula says that witness `0` is distinct
from every older witness in the repeated base context. -/
theorem denote_newestDistinctFromPreviousBaseFormula_iff_forall_ne_val
    (M : HenkinModel.{u, v, w} Base Const)
    (b : Base) {n : Nat}
    (xs : Fin (n + 1) → PredicateObject (Base := Base) (Const := Const) M (.base b)) :
    (HenkinModel.denote M
      (newestDistinctFromPreviousBaseFormula (Base := Base) (Const := Const) b n)
      (repeatedBaseValuation (Base := Base) (Const := Const) M b xs)).down ↔
      ∀ i : Fin n, (xs 0).1 ≠ (xs i.succ).1 := by
  change (HenkinModel.denote M
      (finiteConjunctionFormula (fun i : Fin n =>
        .not (.eq (.var .vz)
          (.var (.vs (Var.ofFinRepeat (.base b) n i))))))
      (HenkinModel.extend M
        (repeatedBaseValuation (Base := Base) (Const := Const) M b
          (fun i : Fin n => xs i.succ)) (xs 0).1)).down ↔
      ∀ i : Fin n, (xs 0).1 ≠ (xs i.succ).1
  rw [denote_finiteConjunctionFormula_iff_forall]
  constructor
  · intro h i heq
    have hden : (HenkinModel.denote M
        (.eq (.var .vz) (.var (.vs (Var.ofFinRepeat (.base b) n i))))
        (HenkinModel.extend M
          (repeatedBaseValuation (Base := Base) (Const := Const) M b
            (fun i : Fin n => xs i.succ)) (xs 0).1)).down := by
      change PreModel.Eqv M.toPreModel (.base b) (xs 0).1
        (repeatedBaseValuation (Base := Base) (Const := Const) M b
          (fun i : Fin n => xs i.succ) (Var.ofFinRepeat (.base b) n i))
      rw [repeatedBaseValuation_ofFinRepeat]
      exact heq
    exact h i hden
  · intro h i heq
    ...
    change PreModel.Eqv M.toPreModel (.base b) (xs 0).1
      (repeatedBaseValuation (Base := Base) (Const := Const) M b
        (fun i : Fin n => xs i.succ) (Var.ofFinRepeat (.base b) n i)) at heq
    rw [repeatedBaseValuation_ofFinRepeat] at heq
    exact heq

/-- Weakening the pairwise-distinctness formula into one more repeated
variable drops back to the tail tuple under the repeated-base valuation. -/
theorem denote_weaken_pairwiseDistinctBaseFormula_tail
    (M : HenkinModel.{u, v, w} Base Const)
    (b : Base)
    {n : Nat}
    (xs : Fin (n + 1) → PredicateObject (Base := Base) (Const := Const) M (.base b)) :
...
/-- The pairwise-distinctness formula generated by the uniform counting syntax
is true exactly when the tuple of admissible base witnesses is injective. -/
theorem denote_pairwiseDistinctBaseFormula_iff_injective
    (M : HenkinModel.{u, v, w} Base Const)
    (b : Base) :
    ∀ {n : Nat}
    (xs : Fin n → PredicateObject (Base := Base) (Const := Const) M (.base b)),
  ...
  | zero =>
      intro xs
      simp [pairwiseDistinctBaseFormula, Function.Injective]
  | succ n ih =>
      intro xs
      change ((HenkinModel.denote M
          (newestDistinctFromPreviousBaseFormula (Base := Base) (Const := Const) b n)
      ...
      rw [denote_newestDistinctFromPreviousBaseFormula_iff_forall_ne_val]
      rw [denote_weaken_pairwiseDistinctBaseFormula_tail]
      rw [ih]
      constructor
      · rintro ⟨hnew, htail⟩ i j hij
        cases i using Fin.cases with
        | zero =>
            cases j using Fin.cases with
            | zero => rfl
            | succ j =>
                exact False.elim (hnew j (congrArg Subtype.val hij))
        | succ i =>
            cases j using Fin.cases with
            | zero =>
                exact False.elim (hnew i (congrArg Subtype.val hij.symm))
            | succ j =>
                exact congrArg Fin.succ (htail hij)
      · intro hxs
        constructor
        · intro i hval
          have hsub : xs 0 = xs i.succ := Subtype.ext hval
          have hfin : (0 : Fin (n + 1)) = i.succ := hxs hsub
          have hnat : (0 : Nat) = i.val + 1 := congrArg Fin.val hfin
          exact Nat.succ_ne_zero i.val hnat.symm
        · intro i j hij
          have hfin : i.succ = j.succ := hxs hij
          exact Fin.succ_injective n hfin

/-- Closing a repeated-base formula by `n` existential quantifiers is true
exactly when some `Fin n` tuple of admissible base witnesses makes the body
true. -/
theorem models_closeExistsBaseFormula_iff_exists_tuple
    (M : HenkinModel.{u, v, w} Base Const)
    (b : Base) :
    ∀ {n : Nat} (φ : Formula Const (List.replicate n (.base b))),
    HenkinModel.models M
      ...
      ∃ xs : Fin n → PredicateObject (Base := Base) (Const := Const) M (.base b),
        (HenkinModel.denote M φ
          (repeatedBaseValuation (Base := Base) (Const := Const) M b xs)).down := by
  intro n
  induction n with
  | zero =>
      intro φ
      ...
      · intro h
        ...
        change (HenkinModel.denote M φ (closedValuation M)).down
        change (HenkinModel.denote M φ (closedValuation M)).down at h
        exact h
      · rintro ⟨xs, h⟩
        change (HenkinModel.denote M φ (closedValuation M)).down
        change (HenkinModel.denote M φ (closedValuation M)).down at h
  ...
  | succ n ih =>
      intro φ
      change HenkinModel.models M
          (closeExistsBaseFormula (Base := Base) (Const := Const) b n (.ex φ)) ↔
        ∃ xs : Fin (n + 1) → PredicateObject (Base := Base) (Const := Const) M (.base b),
          (HenkinModel.denote M φ
            (repeatedBaseValuation (Base := Base) (Const := Const) M b xs)).down
      rw [ih (.ex φ)]
      constructor
      · rintro ⟨tail, htail⟩
        change (HenkinModel.denote M (.ex φ)
          (repeatedBaseValuation (Base := Base) (Const := Const) M b tail)).down at htail
        rcases htail with ⟨x, hx, hφ⟩
        refine
          ⟨fun i : Fin (n + 1) => Fin.cases ⟨x, hx⟩ (fun j : Fin n => tail j) i, ?_⟩
        change (HenkinModel.denote M φ
          (HenkinModel.extend M
            (repeatedBaseValuation (Base := Base) (Const := Const) M b tail) x)).down
        exact hφ
      · rintro ⟨xs, hφ⟩
        refine ⟨fun i : Fin n => xs i.succ, ?_⟩
        change (HenkinModel.denote M (.ex φ)
          (repeatedBaseValuation (Base := Base) (Const := Const) M b
            (fun i : Fin n => xs i.succ))).down
        exact ⟨(xs 0).1, (xs 0).2, by
          change (HenkinModel.denote M φ
            (HenkinModel.extend M
              (repeatedBaseValuation (Base := Base) (Const := Const) M b
                (fun i : Fin n => xs i.succ)) (xs 0).1)).down at hφ
          exact hφ⟩

/-- The uniform `at least k` base formula is true exactly when there is an
injective tuple of `k` admissible base witnesses satisfying the predicate. -/
theorem models_predicateAtLeastNBaseFormula_iff_exists_injective
    (M : HenkinModel.{u, v, w} Base Const)
    (b : Base)
    (p : UnaryPredicate (Base := Base) (Const := Const) (.base b))
    (k : Nat) :
    HenkinModel.models M
        (predicateAtLeastNBaseFormula (Base := Base) (Const := Const) b p k) ↔
      ∃ xs : Fin k → PredicateObject (Base := Base) (Const := Const) M (.base b),
        Function.Injective xs ∧
          ∀ i : Fin k,
            predicateHoldsAt (Base := Base) (Const := Const) M (.base b) p (xs i) := by
  change HenkinModel.models M
      (closeExistsBaseFormula (Base := Base) (Const := Const) b k
        (.and
          (predicateWitnessConjunctionBaseFormula (Base := Base) (Const := Const) b p k)
          (pairwiseDistinctBaseFormula (Base := Base) (Const := Const) b k))) ↔
      ∃ xs : Fin k → PredicateObject (Base := Base) (Const := Const) M (.base b),
        Function.Injective xs ∧
          ∀ i : Fin k,
            predicateHoldsAt (Base := Base) (Const := Const) M (.base b) p (xs i)
  rw [models_closeExistsBaseFormula_iff_exists_tuple]
  constructor
  · rintro ⟨xs, h⟩
    change ((HenkinModel.denote M
        (predicateWitnessConjunctionBaseFormula (Base := Base) (Const := Const) b p k)
      ...
      (HenkinModel.denote M
        (pairwiseDistinctBaseFormula (Base := Base) (Const := Const) b k)
        (repeatedBaseValuation (Base := Base) (Const := Const) M b xs)).down) at h
    exact
      ⟨xs,
        (denote_pairwiseDistinctBaseFormula_iff_injective
          (Base := Base) (Const := Const) M b xs).mp h.2,
        (denote_predicateWitnessConjunctionBaseFormula_iff_forall
          (Base := Base) (Const := Const) M b p xs).mp h.1⟩
  · rintro ⟨xs, hinj, hholds⟩
    refine ⟨xs, ?_⟩
    change ((HenkinModel.denote M
        (predicateWitnessConjunctionBaseFormula (Base := Base) (Const := Const) b p k)
        (repeatedBaseValuation (Base := Base) (Const := Const) M b xs)).down ∧
    ...
    exact
      ⟨(denote_predicateWitnessConjunctionBaseFormula_iff_forall
          (Base := Base) (Const := Const) M b p xs).mpr hholds,
        (denote_pairwiseDistinctBaseFormula_iff_injective
          (Base := Base) (Const := Const) M b xs).mpr hinj⟩

/-- On base types, the uniform `at least k` HOL sentence represents exactly
the semantic cardinality-threshold event `k ≤ ncard (ext p)`. -/
theorem models_predicateAtLeastNBaseFormula_iff_k_le_ncard_extension
    (M : HenkinModel.{u, v, w} Base Const)
    (b : Base)
    ...
    HenkinModel.models M
        (predicateAtLeastNBaseFormula (Base := Base) (Const := Const) b p k) ↔
      k ≤ (predicateExtension (Base := Base) (Const := Const) M (.base b) p).ncard := by
  rw [models_predicateAtLeastNBaseFormula_iff_exists_injective]
  constructor
  · rintro ⟨xs, hinj, hholds⟩
    let W : Set (PredicateObject (Base := Base) (Const := Const) M (.base b)) := Set.range xs
    have hWcard : W.ncard = k := by
      have hImg :
          (xs '' (Set.univ : Set (Fin k))).ncard =
            (Set.univ : Set (Fin k)).ncard :=
        Set.ncard_image_of_injective (Set.univ : Set (Fin k)) hinj
      simpa [W, Set.image_univ, Set.ncard_univ, Nat.card_eq_fintype_card] using hImg
    have hW :
        ∀ x ∈ W,
          predicateHoldsAt (Base := Base) (Const := Const) M (.base b) p x := by
      intro x hx
      rcases hx with ⟨i, rfl⟩
      exact hholds i
    exact
      (predicateExtension_ncard_ge_iff_exists_witnessSet
        (Base := Base) (Const := Const) M (.base b) p k).2
        ⟨W, hWcard, hW⟩
  · intro hk
    rcases
      (predicateExtension_ncard_ge_iff_exists_witnessSet
        (Base := Base) (Const := Const) M (.base b) p k).1 hk with
      ⟨W, hWcard, hW⟩
    classical
    have hCardW : Fintype.card W = k := by
      rw [Set.fintypeCard_eq_ncard, hWcard]
    let e : Fin k ≃ W := (Fintype.equivFinOfCardEq hCardW).symm
    refine ⟨fun i : Fin k => (e i).1, ?_, ?_⟩
    · intro i j hij
      exact e.injective (Subtype.ext hij)
    · intro i
      exact hW (e i).1 (e i).2
