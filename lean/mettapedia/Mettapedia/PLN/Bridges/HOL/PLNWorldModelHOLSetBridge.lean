...
namespace Mettapedia.PLN.Bridges.HOL.PLNWorldModelHOLSetBridge
...
theorem setHolEvidence_eq_crispEvidence
    ...
    setHolEvidence W φ =
      Mettapedia.PLN.WorldModel.PLNWorldModelCrispSpecialization.crispEvidence
...
theorem pointwiseIff_iff_queryEq_is_crispSpecialization
    (φ ψ : SetHOLQuery) :
    (∀ S : SetPointed, setHolSatisfies S φ ↔ setHolSatisfies S ψ) ↔
      Mettapedia.PLN.WorldModel.PLNWorldModelCrispSpecialization.CrispQueryEq
        setHolSatisfies φ ψ := by
  simpa using
    (Mettapedia.PLN.WorldModel.PLNWorldModelCrispSpecialization.pointwiseIff_iff_queryEq
...
abbrev stateModelsTheory (T : SetTheory) (W : SetState) : Prop :=
  Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelSetTheoryBridge.stateModelsTheory T W
...
private theorem setHolSatisfies_embedSentence_iff_of_mutual_consequence
  ...
  have hleφψ :=
    (Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelSetTheoryBridge.consequence_iff_all_model_singleton_strength
      (T := T) (φ := φ) (ψ := ψ)).1 hφψ S hT
  have hleψφ :=
    (Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelSetTheoryBridge.consequence_iff_all_model_singleton_strength
      (T := T) (φ := ψ) (ψ := φ)).1 hψφ S hT
  have himpφψ :=
    (Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOLCompleteness.singletonStrengthLE_singleton_iff_imp
      (S := S) (φ := φ) (ψ := ψ)).1 hleφψ
  have himpψφ :=
    (Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOLCompleteness.singletonStrengthLE_singleton_iff_imp
      (S := S) (φ := ψ) (ψ := φ)).1 hleψφ
  rw [show setHolSatisfies S (Mettapedia.Logic.HOL.Embedding.FirstOrder.embedSentence φ) ↔
      Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.folSatisfies S φ by
        unfold setHolSatisfies Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.folSatisfies
        exact
  ...
  rw [show setHolSatisfies S (Mettapedia.Logic.HOL.Embedding.FirstOrder.embedSentence ψ) ↔
      Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.folSatisfies S ψ by
        unfold setHolSatisfies Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.folSatisfies
        exact
          (Mettapedia.Logic.HOL.Semantics.SetBased.pointed_denote_embedSentence_iff
            (S := S) (φ := ψ))]
...
theorem setHolEvidence_embedSentence_eq_folEvidence
    ...
    setHolEvidence W (Mettapedia.Logic.HOL.Embedding.FirstOrder.embedSentence φ) =
      Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.folEvidence W φ := by
  classical
  apply BinaryEvidence.ext'
  · simp [setHolEvidence, Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.folEvidence,
...
theorem queryStrength_embedSentence_eq_setQueryStrength
    ...
    BinaryWorldModel.queryStrength (State := SetState) (Query := SetHOLQuery) W
      ...
      BinaryWorldModel.queryStrength
        (State := Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelSetTheoryBridge.SetState)
        (Query := SetQuery) W φ := by
  change
    BinaryEvidence.toStrength
      (setHolEvidence W (Mettapedia.Logic.HOL.Embedding.FirstOrder.embedSentence φ)) =
    BinaryEvidence.toStrength (Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.folEvidence W φ)
...
theorem multiset_strength_le_of_consequence_embed
  ...
  exact
    Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelSetTheoryBridge.multiset_strength_le_of_consequence
...
theorem multiset_strength_le_of_provable_imp_embed
  ...
  rw [queryStrength_embedSentence_eq_setQueryStrength (W := W) (φ := ψ)]
  exact
    Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelSetTheoryBridge.multiset_strength_le_of_provable_imp
...
theorem canary_singleton_embedTruth_strength_one
  ...
  exact
    Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.queryStrength_singleton_of_satisfies
      (S := S) (φ := (⊤ : LO.FirstOrder.Sentence SetLang)) (by
        simp [Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.folSatisfies])
...
theorem canary_singleton_embedFalsum_strength_zero
  ...
  rw [queryStrength_embedSentence_eq_setQueryStrength (W := ({S} : SetState))
    (φ := (⊥ : LO.FirstOrder.Sentence SetLang))]
  exact
    Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.queryStrength_singleton_of_not_satisfies
      (S := S) (φ := (⊥ : LO.FirstOrder.Sentence SetLang)) (by
        simp [Mettapedia.PLN.Bridges.Logic.WorldModel.PLNWorldModelFOL.folSatisfies])

end Mettapedia.PLN.Bridges.HOL.PLNWorldModelHOLSetBridge
