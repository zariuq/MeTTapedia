...
namespace Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLRules
...
abbrev HOLProvable (φ : HOLQuery Const) : Prop :=
  Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLCore.HOLProvable (Const := Const) φ
...
abbrev HOLProvEq {τ : Ty Base} (t u : Term Const [] τ) : Prop :=
  Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLCore.HOLProvEq (Const := Const) t u
...
theorem holProvIff_refl (φ : HOLQuery Const) :
  ...
  holProvIff_of_mutual (Const := Const)
    (Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLCore.holProvImp_refl φ)
    (Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLCore.holProvImp_refl φ)
...
theorem holProvIff_trans {φ ψ χ : HOLQuery Const}
  ...
  exact .andI
    (Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLCore.holProvImp_trans hφψ_fwd hψχ_fwd)
    (Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLCore.holProvImp_trans hψχ_rev hφψ_rev)
...
end Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLRules
