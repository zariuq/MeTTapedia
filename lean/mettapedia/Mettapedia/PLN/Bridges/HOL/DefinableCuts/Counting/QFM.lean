...
/-! ## Exact-denominator finite-QFM counting cuts -/
...
/-- Uniform exact-denominator finite-QFM `ForAll` threshold cut for the
base-type HOL formula "at least `k` distinct witnesses satisfy `p`".

This is the arbitrary-cardinality capstone for the base-type counting cuts:
the constructed HOL formula is supplied to the generic cardinality-threshold
cut together with the proved representation theorem.  Endpoint tightness is
therefore inherited from `predicateFuzzyForAllCountingCardinalityThresholdCut`
without adding one alias theorem per endpoint. -/
noncomputable def predicateFuzzyForAllCountingAtLeastNBaseCut
    {T : ClosedTheorySet (WithParams Const)}
    (b : Base)
    (params : FuzzyQuantifierParams)
    (p : UnaryPredicate (Base := Base) (Const := WithParams Const) (.base b))
  ...
  predicateFuzzyForAllCountingCardinalityThresholdCut
    ...
    hObj hMeasurable hε0 k N hNpos hPCL hCardEq
    (fun M => by
      letI := hObj M
      exact
...
/-- Uniform exact-denominator finite-QFM `ThereExists` threshold cut for the
base-type HOL formula "at least `k` distinct witnesses satisfy `p`".

As with the `ForAll` variant, this is a thin specialization of the generic
cardinality-threshold cut.  It proves the `represents_ge` obligation once for
arbitrary `k` rather than enumerating threshold-specific witnesses. -/
noncomputable def predicateFuzzyThereExistsCountingAtLeastNBaseCut
    {T : ClosedTheorySet (WithParams Const)}
    (b : Base)
    (params : FuzzyQuantifierParams)
    (p : UnaryPredicate (Base := Base) (Const := WithParams Const) (.base b))
...
/-- Width-zero tightness for arbitrary base-type exact-denominator finite-QFM
`ForAll` counting cuts.  This is the generic capstone behind the concrete
"at least two" / "at least three" witnesses: the interval collapses exactly
when the theory decides the constructed HOL cardinality formula. -/
theorem predicateFuzzyForAllCountingAtLeastNBaseCut_width_eq_zero_iff_decides
    {T : ClosedTheorySet (WithParams Const)}
    (b : Base)
    (params : FuzzyQuantifierParams)
    (p : UnaryPredicate (Base := Base) (Const := WithParams Const) (.base b))
    ...
    (ExtensionalDefinableCut.intervalOfConsistent
      ...
      (predicateFuzzyForAllCountingAtLeastNBaseCut
        (Base := Base) (Const := Const) (T := T)
        b params p hp0 hObj hMeasurable hε0 k N hNpos hPCL hCardEq)
      enum henum hCons hT0 hEM).width = 0 ↔
      ClosedTheorySet.Provable (Const := WithParams Const) T
        (predicateAtLeastNBaseFormula (Base := Base) (Const := WithParams Const) b p k) ∨
      ClosedTheorySet.Provable (Const := WithParams Const) T
...
/-- Width-zero tightness for arbitrary base-type exact-denominator finite-QFM
`ThereExists` counting cuts.  This uses the same constructed HOL
cardinality formula as the `ForAll` capstone, while the numeric score is the
`ThereExists` QFM mass threshold. -/
theorem predicateFuzzyThereExistsCountingAtLeastNBaseCut_width_eq_zero_iff_decides
    {T : ClosedTheorySet (WithParams Const)}
    (b : Base)
    (params : FuzzyQuantifierParams)
    (p : UnaryPredicate (Base := Base) (Const := WithParams Const) (.base b))
