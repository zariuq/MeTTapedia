...
namespace Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLLinkBridge
...
abbrev HOLLinkJudgment :=
  Mettapedia.PLN.RuleFamilies.FirstOrder.PLNLinkCalculus.Judgment (HOLQuery (Base := Base) Const)
...
theorem holdsTermWM_transport_of_holProvIff {φ ψ : HOLQuery Const}
  ...
  intro hφ
  have hEq :=
    Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLConsequence.holProvIff_to_WMStrengthEq
      (Base := Base) (Const := Const) h W
...
end Mettapedia.PLN.Bridges.HOL.PLNHigherOrderHOLLinkBridge
