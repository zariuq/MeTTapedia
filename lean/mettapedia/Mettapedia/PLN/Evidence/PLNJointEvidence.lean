...
namespace Mettapedia.PLN.Evidence.PLNJointEvidence
...
noncomputable def propWTV (κ : ℝ≥0∞) (E : JointEvidence n) (A : Fin n) : Mettapedia.PLN.TruthValues.PLNWeightTV.WTV :=
  BinaryEvidence.toWTV κ (propEvidence (n := n) (E := E) A)

/-- Link view as WTV, using the canonical BinaryEvidence→WTV map with prior κ. -/
noncomputable def linkWTV (κ : ℝ≥0∞) (E : JointEvidence n) (A B : Fin n) : Mettapedia.PLN.TruthValues.PLNWeightTV.WTV :=
  BinaryEvidence.toWTV κ (linkEvidence (n := n) (E := E) A B)

end JointEvidence

end Mettapedia.PLN.Evidence.PLNJointEvidence
