...
namespace Mettapedia.PLN.InferenceControl.PremiseSelection
...
def localEvidence (vals : Fin n → Bool) : Nat × Nat :=
  (Mettapedia.ProbabilityTheory.Exchangeability.countTrue vals,
    Mettapedia.ProbabilityTheory.Exchangeability.countFalse vals)
...
@[simp] theorem depSetOfVals_card (vals : Fin n → Bool) :
    (depSetOfVals vals).card = Mettapedia.ProbabilityTheory.Exchangeability.countTrue vals := by
  simp [depSetOfVals, Mettapedia.ProbabilityTheory.Exchangeability.countTrue]
...
theorem finite_exchangeable_prob_depends_only_on_localEvidence
    (hexch : Mettapedia.ProbabilityTheory.Exchangeability.FiniteExchangeable n X μ)
    (vals₁ vals₂ : Fin n → Bool)
    (hev : localEvidence vals₁ = localEvidence vals₂) :
    μ {ω | ∀ i, X i ω = vals₁ i} = μ {ω | ∀ i, X i ω = vals₂ i} := by
  have hcount :
      Mettapedia.ProbabilityTheory.Exchangeability.countTrue vals₁ =
        Mettapedia.ProbabilityTheory.Exchangeability.countTrue vals₂ := by
    simpa [localEvidence] using congrArg Prod.fst hev
  exact
    Mettapedia.ProbabilityTheory.Exchangeability.exchangeable_same_counts_same_prob
...
noncomputable def localCountKernel
    (_hexch : Mettapedia.ProbabilityTheory.Exchangeability.FiniteExchangeable n X μ) : Nat → ℝ≥0∞ :=
  fun k =>
    if hk : ∃ vals : Fin n → Bool, Mettapedia.ProbabilityTheory.Exchangeability.countTrue vals = k then
...
theorem localCountKernel_spec
    (hexch : Mettapedia.ProbabilityTheory.Exchangeability.FiniteExchangeable n X μ)
    (vals : Fin n → Bool) :
    localCountKernel X μ hexch (Mettapedia.ProbabilityTheory.Exchangeability.countTrue vals) =
      μ {ω | ∀ i, X i ω = vals i} := by
  let k := Mettapedia.ProbabilityTheory.Exchangeability.countTrue vals
  have hk :
      ∃ v : Fin n → Bool, Mettapedia.ProbabilityTheory.Exchangeability.countTrue v = k := ⟨vals, rfl⟩
  unfold localCountKernel
  rw [dif_pos hk]
  have hchoose :
      Mettapedia.ProbabilityTheory.Exchangeability.countTrue (Classical.choose hk) = k :=
...
theorem finite_deFinetti_local_mixture_bridge
    ...
    ∃ q : Nat → ℝ≥0∞, ∀ vals : Fin n → Bool,
      q (Mettapedia.ProbabilityTheory.Exchangeability.countTrue vals) =
        μ {ω | ∀ i, X i ω = vals i} := by
...
theorem localEvidence_coverage_bound
    ...
    dependencyCoverage (depSetOfVals vals) S ≤
      Nat.min S.card (Mettapedia.ProbabilityTheory.Exchangeability.countTrue vals) := by
  have hbase :
...
theorem finite_deFinetti_partitioned_local_mixture_bridge :
    (hexchBin :
      ∀ b : Bin,
        Mettapedia.ProbabilityTheory.Exchangeability.FiniteExchangeable (nBin b) (Xbin b) μ) →
    ∃ q : (b : Bin) → Nat → ℝ≥0∞, ∀ b : Bin, ∀ vals : Fin (nBin b) → Bool,
      q b (Mettapedia.ProbabilityTheory.Exchangeability.countTrue vals) =
...
theorem finite_statistic_l1_mixture_bound
  ...
  have hpush :
    ...
    Mettapedia.ProbabilityTheory.Exchangeability.DiaconisFreedmanFinite.l1_pushforward_le (μ := μ) (ν := ν) (f := stat)
  have hbase : (∑ ω : Fin m → Fin R, abs (μ ω - ν ω))
      ≤ (4 : ℝ) * (m : ℝ) * (m : ℝ) / (R : ℝ) := by
    dsimp [μ, ν, Z]
    simpa using (Mettapedia.ProbabilityTheory.Exchangeability.DiaconisFreedmanFinite.l1_iid_inj_le R m hR hRm)
...
theorem finite_statistic_tv_mixture_bound_choose2
  ...
  simpa using (Mettapedia.ProbabilityTheory.Exchangeability.DiaconisFreedmanFinite.l1_iid_inj_le_choose2 R m hR hRm)
...
end Mettapedia.PLN.InferenceControl.PremiseSelection
