...
namespace Mettapedia.PLN.RuleFamilies.FirstOrder.PLNInferenceRules
...
- The foundational carrier in this repo is `BinaryEvidence := (n⁺, n⁻)` with a partial order and tensor
  (see `Mettapedia.PLN.Evidence.EvidenceQuantale` and `Mettapedia.PLN.Evidence.PLN_KS_Bridge`).
...
- The former exploratory strength-level packaging is archived at
  `Mettapedia.PLN.RuleFamilies.QuantaleSemantics.PLNQuantaleConnectionLegacy`; the live module path
...
end Mettapedia.PLN.RuleFamilies.FirstOrder.PLNInferenceRules
