...
noncomputable def formedConceptEquivCrispConcept
    ...
    Mettapedia.KR.ConceptGeometry.AbstractInheritance.FormedConcept G M ≃ CrispConcept G M where
  toFun A :=
    DualConcept.toConcept A.1 (Mettapedia.KR.ConceptGeometry.AbstractInheritance.formedConcept_isClosed G M A)
  invFun c :=
    ⟨DualConcept.ofConcept c,
      (Mettapedia.KR.ConceptGeometry.AbstractInheritance.mem_finiteConceptFamily_iff G M _).2
        (DualConcept.isClosed_ofConcept c)⟩
  left_inv A := by
    apply Subtype.ext
    exact DualConcept.ofConcept_toConcept A.1
      (Mettapedia.KR.ConceptGeometry.AbstractInheritance.formedConcept_isClosed G M A)
  right_inv c := by
...
noncomputable def formedConceptOrderIsoCrispConcept
    (G : EvidenceGate Q) (M : Obj → Attr → Q) :
    Mettapedia.KR.ConceptGeometry.AbstractInheritance.FormedConcept G M ≃o CrispConcept G M where
  ...
  map_rel_iff' := by
    ...
    exact
      (DualConcept.toConcept_le_iff A.1 B.1
        (Mettapedia.KR.ConceptGeometry.AbstractInheritance.formedConcept_isClosed G M A)
        (Mettapedia.KR.ConceptGeometry.AbstractInheritance.formedConcept_isClosed G M B))

@[simp] theorem formedConceptOrderIsoCrispConcept_apply
    (G : EvidenceGate Q) (M : Obj → Attr → Q)
    (A : Mettapedia.KR.ConceptGeometry.AbstractInheritance.FormedConcept G M) :
...
theorem formedConcept_inherits_iff_crispConcept_le
    (G : EvidenceGate Q) (M : Obj → Attr → Q)
    (A B : Mettapedia.KR.ConceptGeometry.AbstractInheritance.FormedConcept G M) :
    A ≤ B ↔
      formedConceptOrderIsoCrispConcept G M A ≤
