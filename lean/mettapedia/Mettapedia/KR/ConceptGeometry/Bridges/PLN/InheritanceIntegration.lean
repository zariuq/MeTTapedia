...
namespace Mettapedia.KR.ConceptGeometry.ExtensionalIntensionalDivergence

open Mettapedia.KR.ConceptGeometry.AbstractInheritance
open Mettapedia.KR.ConceptGeometry.IntensionalInheritance.Interpretation
...
def stvToWMTruthValue (t : Mettapedia.PLN.RuleFamilies.FirstOrder.PLNDeduction.STV) : TV :=
  ⟨t.strength, t.confidence⟩

@[simp] theorem stvToWMTruthValue_s (t : Mettapedia.PLN.RuleFamilies.FirstOrder.PLNDeduction.STV) :
    (stvToWMTruthValue t).s = t.strength := rfl

@[simp] theorem stvToWMTruthValue_c (t : Mettapedia.PLN.RuleFamilies.FirstOrder.PLNDeduction.STV) :
...
private theorem prior_w2c_lt_one {x : ℝ} (hx : 0 ≤ x) :
    Mettapedia.PLN.TruthValues.PLNWeightTV.w2c x < 1 := by
  unfold Mettapedia.PLN.TruthValues.PLNWeightTV.w2c
...
noncomputable def conceptPriorTV
    (I : Interpretation Carrier Obj Attr) (c : Carrier) : Mettapedia.PLN.RuleFamilies.FirstOrder.PLNDeduction.STV :=
...
@[simp] theorem conceptPriorTV_confidence_eq
    ...
    (conceptPriorTV I c).confidence =
      Mettapedia.PLN.TruthValues.PLNWeightTV.w2c (conceptPriorWeight I c) := rfl
...
end Mettapedia.KR.ConceptGeometry.ExtensionalIntensionalDivergence
