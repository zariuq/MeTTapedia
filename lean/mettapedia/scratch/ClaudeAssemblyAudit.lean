...
type. -/
theorem kimi_hasType_of_mem_elements {language : LanguageDef}
...
theorem kimi_parallelContents_units_eq_nil
...
theorem kimi_canonicalize_parallel_units_around
    (declaration : ReflectivePresentationDecl)
  ...
  have contentsNil : ∀ list : List Pattern,
    ...
    kimi_parallelContents_units_eq_nil declaration
  ...
  have midContents : parallelContents declaration [mid] = [mid] := by
    ...
    simp [midNotUnit, List.filter_cons]
...
splice. -/
theorem kimi_parallelSplice_singleton_of_canonical_notUnit_notParallel
...
theorem kimi_parallelCollapse_members_result_or_unit
    (declaration : ReflectivePresentationDecl)
...
theorem kimi_sibling_unit_of_singleton_collapse
    (declaration : ReflectivePresentationDecl)
    {elements : List Pattern} {result : Pattern}
    (notUnit : result ≠ .apply declaration.parallelUnitConstructor [])
    (notParallel : ∀ nested,
    ...
    canonicalize declaration other =
      .apply declaration.parallelUnitConstructor [] := by
  rcases kimi_parallelCollapse_members_result_or_unit declaration notUnit
  ...
  · -- both active and other canonicalize to result: positional duplicate
    ...
    exact claude_sibling_unit_residual declaration notUnit notParallel
...
theorem kimi_isCanonical_unitApply (declaration : ReflectivePresentationDecl) :
...
theorem kimi_isCanonical_iterDropUnit (declaration : ReflectivePresentationDecl)
    (dropNeQuote : declaration.dropConstructor ≠ declaration.quoteConstructor) :
  ...
  | zero => exact kimi_isCanonical_unitApply declaration
...
theorem kimi_canonicalize_iterDropUnit (declaration : ReflectivePresentationDecl)
    (dropNeQuote : declaration.dropConstructor ≠ declaration.quoteConstructor)
    (level : Nat) :
    canonicalize declaration
      (iterDrop declaration level
  ...
  canonicalize_eq_of_isCanonical declaration
    (kimi_isCanonical_iterDropUnit declaration dropNeQuote level)
...
theorem kimi_contents_nil_of_collapsed_unit
...
theorem kimi_all_units_of_collapsed_unit
    (declaration : ReflectivePresentationDecl) {elements : List Pattern}
    (collapsed : canonicalize declaration
        (.collection declaration.parallelCollection elements none) =
      .apply declaration.parallelUnitConstructor []) :
    ∀ e ∈ elements, canonicalize declaration e =
      .apply declaration.parallelUnitConstructor [] := by
  intro e emem
  have contentsNil := kimi_contents_nil_of_collapsed_unit declaration collapsed
...
theorem kimi_elementPlan_abstractPatterns_length
  ...
  | _, _, .cons head tail => by
      simp only [CostStaticElementPlan.abstractPatterns, List.length_cons,
        kimi_elementPlan_abstractPatterns_length color tail]
...
plan of `element`. -/
theorem kimi_elementSpine_getElem?_abstracts
    (color : CostStaticColor) {targetFree : FreeTypeContext}
    {sourceBound targetBound : List TypeExpr}
    {thinning : CostStaticBinderThinning rhoCIGSLT color sourceBound
      targetBound}
  ...
  | _, _, .cons head tail, i, element, ithEq => by
      ...
      | succ j =>
          ...
          obtain ⟨ou, p, position⟩ := kimi_elementSpine_getElem?_abstracts
            color tail j ithEq
          refine ⟨ou, p, ?_⟩
...
result. -/
theorem kimi_contents_singleton_of_collapse
    (declaration : ReflectivePresentationDecl)
    {elements : List Pattern} {result : Pattern}
    (notUnit : result ≠ .apply declaration.parallelUnitConstructor [])
    (notParallel : ∀ nested,
...
theorem kimi_collapse_duplicate_value_absurd
    (declaration : ReflectivePresentationDecl)
    {result : Pattern}
    (notUnit : result ≠ .apply declaration.parallelUnitConstructor [])
    (notParallel : ∀ nested,
  ...
  · obtain ⟨pre, post, beforeSplit⟩ := List.mem_iff_append.mp prefixMem
    ...
    exact claude_two_rigid_contributors_absurd declaration notUnit notParallel
...
theorem kimi_declC_dropNeQuote (color : CostStaticColor) :
    (costStaticReflectivePresentationDecl rhoCIGSLT color
      rhoReflectivePresentation.toReflectivePresentationDecl
      ).dropConstructor ≠
    (costStaticReflectivePresentationDecl rhoCIGSLT color
...
theorem kimi_canonicalize_collection_ct_transport
...
theorem claude_sibling_unit_positional
    (declaration : ReflectivePresentationDecl)
    ...
    canonicalize declaration other =
      .apply declaration.parallelUnitConstructor [] := by
  have contentsEq := kimi_contents_singleton_of_collapse declaration notUnit
  ...
  · exfalso
    ...
    · obtain ⟨b1, b2, beforeSplit⟩ := List.mem_iff_append.mp inBefore
      subst elementsEq
      subst beforeSplit
      refine claude_two_rigid_contributors_absurd declaration notUnit
...
theorem kimi_canonicalize_collection_congr_canonical_slot
    (declaration : ReflectivePresentationDecl)
    (collectionType : CollType) (rest : Option String)
...
theorem kimi_plan_abstract_iterDropUnit_of_iterDropUnit
    (color : CostStaticColor) {targetFree : FreeTypeContext} :
          ...
          | none =>
              ...
              · rw [parallelType] at towerCanonical
                ...
                | zero =>
                    ...
                    simp only [iterDrop] at towerCanonical
                    have allUnits := kimi_all_units_of_collapsed_unit
                      (costStaticReflectivePresentationDecl rhoCIGSLT color
                        rhoReflectivePresentation.toReflectivePresentationDecl)
                      towerCanonical
                    have abstractsUnits : ∀ b ∈ children.abstractPatterns,
                      ...
                      have aLen : children.abstractPatterns.length =
                          elements.length :=
                        kimi_elementPlan_abstractPatterns_length color children
                      ...
                      obtain ⟨ou, p, pAbstract⟩ :=
                        kimi_elementSpine_getElem?_abstracts color children i
                          eAt
                    ...
                    simp only [CostStaticRegionPlan.abstractPattern,
                      Option.map_none]
                    rw [kimi_canonicalize_collection_ct_transport
                      rhoReflectivePresentation.toReflectivePresentationDecl
                      ((rhoDeclC_parallelCollection_hashBag color).trans
                        auth_parallelCollection.symm)]
                    rw [canonicalize_parallel]
                    rw [normalizeParallelElements_eq_sort_parallelContents,
                      kimi_parallelContents_units_eq_nil
                        rhoReflectivePresentation.toReflectivePresentationDecl
                        _ abstractsUnits]
                ...
                | succ innerLevel =>
                    ...
                    obtain ⟨ouC, pC, pCAbs⟩ :=
                      kimi_elementSpine_getElem?_abstracts color children
                        contextBefore.length carrierAt
                    have contentsEq := kimi_contents_singleton_of_collapse
                      (costStaticReflectivePresentationDecl rhoCIGSLT color
                        (rhoReflectivePresentation.toReflectivePresentationDecl))
                    ...
                    have siblingUnitAbs : ∀ {j : Nat}
                      ...
                      obtain ⟨ouJ, pJ, pJAbs⟩ :=
                        kimi_elementSpine_getElem?_abstracts color children j
                          jAt
                      ...
                      have measureJ : sizeOf (getElem elements j
                        ...
                        omega
                      have elemCases :=
                        kimi_parallelCollapse_members_result_or_unit
                          (costStaticReflectivePresentationDecl rhoCIGSLT
                            color (rhoReflectivePresentation.toReflectivePresentationDecl))
                      ...
                      · exfalso
                        ...
                        · have elemInPrefix : getElem elements j jElements ∈
                          ...
                          exact kimi_collapse_duplicate_value_absurd
                            (costStaticReflectivePresentationDecl rhoCIGSLT
                              color (rhoReflectivePresentation.toReflectivePresentationDecl))
                    ...
                    have towerAuthFixpoint : canonicalize
                      ...
                      kimi_canonicalize_iterDropUnit
                        rhoReflectivePresentation.toReflectivePresentationDecl
                    ...
                    suffices finalForm : canonicalize
                      ...
                      exact finalForm
                    rw [spineSplit,
                      kimi_canonicalize_collection_congr_canonical_slot
                        rhoReflectivePresentation.toReflectivePresentationDecl
                        _ none _ _ slotEq,
                      kimi_canonicalize_collection_ct_transport
                        rhoReflectivePresentation.toReflectivePresentationDecl
                        ((rhoDeclC_parallelCollection_hashBag color).trans
                          auth_parallelCollection.symm)]
                    apply kimi_canonicalize_parallel_units_around
                    · exact beforeUnits
                    · exact afterUnits
      ...
      | application declared rendered current preimage notBare children =>
          ...
          · rw [canonicalize_apply_of_ne_quote _ quoteHead] at towerCanonical
            ...
            | zero =>
                ...
                rw [canonicalize_eq_of_isCanonical _ (kimi_isCanonical_unitApply
                  rhoReflectivePresentation.toReflectivePresentationDecl)]
                simp only [iterDrop]
            | succ innerLevel =>
...
theorem kimi_plan_abstract_unit_of_colour_unit
    (color : CostStaticColor) {targetFree : FreeTypeContext}
    {sourceBound targetBound : List TypeExpr}
    {thinning : CostStaticBinderThinning rhoCIGSLT color sourceBound
      targetBound}
  ...
  have tower := kimi_plan_abstract_iterDropUnit_of_iterDropUnit color
...
carrier position. -/
theorem kimi_elementPlan_activeAt_lenCert
    {color : CostStaticColor} {targetFree : FreeTypeContext} :
  ...
  | sibling :: contextRest, _, _, _, _, _, _, _, _, _, _, spine, middle,
      ...
      | cons head tail =>
          obtain ⟨headEq, tailEq⟩ := List.cons.inj split
          subst headEq
          obtain ⟨active, activeLen⟩ := kimi_elementPlan_activeAt_lenCert
            tail tailEq
          refine
          ...
          | none =>
              ...
              · rw [parallelType] at canonicalElement
                ...
                · -- direct collection typing: recurse the contributor
                  have memberTyped :=
                    kimi_hasType_of_mem_elements elementsTyped memberMem
                  have memberChain :=
                ...
                · -- bare-rule collection typing: the rule is rho's parallel
                  ...
                  have categoryRestricted := kimi_bare_whole_rule_category
...
theorem claude_canonicalize_iterDrop_fvar (level : Nat) (name : String) :
    canonicalize rhoReflectivePresentation.toReflectivePresentationDecl
        (iterDrop rhoReflectivePresentation.toReflectivePresentationDecl level
          (.fvar name)) =
      iterDrop rhoReflectivePresentation.toReflectivePresentationDecl level
...
theorem claude_iterDrop_fvar_ne_unit (level : Nat) (name : String) :
    iterDrop rhoReflectivePresentation.toReflectivePresentationDecl level
        (.fvar name) ≠
...
theorem claude_iterDrop_fvar_ne_parallel (level : Nat) (name : String)
    (nested : List Pattern) :
    iterDrop rhoReflectivePresentation.toReflectivePresentationDecl level
        (.fvar name) ≠
              ...
              · rw [parallelType] at canonical
                have notUnit : iterDrop
                  ...
                  | zero =>
                      ...
                      exact absurd decodedNone (by simp)
                  | succ level =>
                      simp [iterDrop, rhoDecl_dropConstructor,
                        rhoDecl_unitConstructor]
                ...
                obtain ⟨active, lenCert⟩ :=
                  kimi_elementPlan_activeAt_lenCert children elementsEq
                ...
                have beforeUnits : ∀ b ∈ active.beforeAbstracts,
                  ...
                  obtain ⟨ou, plan, planAt⟩ :=
                    kimi_elementSpine_getElem?_abstracts color children i elemsSome
                  have bEq : b = plan.abstractPattern :=
                    Option.some.inj (absAt.symm.trans planAt)
                  subst bEq
                  have eUnit := claude_sibling_unit_positional
                    (costStaticReflectivePresentationDecl rhoCIGSLT color
                      rhoReflectivePresentation.toReflectivePresentationDecl)
                  ...
                  have measureE : sizeOf before[i] ≤ fuel := by
                    ...
                    omega
                  have tower := kimi_plan_abstract_iterDropUnit_of_iterDropUnit
                    color fuel plan 0 measureE (by simpa using eUnit)
                  simpa using tower
                have afterLen : active.afterAbstracts.length = after.length := by
                  have total := kimi_elementPlan_abstractPatterns_length color
                    children
                ...
                have afterUnits : ∀ a ∈ active.afterAbstracts,
                  ...
                  have tower := kimi_plan_abstract_iterDropUnit_of_iterDropUnit
                    color fuel plan 0 measureE (by simpa using eUnit)
                  simpa using tower
                rw [kimi_canonicalize_collection_congr_canonical_slot
                  rhoReflectivePresentation.toReflectivePresentationDecl
                  ...
                  (by simp only [OneHoleContext.fill]
                      rw [childCollapse, claude_canonicalize_iterDrop_fvar])]
                rw [kimi_canonicalize_collection_ct_transport _ authParallel]
                exact kimi_canonicalize_parallel_units_around _ beforeUnits
                  afterUnits (claude_canonicalize_iterDrop_fvar _ _)
                  (claude_iterDrop_fvar_ne_unit _ _)
                  (fun nested => claude_iterDrop_fvar_ne_parallel _ _ nested)
              · rw [canonicalize_collection_of_ne_parallel _ parallelType]
                  at canonical
                cases level with
                | succ level =>
