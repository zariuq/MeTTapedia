...
/-!
# Non-quote foreign-boundary and rigid-leaf canary

This fixture checks whether a foreign boundary whose content uses a wrapped
name binder can coexist with a current-colour rigid leaf using a base name
binder under one admitted bare-parallel process plan.
-/

namespace Mettapedia.Languages.ProcessCalculi.RhoCalculus
namespace ForeignNonquoteRigidTieCanary
...
def baseNameType : TypeExpr := .base (costBaseSortName "Name")
def processType : TypeExpr := .base (costBaseSortName "Proc")
def available : List TypeExpr := [baseNameType]
...
#eval checkOpenPatternWellSorted rhoCIGSLT.costWholeLanguage
  FreeTypeContext.empty available processType leftPattern
...
theorem leftWellSorted :
  ...
  rcases membership with rfl | rfl <;>
    simp [leftPattern, boundaryLeaf, rigidLeaf, available, baseNameType,
...
theorem rightWellSorted :
  ...
  simp only [List.mem_cons, List.not_mem_nil, or_false] at membership
  rcases membership with rfl | rfl <;>
    simp [rightPattern, boundaryLeaf, rigidLeaf, available, baseNameType,
      binderSafeAt, binderSafeListAt,
      CostStaticColor.reflectiveSymbols, costBaseStaticReflectiveSymbols,
      costWrappedStaticReflectiveSymbols, costBaseStaticSymbols,
      costWrappedStaticSymbols, costBasePresentationSymbols,
...
end ForeignNonquoteRigidTieCanary
