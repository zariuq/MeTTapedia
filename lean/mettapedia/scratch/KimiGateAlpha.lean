...
`Proc` or `Name` colour image, optionally wrapped in hash bags. -/
inductive RhoTypedChainColour (color : CostStaticColor) : TypeExpr → Prop
  | nameLeaf : RhoTypedChainColour color
      (mapTypeExpr (color.symbols rhoCIGSLT) (.base "Name"))
  | procLeaf : RhoTypedChainColour color
...
/-- The category computed for a colour-static quote wire is a name leaf. -/
theorem chainColour_quoteCat (color : CostStaticColor) :
    RhoTypedChainColour color
      (mapTypeExpr (color.symbols rhoCIGSLT) (.base "Name")) :=
  .nameLeaf

/-- The category computed for a colour-static drop wire is a proc leaf. -/
theorem chainColour_dropCat (color : CostStaticColor) :
    RhoTypedChainColour color
      (mapTypeExpr (color.symbols rhoCIGSLT) (.base "Proc")) :=
  .procLeaf

/-- **Self-typing of drop-tower contributors.**  Any typed pattern whose
canonical form under the current colour is a colour-static drop tower is
            ...
            · rw [resultEq] at canonicalElement
              exact absurd (Pattern.apply.inj canonicalElement).1 (by
                ...
                exact absurd (costStaticColor_constructor_inj quoteIsDrop)
                  (by decide))
          · rw [canonicalize_apply_of_ne_quote _ quoteHead] at
              canonicalElement
              ...
              · rw [parallelType] at canonicalElement
                have notUnit : .apply
                  ...
                  exact absurd (costStaticColor_constructor_inj wireEq)
                    (by decide)
                have notParallelResult : ∀ nested, .apply
                      (costStaticReflectivePresentationDecl rhoCIGSLT color
                        rhoReflectivePresentation
                          .toReflectivePresentationDecl)
                        .dropConstructor [y] ≠
